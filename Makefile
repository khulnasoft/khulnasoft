...
PROJECT         := github.com/khulnasoft/khulnasoft/pkg/v3/cmd/pulumi

PKG_CODEGEN := github.com/khulnasoft/khulnasoft/pkg/v3/codegen
...
build:: build_proto build_display_wasm go.ensure
	cd pkg && go install -ldflags "-X github.com/khulnasoft/khulnasoft/pkg/v3/version.Version=${VERSION}" ${PROJECT}

build_display_wasm:: go.ensure
	cd pkg && GOOS=js GOARCH=wasm go build -o ../bin/pulumi-display.wasm -ldflags "-X github.com/khulnasoft/khulnasoft/pkg/v3/version.Version=${VERSION}" ./backend/display/wasm

install:: .ensure.phony go.ensure
	cd pkg && GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/khulnasoft/khulnasoft/pkg/v3/version.Version=${VERSION}" ${PROJECT}

build_debug::
	cd pkg && go install -gcflags="all=-N -l" -ldflags "-X github.com/khulnasoft/khulnasoft/pkg/v3/version.Version=${VERSION}" ${PROJECT}

build_cover::
	cd pkg && go build -cover -o ../bin/pulumi \
		-coverpkg github.com/khulnasoft/khulnasoft/pkg/v3/...,github.com/pulumi/pulumi/sdk/v3/... \
		-ldflags "-X github.com/khulnasoft/khulnasoft/pkg/v3/version.Version=${VERSION}" ${PROJECT}
...
test_lifecycle:
	@cd pkg && $(GO_TEST) github.com/khulnasoft/khulnasoft/pkg/v3/engine/lifecycletest
