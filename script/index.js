...
window.__khulnasoft__ = window.__khulnasoft__ || main()
module.exports = window.__khulnasoft__

function supportMiddleware (context, send, next) {
  checkSupport(function (err) {
    if (err) {
      console.log(__('KhulnaSoft Fair Web Analytics does not support this site: %s', err.message))
      console.log(__('No data will be collected. Find out more at "https://www.khulnasoft.com".'))
      return
    }
    next()
  })
