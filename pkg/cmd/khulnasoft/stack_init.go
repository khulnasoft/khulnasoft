...
func newStackInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Create an empty stack with the given name, ready for updates\n" +
			...
			"\n" +
			"* `khulnasoft stack init --secrets-provider=passphrase`\n" +
			"\n" +
			"To use a cloud secrets provider with any backend, use one of the following:\n" +
			"\n" +
			"* `khulnasoft stack init --secrets-provider=\"awskms://alias/ExampleAlias?region=us-east-1\"`\n" +
			"* `khulnasoft stack init --secrets-provider=\"awskms://1234abcd-12ab-34cd-56ef-1234567890ab?region=us-east-1\"`\n" +
			"* `khulnasoft stack init --secrets-provider=\"azurekeyvault://mykeyvaultname.vault.azure.net/keys/mykeyname\"`\n" +
			"* `khulnasoft stack init --secrets-provider=\"gcpkms://projects/<p>/locations/<l>/keyRings/<r>/cryptoKeys/<k>\"`\n" +
			"* `khulnasoft stack init --secrets-provider=\"hashivault://mykey\"`\n" +
			"\n" +
			"A stack can be created based on the configuration of an existing stack by passing the\n" +
			"`--copy-config-from` flag:\n" +
			"\n" +
			"* `khulnasoft stack init --copy-config-from dev`",
...
// stackInitCmd implements the `khulnasoft stack init` command.
