...
func TestStateDeleteNoArgs(t *testing.T) {
	...
	assert.ErrorContains(t, err, "Must supply <resource URN> unless khulnasoft is run interactively")
}
...
func TestNoProject(t *testing.T) {
	...
	cmd := &stateDeleteCmd{}
	err := cmd.Run(context.Background(), []string{`urn:khulnasoft:proj::stk::pkg:index:typ::res`}, ws, lm)
		...
		SnapshotF: func(ctx context.Context, secretsProvider secrets.Provider) (*deploy.Snapshot, error) {
			return &deploy.Snapshot{
				Resources: []*resource.State{
					{
						URN: "urn:khulnasoft:proj::stk::pkg:index:typ::res",
					},
				},
			}, nil
		},
...
func TestStateDeleteDependency(t *testing.T) {
	...
	mockStack := &backend.MockStack{
		SnapshotF: func(ctx context.Context, secretsProvider secrets.Provider) (*deploy.Snapshot, error) {
			return &deploy.Snapshot{
				Resources: []*resource.State{
					{
						URN: "urn:khulnasoft:proj::stk::pkg:index:typ::dependency",
					},
					{
						URN: "urn:khulnasoft:proj::stk::pkg:index:typ::dependee",
						Dependencies: []resource.URN{
							"urn:khulnasoft:proj::stk::pkg:index:typ::dependency",
						},
					},
				},
			}, nil
	...
	err := cmd.Run(context.Background(), []string{`urn:khulnasoft:proj::stk::pkg:index:typ::dependency`}, ws, lm)
	assert.ErrorContains(t, err,
		"urn:khulnasoft:proj::stk::pkg:index:typ::dependency can't be safely deleted "+
			"because the following resources depend on it:\n"+
			" * \"dependee\"      (urn:khulnasoft:proj::stk::pkg:index:typ::dependee)")
}
...
func TestStateDeleteProtected(t *testing.T) {
	...
	err := cmd.Run(context.Background(), []string{`urn:khulnasoft:proj::stk::pkg:index:typ::res`}, ws, lm)
	assert.ErrorContains(t, err,
		"urn:khulnasoft:proj::stk::pkg:index:typ::res can't be safely deleted because it is protected.")
