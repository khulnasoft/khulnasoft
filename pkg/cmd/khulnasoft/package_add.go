...
// Constructs the `khulnasoft package add` command.
...
as in:
  ...
  khulnasoft package add <provider> -- --provider-parameter-flag value`,
		Run: runCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			tempOut, err := os.MkdirTemp("", "khulnasoft-package-add-")
			if err != nil {
...
func printPythonLinkInstructions(ws pkgWorkspace.Context, root string, pkg *schema.Package, out string) error {
	...
	fmt.Println()
	fmt.Printf("  import khulnasoft_%s as %s\n", importName, importName)
	fmt.Println()
	return nil
}
