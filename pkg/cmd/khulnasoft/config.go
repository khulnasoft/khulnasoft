...
func newConfigCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Lists all configuration values for a specific stack. To add a new configuration value, run\n" +
			"`khulnasoft config set`. To remove an existing value run `khulnasoft config rm`. To get the value of\n" +
			"for a specific configuration key, use `khulnasoft config get <key-name>`.",
		Args: cmdutil.NoArgs,
...
func newConfigGetCmd(stack *string) *cobra.Command {
	...
	getCmd := &cobra.Command{
		...
		Long: "Get a single configuration value.\n\n" +
			...
			"  - `khulnasoft config get --path outer.inner` will get the value of the `inner` key, " +
			"if the value of `outer` is a map `inner: value`.\n" +
			"  - `khulnasoft config get --path 'names[0]'` will get the value of the first item, " +
...
func newConfigRmCmd(stack *string) *cobra.Command {
	...
	rmCmd := &cobra.Command{
		...
		Long: "Remove configuration value.\n\n" +
			...
			"  - `khulnasoft config rm --path outer.inner` will remove the `inner` key, " +
			"if the value of `outer` is a map `inner: value`.\n" +
			"  - `khulnasoft config rm --path 'names[0]'` will remove the first item, " +
			"if the value of `names` is a list.",
		Args: cmdutil.SpecificArgs([]string{"key"}),
		Run: runCmdFunc(func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
...
func newConfigRmAllCmd(stack *string) *cobra.Command {
	...
	rmAllCmd := &cobra.Command{
		...
		Long: "Remove multiple configuration values.\n\n" +
			...
			"  - `khulnasoft config rm-all --path  outer.inner 'foo[0]' key1` will remove the \n" +
			"    `inner` key of the `outer` map, the first key of the `foo` list and `key1`.\n" +
			"  - `khulnasoft config rm-all outer.inner 'foo[0]' key1` will remove the literal" +
...
func newConfigSetCmd(stack *string) *cobra.Command {
	...
	setCmd := &cobra.Command{
		...
		Long: "Configuration values can be accessed when a stack is being deployed and used to configure behavior. \n" +
			"If a value is not present on the command line, khulnasoft will prompt for the value. Multi-line values\n" +
			"may be set by piping a file to standard in.\n\n" +
			"The `--path` flag can be used to set a value inside a map or list:\n\n" +
			"  - `khulnasoft config set --path 'names[0]' a` " +
			"will set the value to a list with the first item `a`.\n" +
			"  - `khulnasoft config set --path parent.nested value` " +
			"will set the value of `parent` to a map `nested: value`.\n" +
			"  - `khulnasoft config set --path '[\"parent.name\"][\"nested.name\"]' value` will set the value of \n" +
...
func newConfigSetAllCmd(stack *string) *cobra.Command {
	...
	setCmd := &cobra.Command{
		...
		Long: "khulnasoft set-all allows you to set multiple configuration values in one command.\n\n" +
			"Each key-value pair must be preceded by either the `--secret` or the `--plaintext` flag to denote whether \n" +
			"it should be encrypted:\n\n" +
			"  - `khulnasoft config set-all --secret key1=value1 --plaintext key2=value --secret key3=value3`\n\n" +
			"The `--path` flag can be used to set values inside a map or list:\n\n" +
			"  - `khulnasoft config set-all --path --plaintext \"names[0]\"=a --plaintext \"names[1]\"=b` \n" +
			"    will set the value to a list with the first item `a` and second item `b`.\n" +
			"  - `khulnasoft config set-all --path --plaintext parent.nested=value --plaintext parent.other=value2` \n" +
			"    will set the value of `parent` to a map `{nested: value, other: value2}`.\n" +
			"  - `khulnasoft config set-all --path --plaintext '[\"parent.name\"].[\"nested.name\"]'=value` will set the \n" +
...
func listConfig(
	...
	var khulnasoftEnv esc.Value
	var envCrypter config.Encrypter
	if env != nil {
		khulnasoftEnv = env.Properties["khulnasoftConfig"]
	...
	err = workspace.ApplyProjectConfig(ctx, stackName, project, khulnasoftEnv, cfg, envCrypter)
	if err != nil {
		return err
	}
	...
	if showSecrets {
		log3rdPartySecretsProviderDecryptionEvent(ctx, stack, "", "khulnasoft config")
	}
...
// primarily for use in commands like `khulnasoft destroy`, where it is useful to be
...
func warnOnNoEnvironmentEffects(out io.Writer, env *esc.Environment) {
	...
	_, hasPulumiConfig := env.Properties["khulnasoftConfig"].Value.(map[string]esc.Value)

	//nolint:lll
	if !hasEnvVars && !hasFiles && !hasPulumiConfig {
		color := cmdutil.GetGlobalColorization()
		fmt.Fprintln(out, color.Colorize(colors.SpecWarning+"The stack's environment does not define the `environmentVariables`, `files`, or `khulnasoftConfig` properties."))
