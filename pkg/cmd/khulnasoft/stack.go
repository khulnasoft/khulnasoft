...
func runStack(ctx context.Context, s backend.Stack, out io.Writer, args stackArgs) error {
	...
	if snap != nil {
		...
		fmt.Fprintf(out, "    No updates yet; run `khulnasoft up`\n")
	}
	...
	if resourceCount == 0 {
		...
		if args.showSecrets {
			log3rdPartySecretsProviderDecryptionEvent(ctx, s, "", "khulnasoft stack")
		}
	}
	...
	fmt.Fprintf(out, "Use `khulnasoft stack select` to change stack; `khulnasoft stack ls` lists known ones\n")
