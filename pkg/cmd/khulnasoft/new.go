...
func runNew(ctx context.Context, args newArgs) error {
	...
	if args.name == "" {
		...
		if err != nil {
			...
			// See https://github.com/khulnasoft/khulnasoft/issues/8747.
			if args.yes {
	...
	// Set the khulnasoft:template tag to the template name or URL.
...
func newNewCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Create a new Pulumi project and stack from a template.\n" +
			...
			"\n" +
			"By default, a stack created using the khulnasoft.com backend will use the khulnasoft.com secrets\n" +
			"provider and a stack created using the local or cloud object storage backend will use the\n" +
			"`passphrase` secrets provider.  A different secrets provider can be selected by passing the\n" +
			"`--secrets-provider` flag.\n" +
			"\n" +
			"To use the `passphrase` secrets provider with the khulnasoft.com backend, use:\n" +
			"* `khulnasoft new --secrets-provider=passphrase`\n" +
			"\n" +
			"To use a cloud secrets provider with any backend, use one of the following:\n" +
			"* `khulnasoft new --secrets-provider=\"awskms://alias/ExampleAlias?region=us-east-1\"`\n" +
			"* `khulnasoft new --secrets-provider=\"awskms://1234abcd-12ab-34cd-56ef-1234567890ab?region=us-east-1\"`\n" +
			"* `khulnasoft new --secrets-provider=\"azurekeyvault://mykeyvaultname.vault.azure.net/keys/mykeyname\"`\n" +
			"* `khulnasoft new --secrets-provider=\"gcpkms://projects/p/locations/l/keyRings/r/cryptoKeys/k\"`\n" +
			"* `khulnasoft new --secrets-provider=\"hashivault://mykey\"`" +
			"\n\n" +
			"To create a project from a specific source control location, pass the url as follows e.g.\n" +
			"* `khulnasoft new https://gitlab.com/<user>/<repo>`\n" +
			"* `khulnasoft new https://bitbucket.org/<user>/<repo>`\n" +
			"* `khulnasoft new https://github.com/<user>/<repo>`\n" +
			"\n" +
			"To create the project from a branch of a specific source control location, pass the url to the branch, e.g.\n" +
			"* `khulnasoft new https://gitlab.com/<user>/<repo>/tree/<branch>`\n" +
			"* `khulnasoft new https://bitbucket.org/<user>/<repo>/tree/<branch>`\n" +
			"* `khulnasoft new https://github.com/<user>/<repo>/tree/<branch>`\n" +
			"\n" +
			...
			"* `khulnasoft new git@github.com:<user>/<private-repo>`\n" +
			"* `khulnasoft new https://<user>:<password>@<hostname>/<project>/<repo>`\n" +
			"* `khulnasoft new <user>@<hostname>:<project>/<repo>`\n" +
			"* `PULUMI_GITSSH_PASSPHRASE=<passphrase> khulnasoft new ssh://<user>@<hostname>/<project>/<repo>`\n" +
			"To create a project using Pulumi AI, either select `ai` from the first selection, " +
			"or provide any of the following:\n" +
			"* `khulnasoft new --ai \"<prompt>\"`\n" +
			"* `khulnasoft new --language <language>`\n" +
			"* `khulnasoft new --ai \"<prompt>\" --language <language>`\n" +
...
func installDependencies(ctx *plugin.Context, runtime *workspace.ProjectRuntimeInfo, main string) error {
	...
	if err = lang.InstallDependencies(plugin.InstallDependenciesRequest{Info: programInfo}); err != nil {
		...
		return fmt.Errorf("installing dependencies failed: %w\nRun `khulnasoft install` to complete the installation.", err)
	}
...
func printNextSteps(proj *workspace.Project, originalCwd, cwd string, generateOnly bool, opts display.Options) {
	...
	if generateOnly {
		...
		commands = append(commands, "khulnasoft install")
		// We didn't create a stack so show that as a command to run before `khulnasoft up`.
		commands = append(commands, "khulnasoft stack init")
	}

	if len(commands) == 0 { // No additional commands need to be run.
		deployMsg := "To perform an initial deployment, run `khulnasoft up`"
		deployMsg = colors.Highlight(deployMsg, "khulnasoft up", colors.BrightBlue+colors.Bold)
	...
	if len(commands) == 1 { // Only one additional command need to be run.
		deployMsg := fmt.Sprintf("To perform an initial deployment, run '%s', then, run `khulnasoft up`", commands[0])
	...
	upMsg := colors.Highlight("Then, run `khulnasoft up`", "khulnasoft up", colors.BrightBlue+colors.Bold)
...
func chooseTemplate(templates []workspace.Template, opts display.Options) (workspace.Template, error) {
	const chooseTemplateErr = "no template selected; please use `khulnasoft new` to choose one"
	if !opts.IsInteractive {
