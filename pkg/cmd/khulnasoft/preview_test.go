...
func TestBuildImportFile_ExistingProvider(t *testing.T) {
	...
	providerState := makeStateMetadata(t, "prov", "khulnasoft:providers:pkg", true, stateOptions{
		Inputs: resource.NewPropertyMapFromMap(map[string]interface{}{
			"version": "3.2.1",
		}),
	})
...
func TestBuildImportFile_NewProvider(t *testing.T) {
	...
	assert.ErrorContains(
		t, err,
		"cannot import resource \"urn:khulnasoft:stack::project::pkg:mod:typ::res\" "+
			"with a new explicit provider \"urn:khulnasoft:stack::project::khulnasoft:providers:pkg::prov\"")
}
...
// Regression test for https://github.com/khulnasoft/khulnasoft/issues/15002
...
func TestBuildImportFile_regress_15002(t *testing.T) {
	...
	events <- engine.NewEvent(engine.ResourcePreEventPayload{
		Metadata: engine.StepEventMetadata{
			URN: "urn:khulnasoft:test::test::pkgA:m:a-different-type::resA",
		},
	})
	// Finally, close the events channel to signal that we're done
	close(events)
...
// Regression test for https://github.com/khulnasoft/khulnasoft/issues/15068
