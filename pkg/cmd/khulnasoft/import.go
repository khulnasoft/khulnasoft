...
import (
	...
	"github.com/khulnasoft/khulnasoft/pkg/v3/codegen/convert"
	...
	"github.com/khulnasoft/khulnasoft/pkg/v3/importer"
	...
	javagen "github.com/khulnasoft/khulnasoft-java/pkg/codegen/java"
...
func writeImportFileToTemp(v importFile) (string, error) {
	...
	f, err := os.CreateTemp(wd, "khulnasoft-import-*.json")
	if err != nil {
...
func newImportCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Import resources into an existing stack.\n" +
			...
			"\n" +
			"You can use `khulnasoft preview` with the `--import-file` option to emit an import file\n" +
					...
					if outputFilePath == "" && !jsonDisplay {
						...
						if protectResources {
							fmt.Print(("Please note that the imported resources are marked as protected. " +
								"To destroy them\n" +
								"you will need to remove the `protect` option and run `khulnasoft update` *before*\n" +
