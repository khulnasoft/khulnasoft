...
func TestReadingGitRepo(t *testing.T) {
...

	{
		...
		// assert.Equal(t, "branch-from-ci", name) # see https://github.com/khulnasoft/khulnasoft/issues/5303
	}
}
...
func TestPulumiCLIMetadata(t *testing.T) {
	...
	cmd.SetArgs([]string{"subcommand", "--name", "khulnasoftpus", "--age", "100", "--human"})
	...
	assert.Equal(t, map[string]string{
		...
		"khulnasoft.version": "SKIP",
		"khulnasoft.arch":    "SKIP",
		"khulnasoft.os":      "SKIP",

		// CLI flags
		"khulnasoft.flag.name":  "set",
		"khulnasoft.flag.age":   "set",
		"khulnasoft.flag.human": "true",

		// Environment variables

		// Env Vars are considered truthy if they are "1" or "true" case-insensitive.
		"khulnasoft.env.PULUMI_SKIP_UPDATE_CHECK": "true",
		// The following is a boolean flag that is set to "0", which is falsey.
		"khulnasoft.env.PULUMI_EXPERIMENTAL": "false",
		// The following is truthy, but NOT a boolean flag.
		"khulnasoft.env.PULUMI_STRING_FLAG": "set",
		// The following is falsey, but NOT a boolean flag.
		"khulnasoft.env.PULUMI_DEPRECATED_FLAG": "set",
	}, actualEnv)
}
