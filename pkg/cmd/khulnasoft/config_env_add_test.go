	...
	t.Run("no imports", func(t *testing.T) {
		...
		env := &esc.Environment{
			Properties: map[string]esc.Value{
				"khulnasoftConfig": esc.NewValue(map[string]esc.Value{
...
`
	...
	t.Run("no effects", func(t *testing.T) {
		...
		const expectedOut = "KEY  VALUE\n" +
			"The stack's environment does not define the `environmentVariables`, `files`, or `khulnasoftConfig` properties.\n" +
