...
func TestParseImportFileJustLogicalName(t *testing.T) {
	...
	assert.Equal(t, importer.NameTable{
		"urn:khulnasoft:stack::proj::foo:bar:bar::thing": "thing",
	}, names)
}
...
func TestParseImportFileLogicalName(t *testing.T) {
	...
	assert.Equal(t, importer.NameTable{
		"urn:khulnasoft:stack::proj::foo:bar:bar::different logical name": "thing",
	}, names)
}
...
func TestParseImportFileSameNameDifferentType(t *testing.T) {
	...
	assert.Equal(t, importer.NameTable{
		"urn:khulnasoft:stack::proj::foo:bar:bar::thing": "thing",
		"urn:khulnasoft:stack::proj::foo:bar:baz::thing": "thing",
	}, names)
}
...
func TestParseImportFileAutoURN(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:khulnasoft:stack::proj::foo:bar:a$foo:bar:a::otherThing"), imports[0].Parent)
	assert.Equal(t, resource.URN(""), imports[1].Parent)
	assert.Equal(t, resource.URN("urn:khulnasoft:stack::proj::foo:bar:a::thing"), imports[2].Parent)
...
func TestImportFileMarshal(t *testing.T) {
	...
	t.Run("initial", func(t *testing.T) {
		...
		importFile := importFile{
			NameTable: map[string]resource.URN{
				"foo": "urn:khulnasoft:stack::proj::foo:bar:a::arb",
			},
			Resources: []importSpec{
				{
					Name: "bar",
