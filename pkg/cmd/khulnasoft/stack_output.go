...
func (cmd *stackOutputCmd) Run(ctx context.Context, args []string) error {
	...
	// If there is an argument, just print that property.  Else, print them all (similar to `khulnasoft stack`).
	if len(args) > 0 {
		name := args[0]
		v, has := outputs[name]
		if has {
	...
	if cmd.showSecrets {
		log3rdPartySecretsProviderDecryptionEvent(ctx, s, "", "khulnasoft stack output")
	}
...
// on behalf of 'khulnasoft stack output'.
