...
func newLoginCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Log in to the Pulumi Cloud.\n" +
			...
			"\n" +
			"    $ khulnasoft login https://api.khulnasoft.acmecorp.com\n" +
			"\n" +
			"to log in to a self-hosted Pulumi Cloud running at the api.khulnasoft.acmecorp.com domain.\n" +
			"\n" +
			...
			"\n" +
			"    $ khulnasoft login file://~\n" +
			"\n" +
			"will store your state information on your computer underneath `~/.khulnasoft`. It is then up to you to\n" +
			...
			"AWS S3:\n" +
			"\n" +
			"    $ khulnasoft login s3://my-khulnasoft-state-bucket\n" +
			"\n" +
			"GCP GCS:\n" +
			"\n" +
			"    $ khulnasoft login gs://my-khulnasoft-state-bucket\n" +
			"\n" +
			"Azure Blob:\n" +
			"\n" +
			"    $ khulnasoft login azblob://my-khulnasoft-state-bucket\n",
		...
		Run: runCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			if cloudURL == "" {
				...
				cloudURL, "https://"), "http://"); strings.HasPrefix(url, "app.khulnasoft.com/") ||
				strings.HasPrefix(url, "khulnasoft.com") {
				return fmt.Errorf("%s is not a valid self-hosted backend, "+
					"use `khulnasoft login` without arguments to log into the Pulumi Cloud backend", cloudURL)
			} else {
...
func chooseAccount(accounts map[string]workspace.Account, opts display.Options) (string, error) {
	...
	if err := survey.AskOne(&survey.Select{
		...
		return "", errors.New("no account selected; please use `khulnasoft login --interactive` to choose one")
	}
