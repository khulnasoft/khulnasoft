...
func TestRenameProvider(t *testing.T) {
	...
	provURN := resource.URN("urn:khulnasoft:dev::prog-aws-typescript::khulnasoft:providers:random::my-provider")
	prov := resource.State{
		URN:  provURN,
		ID:   "81cd12dd-2a90-4f21-a521-f4c71c1f11eb",
		Type: "khulnasoft:providers:random",
	}

	providerRefString := string(prov.URN) + "::" + string(prov.ID)

	res1 := resource.State{
		URN:      resource.URN("urn:khulnasoft:dev::prog-aws-typescript::random:index/randomPet:RandomPet::pet-0"),
...
func TestStateRename_invalidName(t *testing.T) {
	...
	prov := resource.URN("urn:khulnasoft:dev::xxx-dev::kubernetes::provider")
	res := resource.URN("urn:khulnasoft:dev::xxx-dev::kubernetes:core/v1:Namespace::amazon_cloudwatchNamespace")
		...
		Resources: []*resource.State{
			{
				URN:  prov,
				ID:   "provider-id",
				Type: "khulnasoft:provider:kubernetes",
			},
			{
				URN:  res,
				ID:   "res-id",
	...
	assert.Panicsf(t, func() {
		_ = stateRenameOperation(
			res,
			"urn:khulnasoft:dev::xxx-dev::eks:index:Cluster$kubernetes:core/v1:Namespace::amazon_cloudwatchNamespace",
			display.Options{},
			&snap,
		)
...
// Regression test for https://github.com/khulnasoft/khulnasoft/issues/13179.
//
...
func TestStateRename_updatesChildren(t *testing.T) {
	...
	provider := resource.URN("urn:khulnasoft:dev::pets::random::provider")
	parent := resource.URN("urn:khulnasoft:dev::pets::random:index/randomPet:RandomPet::parent")
	child := resource.URN("urn:khulnasoft:dev::pets::random:index/randomPet:RandomPet$random:index/randomPet:RandomPet::child")
		...
		Resources: []*resource.State{
			{
				URN:  provider,
				ID:   "provider-id",
				Type: "khulnasoft:provider:random",
			},
			{
				URN:  parent,
				ID:   "parent-id",
