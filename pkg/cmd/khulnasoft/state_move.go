...
func (cmd *stateMoveCmd) Run(
	...
	if destSnapshot.SecretsManager == nil {
		...
		//nolint:lll
		projectError := errors.New("destination stack has no secret manager. To move resources either initialize the stack with a secret manager, or run the khulnasoft state move command from the destination project directory")
	...
	for _, res := range sourceSnapshot.Resources {
		...
		if matchedArg != "" {
			if strings.HasPrefix(string(res.Type), "khulnasoft:providers:") {
				//nolint:lll
...
This is a bug! We would appreciate a report: https://github.com/khulnasoft/khulnasoft/issues/`, err)
	}
		...
		if errDest != nil {
			...
			for i := len(resourcesToMoveOrdered) - 1; i >= 0; i-- {
				deleteCommands += fmt.Sprintf(
					"\n    khulnasoft state delete --stack %s '%s'",
...
func renameStackAndProject(urn urn.URN, stack backend.Stack) (urn.URN, error) {
	...
	if project, ok := stack.Ref().Project(); ok {
		...
		return "", errors.New("cannot get project name. " +
			"Please upgrade your project with `khulnasoft state upgrade` to solve this.")
	}
	return newURN, nil
}
