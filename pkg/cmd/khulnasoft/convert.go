...
import (
	...
	aferoUtil "github.com/khulnasoft/khulnasoft/pkg/v3/util/afero"
...
func loadConverterPlugin(
	...
	if versionSet := util.SetKnownPluginVersion(&pluginSpec); versionSet {
		ctx.Diag.Infof(
			diag.Message("", "Using version %s for khulnasoft-converter-%s"), pluginSpec.Version, pluginSpec.Name)
	}
...
func newConvertCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Convert Pulumi programs from a supported source program into other supported languages.\n" +
			...
			"\n" +
			"    khulnasoft convert --from yaml --language java --out . \n",
...
func runConvert(
	...
	pclDirectory, err := os.MkdirTemp("", "khulnasoft-convert")
	if err != nil {
