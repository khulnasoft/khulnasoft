...
// TestRegress13774 checks that you can run `khulnasoft new` on an existing project as described in the
...
func TestRegress13774(t *testing.T) {
	...
	err = runNew(context.Background(), args)
	defer removeStack(t, tempdir, args.stack)
	assert.NoError(t, err, "should be able to run `khulnasoft new` successfully on an existing project")
}
...
func TestCreatingProjectWithPulumiBackendURL(t *testing.T) {
	...
	assert.True(t, strings.HasPrefix(b.URL(), "https://app.khulnasoft.com"))
