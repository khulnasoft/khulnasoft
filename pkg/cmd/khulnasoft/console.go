	...
	cmd := &cobra.Command{
		...
		Run: runCmdFunc(func(cmd *cobra.Command, args []string) error {
				...
				if stackName != "" {
					...
					if stack == nil {
						fmt.Println("No stack is currently selected. " +
							"Run `khulnasoft stack select` to select a stack.")
						return nil
					}
				}

				// Open the stack specific URL (e.g. app.khulnasoft.com/{org}/{project}/{stack}) for this
				// stack if a stack is selected and is a cloud stack, else open the cloud backend URL
				// home page, e.g. app.khulnasoft.com.
			...
			fmt.Println("This command is not available for your backend. " +
				...
				"please see https://www.khulnasoft.com/docs/intro/concepts/state/#khulnasoft-cloud-backend")
			return nil
		}),
	}
