...
func initializeDeploymentSettingsCmd(
	...
	if !be.SupportsDeployments() {
		unsupportedBackendMsg := fmt.Sprintf("Backends of type %q do not support managed deployments.\n\n"+
			"Create a Pulumi Cloud account to get started, learn more about khulnasoft deployments here: "+
			"https://www.khulnasoft.com/docs/khulnasoft-cloud/deployments/",
			be.Name())
		...
		unsupportedBackendMsg = colors.Highlight(unsupportedBackendMsg,
			"https://www.khulnasoft.com/docs/khulnasoft-cloud/deployments/", colors.BrightBlue+colors.Underline+colors.Bold)
...
func newDeploymentSettingsConfigureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: runCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			if d.Deployment == nil {
				return errors.New("Deployment file not initialized, please run `khulnasoft deployment settings init` instead")
			}
	...
	if useGitHub {
		...
		if !integration.Installed {
			...
			ghAppExplanationMsg := "Pulumi’s GitHub app displays the results of Pulumi stack update previews in " +
				...
				"https://www.khulnasoft.com/docs/iac/packages-and-automation/continuous-delivery/github-app/\n"
...
func configureGitSSH(
	...
	if gitSSHPrivateKeyPath == "" && gitSSHPrivateKeyValue == "" {
		configureMsg := "\nNo SSH private key was provided, run `khulnasoft deployment settings " +
		...
		configureMsg = colors.Highlight(configureMsg, "khulnasoft deployment settings configure", colors.BrightBlue+colors.Bold)
