...
func setCommandGroups(cmd *cobra.Command, rootCgs []commandGroup) {
	...
	cmd.SetHelpFunc(func(c *cobra.Command, args []string) {
		...
		fmt.Println("Usage:")
		fmt.Println("  khulnasoft [command]")
		fmt.Println()
		...
		fmt.Println("Use `khulnasoft [command] --help` for more information about a command.")
	})
}
...
func NewPulumiCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Pulumi - Modern Infrastructure as Code\n" +
			"\n" +
			"To begin working with Pulumi, run the `khulnasoft new` command:\n" +
			"\n" +
			"    $ khulnasoft new\n" +
			"\n" +
			...
			"\n" +
			"    - khulnasoft up       : Deploy code and/or resource changes\n" +
			"    - khulnasoft stack    : Manage instances of your project\n" +
			"    - khulnasoft config   : Alter your stack's configuration or secrets\n" +
			"    - khulnasoft destroy  : Tear down your stack's resources entirely\n" +
			"\n" +
			"For more information, please visit the project page: https://www.khulnasoft.com/docs/",
		PersistentPreRun: runCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			cmdutil.InitTracing("khulnasoft-cli", "khulnasoft", tracingFlag)
	...
	cmd.PersistentFlags().StringVarP(&cwd, "cwd", "C", "",
		"Run khulnasoft as if it had been started in another directory")
	...
	setCommandGroups(cmd, []commandGroup{
		...
		// We have a set of options that are useful for developers of khulnasoft
...
func getUpgradeMessage(latest semver.Version, current semver.Version, isDevVersion bool) string {
	...
	msg += "visit https://khulnasoft.com/docs/install/ for manual instructions and release notes."
	return msg
}
...
func getUpgradeCommand(isDevVersion bool) string {
	...
	if isBrew {
		return "$ brew update && brew upgrade khulnasoft"
	}
	...
	if runtime.GOOS != "windows" {
		command := "$ curl -sSL https://get.khulnasoft.com | sh"
		if isDevVersion {
	...
	powershellCmd = "> " + powershellCmd + ` -NoProfile -InputFormat None -ExecutionPolicy Bypass -Command "iex ` +
		`((New-Object System.Net.WebClient).DownloadString('https://get.khulnasoft.com/install.ps1'))"`
	if isDevVersion {
...
func isBrewInstall(exe string) (bool, error) {
	...
	brewPrefixCmd := exec.Command(brewBin, "--prefix", "khulnasoft")
	...
	if err = brewPrefixCmd.Run(); err != nil {
		...
		return false, fmt.Errorf("'brew --prefix khulnasoft' failed: %w", err)
	}

	brewPrefixCmdOutput := strings.TrimSpace(stdout.String())
	if brewPrefixCmdOutput == "" {
		return false, errors.New("trimmed output from 'brew --prefix khulnasoft' is empty")
	}
	...
	brewPrefixExePath := filepath.Join(brewPrefixPath, "bin", "khulnasoft")
...
func getLatestBrewFormulaVersion() (semver.Version, bool, error) {
	...
	const formulaJSON = "https://formulae.brew.sh/api/formula/khulnasoft.json"
