...
func TestRepoLookup(t *testing.T) {
	...
	t.Run("should handle directories that are a git repo", func(t *testing.T) {
		...
		remote, err := rl.RemoteURL()
		assert.NoError(t, err)
		assert.Equal(t, "https://github.com/khulnasoft/test-repo.git", remote)
...
func TestValidateGitURL(t *testing.T) {
	...
	err := ValidateGitURL("https://github.com/khulnasoft/test-repo.git")
	require.NoError(t, err)
...
func TestDSFileParsing(t *testing.T) {
  ...
  agentPoolID: 51035bee-a4d6-4b63-9ff6-418775c5da8d`), 0o600)
	...
	require.Equal(t, "git@github.com:khulnasoft/test-repo.git", deploymentFile.DeploymentSettings.SourceContext.Git.RepoURL)
...
func setUpGitWorkspace(ctx context.Context, t *testing.T) string {
	workDir, err := os.MkdirTemp("", "khulnasoft_deployment_settings")
	assert.NoError(t, err)
