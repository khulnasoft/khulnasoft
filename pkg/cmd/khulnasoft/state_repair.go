...
func (cmd *stateRepairCmd) manualRepairError(initialErr error, err error) string {
	...
	stateExportCommand := fmt.Sprintf("khulnasoft stack%s export > %s", cmd.FlagsString, stateFile)
	...
	stateImportCommand := fmt.Sprintf("khulnasoft stack%s import < %s", cmd.FlagsString, stateFile)
...
================================================================================
We would appreciate a report: https://github.com/khulnasoft/khulnasoft/issues/
