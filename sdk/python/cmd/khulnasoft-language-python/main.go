...
// khulnasoft-language-python serves as the "language host" for Pulumi programs written in Python.  It is ultimately
...
const (
	...
	pythonDefaultExec = "khulnasoft-language-python-exec" // the exec shim for Pulumi to run Python programs.
...
func (host *pythonLanguageHost) GetRequiredPlugins(ctx context.Context,
	...
	khulnasoftPackages, err := determinePulumiPackages(ctx, opts)
	if err != nil {
		return nil, err
	}
	...
	return &khulnasoftrpc.GetRequiredPluginsResponse{Plugins: plugins}, nil
}
...
func readPulumiPluginJSON(pkg toolchain.PythonPackage) (*plugin.PulumiPluginJSON, error) {
	...
	logging.V(5).Infof("readPulumiPluginJSON: khulnasoft-plugin.json file path: %s", khulnasoftPluginFilePath)

	plugin, err := plugin.LoadPulumiPluginJSON(khulnasoftPluginFilePath)
...
func determinePulumiPackages(ctx context.Context, options toolchain.PythonOptions) ([]toolchain.PythonPackage, error) {
	...
	logging.V(5).Infof("GetRequiredPlugins: Pulumi packages: %#v", khulnasoftPackages)
...
// contains a khulnasoft-plugin.json file and uses the information in that file to determine the plugin. If `resource` in
// khulnasoft-plugin.json is set to false, nil is returned. If the name or version aren't specified in the file, these
...
func determinePluginDependency(pkg toolchain.PythonPackage) (*khulnasoftrpc.PluginDependency, error) {
	...
	result := &khulnasoftrpc.PluginDependency{
		Name:    name,
		Version: version,
		Kind:    "resource",
		Server:  server,
