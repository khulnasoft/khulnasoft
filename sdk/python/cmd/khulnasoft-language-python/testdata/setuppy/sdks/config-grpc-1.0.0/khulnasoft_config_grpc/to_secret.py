# coding=utf-8
# *** WARNING: this file was generated by khulnasoft-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import sys
import khulnasoft
import khulnasoft.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    'ToSecretResult',
    'AwaitableToSecretResult',
    'to_secret',
    'to_secret_output',
]

@khulnasoft.output_type
class ToSecretResult:
    def __init__(__self__, bool1=None, bool2=None, bool3=None, int1=None, int2=None, int3=None, list_bool1=None, list_bool2=None, list_bool3=None, list_int1=None, list_int2=None, list_int3=None, list_num1=None, list_num2=None, list_num3=None, list_secret_bool1=None, list_secret_bool2=None, list_secret_bool3=None, list_secret_int1=None, list_secret_int2=None, list_secret_int3=None, list_secret_num1=None, list_secret_num2=None, list_secret_num3=None, list_secret_string1=None, list_secret_string2=None, list_secret_string3=None, list_string1=None, list_string2=None, list_string3=None, map_bool1=None, map_bool2=None, map_bool3=None, map_int1=None, map_int2=None, map_int3=None, map_num1=None, map_num2=None, map_num3=None, map_secret_bool1=None, map_secret_bool2=None, map_secret_bool3=None, map_secret_int1=None, map_secret_int2=None, map_secret_int3=None, map_secret_num1=None, map_secret_num2=None, map_secret_num3=None, map_secret_string1=None, map_secret_string2=None, map_secret_string3=None, map_string1=None, map_string2=None, map_string3=None, num1=None, num2=None, num3=None, obj_bool1=None, obj_bool2=None, obj_bool3=None, obj_int1=None, obj_int2=None, obj_int3=None, obj_num1=None, obj_num2=None, obj_num3=None, obj_secret_bool1=None, obj_secret_bool2=None, obj_secret_bool3=None, obj_secret_int1=None, obj_secret_int2=None, obj_secret_int3=None, obj_secret_num1=None, obj_secret_num2=None, obj_secret_num3=None, obj_secret_string1=None, obj_secret_string2=None, obj_secret_string3=None, obj_string1=None, obj_string2=None, obj_string3=None, secret_bool1=None, secret_bool2=None, secret_bool3=None, secret_int1=None, secret_int2=None, secret_int3=None, secret_num1=None, secret_num2=None, secret_num3=None, secret_string1=None, secret_string2=None, secret_string3=None, string1=None, string2=None, string3=None):
        if bool1 and not isinstance(bool1, bool):
            raise TypeError("Expected argument 'bool1' to be a bool")
        khulnasoft.set(__self__, "bool1", bool1)
        if bool2 and not isinstance(bool2, bool):
            raise TypeError("Expected argument 'bool2' to be a bool")
        khulnasoft.set(__self__, "bool2", bool2)
        if bool3 and not isinstance(bool3, bool):
            raise TypeError("Expected argument 'bool3' to be a bool")
        khulnasoft.set(__self__, "bool3", bool3)
        if int1 and not isinstance(int1, int):
            raise TypeError("Expected argument 'int1' to be a int")
        khulnasoft.set(__self__, "int1", int1)
        if int2 and not isinstance(int2, int):
            raise TypeError("Expected argument 'int2' to be a int")
        khulnasoft.set(__self__, "int2", int2)
        if int3 and not isinstance(int3, int):
            raise TypeError("Expected argument 'int3' to be a int")
        khulnasoft.set(__self__, "int3", int3)
        if list_bool1 and not isinstance(list_bool1, list):
            raise TypeError("Expected argument 'list_bool1' to be a list")
        khulnasoft.set(__self__, "list_bool1", list_bool1)
        if list_bool2 and not isinstance(list_bool2, list):
            raise TypeError("Expected argument 'list_bool2' to be a list")
        khulnasoft.set(__self__, "list_bool2", list_bool2)
        if list_bool3 and not isinstance(list_bool3, list):
            raise TypeError("Expected argument 'list_bool3' to be a list")
        khulnasoft.set(__self__, "list_bool3", list_bool3)
        if list_int1 and not isinstance(list_int1, list):
            raise TypeError("Expected argument 'list_int1' to be a list")
        khulnasoft.set(__self__, "list_int1", list_int1)
        if list_int2 and not isinstance(list_int2, list):
            raise TypeError("Expected argument 'list_int2' to be a list")
        khulnasoft.set(__self__, "list_int2", list_int2)
        if list_int3 and not isinstance(list_int3, list):
            raise TypeError("Expected argument 'list_int3' to be a list")
        khulnasoft.set(__self__, "list_int3", list_int3)
        if list_num1 and not isinstance(list_num1, list):
            raise TypeError("Expected argument 'list_num1' to be a list")
        khulnasoft.set(__self__, "list_num1", list_num1)
        if list_num2 and not isinstance(list_num2, list):
            raise TypeError("Expected argument 'list_num2' to be a list")
        khulnasoft.set(__self__, "list_num2", list_num2)
        if list_num3 and not isinstance(list_num3, list):
            raise TypeError("Expected argument 'list_num3' to be a list")
        khulnasoft.set(__self__, "list_num3", list_num3)
        if list_secret_bool1 and not isinstance(list_secret_bool1, list):
            raise TypeError("Expected argument 'list_secret_bool1' to be a list")
        khulnasoft.set(__self__, "list_secret_bool1", list_secret_bool1)
        if list_secret_bool2 and not isinstance(list_secret_bool2, list):
            raise TypeError("Expected argument 'list_secret_bool2' to be a list")
        khulnasoft.set(__self__, "list_secret_bool2", list_secret_bool2)
        if list_secret_bool3 and not isinstance(list_secret_bool3, list):
            raise TypeError("Expected argument 'list_secret_bool3' to be a list")
        khulnasoft.set(__self__, "list_secret_bool3", list_secret_bool3)
        if list_secret_int1 and not isinstance(list_secret_int1, list):
            raise TypeError("Expected argument 'list_secret_int1' to be a list")
        khulnasoft.set(__self__, "list_secret_int1", list_secret_int1)
        if list_secret_int2 and not isinstance(list_secret_int2, list):
            raise TypeError("Expected argument 'list_secret_int2' to be a list")
        khulnasoft.set(__self__, "list_secret_int2", list_secret_int2)
        if list_secret_int3 and not isinstance(list_secret_int3, list):
            raise TypeError("Expected argument 'list_secret_int3' to be a list")
        khulnasoft.set(__self__, "list_secret_int3", list_secret_int3)
        if list_secret_num1 and not isinstance(list_secret_num1, list):
            raise TypeError("Expected argument 'list_secret_num1' to be a list")
        khulnasoft.set(__self__, "list_secret_num1", list_secret_num1)
        if list_secret_num2 and not isinstance(list_secret_num2, list):
            raise TypeError("Expected argument 'list_secret_num2' to be a list")
        khulnasoft.set(__self__, "list_secret_num2", list_secret_num2)
        if list_secret_num3 and not isinstance(list_secret_num3, list):
            raise TypeError("Expected argument 'list_secret_num3' to be a list")
        khulnasoft.set(__self__, "list_secret_num3", list_secret_num3)
        if list_secret_string1 and not isinstance(list_secret_string1, list):
            raise TypeError("Expected argument 'list_secret_string1' to be a list")
        khulnasoft.set(__self__, "list_secret_string1", list_secret_string1)
        if list_secret_string2 and not isinstance(list_secret_string2, list):
            raise TypeError("Expected argument 'list_secret_string2' to be a list")
        khulnasoft.set(__self__, "list_secret_string2", list_secret_string2)
        if list_secret_string3 and not isinstance(list_secret_string3, list):
            raise TypeError("Expected argument 'list_secret_string3' to be a list")
        khulnasoft.set(__self__, "list_secret_string3", list_secret_string3)
        if list_string1 and not isinstance(list_string1, list):
            raise TypeError("Expected argument 'list_string1' to be a list")
        khulnasoft.set(__self__, "list_string1", list_string1)
        if list_string2 and not isinstance(list_string2, list):
            raise TypeError("Expected argument 'list_string2' to be a list")
        khulnasoft.set(__self__, "list_string2", list_string2)
        if list_string3 and not isinstance(list_string3, list):
            raise TypeError("Expected argument 'list_string3' to be a list")
        khulnasoft.set(__self__, "list_string3", list_string3)
        if map_bool1 and not isinstance(map_bool1, dict):
            raise TypeError("Expected argument 'map_bool1' to be a dict")
        khulnasoft.set(__self__, "map_bool1", map_bool1)
        if map_bool2 and not isinstance(map_bool2, dict):
            raise TypeError("Expected argument 'map_bool2' to be a dict")
        khulnasoft.set(__self__, "map_bool2", map_bool2)
        if map_bool3 and not isinstance(map_bool3, dict):
            raise TypeError("Expected argument 'map_bool3' to be a dict")
        khulnasoft.set(__self__, "map_bool3", map_bool3)
        if map_int1 and not isinstance(map_int1, dict):
            raise TypeError("Expected argument 'map_int1' to be a dict")
        khulnasoft.set(__self__, "map_int1", map_int1)
        if map_int2 and not isinstance(map_int2, dict):
            raise TypeError("Expected argument 'map_int2' to be a dict")
        khulnasoft.set(__self__, "map_int2", map_int2)
        if map_int3 and not isinstance(map_int3, dict):
            raise TypeError("Expected argument 'map_int3' to be a dict")
        khulnasoft.set(__self__, "map_int3", map_int3)
        if map_num1 and not isinstance(map_num1, dict):
            raise TypeError("Expected argument 'map_num1' to be a dict")
        khulnasoft.set(__self__, "map_num1", map_num1)
        if map_num2 and not isinstance(map_num2, dict):
            raise TypeError("Expected argument 'map_num2' to be a dict")
        khulnasoft.set(__self__, "map_num2", map_num2)
        if map_num3 and not isinstance(map_num3, dict):
            raise TypeError("Expected argument 'map_num3' to be a dict")
        khulnasoft.set(__self__, "map_num3", map_num3)
        if map_secret_bool1 and not isinstance(map_secret_bool1, dict):
            raise TypeError("Expected argument 'map_secret_bool1' to be a dict")
        khulnasoft.set(__self__, "map_secret_bool1", map_secret_bool1)
        if map_secret_bool2 and not isinstance(map_secret_bool2, dict):
            raise TypeError("Expected argument 'map_secret_bool2' to be a dict")
        khulnasoft.set(__self__, "map_secret_bool2", map_secret_bool2)
        if map_secret_bool3 and not isinstance(map_secret_bool3, dict):
            raise TypeError("Expected argument 'map_secret_bool3' to be a dict")
        khulnasoft.set(__self__, "map_secret_bool3", map_secret_bool3)
        if map_secret_int1 and not isinstance(map_secret_int1, dict):
            raise TypeError("Expected argument 'map_secret_int1' to be a dict")
        khulnasoft.set(__self__, "map_secret_int1", map_secret_int1)
        if map_secret_int2 and not isinstance(map_secret_int2, dict):
            raise TypeError("Expected argument 'map_secret_int2' to be a dict")
        khulnasoft.set(__self__, "map_secret_int2", map_secret_int2)
        if map_secret_int3 and not isinstance(map_secret_int3, dict):
            raise TypeError("Expected argument 'map_secret_int3' to be a dict")
        khulnasoft.set(__self__, "map_secret_int3", map_secret_int3)
        if map_secret_num1 and not isinstance(map_secret_num1, dict):
            raise TypeError("Expected argument 'map_secret_num1' to be a dict")
        khulnasoft.set(__self__, "map_secret_num1", map_secret_num1)
        if map_secret_num2 and not isinstance(map_secret_num2, dict):
            raise TypeError("Expected argument 'map_secret_num2' to be a dict")
        khulnasoft.set(__self__, "map_secret_num2", map_secret_num2)
        if map_secret_num3 and not isinstance(map_secret_num3, dict):
            raise TypeError("Expected argument 'map_secret_num3' to be a dict")
        khulnasoft.set(__self__, "map_secret_num3", map_secret_num3)
        if map_secret_string1 and not isinstance(map_secret_string1, dict):
            raise TypeError("Expected argument 'map_secret_string1' to be a dict")
        khulnasoft.set(__self__, "map_secret_string1", map_secret_string1)
        if map_secret_string2 and not isinstance(map_secret_string2, dict):
            raise TypeError("Expected argument 'map_secret_string2' to be a dict")
        khulnasoft.set(__self__, "map_secret_string2", map_secret_string2)
        if map_secret_string3 and not isinstance(map_secret_string3, dict):
            raise TypeError("Expected argument 'map_secret_string3' to be a dict")
        khulnasoft.set(__self__, "map_secret_string3", map_secret_string3)
        if map_string1 and not isinstance(map_string1, dict):
            raise TypeError("Expected argument 'map_string1' to be a dict")
        khulnasoft.set(__self__, "map_string1", map_string1)
        if map_string2 and not isinstance(map_string2, dict):
            raise TypeError("Expected argument 'map_string2' to be a dict")
        khulnasoft.set(__self__, "map_string2", map_string2)
        if map_string3 and not isinstance(map_string3, dict):
            raise TypeError("Expected argument 'map_string3' to be a dict")
        khulnasoft.set(__self__, "map_string3", map_string3)
        if num1 and not isinstance(num1, float):
            raise TypeError("Expected argument 'num1' to be a float")
        khulnasoft.set(__self__, "num1", num1)
        if num2 and not isinstance(num2, float):
            raise TypeError("Expected argument 'num2' to be a float")
        khulnasoft.set(__self__, "num2", num2)
        if num3 and not isinstance(num3, float):
            raise TypeError("Expected argument 'num3' to be a float")
        khulnasoft.set(__self__, "num3", num3)
        if obj_bool1 and not isinstance(obj_bool1, dict):
            raise TypeError("Expected argument 'obj_bool1' to be a dict")
        khulnasoft.set(__self__, "obj_bool1", obj_bool1)
        if obj_bool2 and not isinstance(obj_bool2, dict):
            raise TypeError("Expected argument 'obj_bool2' to be a dict")
        khulnasoft.set(__self__, "obj_bool2", obj_bool2)
        if obj_bool3 and not isinstance(obj_bool3, dict):
            raise TypeError("Expected argument 'obj_bool3' to be a dict")
        khulnasoft.set(__self__, "obj_bool3", obj_bool3)
        if obj_int1 and not isinstance(obj_int1, dict):
            raise TypeError("Expected argument 'obj_int1' to be a dict")
        khulnasoft.set(__self__, "obj_int1", obj_int1)
        if obj_int2 and not isinstance(obj_int2, dict):
            raise TypeError("Expected argument 'obj_int2' to be a dict")
        khulnasoft.set(__self__, "obj_int2", obj_int2)
        if obj_int3 and not isinstance(obj_int3, dict):
            raise TypeError("Expected argument 'obj_int3' to be a dict")
        khulnasoft.set(__self__, "obj_int3", obj_int3)
        if obj_num1 and not isinstance(obj_num1, dict):
            raise TypeError("Expected argument 'obj_num1' to be a dict")
        khulnasoft.set(__self__, "obj_num1", obj_num1)
        if obj_num2 and not isinstance(obj_num2, dict):
            raise TypeError("Expected argument 'obj_num2' to be a dict")
        khulnasoft.set(__self__, "obj_num2", obj_num2)
        if obj_num3 and not isinstance(obj_num3, dict):
            raise TypeError("Expected argument 'obj_num3' to be a dict")
        khulnasoft.set(__self__, "obj_num3", obj_num3)
        if obj_secret_bool1 and not isinstance(obj_secret_bool1, dict):
            raise TypeError("Expected argument 'obj_secret_bool1' to be a dict")
        khulnasoft.set(__self__, "obj_secret_bool1", obj_secret_bool1)
        if obj_secret_bool2 and not isinstance(obj_secret_bool2, dict):
            raise TypeError("Expected argument 'obj_secret_bool2' to be a dict")
        khulnasoft.set(__self__, "obj_secret_bool2", obj_secret_bool2)
        if obj_secret_bool3 and not isinstance(obj_secret_bool3, dict):
            raise TypeError("Expected argument 'obj_secret_bool3' to be a dict")
        khulnasoft.set(__self__, "obj_secret_bool3", obj_secret_bool3)
        if obj_secret_int1 and not isinstance(obj_secret_int1, dict):
            raise TypeError("Expected argument 'obj_secret_int1' to be a dict")
        khulnasoft.set(__self__, "obj_secret_int1", obj_secret_int1)
        if obj_secret_int2 and not isinstance(obj_secret_int2, dict):
            raise TypeError("Expected argument 'obj_secret_int2' to be a dict")
        khulnasoft.set(__self__, "obj_secret_int2", obj_secret_int2)
        if obj_secret_int3 and not isinstance(obj_secret_int3, dict):
            raise TypeError("Expected argument 'obj_secret_int3' to be a dict")
        khulnasoft.set(__self__, "obj_secret_int3", obj_secret_int3)
        if obj_secret_num1 and not isinstance(obj_secret_num1, dict):
            raise TypeError("Expected argument 'obj_secret_num1' to be a dict")
        khulnasoft.set(__self__, "obj_secret_num1", obj_secret_num1)
        if obj_secret_num2 and not isinstance(obj_secret_num2, dict):
            raise TypeError("Expected argument 'obj_secret_num2' to be a dict")
        khulnasoft.set(__self__, "obj_secret_num2", obj_secret_num2)
        if obj_secret_num3 and not isinstance(obj_secret_num3, dict):
            raise TypeError("Expected argument 'obj_secret_num3' to be a dict")
        khulnasoft.set(__self__, "obj_secret_num3", obj_secret_num3)
        if obj_secret_string1 and not isinstance(obj_secret_string1, dict):
            raise TypeError("Expected argument 'obj_secret_string1' to be a dict")
        khulnasoft.set(__self__, "obj_secret_string1", obj_secret_string1)
        if obj_secret_string2 and not isinstance(obj_secret_string2, dict):
            raise TypeError("Expected argument 'obj_secret_string2' to be a dict")
        khulnasoft.set(__self__, "obj_secret_string2", obj_secret_string2)
        if obj_secret_string3 and not isinstance(obj_secret_string3, dict):
            raise TypeError("Expected argument 'obj_secret_string3' to be a dict")
        khulnasoft.set(__self__, "obj_secret_string3", obj_secret_string3)
        if obj_string1 and not isinstance(obj_string1, dict):
            raise TypeError("Expected argument 'obj_string1' to be a dict")
        khulnasoft.set(__self__, "obj_string1", obj_string1)
        if obj_string2 and not isinstance(obj_string2, dict):
            raise TypeError("Expected argument 'obj_string2' to be a dict")
        khulnasoft.set(__self__, "obj_string2", obj_string2)
        if obj_string3 and not isinstance(obj_string3, dict):
            raise TypeError("Expected argument 'obj_string3' to be a dict")
        khulnasoft.set(__self__, "obj_string3", obj_string3)
        if secret_bool1 and not isinstance(secret_bool1, bool):
            raise TypeError("Expected argument 'secret_bool1' to be a bool")
        khulnasoft.set(__self__, "secret_bool1", secret_bool1)
        if secret_bool2 and not isinstance(secret_bool2, bool):
            raise TypeError("Expected argument 'secret_bool2' to be a bool")
        khulnasoft.set(__self__, "secret_bool2", secret_bool2)
        if secret_bool3 and not isinstance(secret_bool3, bool):
            raise TypeError("Expected argument 'secret_bool3' to be a bool")
        khulnasoft.set(__self__, "secret_bool3", secret_bool3)
        if secret_int1 and not isinstance(secret_int1, int):
            raise TypeError("Expected argument 'secret_int1' to be a int")
        khulnasoft.set(__self__, "secret_int1", secret_int1)
        if secret_int2 and not isinstance(secret_int2, int):
            raise TypeError("Expected argument 'secret_int2' to be a int")
        khulnasoft.set(__self__, "secret_int2", secret_int2)
        if secret_int3 and not isinstance(secret_int3, int):
            raise TypeError("Expected argument 'secret_int3' to be a int")
        khulnasoft.set(__self__, "secret_int3", secret_int3)
        if secret_num1 and not isinstance(secret_num1, float):
            raise TypeError("Expected argument 'secret_num1' to be a float")
        khulnasoft.set(__self__, "secret_num1", secret_num1)
        if secret_num2 and not isinstance(secret_num2, float):
            raise TypeError("Expected argument 'secret_num2' to be a float")
        khulnasoft.set(__self__, "secret_num2", secret_num2)
        if secret_num3 and not isinstance(secret_num3, float):
            raise TypeError("Expected argument 'secret_num3' to be a float")
        khulnasoft.set(__self__, "secret_num3", secret_num3)
        if secret_string1 and not isinstance(secret_string1, str):
            raise TypeError("Expected argument 'secret_string1' to be a str")
        khulnasoft.set(__self__, "secret_string1", secret_string1)
        if secret_string2 and not isinstance(secret_string2, str):
            raise TypeError("Expected argument 'secret_string2' to be a str")
        khulnasoft.set(__self__, "secret_string2", secret_string2)
        if secret_string3 and not isinstance(secret_string3, str):
            raise TypeError("Expected argument 'secret_string3' to be a str")
        khulnasoft.set(__self__, "secret_string3", secret_string3)
        if string1 and not isinstance(string1, str):
            raise TypeError("Expected argument 'string1' to be a str")
        khulnasoft.set(__self__, "string1", string1)
        if string2 and not isinstance(string2, str):
            raise TypeError("Expected argument 'string2' to be a str")
        khulnasoft.set(__self__, "string2", string2)
        if string3 and not isinstance(string3, str):
            raise TypeError("Expected argument 'string3' to be a str")
        khulnasoft.set(__self__, "string3", string3)

    @property
    @khulnasoft.getter
    def bool1(self) -> bool:
        return khulnasoft.get(self, "bool1")

    @property
    @khulnasoft.getter
    def bool2(self) -> bool:
        return khulnasoft.get(self, "bool2")

    @property
    @khulnasoft.getter
    def bool3(self) -> bool:
        return khulnasoft.get(self, "bool3")

    @property
    @khulnasoft.getter
    def int1(self) -> int:
        return khulnasoft.get(self, "int1")

    @property
    @khulnasoft.getter
    def int2(self) -> int:
        return khulnasoft.get(self, "int2")

    @property
    @khulnasoft.getter
    def int3(self) -> int:
        return khulnasoft.get(self, "int3")

    @property
    @khulnasoft.getter(name="listBool1")
    def list_bool1(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_bool1")

    @property
    @khulnasoft.getter(name="listBool2")
    def list_bool2(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_bool2")

    @property
    @khulnasoft.getter(name="listBool3")
    def list_bool3(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_bool3")

    @property
    @khulnasoft.getter(name="listInt1")
    def list_int1(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_int1")

    @property
    @khulnasoft.getter(name="listInt2")
    def list_int2(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_int2")

    @property
    @khulnasoft.getter(name="listInt3")
    def list_int3(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_int3")

    @property
    @khulnasoft.getter(name="listNum1")
    def list_num1(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_num1")

    @property
    @khulnasoft.getter(name="listNum2")
    def list_num2(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_num2")

    @property
    @khulnasoft.getter(name="listNum3")
    def list_num3(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_num3")

    @property
    @khulnasoft.getter(name="listSecretBool1")
    def list_secret_bool1(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_secret_bool1")

    @property
    @khulnasoft.getter(name="listSecretBool2")
    def list_secret_bool2(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_secret_bool2")

    @property
    @khulnasoft.getter(name="listSecretBool3")
    def list_secret_bool3(self) -> Sequence[bool]:
        return khulnasoft.get(self, "list_secret_bool3")

    @property
    @khulnasoft.getter(name="listSecretInt1")
    def list_secret_int1(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_secret_int1")

    @property
    @khulnasoft.getter(name="listSecretInt2")
    def list_secret_int2(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_secret_int2")

    @property
    @khulnasoft.getter(name="listSecretInt3")
    def list_secret_int3(self) -> Sequence[int]:
        return khulnasoft.get(self, "list_secret_int3")

    @property
    @khulnasoft.getter(name="listSecretNum1")
    def list_secret_num1(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_secret_num1")

    @property
    @khulnasoft.getter(name="listSecretNum2")
    def list_secret_num2(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_secret_num2")

    @property
    @khulnasoft.getter(name="listSecretNum3")
    def list_secret_num3(self) -> Sequence[float]:
        return khulnasoft.get(self, "list_secret_num3")

    @property
    @khulnasoft.getter(name="listSecretString1")
    def list_secret_string1(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_secret_string1")

    @property
    @khulnasoft.getter(name="listSecretString2")
    def list_secret_string2(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_secret_string2")

    @property
    @khulnasoft.getter(name="listSecretString3")
    def list_secret_string3(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_secret_string3")

    @property
    @khulnasoft.getter(name="listString1")
    def list_string1(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_string1")

    @property
    @khulnasoft.getter(name="listString2")
    def list_string2(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_string2")

    @property
    @khulnasoft.getter(name="listString3")
    def list_string3(self) -> Sequence[str]:
        return khulnasoft.get(self, "list_string3")

    @property
    @khulnasoft.getter(name="mapBool1")
    def map_bool1(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_bool1")

    @property
    @khulnasoft.getter(name="mapBool2")
    def map_bool2(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_bool2")

    @property
    @khulnasoft.getter(name="mapBool3")
    def map_bool3(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_bool3")

    @property
    @khulnasoft.getter(name="mapInt1")
    def map_int1(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_int1")

    @property
    @khulnasoft.getter(name="mapInt2")
    def map_int2(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_int2")

    @property
    @khulnasoft.getter(name="mapInt3")
    def map_int3(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_int3")

    @property
    @khulnasoft.getter(name="mapNum1")
    def map_num1(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_num1")

    @property
    @khulnasoft.getter(name="mapNum2")
    def map_num2(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_num2")

    @property
    @khulnasoft.getter(name="mapNum3")
    def map_num3(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_num3")

    @property
    @khulnasoft.getter(name="mapSecretBool1")
    def map_secret_bool1(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_secret_bool1")

    @property
    @khulnasoft.getter(name="mapSecretBool2")
    def map_secret_bool2(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_secret_bool2")

    @property
    @khulnasoft.getter(name="mapSecretBool3")
    def map_secret_bool3(self) -> Mapping[str, bool]:
        return khulnasoft.get(self, "map_secret_bool3")

    @property
    @khulnasoft.getter(name="mapSecretInt1")
    def map_secret_int1(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_secret_int1")

    @property
    @khulnasoft.getter(name="mapSecretInt2")
    def map_secret_int2(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_secret_int2")

    @property
    @khulnasoft.getter(name="mapSecretInt3")
    def map_secret_int3(self) -> Mapping[str, int]:
        return khulnasoft.get(self, "map_secret_int3")

    @property
    @khulnasoft.getter(name="mapSecretNum1")
    def map_secret_num1(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_secret_num1")

    @property
    @khulnasoft.getter(name="mapSecretNum2")
    def map_secret_num2(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_secret_num2")

    @property
    @khulnasoft.getter(name="mapSecretNum3")
    def map_secret_num3(self) -> Mapping[str, float]:
        return khulnasoft.get(self, "map_secret_num3")

    @property
    @khulnasoft.getter(name="mapSecretString1")
    def map_secret_string1(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_secret_string1")

    @property
    @khulnasoft.getter(name="mapSecretString2")
    def map_secret_string2(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_secret_string2")

    @property
    @khulnasoft.getter(name="mapSecretString3")
    def map_secret_string3(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_secret_string3")

    @property
    @khulnasoft.getter(name="mapString1")
    def map_string1(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_string1")

    @property
    @khulnasoft.getter(name="mapString2")
    def map_string2(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_string2")

    @property
    @khulnasoft.getter(name="mapString3")
    def map_string3(self) -> Mapping[str, str]:
        return khulnasoft.get(self, "map_string3")

    @property
    @khulnasoft.getter
    def num1(self) -> float:
        return khulnasoft.get(self, "num1")

    @property
    @khulnasoft.getter
    def num2(self) -> float:
        return khulnasoft.get(self, "num2")

    @property
    @khulnasoft.getter
    def num3(self) -> float:
        return khulnasoft.get(self, "num3")

    @property
    @khulnasoft.getter(name="objBool1")
    def obj_bool1(self) -> 'outputs.Tbool1':
        return khulnasoft.get(self, "obj_bool1")

    @property
    @khulnasoft.getter(name="objBool2")
    def obj_bool2(self) -> 'outputs.Tbool2':
        return khulnasoft.get(self, "obj_bool2")

    @property
    @khulnasoft.getter(name="objBool3")
    def obj_bool3(self) -> 'outputs.Tbool3':
        return khulnasoft.get(self, "obj_bool3")

    @property
    @khulnasoft.getter(name="objInt1")
    def obj_int1(self) -> 'outputs.Tint1':
        return khulnasoft.get(self, "obj_int1")

    @property
    @khulnasoft.getter(name="objInt2")
    def obj_int2(self) -> 'outputs.Tint2':
        return khulnasoft.get(self, "obj_int2")

    @property
    @khulnasoft.getter(name="objInt3")
    def obj_int3(self) -> 'outputs.Tint3':
        return khulnasoft.get(self, "obj_int3")

    @property
    @khulnasoft.getter(name="objNum1")
    def obj_num1(self) -> 'outputs.Tnum1':
        return khulnasoft.get(self, "obj_num1")

    @property
    @khulnasoft.getter(name="objNum2")
    def obj_num2(self) -> 'outputs.Tnum2':
        return khulnasoft.get(self, "obj_num2")

    @property
    @khulnasoft.getter(name="objNum3")
    def obj_num3(self) -> 'outputs.Tnum3':
        return khulnasoft.get(self, "obj_num3")

    @property
    @khulnasoft.getter(name="objSecretBool1")
    def obj_secret_bool1(self) -> 'outputs.TsecretBool1':
        return khulnasoft.get(self, "obj_secret_bool1")

    @property
    @khulnasoft.getter(name="objSecretBool2")
    def obj_secret_bool2(self) -> 'outputs.TsecretBool2':
        return khulnasoft.get(self, "obj_secret_bool2")

    @property
    @khulnasoft.getter(name="objSecretBool3")
    def obj_secret_bool3(self) -> 'outputs.TsecretBool3':
        return khulnasoft.get(self, "obj_secret_bool3")

    @property
    @khulnasoft.getter(name="objSecretInt1")
    def obj_secret_int1(self) -> 'outputs.TsecretInt1':
        return khulnasoft.get(self, "obj_secret_int1")

    @property
    @khulnasoft.getter(name="objSecretInt2")
    def obj_secret_int2(self) -> 'outputs.TsecretInt2':
        return khulnasoft.get(self, "obj_secret_int2")

    @property
    @khulnasoft.getter(name="objSecretInt3")
    def obj_secret_int3(self) -> 'outputs.TsecretInt3':
        return khulnasoft.get(self, "obj_secret_int3")

    @property
    @khulnasoft.getter(name="objSecretNum1")
    def obj_secret_num1(self) -> 'outputs.TsecretNum1':
        return khulnasoft.get(self, "obj_secret_num1")

    @property
    @khulnasoft.getter(name="objSecretNum2")
    def obj_secret_num2(self) -> 'outputs.TsecretNum2':
        return khulnasoft.get(self, "obj_secret_num2")

    @property
    @khulnasoft.getter(name="objSecretNum3")
    def obj_secret_num3(self) -> 'outputs.TsecretNum3':
        return khulnasoft.get(self, "obj_secret_num3")

    @property
    @khulnasoft.getter(name="objSecretString1")
    def obj_secret_string1(self) -> 'outputs.TsecretString1':
        return khulnasoft.get(self, "obj_secret_string1")

    @property
    @khulnasoft.getter(name="objSecretString2")
    def obj_secret_string2(self) -> 'outputs.TsecretString2':
        return khulnasoft.get(self, "obj_secret_string2")

    @property
    @khulnasoft.getter(name="objSecretString3")
    def obj_secret_string3(self) -> 'outputs.TsecretString3':
        return khulnasoft.get(self, "obj_secret_string3")

    @property
    @khulnasoft.getter(name="objString1")
    def obj_string1(self) -> 'outputs.Tstring1':
        return khulnasoft.get(self, "obj_string1")

    @property
    @khulnasoft.getter(name="objString2")
    def obj_string2(self) -> 'outputs.Tstring2':
        return khulnasoft.get(self, "obj_string2")

    @property
    @khulnasoft.getter(name="objString3")
    def obj_string3(self) -> 'outputs.Tstring3':
        return khulnasoft.get(self, "obj_string3")

    @property
    @khulnasoft.getter(name="secretBool1")
    def secret_bool1(self) -> bool:
        return khulnasoft.get(self, "secret_bool1")

    @property
    @khulnasoft.getter(name="secretBool2")
    def secret_bool2(self) -> bool:
        return khulnasoft.get(self, "secret_bool2")

    @property
    @khulnasoft.getter(name="secretBool3")
    def secret_bool3(self) -> bool:
        return khulnasoft.get(self, "secret_bool3")

    @property
    @khulnasoft.getter(name="secretInt1")
    def secret_int1(self) -> int:
        return khulnasoft.get(self, "secret_int1")

    @property
    @khulnasoft.getter(name="secretInt2")
    def secret_int2(self) -> int:
        return khulnasoft.get(self, "secret_int2")

    @property
    @khulnasoft.getter(name="secretInt3")
    def secret_int3(self) -> int:
        return khulnasoft.get(self, "secret_int3")

    @property
    @khulnasoft.getter(name="secretNum1")
    def secret_num1(self) -> float:
        return khulnasoft.get(self, "secret_num1")

    @property
    @khulnasoft.getter(name="secretNum2")
    def secret_num2(self) -> float:
        return khulnasoft.get(self, "secret_num2")

    @property
    @khulnasoft.getter(name="secretNum3")
    def secret_num3(self) -> float:
        return khulnasoft.get(self, "secret_num3")

    @property
    @khulnasoft.getter(name="secretString1")
    def secret_string1(self) -> str:
        return khulnasoft.get(self, "secret_string1")

    @property
    @khulnasoft.getter(name="secretString2")
    def secret_string2(self) -> str:
        return khulnasoft.get(self, "secret_string2")

    @property
    @khulnasoft.getter(name="secretString3")
    def secret_string3(self) -> str:
        return khulnasoft.get(self, "secret_string3")

    @property
    @khulnasoft.getter
    def string1(self) -> str:
        return khulnasoft.get(self, "string1")

    @property
    @khulnasoft.getter
    def string2(self) -> str:
        return khulnasoft.get(self, "string2")

    @property
    @khulnasoft.getter
    def string3(self) -> str:
        return khulnasoft.get(self, "string3")


class AwaitableToSecretResult(ToSecretResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return ToSecretResult(
            bool1=self.bool1,
            bool2=self.bool2,
            bool3=self.bool3,
            int1=self.int1,
            int2=self.int2,
            int3=self.int3,
            list_bool1=self.list_bool1,
            list_bool2=self.list_bool2,
            list_bool3=self.list_bool3,
            list_int1=self.list_int1,
            list_int2=self.list_int2,
            list_int3=self.list_int3,
            list_num1=self.list_num1,
            list_num2=self.list_num2,
            list_num3=self.list_num3,
            list_secret_bool1=self.list_secret_bool1,
            list_secret_bool2=self.list_secret_bool2,
            list_secret_bool3=self.list_secret_bool3,
            list_secret_int1=self.list_secret_int1,
            list_secret_int2=self.list_secret_int2,
            list_secret_int3=self.list_secret_int3,
            list_secret_num1=self.list_secret_num1,
            list_secret_num2=self.list_secret_num2,
            list_secret_num3=self.list_secret_num3,
            list_secret_string1=self.list_secret_string1,
            list_secret_string2=self.list_secret_string2,
            list_secret_string3=self.list_secret_string3,
            list_string1=self.list_string1,
            list_string2=self.list_string2,
            list_string3=self.list_string3,
            map_bool1=self.map_bool1,
            map_bool2=self.map_bool2,
            map_bool3=self.map_bool3,
            map_int1=self.map_int1,
            map_int2=self.map_int2,
            map_int3=self.map_int3,
            map_num1=self.map_num1,
            map_num2=self.map_num2,
            map_num3=self.map_num3,
            map_secret_bool1=self.map_secret_bool1,
            map_secret_bool2=self.map_secret_bool2,
            map_secret_bool3=self.map_secret_bool3,
            map_secret_int1=self.map_secret_int1,
            map_secret_int2=self.map_secret_int2,
            map_secret_int3=self.map_secret_int3,
            map_secret_num1=self.map_secret_num1,
            map_secret_num2=self.map_secret_num2,
            map_secret_num3=self.map_secret_num3,
            map_secret_string1=self.map_secret_string1,
            map_secret_string2=self.map_secret_string2,
            map_secret_string3=self.map_secret_string3,
            map_string1=self.map_string1,
            map_string2=self.map_string2,
            map_string3=self.map_string3,
            num1=self.num1,
            num2=self.num2,
            num3=self.num3,
            obj_bool1=self.obj_bool1,
            obj_bool2=self.obj_bool2,
            obj_bool3=self.obj_bool3,
            obj_int1=self.obj_int1,
            obj_int2=self.obj_int2,
            obj_int3=self.obj_int3,
            obj_num1=self.obj_num1,
            obj_num2=self.obj_num2,
            obj_num3=self.obj_num3,
            obj_secret_bool1=self.obj_secret_bool1,
            obj_secret_bool2=self.obj_secret_bool2,
            obj_secret_bool3=self.obj_secret_bool3,
            obj_secret_int1=self.obj_secret_int1,
            obj_secret_int2=self.obj_secret_int2,
            obj_secret_int3=self.obj_secret_int3,
            obj_secret_num1=self.obj_secret_num1,
            obj_secret_num2=self.obj_secret_num2,
            obj_secret_num3=self.obj_secret_num3,
            obj_secret_string1=self.obj_secret_string1,
            obj_secret_string2=self.obj_secret_string2,
            obj_secret_string3=self.obj_secret_string3,
            obj_string1=self.obj_string1,
            obj_string2=self.obj_string2,
            obj_string3=self.obj_string3,
            secret_bool1=self.secret_bool1,
            secret_bool2=self.secret_bool2,
            secret_bool3=self.secret_bool3,
            secret_int1=self.secret_int1,
            secret_int2=self.secret_int2,
            secret_int3=self.secret_int3,
            secret_num1=self.secret_num1,
            secret_num2=self.secret_num2,
            secret_num3=self.secret_num3,
            secret_string1=self.secret_string1,
            secret_string2=self.secret_string2,
            secret_string3=self.secret_string3,
            string1=self.string1,
            string2=self.string2,
            string3=self.string3)


def to_secret(bool1: Optional[bool] = None,
              bool2: Optional[bool] = None,
              bool3: Optional[bool] = None,
              int1: Optional[int] = None,
              int2: Optional[int] = None,
              int3: Optional[int] = None,
              list_bool1: Optional[Sequence[bool]] = None,
              list_bool2: Optional[Sequence[bool]] = None,
              list_bool3: Optional[Sequence[bool]] = None,
              list_int1: Optional[Sequence[int]] = None,
              list_int2: Optional[Sequence[int]] = None,
              list_int3: Optional[Sequence[int]] = None,
              list_num1: Optional[Sequence[float]] = None,
              list_num2: Optional[Sequence[float]] = None,
              list_num3: Optional[Sequence[float]] = None,
              list_secret_bool1: Optional[Sequence[bool]] = None,
              list_secret_bool2: Optional[Sequence[bool]] = None,
              list_secret_bool3: Optional[Sequence[bool]] = None,
              list_secret_int1: Optional[Sequence[int]] = None,
              list_secret_int2: Optional[Sequence[int]] = None,
              list_secret_int3: Optional[Sequence[int]] = None,
              list_secret_num1: Optional[Sequence[float]] = None,
              list_secret_num2: Optional[Sequence[float]] = None,
              list_secret_num3: Optional[Sequence[float]] = None,
              list_secret_string1: Optional[Sequence[str]] = None,
              list_secret_string2: Optional[Sequence[str]] = None,
              list_secret_string3: Optional[Sequence[str]] = None,
              list_string1: Optional[Sequence[str]] = None,
              list_string2: Optional[Sequence[str]] = None,
              list_string3: Optional[Sequence[str]] = None,
              map_bool1: Optional[Mapping[str, bool]] = None,
              map_bool2: Optional[Mapping[str, bool]] = None,
              map_bool3: Optional[Mapping[str, bool]] = None,
              map_int1: Optional[Mapping[str, int]] = None,
              map_int2: Optional[Mapping[str, int]] = None,
              map_int3: Optional[Mapping[str, int]] = None,
              map_num1: Optional[Mapping[str, float]] = None,
              map_num2: Optional[Mapping[str, float]] = None,
              map_num3: Optional[Mapping[str, float]] = None,
              map_secret_bool1: Optional[Mapping[str, bool]] = None,
              map_secret_bool2: Optional[Mapping[str, bool]] = None,
              map_secret_bool3: Optional[Mapping[str, bool]] = None,
              map_secret_int1: Optional[Mapping[str, int]] = None,
              map_secret_int2: Optional[Mapping[str, int]] = None,
              map_secret_int3: Optional[Mapping[str, int]] = None,
              map_secret_num1: Optional[Mapping[str, float]] = None,
              map_secret_num2: Optional[Mapping[str, float]] = None,
              map_secret_num3: Optional[Mapping[str, float]] = None,
              map_secret_string1: Optional[Mapping[str, str]] = None,
              map_secret_string2: Optional[Mapping[str, str]] = None,
              map_secret_string3: Optional[Mapping[str, str]] = None,
              map_string1: Optional[Mapping[str, str]] = None,
              map_string2: Optional[Mapping[str, str]] = None,
              map_string3: Optional[Mapping[str, str]] = None,
              num1: Optional[float] = None,
              num2: Optional[float] = None,
              num3: Optional[float] = None,
              obj_bool1: Optional[khulnasoft.InputType['Tbool1']] = None,
              obj_bool2: Optional[khulnasoft.InputType['Tbool2']] = None,
              obj_bool3: Optional[khulnasoft.InputType['Tbool3']] = None,
              obj_int1: Optional[khulnasoft.InputType['Tint1']] = None,
              obj_int2: Optional[khulnasoft.InputType['Tint2']] = None,
              obj_int3: Optional[khulnasoft.InputType['Tint3']] = None,
              obj_num1: Optional[khulnasoft.InputType['Tnum1']] = None,
              obj_num2: Optional[khulnasoft.InputType['Tnum2']] = None,
              obj_num3: Optional[khulnasoft.InputType['Tnum3']] = None,
              obj_secret_bool1: Optional[khulnasoft.InputType['TsecretBool1']] = None,
              obj_secret_bool2: Optional[khulnasoft.InputType['TsecretBool2']] = None,
              obj_secret_bool3: Optional[khulnasoft.InputType['TsecretBool3']] = None,
              obj_secret_int1: Optional[khulnasoft.InputType['TsecretInt1']] = None,
              obj_secret_int2: Optional[khulnasoft.InputType['TsecretInt2']] = None,
              obj_secret_int3: Optional[khulnasoft.InputType['TsecretInt3']] = None,
              obj_secret_num1: Optional[khulnasoft.InputType['TsecretNum1']] = None,
              obj_secret_num2: Optional[khulnasoft.InputType['TsecretNum2']] = None,
              obj_secret_num3: Optional[khulnasoft.InputType['TsecretNum3']] = None,
              obj_secret_string1: Optional[khulnasoft.InputType['TsecretString1']] = None,
              obj_secret_string2: Optional[khulnasoft.InputType['TsecretString2']] = None,
              obj_secret_string3: Optional[khulnasoft.InputType['TsecretString3']] = None,
              obj_string1: Optional[khulnasoft.InputType['Tstring1']] = None,
              obj_string2: Optional[khulnasoft.InputType['Tstring2']] = None,
              obj_string3: Optional[khulnasoft.InputType['Tstring3']] = None,
              secret_bool1: Optional[bool] = None,
              secret_bool2: Optional[bool] = None,
              secret_bool3: Optional[bool] = None,
              secret_int1: Optional[int] = None,
              secret_int2: Optional[int] = None,
              secret_int3: Optional[int] = None,
              secret_num1: Optional[float] = None,
              secret_num2: Optional[float] = None,
              secret_num3: Optional[float] = None,
              secret_string1: Optional[str] = None,
              secret_string2: Optional[str] = None,
              secret_string3: Optional[str] = None,
              string1: Optional[str] = None,
              string2: Optional[str] = None,
              string3: Optional[str] = None,
              opts: Optional[khulnasoft.InvokeOptions] = None) -> AwaitableToSecretResult:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['bool1'] = bool1
    __args__['bool2'] = bool2
    __args__['bool3'] = bool3
    __args__['int1'] = int1
    __args__['int2'] = int2
    __args__['int3'] = int3
    __args__['listBool1'] = list_bool1
    __args__['listBool2'] = list_bool2
    __args__['listBool3'] = list_bool3
    __args__['listInt1'] = list_int1
    __args__['listInt2'] = list_int2
    __args__['listInt3'] = list_int3
    __args__['listNum1'] = list_num1
    __args__['listNum2'] = list_num2
    __args__['listNum3'] = list_num3
    __args__['listSecretBool1'] = list_secret_bool1
    __args__['listSecretBool2'] = list_secret_bool2
    __args__['listSecretBool3'] = list_secret_bool3
    __args__['listSecretInt1'] = list_secret_int1
    __args__['listSecretInt2'] = list_secret_int2
    __args__['listSecretInt3'] = list_secret_int3
    __args__['listSecretNum1'] = list_secret_num1
    __args__['listSecretNum2'] = list_secret_num2
    __args__['listSecretNum3'] = list_secret_num3
    __args__['listSecretString1'] = list_secret_string1
    __args__['listSecretString2'] = list_secret_string2
    __args__['listSecretString3'] = list_secret_string3
    __args__['listString1'] = list_string1
    __args__['listString2'] = list_string2
    __args__['listString3'] = list_string3
    __args__['mapBool1'] = map_bool1
    __args__['mapBool2'] = map_bool2
    __args__['mapBool3'] = map_bool3
    __args__['mapInt1'] = map_int1
    __args__['mapInt2'] = map_int2
    __args__['mapInt3'] = map_int3
    __args__['mapNum1'] = map_num1
    __args__['mapNum2'] = map_num2
    __args__['mapNum3'] = map_num3
    __args__['mapSecretBool1'] = map_secret_bool1
    __args__['mapSecretBool2'] = map_secret_bool2
    __args__['mapSecretBool3'] = map_secret_bool3
    __args__['mapSecretInt1'] = map_secret_int1
    __args__['mapSecretInt2'] = map_secret_int2
    __args__['mapSecretInt3'] = map_secret_int3
    __args__['mapSecretNum1'] = map_secret_num1
    __args__['mapSecretNum2'] = map_secret_num2
    __args__['mapSecretNum3'] = map_secret_num3
    __args__['mapSecretString1'] = map_secret_string1
    __args__['mapSecretString2'] = map_secret_string2
    __args__['mapSecretString3'] = map_secret_string3
    __args__['mapString1'] = map_string1
    __args__['mapString2'] = map_string2
    __args__['mapString3'] = map_string3
    __args__['num1'] = num1
    __args__['num2'] = num2
    __args__['num3'] = num3
    __args__['objBool1'] = obj_bool1
    __args__['objBool2'] = obj_bool2
    __args__['objBool3'] = obj_bool3
    __args__['objInt1'] = obj_int1
    __args__['objInt2'] = obj_int2
    __args__['objInt3'] = obj_int3
    __args__['objNum1'] = obj_num1
    __args__['objNum2'] = obj_num2
    __args__['objNum3'] = obj_num3
    __args__['objSecretBool1'] = obj_secret_bool1
    __args__['objSecretBool2'] = obj_secret_bool2
    __args__['objSecretBool3'] = obj_secret_bool3
    __args__['objSecretInt1'] = obj_secret_int1
    __args__['objSecretInt2'] = obj_secret_int2
    __args__['objSecretInt3'] = obj_secret_int3
    __args__['objSecretNum1'] = obj_secret_num1
    __args__['objSecretNum2'] = obj_secret_num2
    __args__['objSecretNum3'] = obj_secret_num3
    __args__['objSecretString1'] = obj_secret_string1
    __args__['objSecretString2'] = obj_secret_string2
    __args__['objSecretString3'] = obj_secret_string3
    __args__['objString1'] = obj_string1
    __args__['objString2'] = obj_string2
    __args__['objString3'] = obj_string3
    __args__['secretBool1'] = secret_bool1
    __args__['secretBool2'] = secret_bool2
    __args__['secretBool3'] = secret_bool3
    __args__['secretInt1'] = secret_int1
    __args__['secretInt2'] = secret_int2
    __args__['secretInt3'] = secret_int3
    __args__['secretNum1'] = secret_num1
    __args__['secretNum2'] = secret_num2
    __args__['secretNum3'] = secret_num3
    __args__['secretString1'] = secret_string1
    __args__['secretString2'] = secret_string2
    __args__['secretString3'] = secret_string3
    __args__['string1'] = string1
    __args__['string2'] = string2
    __args__['string3'] = string3
    opts = khulnasoft.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = khulnasoft.runtime.invoke('config-grpc:index:toSecret', __args__, opts=opts, typ=ToSecretResult).value

    return AwaitableToSecretResult(
        bool1=khulnasoft.get(__ret__, 'bool1'),
        bool2=khulnasoft.get(__ret__, 'bool2'),
        bool3=khulnasoft.get(__ret__, 'bool3'),
        int1=khulnasoft.get(__ret__, 'int1'),
        int2=khulnasoft.get(__ret__, 'int2'),
        int3=khulnasoft.get(__ret__, 'int3'),
        list_bool1=khulnasoft.get(__ret__, 'list_bool1'),
        list_bool2=khulnasoft.get(__ret__, 'list_bool2'),
        list_bool3=khulnasoft.get(__ret__, 'list_bool3'),
        list_int1=khulnasoft.get(__ret__, 'list_int1'),
        list_int2=khulnasoft.get(__ret__, 'list_int2'),
        list_int3=khulnasoft.get(__ret__, 'list_int3'),
        list_num1=khulnasoft.get(__ret__, 'list_num1'),
        list_num2=khulnasoft.get(__ret__, 'list_num2'),
        list_num3=khulnasoft.get(__ret__, 'list_num3'),
        list_secret_bool1=khulnasoft.get(__ret__, 'list_secret_bool1'),
        list_secret_bool2=khulnasoft.get(__ret__, 'list_secret_bool2'),
        list_secret_bool3=khulnasoft.get(__ret__, 'list_secret_bool3'),
        list_secret_int1=khulnasoft.get(__ret__, 'list_secret_int1'),
        list_secret_int2=khulnasoft.get(__ret__, 'list_secret_int2'),
        list_secret_int3=khulnasoft.get(__ret__, 'list_secret_int3'),
        list_secret_num1=khulnasoft.get(__ret__, 'list_secret_num1'),
        list_secret_num2=khulnasoft.get(__ret__, 'list_secret_num2'),
        list_secret_num3=khulnasoft.get(__ret__, 'list_secret_num3'),
        list_secret_string1=khulnasoft.get(__ret__, 'list_secret_string1'),
        list_secret_string2=khulnasoft.get(__ret__, 'list_secret_string2'),
        list_secret_string3=khulnasoft.get(__ret__, 'list_secret_string3'),
        list_string1=khulnasoft.get(__ret__, 'list_string1'),
        list_string2=khulnasoft.get(__ret__, 'list_string2'),
        list_string3=khulnasoft.get(__ret__, 'list_string3'),
        map_bool1=khulnasoft.get(__ret__, 'map_bool1'),
        map_bool2=khulnasoft.get(__ret__, 'map_bool2'),
        map_bool3=khulnasoft.get(__ret__, 'map_bool3'),
        map_int1=khulnasoft.get(__ret__, 'map_int1'),
        map_int2=khulnasoft.get(__ret__, 'map_int2'),
        map_int3=khulnasoft.get(__ret__, 'map_int3'),
        map_num1=khulnasoft.get(__ret__, 'map_num1'),
        map_num2=khulnasoft.get(__ret__, 'map_num2'),
        map_num3=khulnasoft.get(__ret__, 'map_num3'),
        map_secret_bool1=khulnasoft.get(__ret__, 'map_secret_bool1'),
        map_secret_bool2=khulnasoft.get(__ret__, 'map_secret_bool2'),
        map_secret_bool3=khulnasoft.get(__ret__, 'map_secret_bool3'),
        map_secret_int1=khulnasoft.get(__ret__, 'map_secret_int1'),
        map_secret_int2=khulnasoft.get(__ret__, 'map_secret_int2'),
        map_secret_int3=khulnasoft.get(__ret__, 'map_secret_int3'),
        map_secret_num1=khulnasoft.get(__ret__, 'map_secret_num1'),
        map_secret_num2=khulnasoft.get(__ret__, 'map_secret_num2'),
        map_secret_num3=khulnasoft.get(__ret__, 'map_secret_num3'),
        map_secret_string1=khulnasoft.get(__ret__, 'map_secret_string1'),
        map_secret_string2=khulnasoft.get(__ret__, 'map_secret_string2'),
        map_secret_string3=khulnasoft.get(__ret__, 'map_secret_string3'),
        map_string1=khulnasoft.get(__ret__, 'map_string1'),
        map_string2=khulnasoft.get(__ret__, 'map_string2'),
        map_string3=khulnasoft.get(__ret__, 'map_string3'),
        num1=khulnasoft.get(__ret__, 'num1'),
        num2=khulnasoft.get(__ret__, 'num2'),
        num3=khulnasoft.get(__ret__, 'num3'),
        obj_bool1=khulnasoft.get(__ret__, 'obj_bool1'),
        obj_bool2=khulnasoft.get(__ret__, 'obj_bool2'),
        obj_bool3=khulnasoft.get(__ret__, 'obj_bool3'),
        obj_int1=khulnasoft.get(__ret__, 'obj_int1'),
        obj_int2=khulnasoft.get(__ret__, 'obj_int2'),
        obj_int3=khulnasoft.get(__ret__, 'obj_int3'),
        obj_num1=khulnasoft.get(__ret__, 'obj_num1'),
        obj_num2=khulnasoft.get(__ret__, 'obj_num2'),
        obj_num3=khulnasoft.get(__ret__, 'obj_num3'),
        obj_secret_bool1=khulnasoft.get(__ret__, 'obj_secret_bool1'),
        obj_secret_bool2=khulnasoft.get(__ret__, 'obj_secret_bool2'),
        obj_secret_bool3=khulnasoft.get(__ret__, 'obj_secret_bool3'),
        obj_secret_int1=khulnasoft.get(__ret__, 'obj_secret_int1'),
        obj_secret_int2=khulnasoft.get(__ret__, 'obj_secret_int2'),
        obj_secret_int3=khulnasoft.get(__ret__, 'obj_secret_int3'),
        obj_secret_num1=khulnasoft.get(__ret__, 'obj_secret_num1'),
        obj_secret_num2=khulnasoft.get(__ret__, 'obj_secret_num2'),
        obj_secret_num3=khulnasoft.get(__ret__, 'obj_secret_num3'),
        obj_secret_string1=khulnasoft.get(__ret__, 'obj_secret_string1'),
        obj_secret_string2=khulnasoft.get(__ret__, 'obj_secret_string2'),
        obj_secret_string3=khulnasoft.get(__ret__, 'obj_secret_string3'),
        obj_string1=khulnasoft.get(__ret__, 'obj_string1'),
        obj_string2=khulnasoft.get(__ret__, 'obj_string2'),
        obj_string3=khulnasoft.get(__ret__, 'obj_string3'),
        secret_bool1=khulnasoft.get(__ret__, 'secret_bool1'),
        secret_bool2=khulnasoft.get(__ret__, 'secret_bool2'),
        secret_bool3=khulnasoft.get(__ret__, 'secret_bool3'),
        secret_int1=khulnasoft.get(__ret__, 'secret_int1'),
        secret_int2=khulnasoft.get(__ret__, 'secret_int2'),
        secret_int3=khulnasoft.get(__ret__, 'secret_int3'),
        secret_num1=khulnasoft.get(__ret__, 'secret_num1'),
        secret_num2=khulnasoft.get(__ret__, 'secret_num2'),
        secret_num3=khulnasoft.get(__ret__, 'secret_num3'),
        secret_string1=khulnasoft.get(__ret__, 'secret_string1'),
        secret_string2=khulnasoft.get(__ret__, 'secret_string2'),
        secret_string3=khulnasoft.get(__ret__, 'secret_string3'),
        string1=khulnasoft.get(__ret__, 'string1'),
        string2=khulnasoft.get(__ret__, 'string2'),
        string3=khulnasoft.get(__ret__, 'string3'))
def to_secret_output(bool1: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     bool2: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     bool3: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     int1: Optional[khulnasoft.Input[Optional[int]]] = None,
                     int2: Optional[khulnasoft.Input[Optional[int]]] = None,
                     int3: Optional[khulnasoft.Input[Optional[int]]] = None,
                     list_bool1: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_bool2: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_bool3: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_int1: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_int2: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_int3: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_num1: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_num2: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_num3: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_secret_bool1: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_secret_bool2: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_secret_bool3: Optional[khulnasoft.Input[Optional[Sequence[bool]]]] = None,
                     list_secret_int1: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_secret_int2: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_secret_int3: Optional[khulnasoft.Input[Optional[Sequence[int]]]] = None,
                     list_secret_num1: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_secret_num2: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_secret_num3: Optional[khulnasoft.Input[Optional[Sequence[float]]]] = None,
                     list_secret_string1: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     list_secret_string2: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     list_secret_string3: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     list_string1: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     list_string2: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     list_string3: Optional[khulnasoft.Input[Optional[Sequence[str]]]] = None,
                     map_bool1: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_bool2: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_bool3: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_int1: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_int2: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_int3: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_num1: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_num2: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_num3: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_secret_bool1: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_secret_bool2: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_secret_bool3: Optional[khulnasoft.Input[Optional[Mapping[str, bool]]]] = None,
                     map_secret_int1: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_secret_int2: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_secret_int3: Optional[khulnasoft.Input[Optional[Mapping[str, int]]]] = None,
                     map_secret_num1: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_secret_num2: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_secret_num3: Optional[khulnasoft.Input[Optional[Mapping[str, float]]]] = None,
                     map_secret_string1: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     map_secret_string2: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     map_secret_string3: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     map_string1: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     map_string2: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     map_string3: Optional[khulnasoft.Input[Optional[Mapping[str, str]]]] = None,
                     num1: Optional[khulnasoft.Input[Optional[float]]] = None,
                     num2: Optional[khulnasoft.Input[Optional[float]]] = None,
                     num3: Optional[khulnasoft.Input[Optional[float]]] = None,
                     obj_bool1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tbool1']]]] = None,
                     obj_bool2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tbool2']]]] = None,
                     obj_bool3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tbool3']]]] = None,
                     obj_int1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tint1']]]] = None,
                     obj_int2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tint2']]]] = None,
                     obj_int3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tint3']]]] = None,
                     obj_num1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tnum1']]]] = None,
                     obj_num2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tnum2']]]] = None,
                     obj_num3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tnum3']]]] = None,
                     obj_secret_bool1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretBool1']]]] = None,
                     obj_secret_bool2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretBool2']]]] = None,
                     obj_secret_bool3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretBool3']]]] = None,
                     obj_secret_int1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretInt1']]]] = None,
                     obj_secret_int2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretInt2']]]] = None,
                     obj_secret_int3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretInt3']]]] = None,
                     obj_secret_num1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretNum1']]]] = None,
                     obj_secret_num2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretNum2']]]] = None,
                     obj_secret_num3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretNum3']]]] = None,
                     obj_secret_string1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretString1']]]] = None,
                     obj_secret_string2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretString2']]]] = None,
                     obj_secret_string3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['TsecretString3']]]] = None,
                     obj_string1: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tstring1']]]] = None,
                     obj_string2: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tstring2']]]] = None,
                     obj_string3: Optional[khulnasoft.Input[Optional[khulnasoft.InputType['Tstring3']]]] = None,
                     secret_bool1: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     secret_bool2: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     secret_bool3: Optional[khulnasoft.Input[Optional[bool]]] = None,
                     secret_int1: Optional[khulnasoft.Input[Optional[int]]] = None,
                     secret_int2: Optional[khulnasoft.Input[Optional[int]]] = None,
                     secret_int3: Optional[khulnasoft.Input[Optional[int]]] = None,
                     secret_num1: Optional[khulnasoft.Input[Optional[float]]] = None,
                     secret_num2: Optional[khulnasoft.Input[Optional[float]]] = None,
                     secret_num3: Optional[khulnasoft.Input[Optional[float]]] = None,
                     secret_string1: Optional[khulnasoft.Input[Optional[str]]] = None,
                     secret_string2: Optional[khulnasoft.Input[Optional[str]]] = None,
                     secret_string3: Optional[khulnasoft.Input[Optional[str]]] = None,
                     string1: Optional[khulnasoft.Input[Optional[str]]] = None,
                     string2: Optional[khulnasoft.Input[Optional[str]]] = None,
                     string3: Optional[khulnasoft.Input[Optional[str]]] = None,
                     opts: Optional[khulnasoft.InvokeOptions] = None) -> khulnasoft.Output[ToSecretResult]:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['bool1'] = bool1
    __args__['bool2'] = bool2
    __args__['bool3'] = bool3
    __args__['int1'] = int1
    __args__['int2'] = int2
    __args__['int3'] = int3
    __args__['listBool1'] = list_bool1
    __args__['listBool2'] = list_bool2
    __args__['listBool3'] = list_bool3
    __args__['listInt1'] = list_int1
    __args__['listInt2'] = list_int2
    __args__['listInt3'] = list_int3
    __args__['listNum1'] = list_num1
    __args__['listNum2'] = list_num2
    __args__['listNum3'] = list_num3
    __args__['listSecretBool1'] = list_secret_bool1
    __args__['listSecretBool2'] = list_secret_bool2
    __args__['listSecretBool3'] = list_secret_bool3
    __args__['listSecretInt1'] = list_secret_int1
    __args__['listSecretInt2'] = list_secret_int2
    __args__['listSecretInt3'] = list_secret_int3
    __args__['listSecretNum1'] = list_secret_num1
    __args__['listSecretNum2'] = list_secret_num2
    __args__['listSecretNum3'] = list_secret_num3
    __args__['listSecretString1'] = list_secret_string1
    __args__['listSecretString2'] = list_secret_string2
    __args__['listSecretString3'] = list_secret_string3
    __args__['listString1'] = list_string1
    __args__['listString2'] = list_string2
    __args__['listString3'] = list_string3
    __args__['mapBool1'] = map_bool1
    __args__['mapBool2'] = map_bool2
    __args__['mapBool3'] = map_bool3
    __args__['mapInt1'] = map_int1
    __args__['mapInt2'] = map_int2
    __args__['mapInt3'] = map_int3
    __args__['mapNum1'] = map_num1
    __args__['mapNum2'] = map_num2
    __args__['mapNum3'] = map_num3
    __args__['mapSecretBool1'] = map_secret_bool1
    __args__['mapSecretBool2'] = map_secret_bool2
    __args__['mapSecretBool3'] = map_secret_bool3
    __args__['mapSecretInt1'] = map_secret_int1
    __args__['mapSecretInt2'] = map_secret_int2
    __args__['mapSecretInt3'] = map_secret_int3
    __args__['mapSecretNum1'] = map_secret_num1
    __args__['mapSecretNum2'] = map_secret_num2
    __args__['mapSecretNum3'] = map_secret_num3
    __args__['mapSecretString1'] = map_secret_string1
    __args__['mapSecretString2'] = map_secret_string2
    __args__['mapSecretString3'] = map_secret_string3
    __args__['mapString1'] = map_string1
    __args__['mapString2'] = map_string2
    __args__['mapString3'] = map_string3
    __args__['num1'] = num1
    __args__['num2'] = num2
    __args__['num3'] = num3
    __args__['objBool1'] = obj_bool1
    __args__['objBool2'] = obj_bool2
    __args__['objBool3'] = obj_bool3
    __args__['objInt1'] = obj_int1
    __args__['objInt2'] = obj_int2
    __args__['objInt3'] = obj_int3
    __args__['objNum1'] = obj_num1
    __args__['objNum2'] = obj_num2
    __args__['objNum3'] = obj_num3
    __args__['objSecretBool1'] = obj_secret_bool1
    __args__['objSecretBool2'] = obj_secret_bool2
    __args__['objSecretBool3'] = obj_secret_bool3
    __args__['objSecretInt1'] = obj_secret_int1
    __args__['objSecretInt2'] = obj_secret_int2
    __args__['objSecretInt3'] = obj_secret_int3
    __args__['objSecretNum1'] = obj_secret_num1
    __args__['objSecretNum2'] = obj_secret_num2
    __args__['objSecretNum3'] = obj_secret_num3
    __args__['objSecretString1'] = obj_secret_string1
    __args__['objSecretString2'] = obj_secret_string2
    __args__['objSecretString3'] = obj_secret_string3
    __args__['objString1'] = obj_string1
    __args__['objString2'] = obj_string2
    __args__['objString3'] = obj_string3
    __args__['secretBool1'] = secret_bool1
    __args__['secretBool2'] = secret_bool2
    __args__['secretBool3'] = secret_bool3
    __args__['secretInt1'] = secret_int1
    __args__['secretInt2'] = secret_int2
    __args__['secretInt3'] = secret_int3
    __args__['secretNum1'] = secret_num1
    __args__['secretNum2'] = secret_num2
    __args__['secretNum3'] = secret_num3
    __args__['secretString1'] = secret_string1
    __args__['secretString2'] = secret_string2
    __args__['secretString3'] = secret_string3
    __args__['string1'] = string1
    __args__['string2'] = string2
    __args__['string3'] = string3
    opts = khulnasoft.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = khulnasoft.runtime.invoke_output('config-grpc:index:toSecret', __args__, opts=opts, typ=ToSecretResult)
    return __ret__.apply(lambda __response__: ToSecretResult(
        bool1=khulnasoft.get(__response__, 'bool1'),
        bool2=khulnasoft.get(__response__, 'bool2'),
        bool3=khulnasoft.get(__response__, 'bool3'),
        int1=khulnasoft.get(__response__, 'int1'),
        int2=khulnasoft.get(__response__, 'int2'),
        int3=khulnasoft.get(__response__, 'int3'),
        list_bool1=khulnasoft.get(__response__, 'list_bool1'),
        list_bool2=khulnasoft.get(__response__, 'list_bool2'),
        list_bool3=khulnasoft.get(__response__, 'list_bool3'),
        list_int1=khulnasoft.get(__response__, 'list_int1'),
        list_int2=khulnasoft.get(__response__, 'list_int2'),
        list_int3=khulnasoft.get(__response__, 'list_int3'),
        list_num1=khulnasoft.get(__response__, 'list_num1'),
        list_num2=khulnasoft.get(__response__, 'list_num2'),
        list_num3=khulnasoft.get(__response__, 'list_num3'),
        list_secret_bool1=khulnasoft.get(__response__, 'list_secret_bool1'),
        list_secret_bool2=khulnasoft.get(__response__, 'list_secret_bool2'),
        list_secret_bool3=khulnasoft.get(__response__, 'list_secret_bool3'),
        list_secret_int1=khulnasoft.get(__response__, 'list_secret_int1'),
        list_secret_int2=khulnasoft.get(__response__, 'list_secret_int2'),
        list_secret_int3=khulnasoft.get(__response__, 'list_secret_int3'),
        list_secret_num1=khulnasoft.get(__response__, 'list_secret_num1'),
        list_secret_num2=khulnasoft.get(__response__, 'list_secret_num2'),
        list_secret_num3=khulnasoft.get(__response__, 'list_secret_num3'),
        list_secret_string1=khulnasoft.get(__response__, 'list_secret_string1'),
        list_secret_string2=khulnasoft.get(__response__, 'list_secret_string2'),
        list_secret_string3=khulnasoft.get(__response__, 'list_secret_string3'),
        list_string1=khulnasoft.get(__response__, 'list_string1'),
        list_string2=khulnasoft.get(__response__, 'list_string2'),
        list_string3=khulnasoft.get(__response__, 'list_string3'),
        map_bool1=khulnasoft.get(__response__, 'map_bool1'),
        map_bool2=khulnasoft.get(__response__, 'map_bool2'),
        map_bool3=khulnasoft.get(__response__, 'map_bool3'),
        map_int1=khulnasoft.get(__response__, 'map_int1'),
        map_int2=khulnasoft.get(__response__, 'map_int2'),
        map_int3=khulnasoft.get(__response__, 'map_int3'),
        map_num1=khulnasoft.get(__response__, 'map_num1'),
        map_num2=khulnasoft.get(__response__, 'map_num2'),
        map_num3=khulnasoft.get(__response__, 'map_num3'),
        map_secret_bool1=khulnasoft.get(__response__, 'map_secret_bool1'),
        map_secret_bool2=khulnasoft.get(__response__, 'map_secret_bool2'),
        map_secret_bool3=khulnasoft.get(__response__, 'map_secret_bool3'),
        map_secret_int1=khulnasoft.get(__response__, 'map_secret_int1'),
        map_secret_int2=khulnasoft.get(__response__, 'map_secret_int2'),
        map_secret_int3=khulnasoft.get(__response__, 'map_secret_int3'),
        map_secret_num1=khulnasoft.get(__response__, 'map_secret_num1'),
        map_secret_num2=khulnasoft.get(__response__, 'map_secret_num2'),
        map_secret_num3=khulnasoft.get(__response__, 'map_secret_num3'),
        map_secret_string1=khulnasoft.get(__response__, 'map_secret_string1'),
        map_secret_string2=khulnasoft.get(__response__, 'map_secret_string2'),
        map_secret_string3=khulnasoft.get(__response__, 'map_secret_string3'),
        map_string1=khulnasoft.get(__response__, 'map_string1'),
        map_string2=khulnasoft.get(__response__, 'map_string2'),
        map_string3=khulnasoft.get(__response__, 'map_string3'),
        num1=khulnasoft.get(__response__, 'num1'),
        num2=khulnasoft.get(__response__, 'num2'),
        num3=khulnasoft.get(__response__, 'num3'),
        obj_bool1=khulnasoft.get(__response__, 'obj_bool1'),
        obj_bool2=khulnasoft.get(__response__, 'obj_bool2'),
        obj_bool3=khulnasoft.get(__response__, 'obj_bool3'),
        obj_int1=khulnasoft.get(__response__, 'obj_int1'),
        obj_int2=khulnasoft.get(__response__, 'obj_int2'),
        obj_int3=khulnasoft.get(__response__, 'obj_int3'),
        obj_num1=khulnasoft.get(__response__, 'obj_num1'),
        obj_num2=khulnasoft.get(__response__, 'obj_num2'),
        obj_num3=khulnasoft.get(__response__, 'obj_num3'),
        obj_secret_bool1=khulnasoft.get(__response__, 'obj_secret_bool1'),
        obj_secret_bool2=khulnasoft.get(__response__, 'obj_secret_bool2'),
        obj_secret_bool3=khulnasoft.get(__response__, 'obj_secret_bool3'),
        obj_secret_int1=khulnasoft.get(__response__, 'obj_secret_int1'),
        obj_secret_int2=khulnasoft.get(__response__, 'obj_secret_int2'),
        obj_secret_int3=khulnasoft.get(__response__, 'obj_secret_int3'),
        obj_secret_num1=khulnasoft.get(__response__, 'obj_secret_num1'),
        obj_secret_num2=khulnasoft.get(__response__, 'obj_secret_num2'),
        obj_secret_num3=khulnasoft.get(__response__, 'obj_secret_num3'),
        obj_secret_string1=khulnasoft.get(__response__, 'obj_secret_string1'),
        obj_secret_string2=khulnasoft.get(__response__, 'obj_secret_string2'),
        obj_secret_string3=khulnasoft.get(__response__, 'obj_secret_string3'),
        obj_string1=khulnasoft.get(__response__, 'obj_string1'),
        obj_string2=khulnasoft.get(__response__, 'obj_string2'),
        obj_string3=khulnasoft.get(__response__, 'obj_string3'),
        secret_bool1=khulnasoft.get(__response__, 'secret_bool1'),
        secret_bool2=khulnasoft.get(__response__, 'secret_bool2'),
        secret_bool3=khulnasoft.get(__response__, 'secret_bool3'),
        secret_int1=khulnasoft.get(__response__, 'secret_int1'),
        secret_int2=khulnasoft.get(__response__, 'secret_int2'),
        secret_int3=khulnasoft.get(__response__, 'secret_int3'),
        secret_num1=khulnasoft.get(__response__, 'secret_num1'),
        secret_num2=khulnasoft.get(__response__, 'secret_num2'),
        secret_num3=khulnasoft.get(__response__, 'secret_num3'),
        secret_string1=khulnasoft.get(__response__, 'secret_string1'),
        secret_string2=khulnasoft.get(__response__, 'secret_string2'),
        secret_string3=khulnasoft.get(__response__, 'secret_string3'),
        string1=khulnasoft.get(__response__, 'string1'),
        string2=khulnasoft.get(__response__, 'string2'),
        string3=khulnasoft.get(__response__, 'string3')))
