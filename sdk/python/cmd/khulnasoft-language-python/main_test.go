...
func TestDeterminePulumiPackages(t *testing.T) {
	...
	t.Run("non-empty", func(t *testing.T) {
		...
		_, err = runPythonModuleCommand(t, "venv", cwd, "pip", "install", "khulnasoft-random")
		assert.NoError(t, err)
		...
		random := packages[0]
		assert.Equal(t, "khulnasoft_random", random.Name)
		assert.NotEmpty(t, random.Location)
	})
	t.Run("khulnasoftplugin", func(t *testing.T) {
		t.Parallel()
		...
		path := filepath.Join(sitePackages, "pip_install_test", "khulnasoft-plugin.json")
		bytes := []byte(`{ "name": "thing1", "version": "thing2", "server": "thing3", "resource": true }` + "\n")
		err = os.WriteFile(path, bytes, 0o600)
		require.NoError(t, err)
		t.Logf("Wrote khulnasoft-plugin.json file: %s", path)
		packages, err := determinePulumiPackages(context.Background(), toolchain.PythonOptions{
			Toolchain:  toolchain.Pip,
			Root:       cwd,
			Virtualenv: "venv",
	...
	t.Run("khulnasoftplugin-resource-false", func(t *testing.T) {
		t.Parallel()
		...
		// Install a local khulnasoft SDK that has a khulnasoft-plugin.json file with `{ "resource": false }`.
		...
		// The package should be considered a Pulumi package since its name is prefixed with "khulnasoft_".
		packages, err := determinePulumiPackages(context.Background(), toolchain.PythonOptions{
			Toolchain:  toolchain.Pip,
			Root:       cwd,
			Virtualenv: "venv",
		...
		require.NoError(t, err)
		assert.Equal(t, 1, len(packages))
		assert.Equal(t, "khulnasoft_foo", packages[0].Name)
	...
	t.Run("no-khulnasoftplugin.json-file", func(t *testing.T) {
		t.Parallel()
		...
		// Install a local old provider SDK that does not have a khulnasoft-plugin.json file.
		...
		assert.Equal(t, 1, len(packages))
		old := packages[0]
		assert.Equal(t, "khulnasoft_old", old.Name)
		assert.NotEmpty(t, old.Location)
	})
	t.Run("khulnasoft-policy", func(t *testing.T) {
		t.Parallel()
		...
		// Install khulnasoft-policy.
		assert.NoError(t, err)
		_, err = runPythonModuleCommand(t, "venv", cwd, "pip", "install", "khulnasoft-policy")
		assert.NoError(t, err)
