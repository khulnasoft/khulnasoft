...
class Stack:
    ...
    def up(
        ...
        try:
            up_result = self._run_khulnasoft_cmd_sync(args, on_output)
    ...
    def preview(
        ...
        :param suppress_outputs: Suppress display of stack outputs (in case they contain sensitive values)
        :param suppress_progress: Suppress display of periodic progress dots
        :param import_file: Save any creates seen during the preview into an import file to use with khulnasoft import
        :param attach_debugger: Run the process under a debugger, and pause until a debugger is attached
        ...
        try:
            preview_result = self._run_khulnasoft_cmd_sync(args, on_output)
        finally:
            _cleanup(temp_dir, log_watcher_thread, on_exit)
    ...
    def refresh(
        ...
        try:
            refresh_result = self._run_khulnasoft_cmd_sync(args, on_output)
        finally:
    ...
    def destroy(
        ...
        try:
            destroy_result = self._run_khulnasoft_cmd_sync(args, on_output)
        finally:
            _cleanup(temp_dir, log_watcher_thread)
    ...
    def import_resources(
        ...
        with tempfile.TemporaryDirectory(prefix="khulnasoft-import-") as temp_dir:
            ...
            import_result = self._run_khulnasoft_cmd_sync(args, on_output)
            summary = self.info(show_secrets and not self._remote)
            generated_code = ""
    ...
    def cancel(self) -> None:
        ...
        """
        self._run_khulnasoft_cmd_sync(["cancel", "--yes"])
    ...
    def _run_khulnasoft_cmd_sync(
        ...
        if self.workspace.khulnasoft_home is not None:
            envs = {**envs, "PULUMI_HOME": self.workspace.khulnasoft_home}
        ...
        result = self.workspace.khulnasoft_command.run(
    ...
    def _remote(self) -> bool:
        ...
        from khulnasoft.automation._local_workspace import LocalWorkspace
