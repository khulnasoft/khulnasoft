...
if TYPE_CHECKING:
    from khulnasoft.automation._remote_workspace import RemoteGitAuth
...
class LocalWorkspaceOptions:
    ...
    khulnasoft_home: Optional[str] = None
    ...
    khulnasoft_command: Optional[PulumiCommand] = None
    ...
    def __init__(
        ...
        self.khulnasoft_home = khulnasoft_home
        self.program = program
        self.env_vars = env_vars
        self.secrets_provider = secrets_provider
        self.project_settings = project_settings
        self.stack_settings = stack_settings
        self.khulnasoft_command = khulnasoft_command
...
class LocalWorkspace(Workspace):
    ...
    def __init__(
        ...
        self.khulnasoft_command = khulnasoft_command or PulumiCommand(
    ...
    def khulnasoft_version(self) -> str:  # type: ignore
        v = self.khulnasoft_command.version
        if v:
            return str(v)
    ...
    def __repr__(self):
        return (
            ...
            f"khulnasoft_home={self.khulnasoft_home!r}, env_vars={self.env_vars!r}, "
    ...
    def add_environments(self, stack_name: str, *environment_names: str) -> None:
        ...
        ver = VersionInfo(3)
        if self.khulnasoft_command.version is not None:
            ver = self.khulnasoft_command.version
        ...
        if ver >= VersionInfo(3, 95):
            ...
            self._run_khulnasoft_cmd_sync(args)
        else:
    ...
    def list_environments(self, stack_name: str) -> List[str]:
        ...
        if ver >= VersionInfo(3, 99):
            result = self._run_khulnasoft_cmd_sync(
    ...
    def get_config(
        ...
        result = self._run_khulnasoft_cmd_sync(args)
    ...
    def remove_tag(self, stack_name: str, key: str) -> None:
        self._run_khulnasoft_cmd_sync(["stack", "tag", "rm", key, "--stack", stack_name])
    ...
    def who_am_i(self) -> WhoAmIResult:
        ...
        if ver >= VersionInfo(3, 58):
            result = self._run_khulnasoft_cmd_sync(["whoami", "--json"])
        ...
        result = self._run_khulnasoft_cmd_sync(["whoami"])
    ...
    def install(
        ...
        if ver >= VersionInfo(3, 91):
            ...
            self._run_khulnasoft_cmd_sync(["install", *args], on_output=on_output)
            return
    ...
    def install_plugin(self, name: str, version: str, kind: str = "resource") -> None:
        self._run_khulnasoft_cmd_sync(["plugin", "install", kind, name, version])
    ...
    def list_plugins(self) -> List[PluginInfo]:
        result = self._run_khulnasoft_cmd_sync(["plugin", "ls", "--json"])
        json_list = json.loads(result.stdout)
    ...
    def stack_outputs(self, stack_name: str) -> OutputMap:
        masked_result = self._run_khulnasoft_cmd_sync(
            ["stack", "output", "--json", "--stack", stack_name]
        )
        plaintext_result = self._run_khulnasoft_cmd_sync(
    ...
    def _remote_supported(self) -> bool:
        ...
        result = self._run_khulnasoft_cmd_sync(["preview", "--help"])
        help_string = result.stdout.strip()
        return "--remote" in help_string

    def _remote_inherit_settings_supported(self) -> bool:
        # See if `--remote-inherit-settings` is present in `khulnasoft preview --help`'s output.
        result = self._run_khulnasoft_cmd_sync(["preview", "--help"])
        help_string = result.stdout.strip()
        return "--remote-inherit-settings" in help_string

    def _run_khulnasoft_cmd_sync(
        self, args: List[str], on_output: Optional[OnOutput] = None
    ) -> CommandResult:
        envs = {"PULUMI_HOME": self.khulnasoft_home} if self.khulnasoft_home else {}
        if self._remote:
            envs["PULUMI_EXPERIMENTAL"] = "true"
        envs = {**envs, **self.env_vars}
        return self.khulnasoft_command.run(args, self.work_dir, envs, on_output)
