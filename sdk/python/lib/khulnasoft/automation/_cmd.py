...
class PulumiCommand:
    ...
    def __init__(
        ...
        :param root: The directory where to look for the Pulumi installation. Defaults to running `khulnasoft` from $PATH.
        ...
        """
        self.command = os.path.join(root, "bin", "khulnasoft") if root else "khulnasoft"
        ...
        self.version = _parse_and_validate_khulnasoft_version(
    ...
    def install(
        ...
        $HOME/.khulnasoft/versions/$VERSION. Set `root` to specify a
        different directory, and `version` to install a custom version.

        :param root: The root directory to install the CLI to. Defaults to `~/.khulnasoft/versions/<version>`
        ...
        if not root:
            root = os.path.join(
                os.path.expanduser("~"), ".khulnasoft", "versions", str(version)
            )
    ...
    def _install_windows(cls, root: str, version: VersionInfo):
        ...
        try:
            _download_to_file("https://get.khulnasoft.com/install.ps1", script.name)
    ...
    def _install_posix(cls, root: str, version: VersionInfo):
        ...
        try:
            _download_to_file("https://get.khulnasoft.com/install.sh", script.name)
...
def _fixup_path(env: Dict[str, str], khulnasoftBin: str) -> Dict[str, str]:
    """
    Fixup path so that we prioritize up the bundled plugins next to the khulnasoft binary.
    """
    new_env = dict(env)
    new_env["PATH"] = os.pathsep.join([khulnasoftBin, env["PATH"]])
    return new_env


def _parse_and_validate_khulnasoft_version(
