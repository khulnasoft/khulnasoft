...
# [2] https://github.com/khulnasoft/khulnasoft-kubernetes/issues/984
#
...
async def serialize_properties(
    ...
    if typ is not None:
        py_name_to_khulnasoft_name = _types.input_type_py_to_khulnasoft_names(typ)
        types = _types.input_type_types(typ)
        # pylint: disable=C3001
        translate = lambda k: py_name_to_khulnasoft_name.get(k) or k
        # pylint: disable=C3001
        ...
        if typ is not None:
            if _types.is_input_type(typ):
                ...
                if typ is dict or origin in [dict, Dict, Mapping, abc.Mapping]:
                    ...
                    # is currently typed as `khulnasoft.Input[str]`, but we've allowed a dict to be passed, which will
...
def deserialize_resource(
    ...
    is_provider = pkg_name == "khulnasoft" and mod_name == "providers"
    if is_provider:
...
def resolve_outputs(
    ...
    if transform_using_type_metadata:
        khulnasoft_to_py_names = _types.resource_khulnasoft_to_py_names(resource_cls)

        # pylint: disable=C3001
        translate = lambda prop: khulnasoft_to_py_names.get(prop) or prop
        # pylint: disable=C3001
    ...
    if not settings.is_dry_run() or settings.is_legacy_apply_enabled():
        for key, value in list(serialized_props.items()):
            ...
            if translated_key not in all_properties:
                ...
                # input prop). This allows `khulnasoft up` to work without erroring when there is an input and output prop
...
def resolve_properties(
    ...
    for key, value in all_properties.items():
        ...
        if not settings.is_dry_run():
            # normal 'khulnasoft up'.  resolve the output with the value we got back
