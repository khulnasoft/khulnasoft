...
async def run_khulnasoft_func(func: Callable[[], None]):
    try:
        func()
    finally:
        await wait_for_rpcs()
        ...
        log.debug("run_khulnasoft_func completed")
...
class Stack(ComponentResource):
    ...
    def __init__(self, func: Callable[[], Optional[Awaitable[None]]]) -> None:
        ...
        super().__init__("khulnasoft:khulnasoft:Stack", name, None, None)
        ...
        try:
            awaitable = func()
            # This _should_ be an awaitable but old khulnasoft executors returned modules here, so we need to handle that
