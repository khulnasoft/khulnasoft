...
async def prepare_resource(
    ...
    elif ty != "khulnasoft:khulnasoft:Stack":
...
def create_urn(
    ...
    if parent is not None:
        ...
        parent_prefix = Output.from_input("urn:khulnasoft:" + stack + "::" + project + "::")
...
def _translate_ignore_changes(
    ...
    if ignore_changes is not None:
        if typ is not None:
            ...
            input_names = _types.input_type_py_to_khulnasoft_names(typ)
            ignore_changes = list(
...
def _translate_additional_secret_outputs(
    ...
    if additional_secret_outputs is not None:
        if typ is not None:
            ...
            output_names = _types.resource_py_to_khulnasoft_names(type(res))
