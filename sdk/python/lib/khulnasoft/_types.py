...
#
#   class FooResource(khulnasoft.CustomResource):
#       nested_value: khulnasoft.Output[Nested] = khulnasoft.property("nestedValue")
#
#       def __init__(self, resource_name, nested_value: khulnasoft.InputType[NestedArgs]):
#           super().__init__("my:module:FooResource", resource_name, {"nestedValue": nested_value})
#
#
# The resource declares a single output `nested_value` of type `khulnasoft.Output[Nested]` and uses
# `khulnasoft.property()` to indicate the Pulumi property name.
#
# The resource's `__init__()` method accepts a `nested_value` argument typed as
# `khulnasoft.InputType[NestedArgs]`, which is an alias for accepting either an input type (in this
...
#       def nested_value(self) -> khulnasoft.Output[Nested]:
#           ...
#
#
# Note the `nested_value` property getter function is empty. The `@khulnasoft.getter` decorator replaces
...
#       @khulnasoft.getter(name="nestedValue")
#       def nested_value(self) -> khulnasoft.Output[Nested]:
#           khulnasoft.get(self, "nested_value")
#
#
...
#   class NestedArgs:
#       first_arg: khulnasoft.Input[str] = khulnasoft.property("firstArg")
#       second_arg: Optional[khulnasoft.Input[float]] = khulnasoft.property("secondArg", default=None)
#
#
# The class is decorated with the `@khulnasoft.input_type` decorator, which indicates the class is an
# input type and does some processing of the class (explained below). `NestedArgs` declares two
# inputs (`first_arg` and `second_arg`) and uses type annotations and `khulnasoft.property()` to
...
#   @khulnasoft.input_type
#   class NestedArgs:
#       def __init__(self, *, first_arg: khulnasoft.Input[str], second_arg: Optional[khulnasoft.Input[float]] = None):
#           khulnasoft.set(self, "first_arg", first_arg)
#           if second_arg is not None:
#               khulnasoft.set(self, "second_arg", second_arg)
#
#       @property
#       @khulnasoft.getter(name="firstArg")
#       def first_arg(self) -> khulnasoft.Input[str]:
#           ...
#
#       @first_arg.setter
#       def first_arg(self, value: khulnasoft.Input[str]):
#           ...
#
#       @property
#       @khulnasoft.getter(name="secondArg")
#       def second_arg(self) -> Optional[khulnasoft.Input[float]]:
#           ...
#
#       @second_arg.setter
#       def second_arg(self, value: Optional[khulnasoft.Input[float]]):
#           ...
#
# This latter (more verbose) declaration is equivalent to the former (simpler) declaration;
# the `@khulnasoft.input_type` processes the class and transforms the former declaration into the
# latter declaration.
#
...
# Empty getter functions are automatically replaced by the `@khulnasoft.getter` decorator with an
# actual implementation, and the `@khulnasoft.input_type` decorator will automatically replace any
# empty setter functions associated with a getter decorated with `@khulnasoft.getter` with an actual
...
#       @khulnasoft.getter(name="firstArg")
#       def first_arg(self) -> khulnasoft.Input[str]:
#           return khulnasoft.get(self, "first_arg")
#
#       @first_arg.setter
#       def first_arg(self, value: khulnasoft.Input[str]):
#           khulnasoft.set(self, "first_arg", value)
#
#       @property
#       @khulnasoft.getter(name="secondArg")
#       def second_arg(self) -> Optional[khulnasoft.Input[float]]:
#           return khulnasoft.get(self, "second_arg")
#
#       @second_arg.setter
#       def second_arg(self, value: Optional[khulnasoft.Input[float]]):
#           khulnasoft.set(self, "second_arg", value)
#
#
...
#       second_arg: Optional[float] = khulnasoft.property("secondArg")
#
#
...
#           ...
#
# An `__init__()` method is added to the class by the `@khulnasoft.output_type` decorator (if an
...
# replaced with implementations by the `@khulnasoft.getter` decorator.
#
...
# `khulnasoft.get` and `khulnasoft.set` will call to translate the Pulumi property name to a translated
...
_PULUMI_NAME = "_khulnasoft_name"
_PULUMI_INPUT_TYPE = "_khulnasoft_input_type"
_PULUMI_OUTPUT_TYPE = "_khulnasoft_output_type"
_PULUMI_PYTHON_TO_PULUMI_TABLE = "_khulnasoft_python_to_khulnasoft_table"
_PULUMI_DEPRECATED_CALLABLE = "_khulnasoft_deprecated_callable"
...
def _create_py_property(a_name: str, khulnasoft_name: str, typ: Any, setter: bool = False):
    """
    ...
    setattr(getter_fn, _PULUMI_NAME, khulnasoft_name)
    ...
    for base in reversed(cls.__mro__):
        for python_name, v in base.__dict__.items():
            if isinstance(v, builtins.property):
                ...
                khulnasoft_name = getattr(prop.fget, _PULUMI_NAME, MISSING)
                if khulnasoft_name is not MISSING:
                    yield (python_name, cast(str, khulnasoft_name), prop)
...
def input_type_py_to_khulnasoft_names(input_type_cls: Type) -> Dict[str, str]:
    """
    Returns a dict of Python names to Pulumi names for the input type.
    """
    assert is_input_type(input_type_cls)
    return {
        python_name: khulnasoft_name
        for python_name, khulnasoft_name, _ in _py_properties(input_type_cls)
    }
...
def input_type_to_dict(obj: Any) -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {}
    for _, khulnasoft_name, prop in _py_properties(cls):
        fget = prop.fget

        # If the property has a _khulnasoft_deprecated_callable attribute, use that
...
def output_type(cls: Type[T]) -> Type[T]:
    ...
    # Pulumi names. Calls to khulnasoft.get() will then convert the name passed to khulnasoft.get() from
    ...
    if hasattr(cls, _TRANSLATE_PROPERTY):
        python_to_khulnasoft_table = None
        for python_name, khulnasoft_name, _ in _py_properties(cls):
            if python_name != khulnasoft_name:
                python_to_khulnasoft_table = python_to_khulnasoft_table or {}
                python_to_khulnasoft_table[python_name] = khulnasoft_name
        if python_to_khulnasoft_table is not None:
            setattr(cls, _PULUMI_PYTHON_TO_PULUMI_TABLE, python_to_khulnasoft_table)
...
def output_type_from_dict(cls: Type[T], output: Dict[str, Any]) -> T:
    ...
    args = {}
    for python_name, khulnasoft_name, _ in _py_properties(cls):
        args[python_name] = output.get(khulnasoft_name)
...
def getter(_fn=None, *, name: Optional[str] = None):
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        khulnasoft_name = name if name is not None else fn.__name__
        if _utils.is_empty_function(fn):
            ...
            fn = get_fn
        setattr(fn, _PULUMI_NAME, khulnasoft_name)
        return fn
...
def _types_from_py_properties(cls: type) -> Dict[str, type]:
    ...
    for _, khulnasoft_name, prop in _py_properties(cls):
        ...
        if return_hint is not None:
            ...
            if typ is Output:
                continue
            result[khulnasoft_name] = typ
    return result


def _khulnasoft_to_py_names_from_py_properties(cls: type) -> Dict[str, str]:
    return {
        khulnasoft_name: python_name for python_name, khulnasoft_name, _ in _py_properties(cls)
    }


def _py_to_khulnasoft_names_from_py_properties(cls: type) -> Dict[str, str]:
    return {
        python_name: khulnasoft_name for python_name, khulnasoft_name, _ in _py_properties(cls)
    }
...
def _names_from_annotations(cls: type) -> Iterator[Tuple[str, str]]:
    ...
    def get_khulnasoft_name(a_name: str) -> str:
        default = getattr(cls, a_name, MISSING)
        return default.name if isinstance(default, _Property) else a_name

    for python_name in cls_annotations.keys():
        yield (python_name, get_khulnasoft_name(python_name))


def _khulnasoft_to_py_names_from_annotations(cls: type) -> Dict[str, str]:
    return {
        khulnasoft_name: python_name
        for python_name, khulnasoft_name in _names_from_annotations(cls)
    }


def _py_to_khulnasoft_names_from_annotations(cls: type) -> Dict[str, str]:
...
def resource_khulnasoft_to_py_names(resource_cls: type) -> Dict[str, str]:
    """
    Returns a dict of Pulumi names to types for the resource.
    """
    # First, get the names from the class's type annotations.
    names_from_annotations = _khulnasoft_to_py_names_from_annotations(resource_cls)

    # Next, get the names from the class's Python properties.
    names_from_py_properties = _khulnasoft_to_py_names_from_py_properties(resource_cls)

    # Return the merged dictionaries.
    return {**names_from_annotations, **names_from_py_properties}


def resource_py_to_khulnasoft_names(resource_cls: type) -> Dict[str, str]:
    """
    Returns a dict of Pulumi names to types for the resource.
    """
    # First, get the names from the class's type annotations.
    names_from_annotations = _py_to_khulnasoft_names_from_annotations(resource_cls)

    # Next, get the names from the class's Python properties.
    names_from_py_properties = _py_to_khulnasoft_names_from_py_properties(resource_cls)
