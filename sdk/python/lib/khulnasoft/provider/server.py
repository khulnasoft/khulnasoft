...
from khulnasoft.provider.provider import InvokeResult, Provider, CallResult, ConstructResult
...
from khulnasoft.runtime import known_types, proto, rpc
...
from khulnasoft.runtime.stack import wait_for_rpcs
import khulnasoft
import khulnasoft.resource
import khulnasoft.runtime.config
import khulnasoft.runtime.settings
from khulnasoft.errors import (
...
class ProviderServicer(ResourceProviderServicer):
    ...
    async def _construct(
        ...
        khulnasoft.runtime.settings.reset_options(
        ...
        khulnasoft.runtime.config.set_all_config(
    ...
    @staticmethod
    def _construct_options(request: proto.ConstructRequest) -> khulnasoft.ResourceOptions:
        parent = None
    ...
    async def _construct_response(
        ...
        urn = await khulnasoft.Output.from_input(result.urn).future()
        assert urn is not None

        # Note: property_deps is populated by rpc.serialize_properties.
        property_deps: Dict[str, List[khulnasoft.resource.Resource]] = {}
    ...
    @staticmethod
    async def _call_args(request: proto.CallRequest) -> Dict[str, khulnasoft.Input[Any]]:
        def deps(key: str) -> Set[str]:
            return set(
                urn
    ...
    async def _call_response(self, result: CallResult) -> proto.CallResponse:
        ...
        ret_deps: Dict[str, List[khulnasoft.resource.Resource]] = {}
...
def _create_provider_resource(ref: str) -> ProviderResource:
    ...
    urn_parts = khulnasoft.urn._parse_urn(urn)
