...
`aliases=[Alias(parent=khulnasoft.ROOT_STACK_RESOURCE)]`.
...
class Alias:
    ...
    To specify no original parent, use `Alias(parent=khulnasoft.ROOT_STACK_RESOURCE)`.
    """

    stack: Optional["Input[str]"]
    """
    The name of the previous stack of the resource.  If not provided, defaults to `khulnasoft.getStack()`.
    """

    project: Optional["Input[str]"]
    """
    The previous project of the resource. If not provided, defaults to `khulnasoft.getProject()`.
    """
...
class ResourceOptions:
    ...
    that the resource provider and khulnasoft engine already determine based on inputs to your resource.
...
class Resource:
    ...
    def __init__(
        ...
        :param Optional[ResourceOptions] opts: Optional set of :class:`khulnasoft.ResourceOptions` to use for this
               resource.
    ...
    def _get_providers(
        ...
        if opts.provider:
            ...
            if provider_pkg in opts_providers:
                ...
                deprecation = "This will become an error in a future version. See https://github.com/khulnasoft/khulnasoft/issues/8799 for more details"
...
class ProviderResource(CustomResource):
    ...
    def __init__(
        ...
        # Provider resources are given a well-known type, prefixed with "khulnasoft:providers".
        CustomResource.__init__(
            self, f"khulnasoft:providers:{pkg}", name, props, opts, dependency, package_ref
        )
        self.package = pkg
...
class DependencyProviderResource(ProviderResource):
    ...
    def __init__(self, ref: str) -> None:
        ...
        # `typ` will be `khulnasoft:providers:<package>` and we want the
