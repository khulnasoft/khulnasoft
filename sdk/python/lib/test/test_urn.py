...
from khulnasoft import urn as urn_util
...
def test_parse_urn_with_name():
    ...
    assert res.typ == "khulnasoft:providers:aws"
    assert res.pkg_name == "khulnasoft"
    assert res.mod_name == "providers"
    assert res.typ_name == "aws"


def test_parse_urn_without_name():
    res = urn_util._parse_urn("urn:khulnasoft:stack::project::khulnasoft:providers:aws")
    assert res.urn_name == ""
    assert res.typ == "khulnasoft:providers:aws"
    assert res.pkg_name == "khulnasoft"
    assert res.mod_name == "providers"
