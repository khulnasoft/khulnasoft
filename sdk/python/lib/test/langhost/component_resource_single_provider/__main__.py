...
from khulnasoft import ProviderResource, CustomResource, ComponentResource, ResourceOptions
...
def create_resources(name, create_children=None, parent=None):
    ...
    # Override protect
    Resource(
        f"{name}/r1", create_children, ResourceOptions(parent=parent, protect=False)
    )
    Resource(
        f"{name}/r2", create_children, ResourceOptions(parent=parent, protect=True)
    )

    # Override provider
    prov = Provider(f"{name}-p", ResourceOptions(parent=parent))
    Resource(
        f"{name}/r3", create_children, ResourceOptions(parent=parent, provider=prov)
    )
...
def create_components(name, create_children=None, parent=None):
    ...
    # Override protect
    Component(
        f"{name}/c1", create_children, ResourceOptions(parent=parent, protect=False)
    )
    Component(
        f"{name}/c2", create_children, ResourceOptions(parent=parent, protect=True)
    )

    # Override providers
    provider = Provider(f"{name}-p", ResourceOptions(parent=parent))
    Component(
        f"{name}/c3", create_children, ResourceOptions(parent=parent, provider=provider)
    )
...
create_components(
    "single-nest", lambda name, parent: create_resources(name, None, parent)
)

# Create doubly-nested resources
create_components(
    "double-nest",
    lambda name, parent: create_components(
        name, lambda name, parent: create_resources(name, None, parent), parent
    ),
)
...
create_components(
    "double-nest-2",
    lambda name, parent: create_resources(
        name, lambda name, parent: create_resources(name, None, parent), parent
    ),
)
