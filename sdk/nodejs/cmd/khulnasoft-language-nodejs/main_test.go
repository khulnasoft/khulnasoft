...
func TestArgumentConstruction(t *testing.T) {
	...
	t.Run("DryRun-NoArguments", func(t *testing.T) {
		...
		rr := &khulnasoftrpc.RunRequest{DryRun: true, Info: info}
	...
	t.Run("OptionalArgs-PassedIfSpecified", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &khulnasoftrpc.RunRequest{Project: "foo", Info: info}
	...
	t.Run("OptionalArgs-NotPassedIfNotSpecified", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &khulnasoftrpc.RunRequest{Info: info}
		args := strings.Join(host.constructArguments(rr, "", "", ""), " ")
...
func TestConfig(t *testing.T) {
	...
	t.Run("Config-Empty", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &khulnasoftrpc.RunRequest{Project: "foo"}
...
func TestGetRequiredPlugins(t *testing.T) {
	...
	files := []struct {
		...
		{
			filepath.Join(dir, "node_modules", "@khulnasoft", "foo", "package.json"),
			`{ "name": "@khulnasoft/foo", "version": "1.2.3", "khulnasoft": { "resource": true } }`,
		},
		{
			filepath.Join(dir, "node_modules", "@khulnasoft", "bar", "package.json"),
			`{ "name": "@khulnasoft/bar", "version": "4.5.6", "khulnasoft": { "resource": true } }`,
		},
		{
			filepath.Join(dir, "node_modules", "@khulnasoft", "baz", "package.json"),
			`{ "name": "@khulnasoft/baz", "version": "4.5.6", "khulnasoft": { "resource": false } }`,
		},
		{
	...
	host := &nodeLanguageHost{}
	resp, err := host.GetRequiredPlugins(context.Background(), &khulnasoftrpc.GetRequiredPluginsRequest{
		Program: dir,
		Info: &khulnasoftrpc.ProgramInfo{
			RootDirectory:    dir,
...
func TestGetRequiredPluginsNestedPolicyPack(t *testing.T) {
	...
	files := []struct {
		...
		{
			filepath.Join(dir, "policy", "node_modules", "@khulnasoft", "baz", "package.json"),
			`{ "name": "@khulnasoft/baz", "version": "7.8.9", "khulnasoft": { "resource": true } }`,
		},
	}
	for _, file := range files {
		err := os.MkdirAll(filepath.Dir(file.path), 0o755)
...
func TestGetProgramDependencies(t *testing.T) {
	...
	t.Run("With no package.json, no lock files", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		_, err := host.GetProgramDependencies(context.Background(), &khulnasoftrpc.GetProgramDependenciesRequest{
			Program: testDir,
			Info: &khulnasoftrpc.ProgramInfo{
	...
	t.Run("With package.json in project root, no lock files", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			{
				path:    "package.json",
				content: `{ "name": "@khulnasoft/baz", "dependencies": { "@khulnasoft/khulnasoft": "^3.113.0" } }`,
			},
			{
				path:    "Pulumi.yaml",
				content: `name: test`,
	...
	t.Run("With package.json and yarn.lock in project root", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			...
			{
				path: "yarn.lock",
				content: `"@khulnasoft/khulnasoft@^3.0.0", "@khulnasoft/khulnasoft@^3.113.0":
  version "3.131.0"
  resolved "https://registry.yarnpkg.com/@khulnasoft/khulnasoft/-/khulnasoft-3.131.0.tgz#6233e5ee5e72907b99415b32be6a9ebf9041f096"
  ...
  dependencies:
    ...
    "@npmcli/arborist" "^7.3.1"`,
		...
		host := &nodeLanguageHost{}
		resp, err := host.GetProgramDependencies(context.Background(), &khulnasoftrpc.GetProgramDependenciesRequest{
			Program: testDir,
			Info: &khulnasoftrpc.ProgramInfo{
				RootDirectory:    testDir,
				ProgramDirectory: testDir,
		...
		require.Equal(t, resp.Dependencies[0].Name, "@khulnasoft/khulnasoft")
...
func TestNonblockingStdout(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/khulnasoft/issues/16503
	t.Parallel()
