...
// khulnasoft-language-nodejs serves as the "language host" for Pulumi
...
// `khulnasoft-language-nodejs-exec`. This script is written in the hosted
...
// `khulnasoft-language-nodejs-exec` and forwarding the RPC request arguments
// to the command-line.
package main
...
import (
	...
	codegen "github.com/khulnasoft/khulnasoft/pkg/v3/codegen/nodejs"
...
func main() {
	...
	cmdutil.InitTracing("khulnasoft-language-nodejs", "khulnasoft-language-nodejs", tracing)
...
func locateModule(ctx context.Context, mod, programDir, nodeBin string, isPlugin bool) (string, error) {
	installCommand := "khulnasoft install"
	if isPlugin {
	...
	// https://github.com/khulnasoft/khulnasoft/issues/16393
...
func (host *nodeLanguageHost) connectToEngine() (khulnasoftrpc.EngineClient, io.Closer, error) {
...
func (host *nodeLanguageHost) GetRequiredPlugins(ctx context.Context,
	...
	// "khulnasoft": true } inside of their package.json files.  We begin this search in the same
	...
	// Keep track of the versions of @khulnasoft/khulnasoft that are pulled in.  If they differ on
	// minor version, we will issue a warning to the user.
	khulnasoftPackagePathToVersionMap := make(map[string]semver.Version)
	plugins, err := getPluginsFromDir(
		req.Info.ProgramDirectory,
		khulnasoftPackagePathToVersionMap,
		false, /*inNodeModules*/
	...
	if err == nil {
		...
		for path, version := range khulnasoftPackagePathToVersionMap {
			if first {
				first = false
				firstPath = path
			...
			if ok, message := compatibleVersions(version, firstVersion); !ok {
				fmt.Fprintf(os.Stderr,
					`Found incompatible versions of @khulnasoft/khulnasoft. %s
...
func getPluginsFromDir(
	dir string, khulnasoftPackagePathToVersionMap map[string]semver.Version,
	inNodeModules bool, visitedPaths map[string]struct{},
) ([]*khulnasoftrpc.PluginDependency, error) {
	...
	var plugins []*khulnasoftrpc.PluginDependency
	...
	for _, file := range files {
		...
		if isDir {
			...
			if info.Name == "@khulnasoft/khulnasoft" {
				...
				khulnasoftPackagePathToVersionMap[curr] = version
			}
			...
			if err != nil {
				...
				plugins = append(plugins, &khulnasoftrpc.PluginDependency{
					Name:    name,
					Kind:    "resource",
					Version: version,
					Server:  server,
...
type packageJSON struct {
	...
	Pulumi          plugin.PulumiPluginJSON `json:"khulnasoft"`
...
func getPluginName(info packageJSON) (string, error) {
	// If it's specified in the "khulnasoft" section, return it as-is.
	if info.Pulumi.Name != "" {
		return info.Pulumi.Name, nil
	}

	// Otherwise, derive it from the top-level package name,
	// only if it has @khulnasoft scope, otherwise fail.
	name := info.Name
	if name == "" {
		return "", errors.New("missing expected \"name\" property")
	}

	// If the name has a @khulnasoft scope, we will just use its simple name.  Otherwise, we use the fully scoped name.
	// We do trim the leading @, however, since Pulumi resource providers do not use the same NPM convention.
	if strings.Index(name, "@khulnasoft/") == 0 {
		return name[strings.IndexRune(name, '/')+1:], nil
	}

	// if the package name does not start with @khulnasoft it means that it is a third-party package
	// third-party packages _MUST_ have the plugin name in package.json in the khulnasoft section
	// {
	...
	return "", fmt.Errorf("Missing property \"name\" for the third-party plugin '%v' "+
		"inside package.json under the \"khulnasoft\" section.", name)
}
...
func getPluginVersion(info packageJSON) (string, error) {
	// See if it's specified in the "khulnasoft" section.
...
func (host *nodeLanguageHost) Run(ctx context.Context, req *khulnasoftrpc.RunRequest) (*khulnasoftrpc.RunResponse, error) {
	...
	target := khulnasoftrpc.NewResourceMonitorClient(conn)
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		Cancel: serverCancel,
		Init: func(srv *grpc.Server) error {
			khulnasoftrpc.RegisterResourceMonitorServer(srv, &monitorProxy{target: target})
			return nil
		},
	...
	if err != nil {
		return &khulnasoftrpc.RunResponse{Error: "could not find node on the $PATH: " + err.Error()}, nil
	}
	...
	responseChannel := make(chan *khulnasoftrpc.RunResponse)
...
// `responseChannel` arg.
func (host *nodeLanguageHost) execNodejs(ctx context.Context, req *khulnasoftrpc.RunRequest,
	engineClient khulnasoftrpc.EngineClient, nodeBin, runPath, address, pipesDirectory string,
	...
	if err != nil {
		...
		return &khulnasoftrpc.RunResponse{Error: err.Error()}
	}
	...
	if err := run(); err != nil {
		...
		if exiterr, ok := err.(*exec.ExitError); ok {
			...
			case nodeJSProcessExitedAfterShowingUserActionableMessage:
				...
				// actionable message". In that case, we can simply bail out and terminate `khulnasoft`
				// without showing any more messages.
				return &khulnasoftrpc.RunResponse{Error: "", Bail: true}
			default:
	...
	return &khulnasoftrpc.RunResponse{Error: errResult}
}

// constructArguments constructs a command-line for `khulnasoft-language-nodejs`
// by enumerating all of the optional and non-optional arguments present
// in a RunRequest.
func (host *nodeLanguageHost) constructArguments(
	req *khulnasoftrpc.RunRequest, runPath, address, pipesDirectory string,
) []string {
	args := []string{runPath}
...
// a RunRequest.
func (host *nodeLanguageHost) constructConfig(req *khulnasoftrpc.RunRequest) (string, error) {
	...
	// like the old world. Newer versions of the @khulnasoft/khulnasoft package handle both formats and when we stop supporting
...
// a RunRequest.
func (host *nodeLanguageHost) constructConfigSecretKeys(req *khulnasoftrpc.RunRequest) (string, error) {
...
func (host *nodeLanguageHost) GetPluginInfo(ctx context.Context, req *emptypb.Empty) (*khulnasoftrpc.PluginInfo, error) {
...
func (host *nodeLanguageHost) InstallDependencies(
	...
	if host.forceTsc {
		...
		// We probably want to see about making something like this an explicit "khulnasoft build" step, but for now shim'ing this
...
func parseYarnLockFile(programDirectory, path string) ([]*khulnasoftrpc.DependencyInfo, error) {
	...
	result := make([]*khulnasoftrpc.DependencyInfo, len(leafs))
	...
	for i, v := range leafs {
		...
		result[i] = &khulnasoftrpc.DependencyInfo{
			Name:    name,
			Version: version,
		}
	}
...
func parseNpmLockFile(programDirectory, path string) ([]*khulnasoftrpc.DependencyInfo, error) {
	...
	result := make([]*khulnasoftrpc.DependencyInfo, len(file.Dependencies))
	var i int
...
func crossCheckPackageJSONFile(path string, file []byte,
	packages []*khulnasoftrpc.DependencyInfo,
) ([]*khulnasoftrpc.DependencyInfo, error) {
	...
	result := make([]*khulnasoftrpc.DependencyInfo, len(dependencies))
	i := 0
...
func (host *nodeLanguageHost) GetProgramDependencies(
	...
	var result []*khulnasoftrpc.DependencyInfo
	...
	if !req.TransitiveDependencies {
		...
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("could not find %s. "+
				...
				`khulnasoft about --transitive" to get a list of used packages`,
				packageFile)
		} else if err != nil {
...
func (host *nodeLanguageHost) GeneratePackage(
	...
	files, err := codegen.GeneratePackage("khulnasoft-language-nodejs", pkg, req.ExtraFiles, req.LocalDependencies, req.Local)
	if err != nil {
		return nil, err
	}
...
func (host *nodeLanguageHost) Pack(ctx context.Context, req *khulnasoftrpc.PackRequest) (*khulnasoftrpc.PackResponse, error) {
	...
	if packageJSON["name"] == "@khulnasoft/khulnasoft" {
		// This is pretty much a copy of the makefiles build_package. Short term we should see about changing
		// the makefile to just build the nodejs plugin first and then simply invoke "khulnasoft package
...
// In https://github.com/khulnasoft/khulnasoft/pull/16504 we used Cmd.StdoutPipe/StderrPipe for this.
...
func (o *oomSniffer) Message() string {
	return "Detected a possible out of memory error. Consider increasing the memory available to the nodejs process " +
		...
		"See https://www.khulnasoft.com/docs/concepts/projects/project-file/#runtime-options"
}
	...
	go func() {
		for scanner.Scan() {
			...
			if !o.detected && (strings.Contains(line, "<--- Last few GCs --->") /* "Normal" OOM output */ ||
				...
				// a failed assertion in the debugger https://github.com/khulnasoft/khulnasoft/issues/16596.
				//nolint:lll
