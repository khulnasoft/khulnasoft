...
func TestGetPlugin(t *testing.T) {
	...
	cases := []struct {
		Name          string
		Mod           *modInfo
		Expected      *khulnasoftrpc.PluginDependency
		...
		},
		{
			...
			Mod: &modInfo{
				Path:    "github.com/khulnasoft/khulnasoft/sdk",
				Version: "v1.14.0",
			},
			ExpectedError: "module is not a khulnasoft provider",
		},
...
func testPluginsAndDependencies(t *testing.T, progDir string) {
	...
	t.Run("GetRequiredPlugins", func(t *testing.T) {
		...
		res, err := host.GetRequiredPlugins(ctx, &khulnasoftrpc.GetRequiredPluginsRequest{
			Project: "deprecated",
			Pwd:     progDir,
