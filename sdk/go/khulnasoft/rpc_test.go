...
func (rp *testResourcePackage) ConstructProvider(ctx *Context, name, typ, urn string) (ProviderResource, error) {
	if typ != "khulnasoft:providers:test" {
...
type test struct {
	S                              string                 `khulnasoft:"s"`
	A                              bool                   `khulnasoft:"a"`
	B                              int                    `khulnasoft:"b"`
	StringAsset                    Asset                  `khulnasoft:"cStringAsset"`
	FileAsset                      Asset                  `khulnasoft:"cFileAsset"`
	RemoteAsset                    Asset                  `khulnasoft:"cRemoteAsset"`
	AssetArchive                   Archive                `khulnasoft:"dAssetArchive"`
	FileArchive                    Archive                `khulnasoft:"dFileArchive"`
	RemoteArchive                  Archive                `khulnasoft:"dRemoteArchive"`
	E                              interface{}            `khulnasoft:"e"`
	Array                          []interface{}          `khulnasoft:"fArray"`
	Map                            map[string]interface{} `khulnasoft:"fMap"`
	G                              string                 `khulnasoft:"g"`
	H                              string                 `khulnasoft:"h"`
	I                              string                 `khulnasoft:"i"`
	CustomResource                 CustomResource         `khulnasoft:"jCustomResource"`
	ComponentResource              ComponentResource      `khulnasoft:"jComponentResource"`
	ProviderResource               ProviderResource       `khulnasoft:"jProviderResource"`
	PreviewCustomResource          CustomResource         `khulnasoft:"jPreviewCustomResource"`
	PreviewProviderResource        ProviderResource       `khulnasoft:"jPreviewProviderResource"`
	MissingCustomResource          CustomResource         `khulnasoft:"kCustomResource"`
	MissingComponentResource       ComponentResource      `khulnasoft:"kComponentResource"`
	MissingProviderResource        ProviderResource       `khulnasoft:"kProviderResource"`
	MissingPreviewCustomResource   CustomResource         `khulnasoft:"kPreviewCustomResource"`
	MissingPreviewProviderResource ProviderResource       `khulnasoft:"kPreviewProviderResource"`
}
...
func TestMarshalRoundtrip(t *testing.T) {
	...
	providerURN := resource.NewURN("stack", "project", "", "khulnasoft:providers:test", "test")
	previewCustomURN := resource.NewURN("stack", "project", "", "test:index:custom", "preview")
	previewProviderURN := resource.NewURN("stack", "project", "", "khulnasoft:providers:test", "preview")
	...
	missingProviderURN := resource.NewURN("stack", "project", "", "khulnasoft:providers:missing", "test")
	missingPreviewCustomURN := resource.NewURN("stack", "project", "", "missing:index:custom", "preview")
	missingPreviewProviderURN := resource.NewURN("stack", "project", "", "khulnasoft:providers:missing", "preview")
	...
	inputs := testInputs{
		...
		RemoteAsset: NewRemoteAsset("https://khulnasoft.com/fake/txt"),
		...
		RemoteArchive: NewRemoteArchive("https://khulnasoft.com/fake/archive.zip"),
		E:             out,
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, err) {
			if assert.NotNil(t, resV) {
				...
				assert.Equal(t, "https://khulnasoft.com/fake/txt", res["cRemoteAsset"].(Asset).URI())
				...
				assert.Equal(t, "https://khulnasoft.com/fake/archive.zip", res["dRemoteArchive"].(Archive).URI())
...
type nestedTypeInputs struct {
	Foo StringInput `khulnasoft:"foo"`
	Bar IntInput    `khulnasoft:"bar"`
}
...
type testResourceArgs struct {
	URN URN `khulnasoft:"urn"`
	ID  ID  `khulnasoft:"id"`

	Any     interface{}            `khulnasoft:"any"`
	Archive Archive                `khulnasoft:"archive"`
	Array   []interface{}          `khulnasoft:"array"`
	Asset   Asset                  `khulnasoft:"asset"`
	Bool    bool                   `khulnasoft:"bool"`
	Float64 float64                `khulnasoft:"float64"`
	Int     int                    `khulnasoft:"int"`
	Map     map[string]interface{} `khulnasoft:"map"`
	String  string                 `khulnasoft:"string"`

	Nested nestedType `khulnasoft:"nested"`
}
...
type testResource struct {
	...
	Any     AnyOutput     `khulnasoft:"any"`
	Archive ArchiveOutput `khulnasoft:"archive"`
	Array   ArrayOutput   `khulnasoft:"array"`
	Asset   AssetOutput   `khulnasoft:"asset"`
	Bool    BoolOutput    `khulnasoft:"bool"`
	Float64 Float64Output `khulnasoft:"float64"`
	Int     IntOutput     `khulnasoft:"int"`
	Map     MapOutput     `khulnasoft:"map"`
	String  StringOutput  `khulnasoft:"string"`

	Nested nestedTypeOutput `khulnasoft:"nested"`
}
...
func TestResourceState(t *testing.T) {
	...
	resolved, _, _, _ := marshalInputs(&testResourceInputs{
		Any:     String("foo"),
		Archive: NewRemoteArchive("https://khulnasoft.com/fake/archive.zip"),
...
type foo struct {
	TemplateOptions *TemplateOptions `khulnasoft:"templateOptions"`
}
...
type TemplateOptions struct {
	...
	TagSpecifications []TemplateTagSpecification `khulnasoft:"tagSpecifications"`
}
...
type TemplateTagSpecificationArgs struct {
	Name StringPtrInput `khulnasoft:"name"`
	Tags StringMapInput `khulnasoft:"tags"`
}
...
type bucketObjectArgs struct {
	Source AssetOrArchive `khulnasoft:"source"`
}
...
type myNestedOutputArgs struct {
	Nested interface{} `khulnasoft:"nested"`
}
...
func TestUnmarshalPropertyMap(t *testing.T) {
	...
	t.Run("output", func(t *testing.T) {
		...
		dependencies := []resource.URN{"urn:khulnasoft:test_stack::test_project::pkg:index:type::name"}
		...
		assertDeps := func(actual []Resource) {
			...
			require.NoError(t, err)
			assert.True(t, known)
			assert.Equal(t, URN("urn:khulnasoft:test_stack::test_project::pkg:index:type::name"), value.(URN))
		}
	...
	t.Run("resource", func(t *testing.T) {
		...
		actual, err := unmarshalPropertyMap(ctx, resource.PropertyMap{
			"unknown id": resource.NewResourceReferenceProperty(resource.ResourceReference{
				URN: "urn:khulnasoft:test_stack::test_project::pkg:index:type::name",
			}),
		})
		require.NoError(t, err)
		...
		require.NoError(t, err)
		assert.Equal(t, URN("urn:khulnasoft:test_stack::test_project::pkg:index:type::name"), value)
		assert.True(t, known)
		assert.False(t, secret)
	})
}
