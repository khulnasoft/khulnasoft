...
func marshalInputs(props Input) (resource.PropertyMap, map[string][]URN, []URN, error) {
	...
	case reflect.Struct:
		...
		for i := 0; i < numFields; i++ {
			...
			tag := destField.Tag.Get("khulnasoft")
...
func unmarshalResourceReference(ctx *Context, ref resource.ResourceReference) (Resource, error) {
	...
	isProvider := tokens.Token(resType).HasModuleMember() && resType.Module() == "khulnasoft:providers"
	if isProvider {
...
// keep things as plain types where possible (e.g. a string property value will just be a `khulnasoft.String`, not an
// `OutputString`). It will use `khulnasoft.Output` for values that are either Computed (will always be a
// `khulnasoft.AnyOutput`), secret, or an output property value.
	...
	unmarshal = func(v resource.PropertyValue) (Input, error) {
		...
		case v.IsOutput():
			...
			if v.OutputValue().Known {
				...
				if element != nil {
					// element will be an Input/Output type like khulnasoft.String or khulnasoft.AnyOutput. We want
