...
func construct(ctx context.Context, req *khulnasoftrpc.ConstructRequest, engineConn *grpc.ClientConn,
	...
	khulnasoftCtx, err := NewContext(ctx, runInfo)
	if err != nil {
	...
	for _, urn := range req.GetDependencies() {
		dependencies = append(dependencies, khulnasoftCtx.newDependencyResource(URN(urn)))
	}

	providers := make(map[string]ProviderResource, len(req.GetProviders()))
	for pkg, ref := range req.GetProviders() {
		resource, err := createProviderResource(khulnasoftCtx, ref)
		if err != nil {
			return nil, err
		}
	...
	var parent Resource
	if req.GetParent() != "" {
		parent = khulnasoftCtx.newDependencyResource(URN(req.GetParent()))
	}
	opts := resourceOption(func(ro *resourceOptions) {
		...
		if urn := req.DeletedWith; urn != "" {
			ro.DeletedWith = khulnasoftCtx.newDependencyResource(URN(urn))
		}
	...
	urn, state, err := constructF(khulnasoftCtx, req.GetType(), req.GetName(), inputs, opts)
	if err != nil {
	...
	if err = khulnasoftCtx.wait(); err != nil {
		return nil, err
	}
	...
	rpcProps, err := plugin.MarshalProperties(
		resolvedProps,
		plugin.MarshalOptions{KeepSecrets: true, KeepUnknowns: keepUnknowns, KeepResources: khulnasoftCtx.state.keepResources})
	if err != nil {
		return nil, fmt.Errorf("marshaling properties: %w", err)
	}

	// Convert the property dependencies map for RPC and remove duplicates.
	rpcPropertyDeps := make(map[string]*khulnasoftrpc.ConstructResponse_PropertyDependencies)
	for k, deps := range propertyDeps {
		...
		rpcPropertyDeps[k] = &khulnasoftrpc.ConstructResponse_PropertyDependencies{
			Urns: urns,
		}
	}
...
func copyInputTo(ctx *Context, v resource.PropertyValue, dest reflect.Value) error {
	...
	case v.IsOutput():
		...
		if !dest.Type().Implements(outputType) && !dest.Type().Implements(inputType) {
			return fmt.Errorf("expected destination type to implement khulnasoft.Input or khulnasoft.Output, got %v", dest.Type())
		}
...
func copyToStruct(ctx *Context, v resource.PropertyValue, typ reflect.Type, dest reflect.Value) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		tag := typ.Field(i).Tag.Get("khulnasoft")
...
func constructInputsCopyTo(ctx *Context, inputs map[string]interface{}, args interface{}) error {
	...
	for k, v := range inputs {
		...
		for i := 0; i < typ.NumField(); i++ {
			...
			field := typ.Field(i)
			tag, has := field.Tag.Lookup("khulnasoft")
			tag = strings.Split(tag, ",")[0] // tagName,flag => tagName
			if !has || tag != k {
				continue
			}
			...
			if len(ci.deps) > 0 {
				return fmt.Errorf("copying input %q: %s.%s is typed as %v but must be a type that implements khulnasoft.Input or "+
					"khulnasoft.Output for input with dependencies", k, typ, field.Name, field.Type)
			}
			...
			if secret {
				return fmt.Errorf("copying input %q: %s.%s is typed as %v but must be a type that implements khulnasoft.Input or "+
					"khulnasoft.Output for secret input", k, typ, field.Name, field.Type)
			}
...
func call(ctx context.Context, req *khulnasoftrpc.CallRequest, engineConn *grpc.ClientConn,
	...
	result, failures, err := callF(khulnasoftCtx, req.GetTok(), args)
	if err != nil {
	...
	// Convert the property dependencies map for RPC and remove duplicates.
	rpcPropertyDeps := make(map[string]*khulnasoftrpc.CallResponse_ReturnDependencies)
	for k, deps := range propertyDeps {
		sort.Slice(deps, func(i, j int) bool { return deps[i] < deps[j] })
		...
		rpcPropertyDeps[k] = &khulnasoftrpc.CallResponse_ReturnDependencies{
			Urns: urns,
		}
	}

	var rpcFailures []*khulnasoftrpc.CheckFailure
	if len(failures) > 0 {
		rpcFailures = make([]*khulnasoftrpc.CheckFailure, len(failures))
		for i, v := range failures {
			failure := v.(callFailure)
			rpcFailures[i] = &khulnasoftrpc.CheckFailure{
