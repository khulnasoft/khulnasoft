...
type StackReference struct {
	...
	Name StringOutput `khulnasoft:"name"`
	// Outputs resolves with exports from the named stack
	Outputs MapOutput `khulnasoft:"outputs"`
...
func NewStackReference(ctx *Context, name string, args *StackReferenceArgs,
	...
	if err := ctx.ReadResource("khulnasoft:khulnasoft:StackReference", name, id, args, &ref, opts...); err != nil {
		return nil, err
	}
