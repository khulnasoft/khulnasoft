...
// The test is extracted from a panic using khulnasoft-docker and minified
...
// captured `*khulnasoft.Context` from `ApplyT`. The user program passes a
...
func TestCollapseAliases(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			totalAliasUrns: 1,
			results:        []URN{"urn:khulnasoft:stack::project::test:resource:type$test:resource:child2::myres-child"},
		},
		{
			parentAliases:  []Alias{},
			childAliases:   []Alias{{Name: String("child2")}},
			totalAliasUrns: 1,
			results:        []URN{"urn:khulnasoft:stack::project::test:resource:type$test:resource:child::child2"},
		},
		{
...
func (pr *Prov) i(ctx *Context, t *testing.T) ProviderResource {
	...
	err := ctx.RegisterResource("khulnasoft:providers:"+pr.t, pr.name, nil, p)
	assert.NoError(t, err)
	return p
}
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			err := RunErr(func(ctx *Context) error {
				...
				for k, p := range provMap {
					...
					result = append(result, strings.TrimPrefix(p.getName(), "khulnasoft:providers:"))
				}
...
func TestRegisterResource_aliasesSpecs(t *testing.T) {
	...
	tests := []struct {
		...
		wantAliases   []*khulnasoftrpc.Alias
	...
	}{
		{
			...
			wantAliasURNs: []string{
				"urn:khulnasoft:stack::project::test:resource:type::resA",
				"urn:khulnasoft:stack::project::test:resource:type::resB",
			},
		},
		{
			...
			wantAliases: []*khulnasoftrpc.Alias{
				{
					Alias: &khulnasoftrpc.Alias_Spec_{
						Spec: &khulnasoftrpc.Alias_Spec{
							Name:   "resA",
							Parent: &khulnasoftrpc.Alias_Spec_NoParent{NoParent: true},
						},
					},
				},
				{
		...
		},
		{
			...
			wantAliasURNs: []string{
				"urn:khulnasoft:stack::project::test:resource:parentType$test:resource:type::child",
			},
		},
		{
					...
					Alias: &khulnasoftrpc.Alias_Spec_{
						Spec: &khulnasoftrpc.Alias_Spec{
							...
							Parent: &khulnasoftrpc.Alias_Spec_ParentUrn{
								ParentUrn: "urn:khulnasoft:stack::project::test:resource:parentType::parent",
							},
						},
					},
				},
	...
	for _, tt := range tests {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			var (
				gotAliases   []*khulnasoftrpc.Alias
				gotAliasURNS []string
			)
			monitor := &testMonitor{
				NewResourceF: func(args MockResourceArgs) (string, resource.PropertyMap, error) {
			...
			if !tt.supportsAliasSpecs {
				opts = append(opts, WrapResourceMonitorClient(
					func(rmc khulnasoftrpc.ResourceMonitorClient) khulnasoftrpc.ResourceMonitorClient {
...
func resourceMonitorClientWithoutFeatures(
	cl khulnasoftrpc.ResourceMonitorClient,
	features ...string,
) khulnasoftrpc.ResourceMonitorClient {
