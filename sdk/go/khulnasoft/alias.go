...
type Alias struct {
	...
	// Use `Alias { NoParent: khulnasoft.Bool(true) }` to avoid defaulting to the current parent.
	Parent Resource
	// The previous parent of the resource in URN format, mutually exclusive to `Parent` and `ParentURN`.
	// To specify no original parent, use `Alias { NoParent: khulnasoft.Bool(true) }`.
	ParentURN URNInput
