...
func TestConfig(t *testing.T) {
	...
	assert.EqualError(t, err, "missing required configuration variable 'testpkg:missing'; run `khulnasoft config` to set")
	assert.Equal(t, emptyTestStruct, testStruct)
	...
	assert.EqualError(t, err,
		"missing required configuration variable 'testpkg:missing'; run `khulnasoft config` to set")
	assert.True(t, errors.Is(err, ErrMissingVar))
}
...
func TestSecretConfig(t *testing.T) {
	...
	khulnasoft.All(s1, s2, s3).ApplyT(func(v []interface{}) ([]interface{}, error) {
		for _, val := range v {
	...
	khulnasoft.All(s4, s5, s6).ApplyT(func(v []interface{}) ([]interface{}, error) {
		for _, val := range v {
			ts := val.(*TestStruct)
	...
	khulnasoft.All(s7, s8, s9).ApplyT(func(v []interface{}) ([]interface{}, error) {
		for _, val := range v {
			if val == true {
				resultBool <- val.(bool)
			} else {
	...
	khulnasoft.All(s10, s11, s12).ApplyT(func(v []interface{}) ([]interface{}, error) {
		for _, val := range v {
			if val == 42 {
				resultInt <- val.(int)
			} else {
