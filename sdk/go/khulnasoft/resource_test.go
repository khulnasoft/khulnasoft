...
func TestUninitializedParentResource(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			parent: &myComponent{},
			wantErr: "WARNING: Ignoring component resource *khulnasoft.myComponent " +
		...
		},
		{
			...
			wantErr: "WARNING: Ignoring resource *khulnasoft.myCustomResource " +
				"(parent of my-resource :: test:index:MyResource) " +
...
func TestComponentResourcePropagatesProvider(t *testing.T) {
	...
	t.Run("provider option", func(t *testing.T) {
		...
		err := RunErr(func(ctx *Context) error {
			...
			require.NoError(t,
				ctx.RegisterResource("khulnasoft:providers:test", "prov", nil /* props */, &prov),
...
//
// Regression test for https://github.com/khulnasoft/khulnasoft/issues/12430
func TestRemoteComponentResourcePropagatesProvider(t *testing.T) {
	...
	err := RunErr(func(ctx *Context) error {
		var prov struct{ ProviderResourceState }
		require.NoError(t,
			ctx.RegisterResource("khulnasoft:providers:aws", "myprovider", nil /* props */, &prov),
			"error registering provider")
		...
		NewResourceF: func(args MockResourceArgs) (string, resource.PropertyMap, error) {
			switch args.Name {
			case "myprovider":
				assert.Equal(t, "khulnasoft:providers:aws", args.RegisterRPC.Type)
...
func TestComponentResourceMultipleSingletonProviders(t *testing.T) {
	...
	newTestProvider := func(ctx *Context, pkg, name string) ProviderResource {
		...
		require.NoError(t,
			ctx.RegisterResource("khulnasoft:providers:"+pkg, name, nil /* props */, &prov),
			"error registering provider")
		return &prov
	}
...
func (dt *dependenciesTracker) Wrap(cl khulnasoftrpc.ResourceMonitorClient) khulnasoftrpc.ResourceMonitorClient {
	m := newInterceptingResourceMonitor(cl)
	m.afterRegisterResource = func(in *khulnasoftrpc.RegisterResourceRequest,
		resp *khulnasoftrpc.RegisterResourceResponse,
		err error,
	) {
		var deps []URN
...
type interceptingResourceMonitor struct {
	...
	afterRegisterResource func(req *khulnasoftrpc.RegisterResourceRequest, resp *khulnasoftrpc.RegisterResourceResponse, err error)
}

func newInterceptingResourceMonitor(inner khulnasoftrpc.ResourceMonitorClient) *interceptingResourceMonitor {
...
func (i *interceptingResourceMonitor) RegisterResource(
	ctx context.Context,
	in *khulnasoftrpc.RegisterResourceRequest,
