...
func TestRegisterInputType(t *testing.T) {
	...
	assert.PanicsWithError(t, "expected khulnasoft.Foo to implement internal.Input", func() {
		RegisterInputType(reflect.TypeOf((*Foo)(nil)).Elem(), FooArgs{})
	})
	assert.PanicsWithError(t, "expected khulnasoft.FooArgs to implement interface khulnasoft.FooInput", func() {
...
func TestJSONMarshalNested(t *testing.T) {
	...
	json := JSONMarshal(out)
	v, known, secret, deps, err := await(json)
	assert.EqualError(t, err, "json: error calling MarshalJSON for type khulnasoft.AnyOutput: outputs can not be marshaled to JSON")
	assert.True(t, known)
	assert.False(t, secret)
	assert.Nil(t, deps)
	assert.Nil(t, v)
