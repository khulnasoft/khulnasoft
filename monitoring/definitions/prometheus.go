...
import (
	...
	"github.com/khulnasoft/khulnasoft/monitoring/definitions/shared"
...
func Prometheus() *monitoring.Dashboard {
	const (
		...
		ruleGroupInterpretation = `Rules that Khulnasoft ships with are grouped under '/sg_config_prometheus'. [Custom rules are grouped under '/sg_prometheus_addons'](https://khulnasoft.com/docs/admin/observability/metrics#prometheus-configuration).`
	)
					...
					{
						{
							...
							NextSteps: fmt.Sprintf(`
								...
								- If the rule group taking a long time to evaluate belongs to '/sg_config_prometheus', please [open an issue](https://github.com/khulnasoft/khulnasoft/issues/new?assignees=&labels=&template=bug_report.md&title=).
				...
				Rows: []monitoring.Row{
					{
						...
						},
						{
							...
							NextSteps:      "Ensure that your [`observability.alerts` configuration](https://khulnasoft.com/docs/admin/observability/alerting#setting-up-alerting) (in site configuration) is valid.",
						},
					},
					{
						{
