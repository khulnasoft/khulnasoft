...
Both Sourcegraph Cloud and on-prem instances share the same schema change process through the "[migrator](https://github.com/khulnasoft/khulnasoft/tree/main/cmd/migrator)" service, regardless of the deployment models (Kubernetes, Docker Compose, Single-Docker container).
...
* `up.sql` - contains SQL statements to "upgrade" the database schema ([example](https://github.com/khulnasoft/khulnasoft/blob/main/migrations/frontend/1648524019_int64-ids-user-pending-permissions/up.sql)).
