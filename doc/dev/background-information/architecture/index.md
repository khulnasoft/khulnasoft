...
At its core, Khulnasoft maintains a persistent cache of all repositories that are connected to it. It is persistent, because this data is critical for Khulnasoft to function, but it is ultimately a cache because the code host is the source of truth and our cache is eventually consistent.

- [gitserver](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/cmd/gitserver/README.md) is the sharded service that stores repositories and makes them accessible to other Khulnasoft services.
- [repo-updater](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/cmd/repo-updater/README.md) is the singleton service that is responsible for ensuring all repositories in gitserver are as up-to-date as possible while respecting code host rate limits. It is also responsible for syncing repository metadata from the code host that is stored in the `repo` table of our Postgres database.

If you want to learn more about how repositories are synchronized, read [Life of a repository](life-of-a-repository.md).

## Permission syncing

Repository permissions are by default being mirrored from code hosts to Khulnasoft, it builds the foundation of Khulnasoft authorization for repositories to ensure users see consistent content as on code hosts. Currently, the background permissions syncer resides in the [repo-updater](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/cmd/repo-updater/README.md).
...
Khulnasoft offers the flexibility of customizing settings by users. The settings of a single user is generally the result of merging user settings, organization settings and global settings. Each of these are referred to as a _settings subject_, which are part of the _settings cascade_. They are all exposed over GraphQL.

## Search

Devs can search across all the code that is connected to their Khulnasoft instance.

By default, Khulnasoft uses [zoekt](https://github.com/sourcegraph/zoekt) to create a trigram index of the default branch of every repository so that searches are fast. This trigram index is the reason why Khulnasoft search is more powerful and faster than what is usually provided by code hosts.

- [zoekt-indexserver](https://khulnasoft.com/github.com/sourcegraph/zoekt/-/tree/cmd/zoekt-sourcegraph-indexserver)
- [zoekt-webserver](https://khulnasoft.com/github.com/sourcegraph/zoekt/-/tree/cmd/zoekt-webserver)

Khulnasoft also has a fast search path for code that isn't indexed yet, or for code that will never be indexed (for example: code that is not on a default branch). Indexing every branch of every repository isn't a pragmatic use of resources for most customers, so this decision balances optimizing the common case (searching all default branches) with space savings (not indexing everything).

- [searcher](https://github.com/khulnasoft/khulnasoft/blob/main/cmd/searcher/README.md) implements the non-indexed search.

Syntax highlighting for any code view, including search results, is provided by [Syntect server](https://github.com/khulnasoft/khulnasoft/tree/main/docker-images/syntax-highlighter).
...
- [Khulnasoft extension API](#sourcegraph-extension-api)
...
To create a batch change, users write a [batch spec](../../../batch_changes/references/batch_spec_yaml_reference.md), which is a YAML file that specifies the changes that should be performed, and the repositories that they should be performed upon — either through a Khulnasoft search, or by declaring them directly. This spec is then executed by [src-cli](#src-cli) on the user's machine (or in CI, or some other environment controlled by the user), which results in [changeset specs](../../../batch_changes/explanations/introduction_to_batch_changes.md#changeset-spec) that are sent to Khulnasoft. These changeset specs are then applied by Khulnasoft to create one or more changesets per repository. (A changeset is a pull request or merge request, depending on the code host.)

Once created, changesets are monitored by Khulnasoft, and their current review and CI status can be viewed on the batch change's page, providing a single pane of glass view of all the changesets created as part of the batch change. The batch change can be updated at any time by re-applying the original batch spec: this will transparently add or remove changesets in repositories that now match or don't match the original search as needed.
...
- [Code insights team page](https://handbook.khulnasoft.com/engineering/code-graph/code-insights#code-insights-team)
...
The Khulnasoft browser extensions bring the features of Khulnasoft directly into the UI of code hosts such as GitHub, GitLab and Bitbucket.

With the Khulnasoft browser extension installed, users get Khulnasoft features (including code navigation) on their code host while browsing code, viewing diffs, or reviewing pull requests.

This lets users get value from Khulnasoft without leaving their existing workflows on their code host, while also giving them a convenient way to jump into Khulnasoft at any time (by using the `Open in Khulnasoft` button on any repository or file). The browser extension also adds an address bar search shortcut, allowing you to search on Khulnasoft directly from the browser address bar.

If you want to learn more about browser extensions:

- [Khulnasoft browser extensions documentation](../../../integration/browser_extension.md)
- [Overview of code host integrations](../web/code_host_integrations.md)

## Native integrations (for code hosts)

Native integrations bring Khulnasoft features directly into the UI of code hosts, in a similar way to the browser extension.

Instead of requiring a browser extension, native integrations inject a script by extending the code host directly (for example, using the code host's plugin architecture). The advantage is that Khulnasoft can be enabled for all users of a code host instance, without any action required from each user.
...
src-cli, or `src`, is a command line tool that users can run locally to interact with Khulnasoft.

src-cli is written in Go, and distributed as a standalone binary for Windows, macOS, and Linux. Its features include [running searches](../../../cli/references/search.md), managing Khulnasoft, and [executing batch changes](../../../batch_changes/quickstart.md#create-the-batch-change). src-cli is an integral part of the [batch changes product](#batch-changes).

Note that src-cli is not contained within the Khulnasoft monorepo, and has its own release cadence.
...
Khulnasoft editor extensions will bring Khulnasoft features like search, and code navigation into your IDE. (Switching between Khulnasoft and an IDE when viewing a file is separately powered by Khulnasoft extensions.)
...
Khulnasoft is deployable via three supported methods:
...
- [Docker Single Container](../../../admin/deploy/docker-single-container/index.md) for small environments on a single server. Easiest and quickest to setup with a single command. Little infrastructure knowledge is required. This deployment method is developed in [`cmd/server`](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/cmd/server).

The [resource estimator](https://docs.khulnasoft.com/admin/deploy/resource_estimator can guide you on the requirements for each deployment type.

## Observability

Observability encapsulates the monitoring and debugging of Khulnasoft deployments.
Khulnasoft is designed, and ships with, a number of observability tools and capabilities out-of-the box to enable visibility into the health and state of a Khulnasoft deployment.
...
Learn more about monitoring in the [monitoring architecture overview](https://handbook.khulnasoft.com/engineering/observability/monitoring_architecture).

- [grafana](../observability/grafana.md) is the frontend for service metrics, and ships with customized dashboards for Khulnasoft services.
...
- [cadvisor](../observability/cadvisor.md) provides per-container performance metrics (scraped by Prometheus) in most Khulnasoft environments.
- [Health checks are provided by each Khulnasoft service](../../../admin/observability/health_checks.md).

Debugging includes [tracing](../../../admin/observability/tracing.md) and [logging](../../../admin/observability/logs.md).

- [jaeger](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/docker-images/jaeger-all-in-one) is the distributed tracing service used by Khulnasoft.
- [jaeger-agent](https://khulnasoft.com/github.com/khulnasoft/khulnasoft/-/tree/docker-images/jaeger-agent) is a sidecar used in Kubernetes deployments for collecting traces from services.
- [Logs are provided by each Khulnasoft service](../../../admin/observability/logs.md).
...
- [Observability at Khulnasoft](https://handbook.khulnasoft.com/engineering/observability)
