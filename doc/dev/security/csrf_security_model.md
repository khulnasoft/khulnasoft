# Khulnasoft CSRF security model

This section describes Khulnasoft's security threat model against CSRF (Cross Site Request Forgery) requests, in depth for developers working at Khulnasoft.
...
- [Khulnasoft CSRF security model](#sourcegraph-csrf-security-model)
...
* Nov 8th, 2021: [@slimsag](https://github.com/slimsag) audited all potential instances of pre-fetched content embedded into pages and found we have none, the following is NOT true ([#27236](https://github.com/khulnasoft/khulnasoft/pull/27236)):
  * "Some Khulnasoft pages pre-fetch content: on the backend, data is pre-fetched for the user so that they need not make a request for the data corresponding to the page immediately upon loading it. Instead, we fetch it and embed it into the `GET` page response, giving JavaScript access to it immediately upon page load."
* Nov 8th, 2021: [@slimsag](https://github.com/slimsag) adjusted CORS handling to forbid cross-origin requests on all non-API routes. ([#27240](https://github.com/khulnasoft/khulnasoft/pull/27240), [#27245](https://github.com/khulnasoft/khulnasoft/pull/27245)):
...
* Nov 16th, 2021: [@slimsag](https://github.com/slimsag) removed our CSRF security tokens/cookies entirely, instead having Khulnasoft rely solely on browser's CORS policies to prevent CSRF attacks. [#7658](https://github.com/khulnasoft/khulnasoft/issues/7658)
...
Our CSRF threat model begins and ends at the `sourcegraph-frontend` layer. This is the service in which all HTTP requests reaching Khulnasoft, be they a user's web browser, or via our API, ultimately go through.

This does not cover additional load balancers, proxies, CDNs, etc. that one may put in front of Khulnasoft:

* Some of our customers choose to place Khulnasoft behind nginx or apache, which may offer additional layers of security.
* For Khulnasoft.com we place Khulnasoft behind Cloudflare and it's WAF, for additional security, rate limiting, etc.
* For managed instances, we place Khulnasoft behind Google GCP's Cloud Load Balancer and Cloud Armor. [details here](https://github.com/sourcegraph/security-issues/issues/158#issuecomment-867038398)

## What is CSRF, why is it dangerous?

See also: [OWASP: CSRF](https://owasp.org/www-community/attacks/csrf)

CSRF (Cross Site Request Forgery) is when a legitimate user is browsing another site, say either attacker.com (ran by a malicious actor), or google.com (a legitimate site, perhaps running code by a malicious actor) makes requests to your own site, say khulnasoft.com, and is able to perform actions on behalf of the user that they did not intend to, using their own authentication credentials—often unbeknownst to them.
...
For example, say a Khulnasoft user clicks a malicious link and Khulnasoft is not protected against CSRF. This would mean that, for example, a `<form>` element could be silently submitted in the background to perform destructive actions on behalf of the user using Khulnasoft's API—such as deleting data on Khulnasoft.
...
In Khulnasoft, we delineate between two types of requests that reach the frontend (generally the only place HTTP requests make their way into Khulnasoft):
...
* The Khulnasoft browser extension, which browsers provide a distinct ORIGIN for (separate from the domain they are executing on.)
* The `src` CLI, running on dev laptops, in CI pipelines, on servers, etc.
* Users via `curl` or various programming languages.
* Code host integrations, e.g. the Khulnasoft plugin running server-side on a Bitbucket Server / Bitbucket Data Center instance—or GitLab's integration.
...
* The Khulnasoft application itself (however, most often this goes through `/.internal` which is unauthenticated and never exposed publicly.)
...
`window.context` is served with each request. For example, if you make a request via `curl https://khulnasoft.com/search` you'll find each GET request for a page introduces context to JavaScript. This _only contains unprivileged, public content_—which is very important as otherwise it could be vulnerable to caching (e.g. if Cloudflare caches a GET request for user A and serves it to user B later):

```
	<script ignore-csp>
		window.context = {"externalURL":"https://khulnasoft.com","xhrHeaders":{"X-Requested-With":"Khulnasoft","x-sourcegraph-client":"https://khulnasoft.com"},"userAgentIsBot":false,"assetsRoot":"/.assets","version":"105021_2021-08-13_2c6eb84","isAuthenticatedUser":false,"sentryDSN":"https://ae2f74442b154faf90b5ff0f7cd1c618@sentry.io/1391511","siteID":"KhulnasoftWeb","siteGQLID":"U2l0ZToic2l0ZSI=","debug":false,"needsSiteInit":false,"emailEnabled":true,"site":{"auth.public":true,"authz.enforceForSiteAdmins":true,"update.channel":"release"},"needServerRestart":false,"deployType":"kubernetes","sourcegraphDotComMode":true,"accessTokensAllow":"all-users-create","allowSignup":true,"resetPasswordEnabled":true,"externalServicesUserMode":"public","authProviders":[{"isBuiltin":true,"displayName":"Builtin username-password authentication","serviceType":"builtin","authenticationURL":""},{"isBuiltin":false,"displayName":"GitHub","serviceType":"github","authenticationURL":"/.auth/github/login?pc=https%3A%2F%2Fgithub.com%2F%3A%3Ae917b2b7fa9040e1edd4"},{"isBuiltin":false,"displayName":"GitLab","serviceType":"gitlab","authenticationURL":"/.auth/gitlab/login?pc=https%3A%2F%2Fgitlab.com%2F%3A%3A51686001d882eae9c23bbc3d976ca07ba52c5b14fd099230a1463ac9c37f2b8b"}],"branding":{"brandName":"Khulnasoft"},"batchChangesEnabled":false,"codeIntelAutoIndexingEnabled":true,"productResearchPageEnabled":true,"experimentalFeatures":{"jvmPackages":"enabled","ranking":{"maxReorderQueueSize":16},"rateLimitAnonymous":500,"search.index.branches":{"github.com/go-yaml/yaml":["v2","v3"],"github.com/kubernetes/kubernetes":["release-1.17"],"github.com/khulnasoft/khulnasoft":["3.17","v3.0.0"]}}}
...
They are [registered here in code](https://khulnasoft.com/github.com/sourcegraph/sourcegraph@aefef0dab971608196310402c051f77b4b2e3c22/-/blob/cmd/frontend/internal/app/app.go?L57-65).
...
* THEN we would not be at risk of CSRF attacks at all, because even if `<form>`, `<img>`, and JavaScript on foreign pages could make requests to Khulnasoft's HTTP API:
  * The API routes are protected (they forbid cookie-based authentication unless a custom `X-Requested-With: Khulnasoft` header is present, more on this below.)
...
2. Site admins can configure trusted CORS domains via the site configuration's [`corsOrigin`](https://khulnasoft.com/search?q=context:global+repo:%5Egithub%5C.com/sourcegraph/sourcegraph%24+file:%5Eschema/site%5C.schema%5C.json+corsOrigin&patternType=literal) setting, but this setting DOES NOT affect our response to these non-API endpoints (cross-origin requests to non-API endpoints are always denied.)
...
1. Session cookies, via the [`session.CookieMiddlewareWithCSRFSafety`](https://khulnasoft.com/search?q=context:global+repo:%5Egithub%5C.com/sourcegraph/sourcegraph%24%40aefef0d+CookieMiddlewareWithCSRFSafety%28&patternType=literal) middleware. This allows session cookie authentication iff one of the following is true:
...
2. Authentication tokens, created in the Khulnasoft UI (also via the API)—checked through the [`AccessTokenAuthMiddleware`](https://khulnasoft.com/search?q=context:global+repo:%5Egithub%5C.com/sourcegraph/sourcegraph%24%40aefef0d+AccessTokenAuthMiddleware%28&patternType=literal) and specified by either:
...
1. First and foremost, by restricting session cookie-based authentication. As described above, session cookie authentication is prohibited in non-simple CORS requests which means cookies CANNOT be used to authenticate a Khulnasoft API request unless it comes from an allowed origin, passing the browser's CORS policy checks. This is the primary means by which we protect against CSRF in our API endpoints.
...
Because of point #1 above (we allow session based authentication iff the request passed CORS policy checks), it is not possible for anyone currently to use the Khulnasoft API endpoints from other, non-explicitly-allowed domains. For example:

* We cannot use the Khulnasoft search API from khulnasoft.com
* People cannot use the Khulnasoft.com GraphQL API from their own websites, even in authenticated form (i.e. public resolvers only)
* Customers cannot use the Khulnasoft API from their private instance on their own internal tool websites—even by having users provide an access token. The request would be forbidden as it would not pass the CORS preflight request.
...
I [@slimsag](https://github.com/slimsag) advise we make the following key improvements to our CSRF threat model in order to improve security, product reliability, and reduce risky behavior of both developers at Khulnasoft and customers on their own private instances:
