...
Deno.serve(async (req) => {
  try {
    if (req.method === 'GET') {
      return new Response(
        'What is Supabase not? POST {answer, twitter} to this URL! Need a hint? 👉 https://github.com/khulnasoft/khulnasoft/blob/master/examples/edge-functions/khulnasoft/functions/sentry/index.ts'
      )
    }
    let answer: string
    let twitter: string
...
/* To invoke locally:
  ...
  1. Run `supabase start` (see: https://khuknasoft.com/docs/reference/cli/supabase-start)
