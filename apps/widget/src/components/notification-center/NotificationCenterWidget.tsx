...
import {
  ...
  ITab,
  IStore,
  useKhulnasoftContext,
  ColorScheme,
  IUserPreferenceSettings,
} from '@khulnasoft/notification-center';
import type { IKhulnasoftThemeProvider, INotificationCenterStyles } from '@khulnasoft/notification-center';
import { IMessage, IOrganizationEntity, ButtonTypeEnum } from '@khulnasoft/shared';
...
export function NotificationCenterWidget(props: INotificationCenterWidgetProps) {
  ...
  const [theme, setTheme] = useState<IKhulnasoftThemeProvider>({});
    ...
    <>
      ...
      {frameInitialized && (
        <KhulnasoftProvider
          ...
          styles={styles}
        >
          <KhulnasoftNotificationCenterWrapper doLogout={doLogout} setDoLogout={setDoLogout}>
            <NotificationCenter
            ...
            />
          </KhulnasoftNotificationCenterWrapper>
        </KhulnasoftProvider>
      )}
    </>
  );
...
function KhulnasoftNotificationCenterWrapper({
  children,
  doLogout,
  setDoLogout,
}: {
  ...
  const { logout } = useKhulnasoftContext();
  useEffect(() => {
    if (doLogout) {
      logout();
      setDoLogout(false);
