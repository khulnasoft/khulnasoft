...
const body = 'To join the Khulnasoft project, click the link below';
...
test.beforeEach(async ({ session }) => {
  ...
  const newWorkflow = workflow(workflowId, async ({ step }) => {
    await step.inApp(
      ...
      async (controls) => {
        return {
          subject: `Hi ${controls.name}! You've been invited to join the Khulnasoft project`,
          body,
        };
      },
      {
...
test('sync workflow', async ({ page }) => {
  ...
  await expect(page).toHaveTitle(`${workflowId} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Configure ${inAppStepId} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Edit ${inAppStepId} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(previewElements.subject).toContainText(`Hi Tim! You've been invited to join the Khulnasoft project`);
  ...
  await expect(page).toHaveTitle(`Trigger ${workflowId} | Khulnasoft Cloud Dashboard`);
