...
test('manage workflows', async ({ page }) => {
  ...
  const subject = 'You have been invited to join the Khulnasoft project';
  const body = "Hello {{payload.name}}! You've been invited to join the Khulnasoft project";

  const workflowsPage = new WorkflowsPage(page);
  await workflowsPage.goTo();
  await expect(page).toHaveTitle(`Workflows | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`${workflowName} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`${workflowNameUpdated} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Edit ${inAppStepName} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Configure ${inAppStepName} | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Configure ${inAppStepNameUpdated} | Khulnasoft Cloud Dashboard`);
  ...
  // check the step config sidebar
  await expect(page).toHaveTitle(`Configure Digest Step | Khulnasoft Cloud Dashboard`);
  ...
  await expect(page).toHaveTitle(`Trigger ${workflowNameUpdated} | Khulnasoft Cloud Dashboard`);
