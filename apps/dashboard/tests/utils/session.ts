...
import { DalService, EnvironmentEntity, OrganizationEntity, UserEntity } from '@khulnasoft/dal';
...
export class Session {
  ...
  async initialize() {
    ...
    const email = `${emailPrefix}@khulnasoft.co`;
    ...
    this.user = await this.userService.createKhulnasoftUser({ externalId: clerkUser.id });

    const clerkOrganization = await this.organizationService.createClerkOrganization({
      name: faker.company.name(),
      createdBy: clerkUser.id,
    });
    this.organization = await this.organizationService.createKhulnasoftOrganization({
