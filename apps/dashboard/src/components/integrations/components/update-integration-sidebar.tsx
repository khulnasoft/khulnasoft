...
import { ChannelTypeEnum, providers as khulnasoftProviders } from '@khulnasoft/shared';
...
export function UpdateIntegrationSidebar({ isOpened }: UpdateIntegrationSidebarProps) {
  ...
  const provider = khulnasoftProviders?.find((p) => p.id === integration?.providerId);
  ...
  async function onSubmit(data: IntegrationFormData, skipPrimaryCheck?: boolean) {
     ...
     */
    if (integration?.providerId === 'khulnasoft-email' || integration?.providerId === 'khulnasoft-sms') {
      data.check = false;
    }
    ...
    try {
      await updateIntegration({
        ...
        data: {
          ...
          check: data.check,
          removeKhulnasoftBranding: data.removeKhulnasoftBranding,
        },
      });
