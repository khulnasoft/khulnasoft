...
import { ChannelTypeEnum, ChatProviderIdEnum, IProviderConfig, PushProviderIdEnum } from '@khulnasoft/shared';
import { providers, EmailProviderIdEnum, SmsProviderIdEnum } from '@khulnasoft/shared';
import { ProvidersIdEnum } from '@khulnasoft/shared';
  ...
  const filteredIntegrations = useMemo(() => {
    ...
    const filtered = providers.filter(
      (provider: IProviderConfig) =>
        ...
        provider.id !== EmailProviderIdEnum.Khulnasoft &&
        provider.id !== SmsProviderIdEnum.Khulnasoft
    );
