...
import { ChannelTypeEnum, IIntegration } from '@khulnasoft/shared';
...
function getProviderDescription(providerType: ChannelTypeEnum): string {
  return providerType === ChannelTypeEnum.IN_APP
    ...
    : `Connect your provider to send ${providerType} notifications with Khulnasoft.`;
}
...
function isActiveIntegration(integration: IIntegration, providerType: ChannelTypeEnum): boolean {
  ...
  const isNotKhulnasoftProvider = !integration.providerId.startsWith('khulnasoft-');
  const isConnected = providerType === ChannelTypeEnum.IN_APP ? !!integration.connected : true;

  return isMatchingChannel && isNotKhulnasoftProvider && isConnected;
}
  ...
  const steps = useMemo(
    (): Step[] => [
      ...
      },
      {
        ...
        description: 'Workflows in Khulnasoft, orchestrate notifications across channels.',
