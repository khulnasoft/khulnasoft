import { getV2, KhulnasoftApiError } from '@/api/api.client';
...
import type { IEnvironment, WorkflowListResponseDto, WorkflowResponseDto } from '@khulnasoft/shared';
import { WorkflowOriginEnum, WorkflowStatusEnum } from '@khulnasoft/shared';
...
export function useSyncWorkflow(workflow: WorkflowResponseDto | WorkflowListResponseDto) {
  ...
  const isSyncable = useMemo(
    () => workflow.origin === WorkflowOriginEnum.KHULNASOFT_CLOUD && workflow.status !== WorkflowStatusEnum.ERROR,
  ...
  const getTooltipContent = () => {
    ...
    if (workflow.origin === WorkflowOriginEnum.KHULNASOFT_CLOUD_V1) {
  ...
  const safeSync = async (envId: string) => {
    try {
      ...
      if (error instanceof KhulnasoftApiError && error.status === 404) {
