...
export class KhulnasoftApiError extends Error {
  constructor(
  ...
  try {
    ...
    const config: RequestInit = {
      ...
      headers: {
        ...
        ...(environment && { 'Khulnasoft-Environment-Id': environment._id }),
        ...headers,
      },
      signal,
    };
    ...
    const baseUrl = API_HOSTNAME ?? 'https://api.khulnasoft.co';
    const response = await fetch(`${baseUrl}/${version}${endpoint}`, config);

    if (!response.ok) {
      const errorData = await response.json();
      throw new KhulnasoftApiError(parseErrorMessage(errorData), response.status, errorData);
    }
    ...
    if (error instanceof KhulnasoftApiError) {
      throw error;
    }
...
function parseErrorMessage(errorData: any): string {
  const DEFAULT_ERROR = 'Khulnasoft API error';
