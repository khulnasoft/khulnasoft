...
import { topicsList } from '@khulnasoft/api/funcs/topicsList';
import { FilterTopicsResponseDto } from '@khulnasoft/api/src/models/components/filtertopicsresponsedto';
...
describe('Khulnasoft Node.js package - Retries and idempotency-key', () => {
  it('should retry trigger and generate idempotency-key only once for request', async () => {
    ...
    khulnasoftClient = new Khulnasoft({
      security: {
        secretKey: 'fakeKey',
      },
  ...
  it('should generate different idempotency-key for each request', async () => {
    ...
    await khulnasoftClient.trigger({ workflowId: 'fake-workflow', to: { subscriberId: '123' }, payload: {} });
    await khulnasoftClient.trigger({ workflowId: 'fake-workflow', to: { subscriberId: '123' }, payload: {} });
  ...
  NON_RECOVERABLE_ERRORS.forEach(([status, message]) => {
    it('should not retry on non-recoverable %i error', async () => {
      ...
      const result = await topicsList(khulnasoftClient, {});
  ...
  it('should retry on various errors until it reaches successful response', async () => {
    ...
    const { error, ok, value } = await topicsList(khulnasoftClient, {});
