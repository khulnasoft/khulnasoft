...
import { TracingModule } from '@khulnasoft/application-generic';
import { Client, KhulnasoftModule } from '@khulnasoft/framework/nest';
...
import { usageLimitsWorkflow } from '@khulnasoft/notifications';
...
const enterpriseImports = (): Array<Type | DynamicModule | Promise<DynamicModule> | ForwardReference> => {
  ...
  if (process.env.KHULNASOFT_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
    ...
    if (require('@khulnasoft/ee-billing')?.BillingModule) {
      modules.push(require('@khulnasoft/ee-billing')?.BillingModule.forRoot());
    }
...
const enterpriseQuotaThrottlerInterceptor =
  (process.env.KHULNASOFT_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') &&
  require('@khulnasoft/ee-billing')?.QuotaThrottlerInterceptor
    ? [
        {
          provide: APP_INTERCEPTOR,
          useClass: require('@khulnasoft/ee-billing')?.QuotaThrottlerInterceptor,
        },
      ]
    : [];
...
modules.push(
  KhulnasoftModule.register({
    apiPath: '/bridge/khulnasoft',
    client: new Client({
      secretKey: process.env.KHULNASOFT_INTERNAL_SECRET_KEY,
      strictAuthentication:
        process.env.NODE_ENV === 'production' ||
        process.env.NODE_ENV === 'dev' ||
        process.env.KHULNASOFT_STRICT_AUTHENTICATION_ENABLED === 'true',
    }),
