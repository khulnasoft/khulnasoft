import { DEFAULT_NOTIFICATION_RETENTION_DAYS, FeatureFlagsKeysEnum, StringifyEnv } from '@khulnasoft/shared';
...
export const envValidators = {
  ...
  MONGO_URL: str(),
  KHULNASOFT_API_KEY: str({ default: '' }),
  ...
  API_ROOT_URL: url(),
  KHULNASOFT_INVITE_TEAM_MEMBER_NUDGE_TRIGGER_IDENTIFIER: str({ default: undefined }),
  SUBSCRIBER_WIDGET_JWT_EXPIRATION_TIME: str({ default: '15 days' }),
  KHULNASOFT_REGION: str({ default: 'local' }),
  KHULNASOFT_SECRET_KEY: str({ default: '' }),

  // Khulnasoft Cloud third party services
  ...(processEnv.IS_SELF_HOSTED !== 'true' &&
    processEnv.KHULNASOFT_ENTERPRISE === 'true' && {
      ...
      KHULNASOFT_INTERNAL_SECRET_KEY: str({ default: '' }),
    }),
  ...
  ...(processEnv.STORAGE_SERVICE === 'AZURE' && {
    ...
    AZURE_CONTAINER_NAME: str({ default: 'khulnasoft' }),
  }),
