...
import { EnvironmentEntity, EnvironmentRepository, OrganizationRepository } from '@khulnasoft/dal';
...
export class UpdateVercelConfiguration {
  ...
  private async updateBridgeUrl(environmentId: string, projectIds: string, accessToken: string) {
    try {
      ...
      const fullBridgeUrl = `https://${bridgeUrl}/api/khulnasoft`;
    ...
    const apiKeys = [
      {
        ...
        type,
        value: clientKey,
        key: 'KHULNASOFT_CLIENT_APP_ID',
      },
      {
        target,
        type,
        value: privateKey,
        key: 'KHULNASOFT_SECRET_KEY',
      },
      {
        target,
        type,
        value: clientKey,
        key: 'NEXT_PUBLIC_KHULNASOFT_CLIENT_APP_ID',
      },
    ];
  ...
  private mapProjects(projects: any[], newAndUpdatedProjectIds: string[], removedProjectIds: string[]) {
    return projects.reduce<AllMappedProjectData>(
      (acc, curr) => {
        ...
        const clientEnv = vercelEnvs?.find((e) => e.key === 'KHULNASOFT_CLIENT_APP_ID');
        const secretEnv = vercelEnvs?.find((e) => e.key === 'KHULNASOFT_SECRET_KEY');
