...
describe('Delete Messages By TransactionId - /messages/?transactionId= (DELETE) #khulnasoft-v2', function () {
  ...
  it('should fail to delete non existing message', async function () {
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.messages.deleteByTransactionId('abc-1234')
    );
  ...
  it('should delete messages by transactionId', async function () {
    ...
    const res = await khulnasoftClient.triggerBroadcast({
    ...
    await khulnasoftClient.messages.deleteByTransactionId(transactionId);
  ...
  it('should delete messages by transactionId and channel', async function () {
    const response = await khulnasoftClient.triggerBroadcast({
      name: template.triggers[0].identifier,
      payload: {
        email: 'new-test-email@gmail.com',
        firstName: 'Testing of User Name',
    ...
    await khulnasoftClient.messages.deleteByTransactionId(transactionId, ChannelTypeEnum.EMAIL);
