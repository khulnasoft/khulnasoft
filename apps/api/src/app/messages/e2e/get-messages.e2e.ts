...
import { ChannelTypeEnum } from '@khulnasoft/api/models/components';
import { initKhulnasoftClassSdk } from '../../shared/helpers/e2e/sdk/e2e-sdk.helper';

describe('Get Message - /messages (GET) #khulnasoft-v2', function () {
  ...
  it('should fetch existing messages', async function () {
    ...
    let response = await khulnasoftClient.messages.retrieve({});
    expect(response.result.data.length).to.be.equal(4);

    response = await khulnasoftClient.messages.retrieve({ channel: ChannelTypeEnum.Email });
    expect(response.result.data.length).to.be.equal(2);

    response = await khulnasoftClient.messages.retrieve({ subscriberId: subscriber2.subscriberId });
    expect(response.result.data.length).to.be.equal(2);
  });
  ...
  it('should fetch messages using transactionId filter', async function () {
    ...
    let response = await khulnasoftClient.messages.retrieve({ subscriberId: subscriber3.subscriberId });
    ...
    response = await khulnasoftClient.messages.retrieve({ transactionId: [transactionId1] });
    expect(response.result.data.length).to.be.equal(4);

    response = await khulnasoftClient.messages.retrieve({ transactionId: [transactionId1, transactionId2] });
    expect(response.result.data.length).to.be.equal(6);

    response = await khulnasoftClient.messages.retrieve({ transactionId: [transactionId2] });
    expect(response.result.data.length).to.be.equal(2);
  });
  ...
  async function triggerEventWithTransactionId(
    ...
    return await khulnasoftClient.trigger({
