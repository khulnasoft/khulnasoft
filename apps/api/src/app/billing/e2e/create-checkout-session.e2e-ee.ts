...
describe('Create checkout session #khulnasoft-v2', async () => {
  if (!require('@khulnasoft/ee-billing').CreateCheckoutSession) {
    throw new Error("CreateCheckoutSession doesn't exist");
  }

  const { CreateCheckoutSession } = require('@khulnasoft/ee-billing');
