...
describe('Rename a topic - /topics/:topicKey (PATCH) #khulnasoft-v2', async () => {
  ...
  it('should rename the topic and return it if exists in the database', async () => {
    ...
    const patchResponse = await khulnasoftClient.topics.rename({ name: renamedTopicName }, topicKey);
  ...
  it('should throw a not found error when the topic id provided does not exist in the database', async () => {
    ...
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.topics.rename({ name: renamedTopicName }, nonExistingId)
    );
  ...
  async function createTopicSubscriberAndAttach() {
    ...
    const controllerAssignResponse = await khulnasoftClient.topics.subscribers.assign(
      {
        subscribers,
      },
      topicKey
