...
describe('Add subscribers to topic - /topics/:topicKey/subscribers (POST) #khulnasoft-v2', async () => {
  ...
  it('should add subscriber to topic', async () => {
    ...
    const response = await khulnasoftClient.topics.subscribers.assign({ subscribers }, topicKey);
  ...
  it('should not duplicate subscribers if adding a subscriber already added to topic', async () => {
    const preconditionResponse = await khulnasoftClient.topics.retrieve(topicKey);
  ...
  it('should create topic on the fly when adding existing subscriber to topic if the topic does not exist', async () => {
    ...
    const response = await khulnasoftClient.topics.subscribers.assign({ subscribers }, nonExistingTopicKey);

    expect(response.result).to.eql({
      succeeded: subscribers,
    });

    const getResponse = await khulnasoftClient.topics.retrieve(nonExistingTopicKey);
