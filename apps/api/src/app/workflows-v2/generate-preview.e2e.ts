...
import {
  ...
  HttpError,
  KhulnasoftRestResult,
  RedirectTargetEnum,
  StepTypeEnum,
  WorkflowCreationSourceEnum,
} from '@khulnasoft/shared';
import { EmailControlType, InAppControlType } from '@khulnasoft/application-generic';
...
describe('Generate Preview #khulnasoft-v2', () => {
  ...
  async function patchStepWithControlValues(workflowSlug: string, stepSlug: string, controlValues: InAppControlType) {
    const khulnasoftRestResult1 = await workflowsClient.patchWorkflowStepData(workflowSlug, stepSlug, {
      controlValues,
    });
    if (!khulnasoftRestResult1.isSuccessResult()) {
      throw new Error('shoud patch');
    }

    return khulnasoftRestResult1.value;
  }
  ...
  describe('Generate Preview', () => {
    ...
    describe('payload sanitation', () => {
      ...
      it('Should not fail if inApp is providing partial URL in redirect', async () => {
        ...
        const khulnasoftRestResult = await workflowsClient.createWorkflow(createDto);
        ...
        const workflowSlug = khulnasoftRestResult.value?.slug;
        const stepSlug = khulnasoftRestResult.value?.steps[0].slug;
  ...
  function getHeaders(): HeadersInit {
    return {
      ...
      'Khulnasoft-Environment-Id': session.environment._id,
    };
  }
...
async function assertHttpError(
  description: string,
  khulnasoftRestResult: KhulnasoftRestResult<GeneratePreviewResponseDto, HttpError>,
  dto: GeneratePreviewRequestDto
) {
  if (khulnasoftRestResult.error) {
    return new Error(
      `${description}: Failed to generate preview: ${khulnasoftRestResult.error.message}payload: ${JSON.stringify(dto, null, 2)} `
    );
  }
...
export async function generatePreview(
  ...
  const khulnasoftRestResult = await workflowsClient.generatePreview(workflowId, stepDatabaseId, dto);
  if (khulnasoftRestResult.isSuccessResult()) {
    return khulnasoftRestResult.value;
  }

  throw await assertHttpError(description, khulnasoftRestResult, dto);
}
