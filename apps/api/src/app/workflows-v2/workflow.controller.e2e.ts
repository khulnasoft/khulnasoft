...
function getHeaders(overrideEnv?: string): HeadersInit {
  return {
    ...
    'Khulnasoft-Environment-Id': overrideEnv || session.environment._id,
  };
}

describe('Workflow Controller E2E API Testing #khulnasoft-v2', () => {
  ...
  describe('Error Handling', () => {
    ...
    describe('Workflow Step Body Issues', () => {
      ...
      it('should remove control issues when no longer present', async () => {
        ...
        const khulnasoftRestResult = await workflowsClient.updateWorkflow(workflowCreated._id, {
          ...workflowCreated,
          steps: [{ ...inAppStep, name: 'New Name', controlValues: { body: 'some body here' } }],
        });
        if (!khulnasoftRestResult.isSuccessResult()) {
          throw new Error(khulnasoftRestResult.error!.responseText);
        }

        const updatedWorkflow = khulnasoftRestResult.value;
  ...
  describe('Update Workflow Permutations', () => {
    it('should update control values', async () => {
      ...
      const updateRequest: UpdateWorkflowDto = {
        origin: WorkflowOriginEnum.KHULNASOFT_CLOUD,
  ...
  describe('List Workflow Permutations', () => {
    ...
    async function createV1Workflow() {
      const khulnasoftRestResult = await workflowsClient.createWorkflowsV1({
      ...
      if (!khulnasoftRestResult.isSuccessResult()) {
        throw new Error(`Failed to create V1 Workflow ${JSON.stringify(khulnasoftRestResult.error)}`);
      }

      return khulnasoftRestResult.value;
    }

    async function searchWorkflowsV1(workflowId?: string) {
      const khulnasoftRestResult = await workflowsClient.searchWorkflowsV1(workflowId);
      if (!khulnasoftRestResult.isSuccessResult()) {
  ...
  describe('Get Workflow Permutations', () => {
    ...
    it('should return 404 if workflow does not exist', async () => {
      ...
      const khulnasoftRestResult = await workflowsClient.getWorkflow(notExistingId);
      expect(khulnasoftRestResult.isSuccess).to.be.false;
      expect(khulnasoftRestResult.error).to.be.ok;
      expect(khulnasoftRestResult.error!.status).to.equal(404);
      expect(khulnasoftRestResult.error!.responseText).to.contain('Workflow');
      const parse = JSON.parse(khulnasoftRestResult.error!.responseText);
  ...
  describe('Get Step Data Permutations', () => {
    ...
    describe('Variables', () => {
      ...
      it('should serve previous step variables with payload schema', async () => {
        ...
        const khulnasoftRestResult = await workflowsClient.getWorkflowStepData(res.value._id, res.value.steps[1]._id);
        if (!khulnasoftRestResult.isSuccessResult()) {
          throw new Error(khulnasoftRestResult.error!.responseText);
        }
        const { variables } = khulnasoftRestResult.value;
  ...
  async function patchWorkflowAndReturnResponse(workflowId: string, active: boolean) {
    const khulnasoftRestResult = await workflowsClient.patchWorkflow(workflowId, {
      active,
    });
    if (!khulnasoftRestResult.isSuccessResult()) {
      throw new Error(khulnasoftRestResult.error!.responseText);
  ...
  async function createWorkflowRest(newVar: CreateWorkflowDto): Promise<WorkflowResponseDto> {
    const khulnasoftRestResult = await workflowsClient.createWorkflow(newVar);
    if (!khulnasoftRestResult.isSuccessResult()) {
      throw new Error(khulnasoftRestResult.error!.responseText);
    }
  ...
  async function patchStepRest(workflowDatabaseId: string, stepId1: string, patchStepDataDto: PatchStepDataDto) {
    const khulnasoftRestResult = await workflowsClient.patchWorkflowStepData(
      workflowDatabaseId,
      stepId1,
      patchStepDataDto
    );
  ...
  async function updateWorkflowRest(id: string, workflow: UpdateWorkflowDto): Promise<WorkflowResponseDto> {
    const khulnasoftRestResult = await workflowsClient.updateWorkflow(id, workflow);
    if (khulnasoftRestResult.isSuccessResult()) {
      return khulnasoftRestResult.value;
    }
    throw new Error(khulnasoftRestResult.error!.responseText);
  ...
  async function getStepData(workflowId: string, stepId: string, envId?: string) {
    const khulnasoftRestResult = await createWorkflowClient(session.serverUrl, getHeaders(envId)).getWorkflowStepData(
      workflowId,
      stepId
    );
    if (!khulnasoftRestResult.isSuccessResult()) {
    ...
    const { value } = khulnasoftRestResult;

    return value;
  }

  async function getWorkflowTestData(workflowId: string, envId?: string) {
    const khulnasoftRestResult = await createWorkflowClient(session.serverUrl, getHeaders(envId)).getWorkflowTestData(
      workflowId
    );
    if (!khulnasoftRestResult.isSuccessResult()) {
      throw new Error(khulnasoftRestResult.error!.responseText);
  ...
  function assertWorkflowResponseBodyData(workflowResponseDto: WorkflowResponseDto) {
    ...
    expect(workflowResponseDto.origin, stringify(workflowResponseDto)).to.be.eq(WorkflowOriginEnum.KHULNASOFT_CLOUD);
...
interface ApiResponse {
  req: {
    ...
    headers: {
      ...
      'khulnasoft-environment-id': string; // e.g., "66e929c6667852862a1e50e4"
    };
  };
  header: {
