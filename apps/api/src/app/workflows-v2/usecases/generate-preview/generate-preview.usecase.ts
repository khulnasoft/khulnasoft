...
import { channelStepSchemas, actionStepSchemas } from '@khulnasoft/framework/internal';
  ...
  async execute(command: GeneratePreviewCommand): Promise<GeneratePreviewResponseDto> {
    try {
      ...
      const sanitizedValidatedControls =
        workflow.origin === WorkflowOriginEnum.KHULNASOFT_CLOUD
          ? this.sanitizeControlsForPreview(initialControlValues, stepData)
          : initialControlValues;

      if (!sanitizedValidatedControls && workflow.origin === WorkflowOriginEnum.KHULNASOFT_CLOUD) {
        throw new Error(
