...
import {
  ...
  CalculateLimitKhulnasoftIntegrationCommand,
  NotificationStep,
...
export class GetActiveIntegrationsStatus {
  constructor(
    ...
    private calculateLimitKhulnasoftIntegrationUsecase: CalculateLimitKhulnasoftIntegration
  ) {}
  ...
  async execute(command: GetActiveIntegrationsStatusCommand): Promise<WorkflowResponse[] | WorkflowResponse> {
    ...
    const activeStateByChannelTypeWithKhulnasoft = await this.processKhulnasoftProviders(
      activeIntegrationsByEnv,
      command,
      activeStateByChannelType
    );

    return this.updateActiveIntegrationsStatus(command.workflows, activeStateByChannelTypeWithKhulnasoft);
  }
  ...
  private async processKhulnasoftProviders(
    activeIntegrations: IntegrationResponseDto[],
    command: GetActiveIntegrationsStatusCommand,
    stateByChannelType: WorkflowChannelsIntegrationStatus
  ) {
    const primaryKhulnasoftProviders = activeIntegrations.filter(
      (integration) =>
        (integration.providerId === EmailProviderIdEnum.Khulnasoft ||
          integration.providerId === SmsProviderIdEnum.Khulnasoft) &&
        integration.primary
    );

    for (const primaryKhulnasoftProvider of primaryKhulnasoftProviders) {
      const channelType = primaryKhulnasoftProvider.channel;
      let hasLimitReached = true;
      const limit = await this.calculateLimitKhulnasoftIntegrationUsecase.execute(
        CalculateLimitKhulnasoftIntegrationCommand.create({
          channelType,
          environmentId: command.environmentId,
          organizationId: command.organizationId,
        })
