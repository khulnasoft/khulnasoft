...
import { ChannelTypeEnum, StepTypeEnum } from '@khulnasoft/shared';
import { Khulnasoft } from '@khulnasoft/api';
import { triggerBulk } from '@khulnasoft/api/funcs/triggerBulk';
import { TriggerEventRequestDto } from '@khulnasoft/api/models/components';
import { z } from 'zod';
import { KhulnasoftCore } from '@khulnasoft/api/core';
import {
  expectSdkValidationExceptionGeneric,
  initKhulnasoftClassSdk,
  initKhulnasoftFunctionSdk,
} from '../../shared/helpers/e2e/sdk/e2e-sdk.helper';

describe('Trigger bulk events - /v1/events/trigger/bulk (POST) #khulnasoft-v2', function () {
  ...
  let khulnasoftCore: KhulnasoftCore;
  ...
  beforeEach(async () => {
    ...
    khulnasoftCore = initKhulnasoftFunctionSdk(session);
  });

  it('should return the response array in correct order', async function () {
    const bulkTriggerResponse = await triggerBulk(khulnasoftCore, {
      events: [
        {
          transactionId: '1111',
  ...
  it('should generate message and notification based on a bulk event', async function () {
    await khulnasoftClient.triggerBulk({
      events: [
        {
          workflowId: template.triggers[0].identifier,
          to: [
