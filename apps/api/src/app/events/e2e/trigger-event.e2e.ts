...
import { SubscribersService, UserSession, WorkflowOverrideService } from '@khulnasoft/testing';
import {
  ActorTypeEnum,
  ChannelTypeEnum,
  ChatProviderIdEnum,
...
import { EmailEventStatusEnum } from '@khulnasoft/stateless';
import { DetailEnum } from '@khulnasoft/application-generic';
import { Khulnasoft } from '@khulnasoft/api';
import { SubscriberPayloadDto } from '@khulnasoft/api/src/models/components/subscriberpayloaddto';
import { CreateIntegrationRequestDto, TriggerEventResponseDto } from '@khulnasoft/api/models/components';
...
describe('Trigger event - /v1/events/trigger (POST) #khulnasoft-v2', function () {
  ...
  describe(`Trigger Event - /v1/events/trigger (POST)`, function () {
    ...
    it('should trigger an event successfully', async function () {
      const response = await khulnasoftClient.trigger({
        workflowId: template.triggers[0].identifier,
        to: [subscriber.subscriberId],
        payload: {
    ...
    it('should store jobs & message provider id successfully', async function () {
      ...
      expect(inAppMessage?.providerId).to.equal(InAppProviderIdEnum.Khulnasoft);
      ...
      expect(inAppJob?.providerId).to.equal(InAppProviderIdEnum.Khulnasoft);
    ...
    it('should trigger in-app notification', async function () {
      ...
      await khulnasoftClient.trigger({
        workflowId: template.triggers[0].identifier,
        to: [
          { subscriberId: 'no_type_123', lastName: 'smith_no_type', email: 'test@email.khulnasoft' },
          {
            type: 'Subscriber',
    ...
    it('should use Khulnasoft integration for new orgs', async function () {
      process.env.KHULNASOFT_EMAIL_INTEGRATION_API_KEY = 'true';
      ...
      expect(message!.providerId).to.equal(EmailProviderIdEnum.Khulnasoft);
    });
    ...
    it('should broadcast trigger to all subscribers', async () => {
      ...
      await khulnasoftClient.triggerBroadcast({
        name: template.triggers[0].identifier,
        payload: {
          organizationName: 'Umbrella Corp',
        },
    ...
    describe('Post Mortem', function () {
      ...
      it(`should not create multiple subscribers when multiple triggers are made        
        ...
        for (let i = 0; i < 3; i += 1) {
          ...
          await Promise.all([
            simpleTrigger(khulnasoftClient, template, subscriberId),
            simpleTrigger(khulnasoftClient, template, subscriberId),
          ]);
          await session.waitForJobCompletion(template._id);
    ...
    describe('filters logic', () => {
      ...
      it('should trigger message with override integration identifier', async function () {
        ...
        const { result } = await khulnasoftClient.integrations.create(payload);
      ...
      describe('workflow override', () => {
        ...
        it('should override - active false', async function () {
          ...
          const triggerResponse = await khulnasoftClient.trigger({
          ...
          const triggerResponse2 = await khulnasoftClient.trigger({
            workflowId: workflow.triggers[0].identifier,
            to: [subscriberOverride],
            tenant: tenant.identifier,
            payload: {
  ...
  async function sendTrigger(
    ...
    const request = {
      ...
      to: [{ subscriberId: newSubscriberIdInAppNotification, lastName: 'Smith', email: 'test@email.khulnasoft' }],
      payload: {
        organizationName: 'Umbrella Corp',
        compiledVariable: 'test-env',
        ...payload,
    ...
    return (await khulnasoftClient.trigger(request)).result;
  }
...
function simpleTrigger(khulnasoftClient: Khulnasoft, template, subscriberID: string) {
  return khulnasoftClient.trigger({
    workflowId: template.triggers[0].identifier,
    to: [subscriberID],
    payload: {
      firstName: 'Testing of User Name',
