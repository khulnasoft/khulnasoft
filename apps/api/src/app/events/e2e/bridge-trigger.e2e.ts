...
contexts.forEach((context: Context) => {
  describe('Self-Hosted Bridge Trigger #khulnasoft-v2', async () => {
    ...
    beforeEach(async () => {
      ...
      bridge = context.isStateful ? undefined : { url: `${bridgeServer.serverPath}/khulnasoft` };
    ...
    it(`should skip inApp step and execute email step when userName is John Doe [${context.name}]`, async () => {
            ...
            },
            {
              controlSchema: {
                ...
                properties: {
                  subject: {
                    type: 'string',
                    default: `A Successful Test on Khulnasoft from default_name`,
                  },
                },
              } as const,
            }
      ...
      expect(emailMessages[0].subject).to.include('A Successful Test on Khulnasoft from default_name');
    });
    ...
    it(`should execute both inApp and email steps when userName is not John Doe [${context.name}]`, async () => {
      ...
      const newWorkflow = workflow(
        ...
        async ({ step, payload }) => {
          ...
          await step.email(
            ...
            async () => {
              return {
                subject: `Welcome to Khulnasoft ${payload.userName}`,
                body: `This is your first Khulnasoft Email ${payload.userName}`,
              };
            },
            {
      ...
      expect(emailMessages.length).to.eq(1);
      expect(emailMessages[0].subject).to.include('Welcome to Khulnasoft Jane Doe');
    });
  });
});

describe('Khulnasoft-Hosted Bridge Trigger #khulnasoft-v2', () => {
  let session: UserSession;
  const messageRepository = new MessageRepository();
  let subscriber: SubscriberEntity;
  let subscriberService: SubscribersService;
  ...
  it('should execute a Khulnasoft-managed workflow', async () => {
...
async function syncWorkflow(
  ...
  await session.testAgent.post(`/v1/bridge/sync`).send({
    bridgeUrl: `${bridgeServer.serverPath}/khulnasoft`,
  });
