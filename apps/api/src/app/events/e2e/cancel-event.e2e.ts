...
import { JobRepository, JobStatusEnum, NotificationTemplateEntity, SubscriberEntity } from '@khulnasoft/dal';
import { DelayTypeEnum, DigestTypeEnum, DigestUnitEnum, StepTypeEnum } from '@khulnasoft/shared';
...
describe('Cancel event - /v1/events/trigger/:transactionId (DELETE) #khulnasoft-v2', function () {
  ...
  async function cancelEvent(transactionId: string) {
    // TODO: Replace with await khulnasoftClient.cancel(transactionId) when the response validation error is fixed
  ...
  it('should cancel a digest step', async function () {
    ...
    const { result } = await khulnasoftClient.trigger({
  ...
  it.skip('should cancel a digest after it has already digested some triggers', async function () {
    ...
    const { result: result3 } = await khulnasoftClient.trigger({
      workflowId: template.triggers[0].identifier,
      to: [subscriber.subscriberId],
      payload: {
        customVar: 'trigger_2_data',
  ...
  it.skip('should be able to cancel 1st main digest and then its follower', async function () {
    ...
    const { result: result4 } = await khulnasoftClient.trigger({
      workflowId: template.triggers[0].identifier,
      to: [subscriber.subscriberId],
      payload: {
