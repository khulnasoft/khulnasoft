...
describe('Trigger event - Send message email - /v1/events/trigger (POST) #khulnasoft-v2', function () {
  it('should merge mail data', function () {
    ...
    let result = createMailData(defaultMailData, {
      to: ['override-to@khulnasoft.co'],
      from: 'override-from@khulnasoft.co',
      text: 'text',
      cc: ['cc@khulnasoft.co'],
      bcc: ['bcc@khulnasoft.co'],
    });

    expect(result.to).to.deep.equal(['to-reply@khulnasoft.co', 'override-to@khulnasoft.co']);
    expect(result.from).to.equal('override-from@khulnasoft.co');
    expect(result.text).to.equal('text');
    expect(result.cc).to.deep.equal(['cc@khulnasoft.co']);
    expect(result.bcc).to.deep.equal(['bcc@khulnasoft.co']);
    ...
    expect(result.to).to.deep.equal(['override-to@khulnasoft.co']);
  });
