...
} from '@khulnasoft/api/models/components';
import { initKhulnasoftClassSdk } from '../../shared/helpers/e2e/sdk/e2e-sdk.helper';

describe('Topic Trigger Event #khulnasoft-v2', () => {
  describe('Trigger event for a topic - /v1/events/trigger (POST)', () => {
    ...
    it('should trigger an event successfully', async () => {
      const response = await khulnasoftClient.trigger(buildTriggerRequestPayload(template, to));
    ...
    it('should generate message and notification based on event', async () => {
      ...
      await khulnasoftClient.trigger(buildTriggerRequestPayload(template, to, attachments));
    ...
    it('should trigger SMS notification', async () => {
      ...
      await khulnasoftClient.trigger(buildTriggerRequestPayload(template, to));
  ...
  describe('Trigger event for multiple topics and multiple subscribers - /v1/events/trigger (POST)', () => {
    ...
    beforeEach(async () => {
      ...
      to = [
        ...
        fifthSubscriber.subscriberId,
        {
          ...
          lastName: 'Define',
          email: 'subscribers-define@email.khulnasoft',
        },
      ];
      khulnasoftClient = initKhulnasoftClassSdk(session);
    });
    ...
    it('should trigger an event successfully', async () => {
      ...
      const response = await khulnasoftClient.trigger(buildTriggerRequestPayload(template, localTo));
...
const createTopic = async (session: UserSession, key: TopicKey, name: TopicName): Promise<CreateTopicResponseDto> => {
  const response = await initKhulnasoftClassSdk(session).topics.create({ key, name });
...
const addSubscribersToTopic = async (
  ...
  const response = await initKhulnasoftClassSdk(session).topics.subscribers.assign(
    {
...
const triggerEvent = async (
  ...
  await initKhulnasoftClassSdk(session).trigger({
    workflowId: template.triggers[0].identifier,
    to,
    payload,
  });
