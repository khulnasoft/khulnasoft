...
import { IntegrationEntity, OrganizationRepository } from '@khulnasoft/dal';
import { ChannelTypeEnum, EmailProviderIdEnum, IEmailOptions, WorkflowOriginEnum } from '@khulnasoft/shared';
  ...
  public async execute(command: SendTestEmailCommand) {
    ...
    if (email && integration) {
      const mailData: IEmailOptions = {
        ...
        subject,
        html: html as string,
        from: (command.payload.$sender_email as string) || integration?.credentials.from || 'no-reply@khulnasoft.co',
      };
