...
import { SubscribersControllerSearchSubscribersRequest } from '@khulnasoft/api/src/models/operations';
import { OrderDirection } from '@khulnasoft/api/models/operations';
...
describe('Subscriber Controller E2E API Testing #khulnasoft-v2', () => {
  ...
  describe('Create Subscriber', () => {
    it.skip(`should not create multiple subscribers when multiple triggers are made        
      ...
      for (let i = 0; i < 2; i += 1) {
        ...
        await Promise.all([
          khulnasoftClient.subscribers.create({ subscriberId, firstName: 'TestSubFName', lastName: 'TestSubLName' }),
          khulnasoftClient.subscribers.create({ subscriberId, firstName: 'TestSubFName', lastName: 'TestSubLName' }),
        ]);
  ...
  async function createSubscriberAndValidate(nameSuffix: string = '') {
    ...
    const res = await khulnasoftClient.subscribers.create(createSubscriberDto);
  ...
  async function getListSubscribers(params: SubscribersControllerSearchSubscribersRequest = {}) {
    const res = await khulnasoftClient.subscribers.search(params);
