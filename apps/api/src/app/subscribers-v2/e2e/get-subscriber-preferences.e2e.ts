...
describe('Get Subscriber Preferences - /subscribers/:subscriberId/preferences (GET) #khulnasoft-v2', () => {
  ...
  it('should fetch subscriber preferences with default values', async () => {
    const response = await khulnasoftClient.subscribers.preferences.list(subscriber.subscriberId);
  ...
  it('should return 404 if subscriber does not exist', async () => {
    ...
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.subscribers.preferences.list(invalidSubscriberId)
    );
  ...
  it('should inherit channel preferences from global settings when no workflow override exists', async () => {
    ...
    await khulnasoftClient.subscribers.preferences.update(
      {
        channels: {
          email: false,
          inApp: true,
