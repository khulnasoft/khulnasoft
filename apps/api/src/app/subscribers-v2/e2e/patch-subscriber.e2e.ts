...
describe('Update Subscriber - /subscribers/:subscriberId (PATCH) #khulnasoft-v2', () => {
  ...
  it('should update the fields of the subscriber', async () => {
    ...
    const res = await khulnasoftClient.subscribers.patch(payload, subscriber.subscriberId);
  ...
  it('should return 404 if subscriberId does not exist', async () => {
    ...
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.subscribers.patch(payload, invalidSubscriberId)
    );
  ...
  it('should return the original subscriber if no fields are updated', async () => {
    const res = await khulnasoftClient.subscribers.patch({}, subscriber.subscriberId);
