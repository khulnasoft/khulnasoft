...
describe('Mark as Seen - /widgets/messages/mark-as (POST) #khulnasoft-v2', async () => {
  ...
  it('should change the seen status', async function () {
    await khulnasoftClient.subscribers.messages.markAllAs(
      {
  ...
  it('should throw exception if messages were not provided', async function () {
    ...
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.subscribers.messages.markAllAs(
        {
          messageId: [],
          markAs: MessagesStatusEnum.SEEN,
        },
