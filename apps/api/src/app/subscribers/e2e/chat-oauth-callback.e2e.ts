...
import { IntegrationRepository, SubscriberRepository } from '@khulnasoft/dal';
import { createHash } from '@khulnasoft/application-generic';
...
describe('ChatOauthCallback - /:subscriberId/credentials/:providerId/:environmentId/callback (GET) #khulnasoft-v2', function () {
  ...
  it('should return action script', async () => {
    ...
    const res = await khulnasoftClient.subscribers.authentication.chatAccessOauthCallBack(
      {
        code: 'code_123',
  ...
  it('should throw an exception when looking for integration with invalid environmentId', async () => {
    ...
    const { error } = await expectSdkExceptionGeneric(() =>
      khulnasoftClient.subscribers.authentication.chatAccessOauthCallBack({
        code: 'code_123',
