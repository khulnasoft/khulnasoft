...
import { ChannelTypeEnum, MessagesStatusEnum } from '@khulnasoft/shared';
...
describe('Mark All Subscriber Messages - /subscribers/:subscriberId/messages/mark-all (POST) #khulnasoft-v2', function () {
  ...
  async function markAllSubscriberMessagesAs(subscriberId: string, markAs: MessagesStatusEnum) {
    const res = await khulnasoftClient.subscribers.messages.markAll({ markAs }, subscriberId);

    return res.result;
  }
  async function getSubscriberNotifications(subscriberId: string) {
    const res = await khulnasoftClient.subscribers.notifications.feed({
      subscriberId,
      limit: 100,
    });
