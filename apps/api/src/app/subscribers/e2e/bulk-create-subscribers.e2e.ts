...
import { SubscriberEntity, SubscriberRepository } from '@khulnasoft/dal';
...
describe('Bulk create subscribers - /v1/subscribers/bulk (POST) #khulnasoft-v2', function () {
  ...
  it('should return the response array in correct format', async function () {
    const bulkResult = await khulnasoftClient.subscribers.createBulk({
      subscribers: [
        {
          subscriberId: 'test1',
          firstName: 'sub1',
  ...
  it('should create and update subscribers', async function () {
    const res = await khulnasoftClient.subscribers.createBulk({
      subscribers: [
        {
          subscriberId: 'sub1',
          firstName: 'John',
  ...
  it('should throw an error when sending more than 500 subscribers', async function () {
    ...
    const { error } = await expectSdkValidationExceptionGeneric(() =>
      khulnasoftClient.subscribers.createBulk({
  ...
  it('should recreate deleted subscribers', async function () {
    ...
    let bulkResponse = await khulnasoftClient.subscribers.createBulk({
    ...
    await khulnasoftClient.subscribers.delete(newSubscriber1.subscriberId);
    await khulnasoftClient.subscribers.delete(newSubscriber2.subscriberId);

    bulkResponse = await khulnasoftClient.subscribers.createBulk({
      subscribers: [existingSubscriber, newSubscriber1, newSubscriber2],
    });
