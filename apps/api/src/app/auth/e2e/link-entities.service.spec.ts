...
import { CLERK_ORGANIZATION_1, CLERK_USER_1, ClerkClientMock } from '@khulnasoft/testing';
...
describe('Link external and internal entities #khulnasoft-v2', () => {
  let eeAuth: any;

  try {
    eeAuth = require('@khulnasoft/ee-auth');
  } catch (error) {
    return;
  }
  ...
  const createKhulnasoftIntegrations = {
    execute: sinon.stub().resolves({ _id: new mongoose.Types.ObjectId() }),
  };
  ...
  sinon.stub(CreateKhulnasoftIntegrationsCommand, 'create').returns({});
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: CreateKhulnasoftIntegrations, useValue: createKhulnasoftIntegrations },
  ...
  it.skip('should create new user and organization when no internal entities exist', async () => {
    ...
    sinon.assert.calledTwice(createKhulnasoftIntegrations.execute);
  });
