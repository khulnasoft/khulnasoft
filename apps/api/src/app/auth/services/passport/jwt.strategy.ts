...
import { Instrument, HttpRequestHeaderKeysEnum } from '@khulnasoft/application-generic';
...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  resolveEnvironmentId(req: http.IncomingMessage, session: UserSessionData) {
    const environmentIdFromHeader =
      (req.headers[HttpRequestHeaderKeysEnum.KHULNASOFT_ENVIRONMENT_ID.toLowerCase()] as string) || '';
