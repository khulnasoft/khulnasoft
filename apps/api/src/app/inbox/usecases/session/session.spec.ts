...
import { AnalyticsService, CreateOrUpdateSubscriberUseCase, SelectIntegration } from '@khulnasoft/application-generic';
...
describe('Session', () => {
  ...
  it('should return correct removeKhulnasoftBranding value when is set on the integration', async () => {
    const command: SessionCommand = {
      applicationIdentifier: 'app-id',
      subscriberId: 'subscriber-id',
      subscriberHash: 'hash',
    ...
    const integrationWithoutRemoveKhulnasoftBranding = { ...mockIntegration, credentials: { hmac: false } };
    const notificationCount = { data: [{ count: 10, filter: {} }] };
    const token = 'token';

    environmentRepository.findEnvironmentByIdentifier.resolves(environment as any);
    selectIntegration.execute.resolves(integrationWithoutRemoveKhulnasoftBranding);
    createSubscriber.execute.resolves(subscriber as any);
    notificationsCount.execute.resolves(notificationCount);
    authService.getSubscriberWidgetToken.resolves(token);
    ...
    expect(response.removeKhulnasoftBranding).to.equal(false);

    const integrationWithInvalidRemoveKhulnasoftBranding = {
      ...mockIntegration,
      credentials: { hmac: false },
      removeKhulnasoftBranding: false,
    };
    selectIntegration.execute.resolves(integrationWithInvalidRemoveKhulnasoftBranding as any);

    const responseWithRemoveKhulnasoftBranding: SubscriberSessionResponseDto = await session.execute(command);

    expect(responseWithRemoveKhulnasoftBranding.removeKhulnasoftBranding).to.equal(false);

    const integrationWithValidRemoveKhulnasoftBranding = {
      ...mockIntegration,
      credentials: { hmac: false },
      removeKhulnasoftBranding: true,
    };
    selectIntegration.execute.resolves(integrationWithValidRemoveKhulnasoftBranding);

    const responseWithValidRemoveKhulnasoftBranding: SubscriberSessionResponseDto = await session.execute(command);

    expect(responseWithValidRemoveKhulnasoftBranding.removeKhulnasoftBranding).to.equal(true);
  });
