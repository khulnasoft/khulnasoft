...
import { API_KEY_SWAGGER_SECURITY_NAME, BEARER_SWAGGER_SECURITY_NAME } from '@khulnasoft/application-generic';
...
function buildBaseOptions() {
  const options = new DocumentBuilder()
    .setTitle('Khulnasoft API')
    .setDescription('Khulnasoft REST API. Please see https://docs.khulnasoft.com/api-reference for more details.')
    .setVersion('1.0')
    .setContact('Khulnasoft Support', 'https://discord.gg/khulnasoft', 'support@khulnasoft.co')
    .setExternalDoc('Khulnasoft Documentation', 'https://docs.khulnasoft.co')
    .setTermsOfService('https://khulnasoft.com/terms')
    .setLicense('MIT', 'https://opensource.org/license/mit')
    .addServer('https://api.khulnasoft.co')
    .addServer('https://eu.api.khulnasoft.co')
    ...
    .addTag(
      ...
      { url: 'https://docs.khulnasoft.com/workflows' }
    )
    .addTag(
      'Subscribers',
      `A subscriber in Khulnasoft represents someone who should receive a message. A subscriber’s profile information contains important attributes about the subscriber that will be used in messages (name, email). The subscriber object can contain other key-value pairs that can be used to further personalize your messages.`,
      { url: 'https://docs.khulnasoft.com/subscribers/subscribers' }
    )
    .addTag(
      'Topics',
      `Topics are a way to group subscribers together so that they can be notified of events at once. A topic is identified by a custom key. This can be helpful for things like sending out marketing emails or notifying users of new features. Topics can also be used to send notifications to the subscribers who have been grouped together based on their interests, location, activities and much more.`,
      { url: 'https://docs.khulnasoft.com/subscribers/topics' }
    )
    .addTag(
      'Notification',
      ...
      { url: 'https://docs.khulnasoft.com/getting-started/introduction' }
    )
    .addTag(
      'Integrations',
      `With the help of the Integration Store, you can easily integrate your favorite delivery provider. During the runtime of the API, the Integrations Store is responsible for storing the configurations of all the providers.`,
      { url: 'https://docs.khulnasoft.com/channels-and-providers/integration-store' }
    )
    .addTag(
      'Layouts',
      `Khulnasoft allows the creation of layouts - a specific HTML design or structure to wrap content of email notifications. Layouts can be manipulated and assigned to new or existing workflows within the Khulnasoft platform, allowing users to create, manage, and assign these layouts to workflows, so they can be reused to structure the appearance of notifications sent through the platform.`,
      { url: 'https://docs.khulnasoft.com/content-creation-design/layouts' }
    )
    .addTag(
      'Workflows',
    ...
    .addTag(
      ...
      { url: 'https://docs.khulnasoft.com/platform/environments#promoting-pending-changes-to-production' }
    )
    .addTag(
      'Environments',
      `Khulnasoft uses the concept of environments to ensure logical separation of your data and configuration. This means that subscribers, and preferences created in one environment are never accessible to another.`,
      { url: 'https://docs.khulnasoft.com/platform/environments' }
    )
    .addTag(
      'Inbound Parse',
      ...
      { url: 'https://docs.khulnasoft.com/platform/inbound-parse-webhook' }
    )
    .addTag(
      'Feeds',
      `Khulnasoft provides a notification activity feed that monitors every outgoing message associated with its relevant metadata. This can be used to monitor activity and discover potential issues with a specific provider or a channel type.`,
      { url: 'https://docs.khulnasoft.com/activity-feed' }
    )
    .addTag(
      'Tenants',
      `A tenant represents a group of users. As a developer, when your apps have organizations, they are referred to as tenants. Tenants in Khulnasoft provides the ability to tailor specific notification experiences to users of different groups or organizations.`,
      { url: 'https://docs.khulnasoft.com/tenants' }
    )
    .addTag(
      'Messages',
      `A message in Khulnasoft represents a notification delivered to a recipient on a particular channel. Messages contain information about the request that triggered its delivery, a view of the data sent to the recipient, and a timeline of its lifecycle events. Learn more about messages.`,
      { url: 'https://docs.khulnasoft.com/workflows/messages' }
    )
    .addTag(
      'Organizations',
      `An organization serves as a separate entity within your Khulnasoft account. Each organization you create has its own separate integration store, workflows, subscribers, and API keys. This separation of resources allows you to manage multi-tenant environments and separate domains within a single account.`,
      { url: 'https://docs.khulnasoft.com/platform/organizations' }
    )
    .addTag(
      'Execution Details',
...
function buildOpenApiBaseDocument(internalSdkGeneration: boolean | undefined) {
  ...
  if (process.env.KHULNASOFT_ENTERPRISE === 'true' || process.env.CI_EE_TEST === 'true') {
    options.addTag(
      'Translations',
      `Translations are used to localize your messages for different languages and regions. Khulnasoft provides a way to create and manage translations for your messages. You can create translations for your messages in different languages and regions, and assign them to your subscribers based on their preferences.`,
      { url: 'https://docs.khulnasoft.com/content-creation-design/translations' }
    );
  }
