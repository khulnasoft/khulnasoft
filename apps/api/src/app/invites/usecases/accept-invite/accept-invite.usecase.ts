...
import { MemberEntity, MemberRepository, OrganizationRepository, UserEntity, UserRepository } from '@khulnasoft/dal';
...
export class AcceptInvite {
  ...
  async sendInviterAcceptedEmail(inviter: UserEntity, member: MemberEntity) {
    ...
    try {
      if ((process.env.NODE_ENV === 'dev' || process.env.NODE_ENV === 'production') && process.env.KHULNASOFT_API_KEY) {
        const khulnasoft = new Khulnasoft({ security: { secretKey: process.env.KHULNASOFT_API_KEY } });

        await khulnasoft.trigger({
          workflowId: process.env.KHULNASOFT_TEMPLATEID_INVITE_ACCEPTED || 'invite-accepted-dEQAsKD1E',
          to: [
            {
