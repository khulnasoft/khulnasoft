import { CommunityOrganizationRepository, EnvironmentRepository, IntegrationRepository } from '@khulnasoft/dal';
...
describe('Update Integration - /integrations/:integrationId (PUT) #khulnasoft-v2', function () {
  ...
  it('should not set the primary if there is only Khulnasoft active integration', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
  ...
  it('should not disable the khulnasoft integration and clear the primary flag if the integration is updated', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
  ...
  it('should update removeKhulnasoftBranding when organization is not on free tier', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
    ...
    expect(data.removeKhulnasoftBranding).to.equal(true);
    ...
    expect(updatedIntegration?.removeKhulnasoftBranding).to.equal(true);
  });

  it('should not update removeKhulnasoftBranding when organization is on free tier', async function () {
    await integrationRepository.deleteMany({
      _organizationId: session.organization._id,
      _environmentId: session.environment._id,
    });
    ...
    expect(data.removeKhulnasoftBranding).to.be.undefined;
    ...
    expect(updatedIntegration?.removeKhulnasoftBranding).to.be.undefined;
  });
});
