...
describe('Get Integrations - /integrations (GET) #khulnasoft-v2', function () {
  ...
  it('should retrieve all the integrations of all environments from an organization from the prefilled test data', async () => {
    ...
    const emailIntegrations = integrations
      ...
      .filter((integration) => integration.providerId !== EmailProviderIdEnum.Khulnasoft);
    ...
    const smsIntegrations = integrations
      ...
      .filter((integration) => integration.providerId !== SmsProviderIdEnum.Khulnasoft);
