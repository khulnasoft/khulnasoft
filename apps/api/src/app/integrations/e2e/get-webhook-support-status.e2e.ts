...
import { IntegrationEntity, IntegrationRepository } from '@khulnasoft/dal';

describe('Get Webhook Support Status - /webhook/provider/:providerOrIntegrationId/status (GET) #khulnasoft-v0', function () {
  ...
  it('should throw bad request error for chat, push, in-app integrations', async () => {
    ...
    await checkBadRequestIntegration(
      khulnasoftIntegration,
      `Webhook for ${khulnasoftIntegration.providerId}-${khulnasoftIntegration.channel} is not supported yet`
    );
  });
