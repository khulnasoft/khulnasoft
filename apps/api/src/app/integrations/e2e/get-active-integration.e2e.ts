import { UserSession, IntegrationService } from '@khulnasoft/testing';
...
describe('Get Active Integrations - Multi-Provider Configuration - /integrations/active (GET) #khulnasoft-v2', function () {
  ...
  it('should have return empty array if no active integration are exist', async function () {
    ...
    const normalizeIntegration = response.body.data.filter(
      (integration) => !integration.providerId.includes('khulnasoft')
    );
