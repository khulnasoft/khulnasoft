...
import { DalException, IntegrationEntity, IntegrationQuery, IntegrationRepository } from '@khulnasoft/dal';
import {
...
export class CreateIntegration {
  ...
  private async validate(command: CreateIntegrationCommand): Promise<void> {
    ...
    if (
      existingIntegration &&
      command.providerId === InAppProviderIdEnum.Khulnasoft &&
    ...
    if (
      (command.providerId === SmsProviderIdEnum.Khulnasoft && !areKhulnasoftSmsCredentialsSet()) ||
      (command.providerId === EmailProviderIdEnum.Khulnasoft && !areKhulnasoftEmailCredentialsSet())
    ) {
      throw new BadRequestException(
        `Creating Khulnasoft integration for ${command.providerId} provider is not allowed`
      );
    }

    if (command.providerId === SmsProviderIdEnum.Khulnasoft || command.providerId === EmailProviderIdEnum.Khulnasoft) {
      ...
      if (count > 0) {
        throw new ConflictException(
          `Integration with khulnasoft provider for ${command.channel.toLowerCase()} channel already exists`
        );
      }
    }
