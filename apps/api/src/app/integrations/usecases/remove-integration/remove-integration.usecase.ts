...
import { IntegrationRepository, DalException, IntegrationEntity } from '@khulnasoft/dal';
import { CHANNELS_WITH_PRIMARY, ChannelTypeEnum, EmailProviderIdEnum, SmsProviderIdEnum } from '@khulnasoft/shared';
import { buildIntegrationKey, InvalidateCacheService } from '@khulnasoft/application-generic';
  ...
  async execute(command: RemoveIntegrationCommand) {
    try {
      ...
      if (this.isBuiltInIntegration(existingIntegration)) {
        throw new BadRequestException('Khulnasoft demo integration or In-App integration cannot be deleted');
      }
