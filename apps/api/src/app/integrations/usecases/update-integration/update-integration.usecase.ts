...
import { ApiServiceLevelEnum, CHANNELS_WITH_PRIMARY, FeatureFlagsKeysEnum } from '@khulnasoft/shared';
...
export class UpdateIntegration {
  ...
  private async shouldUpdateRemoveKhulnasoftBranding(
    ...
    const isRemoveKhulnasoftBrandingDefined = typeof command.removeKhulnasoftBranding !== 'undefined';
    const isRemoveKhulnasoftBrandingChanged =
      isRemoveKhulnasoftBrandingDefined &&
      existingIntegration.removeKhulnasoftBranding !== command.removeKhulnasoftBranding;

    if (!isRemoveKhulnasoftBrandingChanged) {
      return false;
    }
  ...
  async execute(command: UpdateIntegrationCommand): Promise<IntegrationEntity> {
    ...
    const shouldUpdateRemoveKhulnasoftBranding = await this.shouldUpdateRemoveKhulnasoftBranding(
      command,
      existingIntegration
    );
    if (shouldUpdateRemoveKhulnasoftBranding) {
      updatePayload.removeKhulnasoftBranding = command.removeKhulnasoftBranding;
    }
