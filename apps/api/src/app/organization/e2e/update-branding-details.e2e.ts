import { processTestAgentExpectedStatusCode, UserSession } from '@khulnasoft/testing';
import { expect } from 'chai';

describe('Update Branding Details - /organizations/branding (PUT) #khulnasoft-v0-os', function () {
  ...
  it('should update the branding details', async function () {
    const payload = {
      ...
      fontFamily: 'Nunito',
      logo: 'https://s3.us-east-1.amazonaws.com/khulnasoft-app-bucket/2/1/3.png',
    };
  ...
  it('logo should be an https protocol', async () => {
    const payload = {
      logo: 'http://s3.us-east-1.amazonaws.com/khulnasoft-app-bucket/2/1/3.png',
    };
  ...
  ['png', 'jpg', 'jpeg', 'gif', 'svg'].forEach((extension) => {
    it(`should update if logo is a valid image URL with ${extension} extension`, async function () {
      const payload = {
        logo: `https://s3.us-east-1.amazonaws.com/khulnasoft-app-bucket/2/1/3.${extension}`,
      };
