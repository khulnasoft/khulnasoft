...
import { IntegrationRepository, EnvironmentRepository, OrganizationRepository } from '@khulnasoft/dal';
import { EmailProviderIdEnum, SmsProviderIdEnum } from '@khulnasoft/shared';

import { AppModule } from '../../src/app.module';

export async function run() {
  console.log('Update the primary and priority fields for inactive Khulnasoft integrations\n');
  ...
  for (const environment of environments) {
    ...
    const integrations = await integrationRepository.find(
      {
        ...
        primary: true,
        providerId: {
          $in: [EmailProviderIdEnum.Khulnasoft, SmsProviderIdEnum.Khulnasoft],
        },
      },
      undefined,
      { sort: { createdAt: -1 } }
    ...
    console.log(
      `Found ${integrations.length} inactive and primary Khulnasoft integrations ` +
    ...
    if (ids.length > 0) {
      console.log(`Updating Khulnasoft integrations with: `, { primary: false, priority: 0 });
