...
- `api_key`: KHULNASOFT API Key to use for authentication, defaults to `KHULNASOFT_API_KEY` environment variable
...
- `api_key`: API key to use for authentication, defaults to `KHULNASOFT_API_KEY` environment variable
- `request_timeout`: Timeout for the request in **seconds**
