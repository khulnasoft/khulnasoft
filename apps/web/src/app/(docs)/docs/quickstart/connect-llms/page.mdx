# Connect LLMs to KHULNASOFT
KhulnaSoft can work with any LLM and AI framework. The easiest way to connect an LLM to KhulnaSoft is to use the tool use capabilities of the LLM (sometimes known as function calling).
...
<CodeGroup>
```python
# pip install openai khulnasoft-code-interpreter
...
# Execute code in KhulnaSoft Sandbox
if code:
...
<CodeGroup>
```python
# pip install anthropic khulnasoft-code-interpreter
...
<CodeGroup>
```python
# pip install mistralai khulnasoft-code-interpreter
import os
...
# pip install groq khulnasoft-code-interpreter
import os
...
<CodeGroup>
```js
// npm install ai @ai-sdk/openai @khulnasoft/code-interpreter
...
// Run the code in KhulnaSoft Sandbox
...
<CodeGroup>
```js
// npm install ai @ai-sdk/openai zod @khulnasoft/code-interpreter
import { openai } from '@ai-sdk/openai'
import { generateText } from 'ai'
import z from 'zod'
import { Sandbox } from '@khulnasoft/code-interpreter'
...
<CodeGroup>
```python
# pip install crewai crewai[tools] khulnasoft-code-interpreter
...
<CodeGroup>
```python
# pip install langchain langchain-openai khulnasoft-code-interpreter
...
# Define a tool that uses the KhulnaSoft Sandbox
khulnasoft_sandbox_tool = Tool(
    name="execute_python",
    func=execute_python,
    description="Execute python code in a Jupyter notebook cell and return result"
)
...
llm = ChatOpenAI(model="gpt-4o").bind_tools([khulnasoft_sandbox_tool])
...
<CodeGroup>
```python
# pip install llama-index khulnasoft-code-interpreter
...
khulnasoft_sandbox_tool = FunctionTool.from_defaults(
    name="execute_python",
    description="Execute python code in a Jupyter notebook cell and return result",
    fn=execute_python
)
...
agent = ReActAgent.from_tools([khulnasoft_sandbox_tool], llm=llm, verbose=True)
