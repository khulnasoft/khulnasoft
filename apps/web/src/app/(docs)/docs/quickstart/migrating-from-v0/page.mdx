# Migrating KhulnaSoft SDK from v0.* to v1.0 using Grit

This guide explains how to migrate your project from **KhulnaSoft SDK `v0.*`** to **KhulnaSoft SDK `v1.0`** using [Grit](https://www.grit.io/) and our custom migrations.
...
```bash
grit apply github.com/khulnasoft/khulnasoft-cookbook#khulnasoft_v0_to_v1_js --interactive
```

### For Python:

```bash
grit apply github.com/khulnasoft/khulnasoft-cookbook#khulnasoft_v0_to_v1_py --interactive
```
...
- ⚠️ If non-KhulnaSoft objects with `.close()` methods, `.id` attributes, etc. exist in files that import KhulnaSoft libraries, they will be incorrectly changed. You need to reject these changes manually.
...
```bash
git add .
git commit -m "Migrate project to KhulnaSoft SDK v1.0"
```
...
By following these steps, you can easily migrate your TypeScript or Python project from **KhulnaSoft SDK `v0.*`** to **KhulnaSoft SDK `v1.0`** using Grit. Don’t forget to use version control to track all changes throughout the process and to thoroughly test your application after migrating.
