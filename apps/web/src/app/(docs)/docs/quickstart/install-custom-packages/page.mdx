...
There are two ways to install custom packages in the KhulnaSoft Sandbox.
...
Install the [KhulnaSoft CLI](https://npmjs.com/package/@khulnasoft/cli) globally on your machine with NPM.
...
### 2. Login to KhulnaSoft CLI
Before you can create a custom sandbox, you need to login to KhulnaSoft CLI.
<CodeGroup isTerminalCommand>
```bash
...
### 3. Specify the packages you need in `khulnasoft.Dockerfile`
Edit the KhulnaSoft Dockerfile to install the packages you need.

<Note>
You need to use the `khulnasoft/code-interpreter:latest` base image.
</Note>
...
Since KhulnaSoft Sandboxes are Debian based machines, you can use any package manager supported by Debian.
