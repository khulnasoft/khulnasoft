...
To create a sandbox template, you specify the `khulnasoft.Dockerfile`. We then take this Dockerfile and create a new sandbox template from it and give you back a template ID.
...
**Steps**
1. [Install KhulnaSoft CLI](#1-install-khulnasoft-cli)
1. [Initialize sandbox template](#2-initialize-sandbox-template)
1. [Customize `khulnasoft.Dockerfile`](#3-customize-khulnasoftdockerfile)
...
Install KhulnaSoft CLI with npm:
<CodeGroup isTerminalCommand>
```bash
npm i @khulnasoft/cli
```
</CodeGroup>

### 2. Initialize sandbox template
The following command will create a basic `khulnasoft.Dockerfile` in the current directory.
...
### 3. Customize `khulnasoft.Dockerfile`
Now you can customize your sandbox template by editing the `khulnasoft.Dockerfile` file.
<CodeGroup title="khulnasoft.Dockerfile">
```bash
# Make sure to use this base image
FROM khulnasoft/code-interpreter:latest # $HighlightLine
...
Now you can build your sandbox template. We'll use Docker and the KhulnaSoft CLI.
What is going to happen is that KhulnaSoft CLI will call Docker to build the image and then push it to the KhulnaSoft cloud.
...
import cowsay
cowsay.say('Hello from KHULNASOFT!')
`)
