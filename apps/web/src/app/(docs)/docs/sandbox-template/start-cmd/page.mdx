...
When you are building a sandbox template you can specify the start command by using the [`-c`](/docs/sdk-reference/cli/v1.0.9/template#khulnasoft-template-build) option:

```bash
khulnasoft build -c "<your-start-command>"
```
...
## How it works
Every time you are building a [custom sandbox](/docs/sandbox-template), we create a container based on the [`khulnasoft.Dockerfile`](/docs/sandbox-template#3-customize-khulnasoft-dockerfile) file you create in the process.
...
```bash {{ language: 'bash' }}
khulnasoft sandbox logs <sandbox-id>
```
</CodeGroup>

## Sandbox template config
The start command is specified inside the `khulnasoft.toml` in the same directory where you ran `khulnasoft build -c "<your-start-command>"`.
