...
Sometimes it's useful to know if the code is running inside a sandbox. Upon creating a sandbox, an environment variable `KHULNASOFT_SANDBOX` is automatically set to `true`.
You can try it out by running the following code in the sandbox:

<CodeGroup>
```js
const sandbox = await Sandbox.create()
const result = await sandbox.commands.run('echo $KHULNASOFT_SANDBOX')
```
```python
sandbox = Sandbox()
result = sandbox.commands.run("echo $KHULNASOFT_SANDBOX")
```
</CodeGroup>
