...
1. Pass the template ID to our SDK like this:
    <CodeGroup title="Using custom sandbox with SDK" isRunnable={false}>
      ...
      const sandbox = await Sandbox.create({
        template: '<sandbox-template-id>', // You get sandbox template ID from the CLI after you run `$ khulnasoft build`
      })
      ...
      # Create new sandbox
      sandbox = Sandbox(
        template="<sandbox-template>", # You get sandbox template from the CLI after you run `$ khulnasoft build`
      )
