...
<Note>
The KhulnaSoft Sandbox is a secure way to run your AI app. It is a long-running cloud environment where you can let any LLM (GPTs, Claude, local LLMs, etc) use tools exactly like you would do locally.

Read more about KhulnaSoft Sandboxes [here](/docs/sandbox/overview).
</Note>
...
1. Pass the template ID to our SDK like this:
    <CodeGroup title="Using custom sandbox with SDK" isRunnable={false}>
      ...
      const sandbox = await Sandbox.create({
        // You get sandbox template from the CLI after you run `$ khulnasoft build`
