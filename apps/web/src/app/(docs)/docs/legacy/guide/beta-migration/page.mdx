...
The following guide shows how to migrate your code from the current Core SDKs (`khulnasoft`) and Code Interpreter SDKs (`@khulnasoft/code-interpreter`/`khulnasoft-code-interpreter`) to the beta versions.

The latest beta versions you should install can be found in NPM/PyPi release history:
- **JavaScript & TypeScript**
	- [Latest release](https://www.npmjs.com/package/khulnasoft?activeTab=versions) with *rc* tag for `khulnasoft`
	- [Latest release](https://www.npmjs.com/package/@khulnasoft/code-interpreter?activeTab=versions) with *rc* tag for `@khulnasoft/code-interpreter`
- **Python**
	- [Latest release](https://pypi.org/project/khulnasoft/#history) with *PRE-RELEASE* for `khulnasoft`
	- [Latest release](https://pypi.org/project/khulnasoft-code-interpreter/#history) with *PRE-RELEASE* for `khulnasoft-code-interpreter`
...
These changes affect both the `khulnasoft` Core SDKs and Code Interpreter SDKs (`@khulnasoft/code-interpreter`/`khulnasoft-code-interpreter`) because the Code Interpreter uses the Core SDK.

<Note>
If you are using a custom template you need to rebuild it by calling `khulnasoft template build` in the directory with the template.
</Note>
...
If you need this feature please reach out to us on [Discord](https://discord.com/invite/U7KEcGErtQ) or [GitHub](https://github.com/khulnasoft/khulnasoft) so we can prioritize it.
...
```
if you use the KhulnaSoft Docker image as base in your `khulnasoft.Dockerfile`
```docker
...
Or if you are using custom base you need to copy all files excluding the `khulnasoft.toml` from [beta template](https://github.com/khulnasoft/code-interpreter/tree/beta/template)
to the directory with your template and then run `khulnasoft template build -c "/root/.jupyter/start-up.sh"`.
...
from khulnasoft_code_interpreter import AsyncCodeInterpreter
