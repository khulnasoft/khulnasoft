...
To customize your sandbox compute, you need to build your custom sandbox template using the [KhulnaSoft CLI](/docs/cli/installation).
Run [`khulnasoft template build`](/docs/cli/commands#template-build) and specify the `--cpu-count` and `--memory-mb` options.

For example this custom build gives your sandbox 8 vCPU and 4GiB of RAM
```sh
khulnasoft template build --cpu-count 8 --memory-mb 4096
```
</Note>
