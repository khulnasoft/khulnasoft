...
import { CustomDataType, WorkflowPreferences } from '@khulnasoft/shared';
...
axios.interceptors.request.use(async (config) => {
  config.headers.set('Khulnasoft-Environment-Id', getEnvironmentId());
...
export function buildApiHttpClient({
  baseURL = API_ROOT || 'https://api.khulnasoft.co',
  secretKey,
  ...
  httpClient.interceptors.request.use(async (config) => {
    ...
    if (secretKey) {
      ...
      config.headers.set('Khulnasoft-Environment-Id', environmentId);
    }
