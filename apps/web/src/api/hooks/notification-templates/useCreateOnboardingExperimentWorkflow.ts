...
import { EmailProviderIdEnum, StepTypeEnum } from '@khulnasoft/shared';
import type { IResponseError, ICreateNotificationTemplateDto, INotificationTemplate } from '@khulnasoft/shared';
...
export const useCreateOnboardingExperimentWorkflow = () => {
  ...
  const createOnboardingExperimentWorkflow = useCallback(() => {
    ...
    const khulnasoftEmailIntegration = integrations?.find(
      (integration) => integration.providerId === EmailProviderIdEnum.Khulnasoft
    );

    if (khulnasoftEmailIntegration && !khulnasoftEmailIntegration?.primary) {
      makePrimaryIntegration({ id: khulnasoftEmailIntegration?._id as string });
    }
    ...
    if (onboardingExperimentWorkflowExists) {
      ...
      const payload = {
        ...
        steps: [
          {
            template: {
              subject: 'Your first email notification from Khulnasoft!',
              senderName: 'Khulnasoft Onboarding',
