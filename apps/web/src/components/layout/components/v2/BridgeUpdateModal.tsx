import { Modal, successMessage } from '@khulnasoft/design-system';
import { css } from '@khulnasoft/khulnasofti/css';
import { text } from '@khulnasoft/khulnasofti/recipes';
import { Button, Input, Title } from '@khulnasoft/khulnasofti';
import { IconOutlineMenuBook } from '@khulnasoft/khulnasofti/icons';
import { HStack, Box, styled } from '@khulnasoft/khulnasofti/jsx';
...
export const BridgeUpdateModal: FC<BridgeUpdateModalProps> = ({ isOpen, toggleOpen }) => {
  ...
  const onUpdateClick = async () => {
    ...
    try {
      ...
      successMessage('You have successfully updated your Khulnasoft Endpoint configuration');
      toggleOpen();
    } catch (error) {
  ...
  return (
    <>
      <Modal opened={isOpen} title={<Title variant="section">Update Khulnasoft Endpoint URL</Title>} onClose={toggleOpen}>
        <Box colorPalette={'mode.local'}>
          <Input
            label={'Khulnasoft Endpoint URL'}
            description={
              'This url should be a full URL to the Khulnasoft Endpoint including the /api/khulnasoft path, e.g. https://your.api.com/api/khulnasoft'
            }
