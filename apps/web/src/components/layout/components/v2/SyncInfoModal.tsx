...
// TODO: replace with Khulnasofti Code Block when available
import { Prism } from '@mantine/prism';

// TODO: replace with Khulnasofti Modal when available
import { Modal } from '@khulnasoft/design-system';
import { Button, Input, Tabs, Text, Title } from '@khulnasoft/khulnasofti';
      ...
      content: (
        <>
          ...
          <Input
            ...
            placeholder="https://your-deployed-application.com/api/khulnasoft"
          />
          ...
          {bridgeUrl === manualUrl ? (
            <Text variant={'secondary'} style={{ marginTop: 10 }}>
              ...
              to actively sync with Khulnasoft Cloud.
              <br /> <br />
...
function getGithubYamlContent({ bridgeUrl }: { bridgeUrl: string }) {
  return `# .github/workflows/khulnasoft.yml
...
function getOtherCodeContent({ secretKey, bridgeUrl }: { secretKey: string; bridgeUrl: string }) {
  let command = `npx khulnasoft@latest sync \\
  --bridge-url ${bridgeUrl || BRIDGE_ENDPOINT_PLACEHOLDER} \\
