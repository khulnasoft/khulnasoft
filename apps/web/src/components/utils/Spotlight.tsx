...
import { Activity, Bolt, Box, Brand, Chat, IconLogout, Repeat, Settings, Team } from '@khulnasoft/design-system';
  ...
  useEffect(() => {
    addItem([
      ...
      },
      {
        ...
        onTrigger: () => {
          window?.open(`https://docs.khulnasoft.co${UTM_CAMPAIGN_QUERY_PARAM}`, '_blank')?.focus();
        },
      },
      {
        id: 'navigate-support',
        title: 'Go to Support',
        onTrigger: () => {
          window?.open('https://discord.com/invite/khulnasoft', '_blank')?.focus();
        },
      },
      {
        ...
        onTrigger: () => {
          window?.open('https://github.com/khulnasoft/khulnasoft/issues/new/choose', '_blank')?.focus();
        },
        icon: <Chat />,
      },
      {
