...
const khulnasoftRedirectURLCookie = createCookieHandler('nv_redirect_url');
...
export function useRedirectURL() {
  const setRedirectURL = useCallback(() => {
    ...
    if (redirectURLFromParams) {
      ...
      // Expires in 7 days.
      khulnasoftRedirectURLCookie.set(redirectURLFromParams, { expires: REDIRECT_COOKIE_EXPIRY_DAYS });
  ...
  const getRedirectURL = useCallback(() => {
    const redirectURLFromCookie = khulnasoftRedirectURLCookie.get();
    ...
    if (redirectURLFromCookie) {
      ...
      khulnasoftRedirectURLCookie.remove();
