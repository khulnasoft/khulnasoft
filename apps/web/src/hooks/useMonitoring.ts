...
export function useMonitoring() {
  ...
  const isKhulnasoftUser = currentUser && currentUser._id && !currentUser._id.startsWith('user_');
  const isKhulnasoftOrganization =
    currentOrganization && currentOrganization._id && !currentOrganization._id.startsWith('org_');

  /*
   * if the identifier present isn't a khulnasoft identifier, we don't want to pollute our data with
   * clerk identifiers, so we will skip monitoring.
   */
  const shouldMonitor = isKhulnasoftUser && isKhulnasoftOrganization;
