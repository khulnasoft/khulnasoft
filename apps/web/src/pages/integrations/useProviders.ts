...
import {
  ...
  IConfigCredentials,
  IProviderConfig,
  KHULNASOFT_SMS_EMAIL_PROVIDERS,
  providers,
  PushProviderIdEnum,
...
function initializeProvidersByIntegration(integrations: IntegrationEntity[]): IIntegratedProvider[] {
  return integrations
    .filter((integrationItem) => {
      ...
      return !KHULNASOFT_SMS_EMAIL_PROVIDERS.includes(integrationItem.providerId);
    })
    .map((integrationItem) => {
      ...
      return {
        ...
        removeKhulnasoftBranding: integrationItem?.removeKhulnasoftBranding ?? false,
      };
    });
}
