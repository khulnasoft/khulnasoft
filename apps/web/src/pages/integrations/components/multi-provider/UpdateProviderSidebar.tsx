...
import { Button, colors, Input, Sidebar, Text, Check, Copy } from '@khulnasoft/design-system';
...
import { KhulnasoftInAppFrameworks } from '../KhulnasoftInAppFrameworks';
...
import { KhulnasoftInAppFrameworkHeader } from '../KhulnasoftInAppFrameworkHeader';
import { KhulnasoftInAppSetupWarning } from '../KhulnasoftInAppSetupWarning';
import { KhulnasoftProviderSidebarContent } from './KhulnasoftProviderSidebarContent';
...
import { KhulnasoftInAppRemoveBranding } from '../KhulnasoftInAppRemoveBranding';
...
export function UpdateProviderSidebar({
  ...
  const isKhulnasoftInAppProvider = selectedProvider?.providerId === InAppProviderIdEnum.Khulnasoft;
  ...
  useEffect(() => {
    ...
    reset({
      ...
      removeKhulnasoftBranding: foundProvider.removeKhulnasoftBranding,
    });
  ...
  if (
    SmsProviderIdEnum.Khulnasoft === selectedProvider?.providerId ||
    EmailProviderIdEnum.Khulnasoft === selectedProvider?.providerId
  ) {
    return (
      <FormProvider {...methods}>
        <Sidebar
          ...
          data-test-id="update-provider-sidebar-khulnasoft"
          customFooter={
        ...
        >
          <KhulnasoftProviderSidebarContent provider={selectedProvider} />
    ...
    <FormProvider {...methods}>
      <Sidebar
        ...
        customHeader={
          sidebarState === SidebarStateEnum.NORMAL ? (
            ...
            <>
              <When truthy={isKhulnasoftInAppProvider}>
                <KhulnasoftInAppFrameworkHeader framework={framework} />
              </When>
            </>
          )
        }
      ...
      >
        <When truthy={sidebarState === SidebarStateEnum.NORMAL}>
          ...
          />
          {isKhulnasoftInAppProvider && <KhulnasoftInAppSetupWarning provider={selectedProvider} />}
          <UpdateIntegrationCommonFields provider={selectedProvider} />
          ...
          {isKhulnasoftInAppProvider && <KhulnasoftInAppRemoveBranding control={control} />}
          {isWebhookEnabled && (
            <InputWrapper>
              <Input
                label="Webhook URL"
          ...
          {isKhulnasoftInAppProvider && <KhulnasoftInAppFrameworks onFrameworkClick={onFrameworkClickCallback} />}
        </When>
        <When truthy={isKhulnasoftInAppProvider && sidebarState === SidebarStateEnum.EXPANDED}>
          <SetupTimeline
            framework={framework?.toString() ?? ''}
            onDone={() => {
              setSidebarState(SidebarStateEnum.NORMAL);
              successMessage('Successfully configured Khulnasoft In-App');
            }}
            onConfigureLater={() => {
              setSidebarState(SidebarStateEnum.NORMAL);
            }}
