...
import { ErrorIcon, colors, Text } from '@khulnasoft/design-system';
import { LimitBar } from '../LimitBar';
import { useIntegrationLimit } from '../../../../hooks';
import type { IIntegratedProvider } from '../../types';

export const KhulnasoftProviderSidebarContent = ({ provider }: { provider: IIntegratedProvider | null }) => {
  const {
    data: { limit, count },
    loading,
  ...
  return (
    <Stack spacing={16}>
      <Group pt={10} pb={10} spacing={16}>
        ...
        <Text color={descriptionColor} data-test-id="khulnasoft-provider-limits">
          Khulnasoft provider allows sending max {limit} {provider?.channel === ChannelTypeEnum.EMAIL ? 'emails' : 'messages'}{' '}
          per month,
          <br />
