...
import { ChannelTypeEnum, KHULNASOFT_PROVIDERS, providers } from '@khulnasoft/shared';
...
export function CreateProviderInstanceSidebar({
  ...
  const showKhulnasoftProvidersErrorMessage = useMemo(() => {
    if (!provider || integrations.length === 0 || !KHULNASOFT_PROVIDERS.includes(provider.id)) {
      return false;
    }
  ...
  return (
    <Sidebar
      ...
      customFooter={
        <Group ml="auto">
          ...
          <Button
            disabled={isLoading || isLoadingCreate || showKhulnasoftProvidersErrorMessage}
    ...
    >
      ...
      <When truthy={showKhulnasoftProvidersErrorMessage}>
        <WarningMessage>
          <Text data-test-id="khulnasoft-provider-error">
