...
import { colors, Close } from '@khulnasoft/design-system';
...
interface KhulnasoftProviderBaseProps {
  onClose: () => void;
  senderInformation?: React.ReactNode;
  channel: ChannelTypeEnum;
}

export function KhulnasoftProviderBase({ onClose, senderInformation, channel }: KhulnasoftProviderBaseProps) {
  const {
    data: { limit, count },
    loading,
    ...
    >
      ...
      <Stack spacing={24}>
        ...
        <Stack spacing={24}>
          <Stack spacing={16}>
            <Text>
              The free Khulnasoft provider allows sending {limit} {unit} per month
            </Text>
