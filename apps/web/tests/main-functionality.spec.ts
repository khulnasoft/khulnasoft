...
test('should show the brand logo on main page', async ({ page }) => {
  ...
  await expect(brandLogo).toHaveAttribute('src', 'https://dashboard.khulnasoft.com/static/images/logo-light.png');
});
...
test('should create an SMS channel message', async ({ page }) => {
  ...
  await expect(triggerCodeSnippet).toContainText("import { Khulnasoft } from '@khulnasoft/node'");
