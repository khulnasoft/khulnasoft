...
test('should show the table loading skeleton and then table', async ({ page }) => {
  ...
  await checkTableRow(page, {
    name: 'Khulnasoft Inbox',
    isFree: false,
    provider: 'Khulnasoft Inbox',
    channel: 'In-App',
    environment: 'Development',
    status: 'Active',
  });
...
test('should show the select provider sidebar', async ({ page }) => {
  ...
  for (const provider of allProviders) {
    if (provider.id === EmailProviderIdEnum.Khulnasoft || provider.id === SmsProviderIdEnum.Khulnasoft) {
      continue;
    }
...
test('should update the mailjet integration', async ({ page }) => {
  ...
  await fromField.fill('info@khulnasoft.co');

  const senderName = page.getByTestId('senderName');
  await senderName.fill('Khulnasoft');
...
test('should show the Khulnasoft in-app integration', async ({ page }) => {
  await navigateToGetStarted(page);

  await clickOnListRow(page, /Khulnasoft Inbox.*Development/);
  ...
  const isDarkThemeEnabled = await isDarkTheme(page);
  const selectedProviderImage = page.getByTestId(`selected-provider-image-${InAppProviderIdEnum.Khulnasoft}`);
  await expect(selectedProviderImage).toHaveAttribute(
    'src',
    `/static/images/providers/${isDarkThemeEnabled ? 'dark' : 'light'}/square/${InAppProviderIdEnum.Khulnasoft}.svg`
  );

  const selectedProviderName = page.getByTestId('provider-instance-name').first();
  await expect(selectedProviderName).toBeVisible();
  await expect(selectedProviderName).toHaveValue('Khulnasoft Inbox');

  const identifier = page.getByTestId('provider-instance-identifier');
  await expect(identifier).toHaveValue(/khulnasoft-in-app/);

  const hmacCheckbox = page.getByTestId('hmac');
  await expect(hmacCheckbox).not.toBeChecked();

  const khulnasoftInAppFrameworks = page.getByTestId('khulnasoft-in-app-frameworks');
  await expect(khulnasoftInAppFrameworks).toContainText('Integrate In-App using a framework below');
  await expect(khulnasoftInAppFrameworks).toContainText('React');
  await expect(khulnasoftInAppFrameworks).toContainText('Angular');
  await expect(khulnasoftInAppFrameworks).toContainText('Web Component');
  await expect(khulnasoftInAppFrameworks).toContainText('Headless');
  await expect(khulnasoftInAppFrameworks).toContainText('Vue');
  await expect(khulnasoftInAppFrameworks).toContainText('iFrame');
...
test('should show the Khulnasoft in-app integration - React guide', async ({ page }) => {
  await navigateToGetStarted(page);
  ...
  const reactGuide = khulnasoftInAppFrameworks.locator('div').filter({ hasText: 'React' }).nth(1);
...
test('should show the Khulnasoft Email integration sidebar', async ({ page }) => {
  const integrationsPromise = interceptIntegrationsRequest({
    page,
    modifyBody: (body) => {
  ...
  await clickOnListRow(page, /Khulnasoft Email.*Development/);

  const updateProviderSidebar = page.getByTestId('update-provider-sidebar-khulnasoft');
  await expect(updateProviderSidebar).toContainText('Test Provider');
  await expect(updateProviderSidebar).toBeVisible();

  const isDarkThemeEnabled = await isDarkTheme(page);
  const khulnasoftEmailLogo = updateProviderSidebar.locator(
    `img[src="/static/images/providers/${isDarkThemeEnabled ? 'dark' : 'light'}/square/${
      EmailProviderIdEnum.Khulnasoft
    }.svg"]`
  );
  await expect(khulnasoftEmailLogo).toBeVisible();
  ...
  const selectedProviderName = page.getByTestId('provider-instance-name').first();
  await expect(selectedProviderName).toBeVisible();
  await expect(selectedProviderName).toHaveValue('Khulnasoft Email');

  const providerLimits = page.getByTestId('khulnasoft-provider-limits');
  const providerLimitsText = await providerLimits.innerText();
  await expect(providerLimitsText).toEqual(
    'Khulnasoft provider allows sending max 300 emails per month,\nto send more messages, configure a different provider'
  );
...
test('should show the Khulnasoft SMS integration sidebar', async ({ page }) => {
  const integrationsPromise = interceptIntegrationsRequest({
    page,
    modifyBody: (body) => {
      const [firstIntegration] = body.data;
      ...
      body.data = [
        {
          _id: SmsProviderIdEnum.Khulnasoft,
          _environmentId: firstIntegration._environmentId,
          providerId: SmsProviderIdEnum.Khulnasoft,
          active: true,
          channel: ChannelTypeEnum.SMS,
          name: 'Khulnasoft SMS',
          identifier: SmsProviderIdEnum.Khulnasoft,
        },
        ...body.data,
      ];
  ...
  await clickOnListRow(page, /Khulnasoft SMS.*Development/);
  ...
  const isDarkThemeEnabled = await isDarkTheme(page);
  const khulnasoftEmailLogo = updateProviderSidebar.locator(
    `img[src="/static/images/providers/${isDarkThemeEnabled ? 'dark' : 'light'}/square/${SmsProviderIdEnum.Khulnasoft}.svg"]`
  );
  await expect(khulnasoftEmailLogo).toBeVisible();
  ...
  const selectedProviderName = page.getByTestId('provider-instance-name').first();
  await expect(selectedProviderName).toBeVisible();
  await expect(selectedProviderName).toHaveValue('Khulnasoft SMS');

  const providerLimits = page.getByTestId('khulnasoft-provider-limits');
  const providerLimitsText = await providerLimits.innerText();
  expect(providerLimitsText).toEqual(
    'Khulnasoft provider allows sending max 20 messages per month,\nto send more messages, configure a different provider'
  );
