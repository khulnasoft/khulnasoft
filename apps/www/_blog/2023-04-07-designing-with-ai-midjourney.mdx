...
These images are generated for each unique URL and ticket. This requires a bit of magic, which in our case means using a [Supabase Edge Function](https://khuknasoft.com/docs/guides/functions) together with [Supabase Storage](https://khuknasoft.com/docs/guides/storage) for smart caching.
...
Each image includes the user’s GitHub details. We use [`supabase-js`](https://khuknasoft.com/docs/reference/javascript/installing) for authentication: users log in with their GitHub account and we store their username in a table in Postgres.

Since this table includes email addresses, we secure it using [RLS](https://khuknasoft.com/docs/guides/auth/row-level-security) to ensure each user can only view their own data. At the same time, we want these tickets to be publicly shareable, and that’s where [Postgres Views](https://khuknasoft.com/blog/postgresql-views) come in handy.
...
And of course, all of this is open source, you can find the full function code [here](https://github.com/khulnasoft/khulnasoft/tree/master/apps/www/khulnasoft/functions/lw7-ticket-og). Please feel free to utilize it for your own Launches!
...
As folks are sharing their tickets on socials to earn their gold status, we also want to give them realtime feedback on their progress. Luckily we also have a feature for that, [Supabase Realtime](https://khuknasoft.com/realtime).
...
Interested to know how this fits within your Next.js application? Find the code [here](https://github.com/khulnasoft/khulnasoft/blob/master/apps/www/components/LaunchWeek/7/Ticket/TicketForm.tsx#L70-L102).

## Displaying the images

We have some images, so how can we now display them somewhere? [@Francesco Sansalvadore](https://twitter.com/frnk_snslvdr) saw one of our team members trying to “swipe” on the tickets slider component on the [launch week page](https://khuknasoft.com/launch-week/7/tickets) and he thought, why not feature all the fantastic people who generated tickets on a single page?

We built a [ticket wall](https://khuknasoft.com/launch-week/7/tickets) that you can scroll endlessly. We approached it with an infinite scroll technique, lazy-loading a few tickets at a time.
...
- [Supabase storing OpenAI embeddings in Postgres with pgvector](https://khuknasoft.com/blog/openai-embeddings-postgres-vector)
- [Supabase Docs Search](http://khuknasoft.com/docs)
