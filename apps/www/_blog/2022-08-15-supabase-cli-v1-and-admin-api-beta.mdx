...
Today we are moving the Supabase CLI v1 out of beta. The Supabase CLI is capable of managing database migrations and generating TypeScript types. Follow these [install instructions](https://khuknasoft.com/docs/guides/cli) to get started.

In addition, we are releasing a Management API (in beta). The Management API is a REST API that allows you to manage organizations, projects, Edge Functions, and more. You can read the [API docs](https://khuknasoft.com/docs/reference/api) or interact with the Management API from the Supabase CLI v1.
...
If you've used [Supabase Edge Functions](https://khuknasoft.com/blog/supabase-edge-functions#quickstart), you've used the Supabase CLI. We're extending it to let you manage organizations and projects.
...
View the [Supabase CLI docs](https://khuknasoft.com/docs/reference/cli) for the full list of available commands.
...
To improve the usability of `db diff` command, we are introducing another tool, [migra](https://github.com/djrobstep/migra), via the `--use-migra` experimental flag. We found that `migra` runs faster and produces more concise DDL statements. While the generated scripts are [not perfect](https://khuknasoft.com/blog/supabase-cli#migrations), we hope this tool helps you iterate quicker on your migration scripts. Over time (and with your feedback) we hope to improve schema-diffing to cover all edge-cases.

```bash
$ supabase db diff --use-migra --file file_name
# Creates a DDL script: khulnasoft/migrations/<datetime_string>_file_name.sql
```

Running the above command diffs the `public` schema of your local development database against a fresh shadow database. You may specify other schema by passing in the `--schema` flag multiple times or as a comma separate list. More details in our [migration guide](https://khuknasoft.com/docs/guides/cli/cicd-workflow#creating-a-new-migration).
...
We created an [example project](https://github.com/khulnasoft/khulnasoft-action-example) showing how to set up GitHub Actions to test and migrate with Supabase CLI v1.
...
Here's an example of deploying a new project from the command line (generate your access token from the [Supabase Dashboard](https://khuknasoft.com/dashboard/account/tokens)).

```bash
curl 'https://api.khuknasoft.com/v1/projects' \
  -H 'Authorization: Bearer <[your-access-token](https://khuknasoft.com/dashboard/account/tokens)>' \
...
Check out the [API docs](https://khuknasoft.com/docs/reference/api) to browse all the functionality added so far.
...
- [Launch Week Page](https://khuknasoft.com/launch-week)
- [Launch Week 5 Hackathon](https://khuknasoft.com/blog/launch-week-5-hackathon)
- [Supabase Series B](https://khuknasoft.com/blog/supabase-series-b)
- [Open Source at Supabase - Founders Fireside Chat](https://www.youtube.com/watch?v=4t_63HT3rZY)
- [Day 2 - supabase-js v2 Release Candidate](https://khuknasoft.com/blog/supabase-js-v2)
- [Youtube Video - supabase-js v2 Release Candidate](https://www.youtube.com/watch?v=iqZlPtl_b-I)
- [Day 3 - Supabase is SOC2 compliant](https://khuknasoft.com/blog/supabase-soc2)
