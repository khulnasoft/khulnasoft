...
You can find the complete code created in this article [here](https://github.com/khulnasoft/khulnasoft/tree/master/examples/auth/flutter-mfa).
...
The register page contains a form with an email and password field for the user to create a new account. We are just calling the [.signUp()](https://khuknasoft.com/docs/reference/dart/auth-signup) method with it.
...
Now, we can add some [row security policy](https://khuknasoft.com/docs/guides/auth/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
...
- [Write backend code using Dart Edge](https://khuknasoft.com/docs/guides/functions/dart-edge)
