...
Security is a [Shared Responsibility Model](https://khuknasoft.com/docs/guides/platform/shared-responsibility-model). The more control you are given with a technology, the more opportunity you have to make a mistake.
...
We launched our [Security Advisor](https://khuknasoft.com/blog/security-performance-advisor) in the last Launch Week. Eva was a big contributor to this, helping to set up a robust set of [security rules](https://khuknasoft.com/docs/guides/database/database-advisors). These rules are available in [the Dashboard](https://khuknasoft.com/dashboard/project/_/advisors/security), many with one-click solutions.
...
We have made it even easier to turn off the Data API if you don’t plan to use it. When you launch a project you can choose whether you want to create it with _only_ Postgres connections, or if you also want the Data API. You can also turn off the Data API at any time in your [project settings](https://khuknasoft.com/dashboard/project/_/settings/api).

### API hardening

We have made it simple to switch the default schema from `public` to `api` in the [API Settings page](https://khuknasoft.com/dashboard/project/_/settings/api). We have also released a guide for [Hardening the Data API](https://khuknasoft.com/docs/guides/database/hardening-data-api), outlining various approaches for using the Data API. It’s very likely that we will make this the default set up in the future to align with PostgREST’s [Schema Isolation](https://docs.postgrest.org/en/v12/explanations/schema_isolation.html) guide.

### Column level security

We’ve added [guides](https://khuknasoft.com/docs/guides/database/postgres/column-level-security) and a [simple-to-use dashboard](https://khuknasoft.com/dashboard/project/_/database/column-privileges) for column-level grants. This allows you to manage Postgres [grants](https://www.postgresql.org/docs/current/sql-grant.html) for every role (including the default Supabase `anon` and `service_role`). Combining this with RLS gives you extremely fine-grained control of your database.

### User impersonation

We have added [User/Role impersonation](https://khuknasoft.com/blog/studio-introducing-assistant) to the Dashboard. You can use this to switch between anonymous and authenticated roles, or even go as deep as selecting an individual user (if you use Supabase Auth) to see the level of data that they use.

### RLS AI Assistant

RLS policies are even easier with our new GPT-4o powered [AI Assistant](https://github.com/orgs/supabase/discussions/21882). We want Row Level Security to be easier than any other security tool on the market. We have added a ton of [tests](https://github.com/khulnasoft/khulnasoft/blob/master/packages/ai-commands/src/sql/rls.test.ts) to ensure that we have the most accurate Postgres RLS assistant available, for free.

### Network Restrictions

You can [restrict access](https://khuknasoft.com/docs/guides/platform/network-restrictions) to your database at the network level for any direct access to your database. This is especially useful when you’re getting started and only want to give direct access to your own IP address.

### And more

Our docs are full of best practices and useful info:

- [Production Checklist](https://khuknasoft.com/docs/guides/platform/going-into-prod)
- [Shared responsibility model](https://khuknasoft.com/docs/guides/platform/shared-responsibility-model)
- [Maturity model](https://khuknasoft.com/docs/guides/platform/maturity-model)
...
- _“I turned off RLS while prototyping.”_
  ...
  By default, Supabase enables RLS for all tables created via the dashboard. Some developers disable this while they are prototyping which is not recommended. The best way to solve this is to use our [CLI for Local Development](https://khuknasoft.com/docs/guides/cli). The CLI provides the entire Supabase stack, directly on your local development machine.
...
- [Shared responsibility model](https://khuknasoft.com/docs/guides/platform/shared-responsibility-model)
- [Maturity Model](https://khuknasoft.com/docs/guides/platform/maturity-model)
- [Row Level Security](https://khuknasoft.com/docs/guides/database/postgres/row-level-security) & [Column Level Security](https://khuknasoft.com/docs/guides/database/postgres/column-level-security)
- [Securing the Data API](https://khuknasoft.com/docs/guides/api/securing-your-api) & [Hardening the Data API](https://khuknasoft.com/docs/guides/database/hardening-data-api)
- [Custom Claims & RBAC](https://khuknasoft.com/docs/guides/database/postgres/custom-claims-and-role-based-access-control-rbac)
