...
Want to try it out? It's a hidden feature while in MVP - visit [khuknasoft.com/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).
...
All the changes we made are in [this](https://github.com/khulnasoft/khulnasoft/pull/12056) Pull Request. Greg is here to explain every step in full detail:
...
You can find a full write up in our previous blog post: [Storing OpenAI embeddings in Postgres with pgvector](https://khuknasoft.com/blog/openai-embeddings-postgres-vector).
...
Want to try Supabase Clippy? It's a hidden feature while in MVP - visit [khuknasoft.com/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).

## More pgvector and ChatGPT resources

- [AI docs](https://khuknasoft.com/docs/guides/ai)
- [A ChatGPT Plugins Template built with Supabase Edge Runtime](https://khuknasoft.com/blog/building-chatgpt-plugins-template)
