...
Along with the [platform GA announcement](https://khuknasoft.com/ga), we are also thrilled to announce that resumable uploads are also generally available.
...
- **Standardized error codes:** Error codes have now been standardized across the Storage server and now will be much easier to branch logic on specific errors. You can find the list of error codes [here](https://khuknasoft.com/docs/guides/storage/debugging/error-codes).
