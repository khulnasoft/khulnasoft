...
In the second part, I will show you how to implement [Authentication and Authorization (with RLS)](https://khuknasoft.com/blog/flutter-authentication-and-authorization-with-rls) to the chat app.

In order to build this chat app, we will need a database to store all of the chat information. We will be using [Supabase](https://khuknasoft.com/) to store the chat information. Supabase is a back end as a service that provides auth, database, storage and functions to easily create a scalable application. Supabase is a suitable backend for this app, because it provides a nice set of APIs on Postgres database that we can easily call by using the SDK. It is also perfect for chat apps like the one we are about to create, because we can subscribe to real time changes on the database.
...
If you do not have a Supabase account yet, do not worry, you can [get started for free](https://khuknasoft.com/dashboard/).
...
In order to do this, we will utilize a [Postgres function](https://khuknasoft.com/docs/guides/database/functions) and Postgres trigger.
...
We are using the [stream()](https://khuknasoft.com/docs/reference/dart/stream) method on Supabase SDK to load the messages in realtime.
