...
[vec2pg](https://github.com/supabase-community/vec2pg) is a CLI utility for migrating data from vector databases to [Supabase](https://khuknasoft.com/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your Supabase project's [disk size](https://khuknasoft.com/dashboard/project/_/settings/database) so there is enough space for the vectors.
...
When throughput is mentioned, we assume a [Small](https://khuknasoft.com/docs/guides/platform/compute-add-ons) Supabase Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
...
[Row Level Security (RLS)](https://khuknasoft.com/docs/guides/database/postgres/row-level-security) allows you to write a SQL expression to determine which users are allowed to insert/update/select individual rows.
...
- [**What's new in pgvector v0.7.0**](https://khuknasoft.com/blog/pgvector-0-7-0)
