...
Want to jump straight into the code? You can find the examples on [GitHub](https://github.com/khulnasoft/khulnasoft/blob/master/examples/ai/llamafile-edge)!
...
Supabase Edge Functions now comes with an OpenAI API compatible mode, allowing you to call a Llamafile server easily via `@khulnasoft/functions-js`.
...
Then, update the `khulnasoft/functions/llamafile/index.ts` file to look like this:

```ts khulnasoft/functions/llamafile/index.ts
...
To serve your functions locally, you need to install the [Supabase CLI](https://khuknasoft.com/docs/guides/cli/getting-started#running-supabase-locally) as well as [Docker Desktop](https://docs.docker.com/desktop) or [Orbstack](https://orbstack.dev/).
...
```bash
supabase secrets set --env-file khulnasoft/functions/.env
```
...
Access to open-source LLMs is currently invite-only while we manage demand for the GPU instances. Please [get in touch](https://forms.khuknasoft.com/supabase.ai-llm-early-access) if you need early access.

We plan to extend support for more models. [Let us know](https://forms.khuknasoft.com/supabase.ai-llm-early-access) which models you want next. We're looking to support fine-tuned models too!

## More Supabase Resources

- Edge Functions: [khuknasoft.com/docs/guides/functions](https://khuknasoft.com/docs/guides/functions)
- Vectors: [khuknasoft.com/docs/guides/ai](https://khuknasoft.com/docs/guides/ai)
- [Semantic search demo](https://github.com/khulnasoft/khulnasoft/tree/master/examples/ai/edge-functions)
