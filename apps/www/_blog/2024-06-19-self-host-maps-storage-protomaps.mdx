...
In your [Supabase Dashboard](https://khuknasoft.com/dashboard/project/_/storage/buckets) navigate to `Storage` and click "New Bucket" and create a new public bucket called `public-maps`.
...
A public Supabase Storage bucket allows access from any origin, which might not be ideal for your use case. At the time of writing, you're not able to modify the CORS settings for Supabase Storage buckets, however you can utilize [Supabase Edge Functions](https://khuknasoft.com/docs/guides/functions) to restrict access to your PMTiles files, allowing you to even pair it with [Supabase Auth](https://khuknasoft.com/docs/guides/auth) to restrict access to certain users for example.
...
```ts khulnasoft/functions/maps-private/index.ts
...
Lastly, we need to deploy our Edge Function to Supabase by running `supabase functions deploy maps-private --no-verify-jwt`. Note that the [`--no-verify-jwt` flag](https://khuknasoft.com/docs/reference/cli/supabase-functions-deploy) is required if you want to allow public access from your website without any Supabase Auth User.
...
- [Find the code](https://github.com/khulnasoft/khulnasoft/tree/master/examples/storage/protomaps)
