...
One of the unique features of Supabase is the ability to access your Postgres database directly from a browser or mobile app through our [Serverless APIs](https://khuknasoft.com/docs/guides/api). This means that developers are writing more [PL/pgSQL](https://www.postgresql.org/docs/current/plpgsql.html).
...
We’ve added [debugging utilities](https://khuknasoft.com/docs/guides/database/inspect) to our CLI to identify production issues via the `supabase inspect db` command. This interface includes 19 different commands to help you solve everything from slow queries to redundant indexes.
...
{/* prettier-ignore */}
  <div>
    - [Show most frequently run queries](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-calls)
    - [Show long running queries](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-long-running-queries)
    - [Show queries ordered by total execution time](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-outliers)
    - [Show queries waiting and holding locks](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-blocking)
    - [Show queries taking exclusive locks](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-locks)
    - [Show total size of all indexes](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-total-index-size)
    - [Show sizes of individual indexes](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-index-sizes)
    - [Show information about index efficiency](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-index-usage)
    - [Show indexes with low usage](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-unused-indexes)
    - [Show total size of all tables](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-total-table-sizes)
    - [Show sizes of individual tables](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-table-sizes)
    - [Show index sizes of individual tables](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-table-index-sizes)
    - [Show cache hit rates for tables and indices](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-cache-hit)
    - [Show estimated number of rows per table](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-table-record-counts)
    - [Show number of sequential scans for all tables](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-seq-scans)
    - [Show information about replication slots](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-replication-slots)
    - [Show number of active connections](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-role-connections)
    - [Show estimated database bloat](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-bloat)
    - [Show statistics related to vacuum operations](https://khuknasoft.com/docs/reference/cli/supabase-inspect-db-vacuum-stats)
  </div>
...
Finally, we’ve added a Migrations view [in the dashboard](https://app.khuknasoft.com/project/_/database/migrations) to track your migration history to improve the discoverability of migrations.
...
Finally, while you wait for us to make progress on the Language Server, we’ve added [support for linting](https://khuknasoft.com/docs/guides/cli/testing-and-linting#linting-your-database) through the excellent [plpgsql_check](https://github.com/okbob/plpgsql_check) extension.
...
We’ve added [support for seeding](https://khuknasoft.com/docs/guides/cli/seeding-your-database) to populate your local databases with data whenever you run `supabase start` or `supabase db reset`.

We’ve also worked with our friends at Snaplet to [generate seed data](https://khuknasoft.com/docs/guides/cli/seeding-your-database#generating-seed-data) directly from your database:

```markdown
npx snaplet generate --sql > khulnasoft/seed.sql
```
...
We’ve developed an official [GitHub Action](https://github.com/marketplace/actions/supabase-cli-action) which leverages the CLI. You can [generate types on every PR](https://khuknasoft.com/docs/guides/cli/github-action/generating-types), or run [your tests on every commit](https://khuknasoft.com/docs/guides/cli/github-action/testing).
...
Supabase isn’t simply a database, it’s an entire backend - everything from your Postgres database to your [50GB videos](https://khuknasoft.com/blog/storage-v3-resumable-uploads). Branching improves the experience of managing environments so that developers and teams spend less time on DevOps and more time building.

### Supabase branching is hard

Every project is a Postgres database, wrapped in a suite of tools like [Auth](https://khuknasoft.com/auth), [Storage](https://khuknasoft.com/storage), [Edge Functions](https://khuknasoft.com/edge-functions), [Realtime](https://khuknasoft.com/realtime) and [Vectors](https://khuknasoft.com/vector), and encompassed by [API middleware](https://khuknasoft.com/docs/guides/api) and [logs](https://khuknasoft.com/blog/supabase-logs-self-hosted).
...
Every time you open a new Pull Request on GitHub, a corresponding “Preview Environment” is created. Each preview branch is an isolated Firecracker instance that pauses automatically after a period of inactivity. Every time a change is pushed to GitHub, the migrations within the `./khulnasoft/migrations` folder are run against the Preview Branch so that your entire team is working from the same source of truth.

When you hit merge on your Pull Request we run the migrations on your Production database.

### What about data?

We’re starting with seed data. Any SQL with `./khulnasoft/seed.sql` will populate your Preview Branch. This provides your entire team with an isolated and reproducible environment, safe from any data privacy concerns.
...
We are [rolling out access](https://khuknasoft.com/blog/supabase-branching) and we'll be onboarding organizations in batches over the next few weeks. You can still [sign up for access](https://forms.khuknasoft.com/branching-request).

</Admonition>

## Get started

Jump into our updated [Local Development documentation](https://khuknasoft.com/docs/guides/cli) to get started with the CLI.

If you’re an existing user simply [update your CLI](https://khuknasoft.com/docs/guides/cli/getting-started#updating-the-supabase-cli) and check out the [full command reference](https://khuknasoft.com/docs/reference/cli/introduction) for all the latest commands.

## More Launch Week 8

- [Hugging Face is now supported in Supabase](https://khuknasoft.com/blog/hugging-face-supabase)
- [Launch Week 8](https://khuknasoft.com/launch-week)
- [Coding the stars - an interactive constellation with Three.js and React Three Fiber](https://khuknasoft.com/blog/interactive-constellation-threejs-react-three-fiber)
- [Why we'll stay remote](https://khuknasoft.com/blog/why-supabase-remote)
