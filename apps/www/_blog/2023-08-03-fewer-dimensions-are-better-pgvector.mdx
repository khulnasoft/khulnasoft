...
IVF [indexes](https://khuknasoft.com/docs/guides/ai/vector-indexes/ivf-indexes#how-does-ivfflat-work) work by clustering vectors into `lists`, and then querying only vectors within the same list (or multiple nearby lists, depending on the value of `probes`).
...
In the real-world it's common for an index to reduce the number of distance computations needed to estimate nearest neighbors from 100% of the dataset to 5-20%. At 1M records, that's still 50k-200k distance calculations being performed for a single query. Given how different the [resource requirements](https://khuknasoft.com/docs/guides/ai/choosing-compute-addon) are to support heavy vector workloads, it's not surprising that one of the most common issues we see is significant under provisioning of hardware.
...
- **Process:** We followed [our optimization guide and tips](https://khuknasoft.com/docs/guides/ai/going-to-prod#performance-tips-when-using-indexes).
...
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://khuknasoft.com/blog/increase-performance-pgvector-hnsw)
