...
import { Alert, Button, ButtonMenuItem, Dialog, MenuItem } from 'design-system';
...
function EditAppDialog({ app, open, onClose }: EditAppDialogProps) {
  ...
  const khulnasoftDomain = app.domains.find(hasProperty('type', 'autoassigned'));
  const [subdomain = '', domainSuffix] = splitDomain(khulnasoftDomain);
  ...
  const mutation = useMutation({
    async mutationFn(values: FormValues<typeof form>) {
      ...
      if (khulnasoftDomain && values.subdomain !== subdomain) {
        promises.push(
          api.editDomain({
            token,
            path: { id: khulnasoftDomain.id },
            query: {},
