...
import { sendDataToKhulnasoftTrace } from '../utils/sendDataToKhulnasoftTrace.utils';
...
export class UserInfoService {
  ...
  async sendDailyUserTelemetry() {
    ...
    const telemetryEnabled = process.env.KHULNASOFT_TELEMETRY !== 'false';

    if (isSelfHosted && telemetryEnabled) {
      const userData = await this.getUserData();
      await sendDataToKhulnasoftTrace(this.httpService, 'User Info - [OS Telemetry]', userData);
    }
  }
}
