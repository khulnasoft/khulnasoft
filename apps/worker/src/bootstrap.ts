...
import { BullMqService, getErrorInterceptor, Logger as PinoLogger } from '@khulnasoft/application-generic';
...
export async function bootstrap(): Promise<INestApplication> {
  ...
  try {
    ...
    Logger.error('[@khulnasoft/worker]: Failed to start app infra', e.message, e.start);
    process.exit(1);
  }

  await app.listen(process.env.PORT);

  Logger.log(`[@khulnasoft/worker]: Listening for NODE_ENV=${process.env.NODE_ENV} on port ${process.env.PORT}`);
