...
If you'd like to contribute, see our list of [recommended issues](https://github.com/khulnasoft/khulnasoft/issues?q=is%3Aopen+is%3Aissue+label%3Adocumentation+label%3A%22help+wanted%22). We also welcome you to open a PR or a new issue with your question.
...
For inspiration, see [an example of a tutorial](https://khuknasoft.com/docs/guides/getting-started/tutorials/with-nextjs).
...
For inspiration, see [an example of a guide](https://khuknasoft.com/docs/guides/auth/auth-email).
...
The navigation is defined in [`NavigationMenu.constants.ts`](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/components/Navigation/NavigationMenu/NavigationMenu.constants.ts).
...
Each type of library (for example, language SDK or CLI) has a common spec file. For example, see the [spec file for the language SDKs](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/spec/common-client-libs-sections.json). This file contains definitions for the common SDK functions:
...
Each library also has its own spec file containing library-specific details. For example, see the [JavaScript SDK spec file](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/spec/supabase_js_v2.yml).

The functions listed in this file match the ones defined in the common spec file.

Each function contains a description, code examples, and optional notes. The parameters are pulled from the source code via the `$ref` property, which references a function definition in the source code repo. These references are pulled down and transformed using commands in the spec [Makefile](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/spec/Makefile). Unless you're a library maintainer, you don't need to worry about this.
...
If you copy the same content multiple times across different files, create a **partial** for content reuse instead. Partials are MDX files contained in [`apps/docs/components/MDX`](https://github.com/khulnasoft/khulnasoft/tree/master/apps/docs/components/MDX). They contain reusable snippets that can be inserted in multiple pages. For example, you can create a partial to define a common setup step for a group of tutorials.

To use a partial, import it into your MDX file. You can also set up a partial to automatically import by including it in the `components` within [`apps/docs/components/index.tsx`](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/components/index.tsx).
...
Use relative links when linking within the `khuknasoft.com` domain. For example, `[link to another page in Supabase docs](/docs/guides/getting-started)`.
...
The `queryGroup` param is optional. It lets you link directly to a tab by using the query group as a query param in the URL, for example: `https://khuknasoft.com/docs/my-page?packagemanager=ts`
...
Search is handled using a Supabase instance. During CI, [a script](https://github.com/khulnasoft/khulnasoft/blob/master/apps/docs/scripts/search/generate-embeddings.ts) aggregates all content sources (eg. guides, reference docs, etc), indexes them using OpenAI embeddings, and stores them in a Supabase database.

Search uses a hybrid of native Postgres FTS and embedding similarity search based on [`pgvector`](https://github.com/pgvector/pgvector). At runtime, a PostgREST call triggers the RPC that runs the weighted FTS search, and an [Edge Function](https://github.com/supabase/blob/master/khulnasoft/functions) is executed to perform the embedding search.
