...
1. If you're new to Supabase, [create a project](https://database.new). Make a note of your password, you will need this later. If you forget it, you can [reset it here](https://khuknasoft.com/dashboard/project/_/settings/database).
...
2. Create a configuration file (e.g., config.load).
   ...
   For your destination, use your Supabase connection string with `Use connection pooling` enabled, and the mode set to `Session`. You can get the string from your [`Database Settings`](https://khuknasoft.com/dashboard/project/_/settings/general).

   ```sql
   load database
     from mysql://user:password@host/source_db
     into postgres://postgres.xxxx:password@xxxx.pooler.khuknasoft.com:5432/postgres
...
2. Create a configuration file (e.g., config.load).
   ...
   LOAD DATABASE
       ...
       INTO postgres://postgres.xxxx:password@xxxx.pooler.khuknasoft.com:6543/postgres
   ALTER SCHEMA 'public' OWNER TO 'postgres';
   set wal_buffers = '64MB', max_wal_senders = 0, statement_timeout = 0, work_mem to '2GB';
   ```
