...
The Preview Branch is also seeded with sample data based on `./khulnasoft/seed.sql` by default, if that file exists.
...
The Git integration can read files from your Git provider, watching every commit and pull request. Each time a commit is pushed with new migrations in the `./khulnasoft/migrations` directory, the migrations are run on the matching Supabase Preview environment:
...
The Preview Branch is seeded with sample data based on `./khulnasoft/seed.sql` by default, if that file exists.
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Pull your database migration" fullWidth>
            ...
            Pull your database changes using `supabase db pull`. You can find your database URL in your [database settings](https://khuknasoft.com/dashboard/project/_/settings/database), under the URI tab of the Connection String settings panel. Make sure **Use connection pooling** is checked so you can use the IPv4-enabled connection pooler. (Without connection pooling, your database is only accessible over IPv6, which isn't yet supported by all network providers.)
            ...
            # postgres://postgres.xxxx:password@xxxx.pooler.khuknasoft.com:6543/postgres
            ```
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Generate a migration file" fullWidth>
            ...
            This will create a SQL file called `./khulnasoft/migrations/[timestamp]add_employees_table.sql`. This file will reflect the changes that you made in your local dashboard.
...
<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        ...
        Select the branch you wish to use in your Supabase [project](https://khuknasoft.com/dashboard/project/_).
    ...
    <StepHikeCompact.Step step={2}>
        ...
        Make changes to your schema with either with the [Table Editor](https://khuknasoft.com/dashboard/project/_/editor) or the [SQL Editor]((https://khuknasoft.com/dashboard/project/_/sql)).
    ...
    <StepHikeCompact.Step step={3}>
        ...
        If you don't know the password, you must Reset the database password so you know the password. Go to the [database setting page](https://khuknasoft.com/dashboard/project/_/settings/database) and click `Reset database password`.
    ...
    <StepHikeCompact.Step step={4}>
        ...
        ```bash
        supabase db pull --db-url "postgres://postgres.xxxx:password@xxxx.pooler.khuknasoft.com:6543/postgres"
        ```
...
Every time a new commit is pushed that changes the migration files in `./khulnasoft/migrations`, the new migrations are run against the preview branch. You can check the status of these runs in the comment's Tasks table.
...
The new preview branch is reseeded from your `./khulnasoft/seed.sql` file by default. Any additional data changes you made on the old preview branch are lost. This is equivalent to running `supabase db reset` locally. All migrations are rerun in sequential order.

### Seeding behavior

Your Preview Branches are seeded with sample data from the file `./khulnasoft/seed.sql` by default.
...
The GitHub integration automatically checks for new migrations on every commit. It runs any new migrations found in `./khulnasoft/migrations`.
