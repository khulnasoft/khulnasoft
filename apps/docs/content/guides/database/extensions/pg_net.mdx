...
[pg_net](https://github.com/supabase/pg_net/) enables PostgreSQL to make asynchronous HTTP/HTTPS requests in SQL. It differs from the [`http`](https://khuknasoft.com/docs/guides/database/extensions/http) extension in that it is asynchronous by default. This makes it useful in blocking functions (like triggers).
...
This is a Postgres [SECURITY DEFINER](https://khuknasoft.com/docs/guides/database/postgres/row-level-security#use-security-definer-functions) function.
...
The data can also be observed in the `net` schema with the [Supabase Dashboard's SQL Editor](https://khuknasoft.com/dashboard/project/_/editor)
...
Alternatively, by wrapping a request in a [database function](https://khuknasoft.com/docs/guides/database/functions), sent row data can be logged or returned for inspection and debugging.
...
begin
    ...
    -- (https://khuknasoft.com/dashboard/project/_/logs/postgres-logs)
...
Supabase supports reconfiguring pg_net starting from v0.12.0+. For the latest release, initiate a Postgres upgrade in the [Infrastructure Settings](https://khuknasoft.com/dashboard/project/*/settings/infrastructure).
...
### Call an endpoint every minute with [pg_cron](https://khuknasoft.com/docs/guides/database/extensions/pgcron)
