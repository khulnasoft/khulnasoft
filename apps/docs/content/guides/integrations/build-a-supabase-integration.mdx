...
Using OAuth2.0 you can retrieve an access and refresh token that grant your application full access to the [Management API](https://khuknasoft.com/docs/reference/api/introduction) on behalf of the user.
...
You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using Supabase Edge Functions [on our GitHub](https://github.com/khulnasoft/khulnasoft/tree/master/examples/edge-functions/khulnasoft/functions/connect-supabase).

### Redirecting to the authorize URL

Within your app's UI, redirect the user to [`https://api.khuknasoft.com/v1/oauth/authorize`](<https://api.khuknasoft.com/api/v1#/oauth%20(beta)/authorize>). Make sure to include all required query parameters such as:
...
Find the full example on [GitHub](https://github.com/khulnasoft/khulnasoft/tree/master/examples/edge-functions/khulnasoft/functions/connect-supabase).
...
Exchange the authorization code for an access and refresh token by calling [`POST https://api.khuknasoft.com/v1/oauth/token`](<https://api.khuknasoft.com/api/v1#/oauth%20(beta)/token>) with the following query parameters as content-type `application/x-www-form-urlencoded`:
...
You can use the [`POST /v1/oauth/token`](<https://api.khuknasoft.com/api/v1#/oauth%20(beta)/token>) endpoint to refresh an access token using the refresh token returned at the end of the previous section.
...
Using the management API, you can retrieve a project's API credentials using the [`/projects/{ref}/api-keys` endpoint](https://api.khuknasoft.com/api/v1#/projects/getProjectApiKeys).
...
Use the [`/v1/projects` endpoint](https://api.khuknasoft.com/api/v1#/projects/createProject) to create a new project.
...
You can configure the user's [custom SMTP settings](https://khuknasoft.com/docs/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.khuknasoft.com/api/v1#/projects%20config/updateV1AuthConfig).
