...
Edge functions natively support NPM modules and Node built-in APIs. [Link](https://khuknasoft.com/blog/edge-functions-node-npm).
...
The alpha stage indicates that the API might change in the future, not that the service isn’t stable. Even though the [uptime Service Level Agreement](https://khuknasoft.com/sla) does not cover products in Alpha, we do our best to have the service as stable as possible.
...
In addition to the Beta requirements, features in GA are covered by the [uptime SLA](https://khuknasoft.com/sla).
