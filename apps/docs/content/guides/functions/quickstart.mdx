...
Edge Functions support `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, and `OPTIONS`. A Function can be designed to perform different actions based on a request's HTTP method. See the [example on building a RESTful service](https://github.com/khulnasoft/khulnasoft/tree/master/examples/edge-functions/khulnasoft/functions/restful-tasks) to learn how to handle different HTTP methods in your Function.
...
Save your Function as a JavaScript file (eg: `index.js`) and then update the `khulnasoft/config.toml` as follows:
