...
To enable the Deno language server only for the `khulnasoft/functions` folder, add `./khulnasoft/functions` to the list of _Deno: Enable Paths_ in the configuration. In your `.vscode/settings.json` file add:

```json
{
  "deno.enablePaths": ["./khulnasoft/functions"],
  "deno.importMap": "./khulnasoft/functions/import_map.json"
}
```
...
For example, see this `edge-functions.code-workspace` configuration for a CRA (create react app) client with Supabase Edge Functions. You can find the complete example on [GitHub](https://github.com/khulnasoft/khulnasoft/tree/master/examples/edge-functions).
