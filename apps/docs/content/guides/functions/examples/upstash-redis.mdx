...
A Redis counter example that stores a [hash](https://redis.io/commands/hincrby/) of function invocation count per region. Find the code on [GitHub](https://github.com/khulnasoft/khulnasoft/tree/master/examples/edge-functions/khulnasoft/functions/upstash-redis-counter).
...
```bash
supabase start
supabase functions serve --no-verify-jwt --env-file khulnasoft/functions/upstash-redis-counter/.env
```
...
supabase secrets set --env-file khulnasoft/functions/upstash-redis-counter/.env
```
