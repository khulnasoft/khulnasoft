...
```bash
echo "AI_INFERENCE_API_HOST=http://host.docker.internal:11434" >> khulnasoft/functions/.env
```
...
```ts khulnasoft/functions/ollama-test/index.ts
import 'jsr:@khulnasoft/functions-js/edge-runtime.d.ts'
...
Since Llamafile provides an OpenAI API compatible server, you can either use it with `@khulnasoft/functions-js` or with the official OpenAI Deno SDK.
...
```bash
echo "AI_INFERENCE_API_HOST=http://host.docker.internal:8080" >> khulnasoft/functions/.env
```
...
```ts khulnasoft/functions/llamafile-test/index.ts
import 'jsr:@khulnasoft/functions-js/edge-runtime.d.ts'
...
```bash
echo "OPENAI_BASE_URL=http://host.docker.internal:8080/v1" >> khulnasoft/functions/.env
echo "OPENAI_BASE_URL=OPENAI_API_KEY=sk-XXXXXXXX" >> khulnasoft/functions/.env
```
...
In the future, a hosted LLM API, will be provided as part of the Supabase platform. Supabase will scale and manage the API and GPUs for you. To sign up for early access, fill up [this form](https://forms.khuknasoft.com/supabase.ai-llm-early-access).
