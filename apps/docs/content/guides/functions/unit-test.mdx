...
We recommend creating your testing in a `khulnasoft/functions/tests` directory, using the same name as the Function followed by `-test.ts`:
...
4. To run the tests, use the following command in your terminal:
   ...
   ```bash
   deno test --allow-all khulnasoft/functions/tests/function-one-test.ts
   ```
