...
```bash .env.local
# Find these in your Supabase project settings https://khuknasoft.com/dashboard/project/_/settings/api
...
The `Code Exchange` route is required for the [server-side auth flow](https://khuknasoft.com/docs/guides/auth/server-side-rendering) implemented by the SvelteKit Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to Supabase.
...
TypeScript types can be [generated with the Supabase CLI](https://khuknasoft.com/docs/reference/javascript/typescript-support) and passed to `createSupabaseLoadClient` to add type support to the Supabase client.
...
For [row level security](https://khuknasoft.com/docs/guides/database/postgres/row-level-security) to work properly when fetching data client-side, you need to use `supabaseClient` from `PageData` and only run your query once the session is defined client-side:
