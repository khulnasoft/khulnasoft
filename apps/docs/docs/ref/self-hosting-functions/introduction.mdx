...
```bash
./run.sh start --main-service /path/to/khulnasoft/functions -p 9000
```
...
docker run -it --rm -p 9000:9000 -v /path/to/khulnasoft/functions:/usr/services supabase/edge-runtime start --main-service /usr/services
```
...
You can view the logs for the Edge Runtime, by visiting Fly.io’s Dashboard > Your App > Metrics. Also, you can serve edge-runtime from multiple regions by running `fly regions add [REGION]`.
  ...
  <RefSubLayout.Examples>
    ...
    ### Client libraries
    - [JavaScript](https://khuknasoft.com/docs/reference/javascript/functions-invoke)
    - [Dart](https://khuknasoft.com/docs/reference/dart/functions-invoke)
