...
test('log out', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'khulnasoft e2e' }).last().click();
...
test('switch organization', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'khulnasoft-e2e-2' }).click();

  await expect(page.getByTestId('organization-switcher')).toHaveText(/khulnasoft-e2e-2/);
});
