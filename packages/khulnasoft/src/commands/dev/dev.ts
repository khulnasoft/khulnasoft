...
export const TUNNEL_URL = 'https://khulnasoft.sh/api/tunnels';
...
export async function devCommand(options: DevCommandOptions, anonymousId?: string) {
  ...
  const KHULNASOFT_ENDPOINT_PATH = options.route;
  ...
  if (parsedOptions.tunnel) {
    ...
    tunnelOrigin = await createTunnel(parsedOptions.origin, KHULNASOFT_ENDPOINT_PATH);
  }
  devSpinner.succeed(`🛣️  Tunnel    → ${tunnelOrigin}${KHULNASOFT_ENDPOINT_PATH}`);
  ...
  await monitorEndpointHealth(parsedOptions, KHULNASOFT_ENDPOINT_PATH);
}
...
async function monitorEndpointHealth(parsedOptions: DevCommandOptions, endpointRoute: string) {
  ...
  Starting out? Use our starter ${chalk.bold('npx khulnasoft@latest init')}
