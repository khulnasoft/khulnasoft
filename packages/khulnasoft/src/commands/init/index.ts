...
const programName = 'khulnasoft init';
...
export async function init(program: IInitCommandOptions, anonymousId?: string): Promise<void> {
  if (anonymousId) {
    analytics.track({
      ...
      data: {},
      event: 'Run Khulnasoft Init Command',
    });
  }
  ...
  if (!projectPath) {
    console.log(
      '\nPlease specify the project directory:\n' +
        ...
        'For example:\n' +
        `  ${cyan(programName)} ${green('my-khulnasoft-app')}\n\n` +
  ...
  if (!program.secretKey) {
    ...
    try {
      ...
      console.error(
        ...
        `Failed to verify your secret key against ${program.apiUrl}. For EU instances use --api-url https://eu.api.khulnasoft.co or provide the correct secret key`
      );
