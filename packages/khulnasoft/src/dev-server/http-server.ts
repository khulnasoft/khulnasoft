...
export const WELL_KNOWN_ROUTE = '/.well-known/khulnasoft';
  ...
  private serveStudio(req: http.IncomingMessage, res: http.ServerResponse) {
    ...
    <html class="dark">
      <head>
        ...
        <title>Khulnasoft Studio</title>
      </head>
      <body style="padding: 0; margin: 0; overflow: hidden;">
        <script>
          const KHULNASOFT_CLOUD_STUDIO_ORIGIN = '${this.options.dashboardUrl}';
          ...
          function redirectToCloud() {
            // Replace the local tunnel with Khulnasoft Web Dashboard on build time.
            const url = new URL('/local-studio/auth', KHULNASOFT_CLOUD_STUDIO_ORIGIN);
