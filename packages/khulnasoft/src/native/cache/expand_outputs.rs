...
use crate::native::walker::{khulnasoft_walker, khulnasoft_walker_sync};
    ...
    P: AsRef<Path>,
{
    ...
    let found_paths = khulnasoft_walker_sync(directory, Some(&negated_globs))
        .filter_map(|path| {
...
pub fn get_files_for_outputs(
    ...
    if !globs.is_empty() {
        ...
        for (root, patterns) in partitioned_globs {
            ...
            let found_paths: Vec<String> = khulnasoft_walker(&root_path, false)
                .filter_map(|file| {
    ...
    if !directories.is_empty() {
        for dir in directories {
            ...
            let files_in_dir = khulnasoft_walker(&dir_path, false).filter_map(|e| {
...
mod test {
    ...
    fn setup_fs() -> TempDir {
        ...
        temp.child("nested")
            .child("deeply")
            .child("khulnasoft.darwin-arm64.node")
            .touch()
            .unwrap();
    ...
    fn should_expand_outputs() {
        let temp = setup_fs();
        let entries = vec![
            "packages/khulnasoft/src/native/*.node".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "packages/khulnasoft/src/native/khulnasoft.darwin-arm64.node",
                "test.txt"
            ]
        );
    }
