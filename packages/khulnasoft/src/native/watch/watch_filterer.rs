...
use crate::native::watch::utils::{get_ignore_files, get_khulnasoft_ignore, transform_event};

#[derive(Debug)]
pub struct WatchFilterer {
    pub khulnasoft_ignore: Option<IgnoreFilter>,
...
impl WatchFilterer {
    fn filter_event(&self, event: &Event, priority: Priority) -> bool {
        ...
        for (path, file_type) in event.paths() {
            ...
            let khulnasoft_ignore_match_type =
                if let Some(khulnasoft_ignore) = &self.khulnasoft_ignore {
                    khulnasoft_ignore.match_path(path, is_dir)
                } else {
                    Match::None
                };

            // if the khulnasoftignore file contains this file as a whitelist,
            // we do not want gitignore to filter it out, so it will always pass as true
            if matches!(khulnasoft_ignore_match_type, Match::Whitelist(_)) {
                trace!(
                    ?path,
                    "khulnasoftignore whitelist match, ignoring gitignore"
                );
                pass &= true;
            // If the khulnasoftignore file contains this file as an ignore,
            // then there's no point in checking the gitignore file
            } else if matches!(khulnasoft_ignore_match_type, Match::Ignore(_)) {
                trace!(?path, "khulnasoftignore ignore match, ignoring gitignore");
                pass &= false;
            } else {
                pass &= self
                    .git_ignore
...
pub(super) async fn create_filter(
    ...
    let khulnasoft_ignore_file = get_khulnasoft_ignore(origin);
    ...
    let khulnasoft_ignore = if let Some(khulnasoft_ignore_file) = khulnasoft_ignore_file {
        Some(
            IgnoreFilter::new(origin, &[khulnasoft_ignore_file])
                .await
                .map_err(anyhow::Error::from)?,
        )
    } else {
