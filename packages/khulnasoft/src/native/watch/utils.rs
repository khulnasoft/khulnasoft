...
pub(super) fn get_khulnasoft_ignore<P: AsRef<Path>>(origin: P) -> Option<IgnoreFile> {
    let khulnasoft_ignore_path = PathBuf::from(origin.as_ref()).join(".khulnasoftignore");
    if khulnasoft_ignore_path.exists() {
        Some(IgnoreFile {
            path: khulnasoft_ignore_path,
