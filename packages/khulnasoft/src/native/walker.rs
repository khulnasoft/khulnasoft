...
pub fn khulnasoft_walker_sync<'a, P>(
    directory: P,
    ...
    P: AsRef<Path> + 'a,
{
    ...
    let mut base_ignores: Vec<String> = vec![
        "**/node_modules".into(),
        "**/.git".into(),
        "**/.khulnasoft/cache".into(),
        "**/.khulnasoft/workspace-data".into(),
        "**/.yarn/cache".into(),
    ];
...
/// Walk the directory and ignore files from .gitignore and .khulnasoftignore
#[cfg(target_arch = "wasm32")]
pub fn khulnasoft_walker<P>(directory: P, use_ignores: bool) -> impl Iterator<Item = KhulnasoftFile>
where
    P: AsRef<Path>,
{
    ...
    P: AsRef<Path>,
{
    ...
    if use_ignores {
        walker.add_custom_ignore_filename(".khulnasoftignore");
    }
...
mod test {
    ...
    fn it_walks_a_directory() {
        ...
        let content = khulnasoft_walker("/does/not/exist", true).collect::<Vec<_>>();
        assert!(content.is_empty());

        let temp_dir = setup_fs();

        let mut content = khulnasoft_walker(&temp_dir, true).collect::<Vec<_>>();
        content.sort();
        let content = content
            .into_iter()
    ...
    fn handles_khulnasoft_ignore() {
        ...
        // add khulnasoftignore file
        temp_dir
            .child(".khulnasoftignore")
            .write_str(
                r"baz/
nested/child.txt
nested/child-two/
...
/packages
    ",
        ...
        let mut file_names = khulnasoft_walker(temp_dir, true)
            .map(
                |KhulnasoftFile {
