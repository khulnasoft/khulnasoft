...
use crate::native::walker::{khulnasoft_walker, KhulnasoftFile};
use crate::native::workspace::files_archive::{KhulnasoftFileHashed, KhulnasoftFileHashes};

pub fn full_files_hash(workspace_root: &Path) -> KhulnasoftFileHashes {
    let files = khulnasoft_walker(workspace_root, true).collect::<Vec<_>>();
...
pub fn selective_files_hash(
    workspace_root: &Path,
    mut archived_files: KhulnasoftFileHashes,
...
fn hash_files(files: Vec<KhulnasoftFile>) -> Vec<(String, KhulnasoftFileHashed)> {
            ...
            .filter_map(|file| {
                hash_file_path(&file.full_path).map(|hash| {
                    (
                        ...
                        KhulnasoftFileHashed(hash, file.mod_time),
                    )
                })
            })
            .collect::<Vec<_>>()
    ...
    fn should_selectively_hash_workspace() {
        ...
        let archived_files = vec![
            ...
            (
                ...
                KhulnasoftFileHashed(String::from("hash5"), 0),
            ),
        ]
        .into_iter()
        .collect::<KhulnasoftFileHashes>();
