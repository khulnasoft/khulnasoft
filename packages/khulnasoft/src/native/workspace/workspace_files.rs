...
use crate::native::workspace::types::{
    FileLocation, KhulnasoftWorkspaceFiles, KhulnasoftWorkspaceFilesExternals,
};

pub(super) fn get_files(
    project_root_map: HashMap<String, String>,
    files: Vec<FileData>,
) -> napi::Result<KhulnasoftWorkspaceFiles> {
    if files.is_empty() {
    ...
    Ok(KhulnasoftWorkspaceFiles {
        project_file_map,
        global_files,
        external_references: Some(KhulnasoftWorkspaceFilesExternals {
