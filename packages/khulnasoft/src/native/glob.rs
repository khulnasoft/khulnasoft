...
pub struct KhulnasoftGlobSetBuilder {
    included_globs: GlobSetBuilder,
    excluded_globs: GlobSetBuilder,
}

impl KhulnasoftGlobSetBuilder {
    pub fn new<S: AsRef<str>>(globs: &[S]) -> anyhow::Result<Self> {
        let mut glob_set_builder = KhulnasoftGlobSetBuilder {
    ...
    pub fn add(&mut self, glob: &str) -> anyhow::Result<&mut KhulnasoftGlobSetBuilder> {
    ...
    pub fn build(&self) -> anyhow::Result<KhulnasoftGlobSet> {
        Ok(KhulnasoftGlobSet {
...
#[derive(Debug)]
pub struct KhulnasoftGlobSet {
    included_globs: GlobSet,
    excluded_globs: GlobSet,
}
impl KhulnasoftGlobSet {
...
pub(crate) fn build_glob_set<S: AsRef<str> + Debug>(
    globs: &[S],
) -> anyhow::Result<KhulnasoftGlobSet> {
    let result = globs
        .iter()
    ...
    KhulnasoftGlobSetBuilder::new(&result)?.build()
}
...
mod test {
    ...
    fn should_work_with_simple_globs() {
        ...
        assert!(glob_set.is_match("packages/khulnasoft/package.json"));
    ...
    fn should_not_detect_deeply_nested_package_json() {
        let glob_set = build_glob_set(&["packages/*/package.json"]).unwrap();
        assert!(!glob_set.is_match("packages/khulnasoft/test-files/package.json"))
    }
    ...
    fn should_detect_node_modules() {
        ...
        assert!(glob_set.is_match("packages/khulnasoft/node_modules"));
    }
