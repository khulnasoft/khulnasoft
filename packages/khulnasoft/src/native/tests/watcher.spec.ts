...
describe('watcher', () => {
  ...
  it('should trigger the callback when files are updated', async () => {
    return new Promise<void>(async (done) => {
      ...
      await wait();
      // khulnasoftignored file should not trigger a callback
  ...
  it('prioritize khulnasoftignore over gitignores', async () => {
    return new Promise<void>(async (done) => {
      await wait();
      watcher = new Watcher(temp.tempDir);
      watcher.watch((err, paths) => {
        expect(paths.some(({ path }) => path === '.env.local')).toBeTruthy();
        expect(paths.some(({ path }) => path === 'inner/.env.inner')).toBeTruthy();
