import { TaskDetails, KhulnasoftCache } from '../index';
...
describe('Cache', () => {
  ...
  beforeEach(() => {
    ...
    cache = new KhulnasoftCache(tempFs.tempDir, join(tempFs.tempDir, '.cache'), dbConnection);
  ...
  it('should store results into cache', async () => {
    ...
    expect(await tempFs.readFile('dist/output.txt')).toEqual('output contents 123');
  });
