...
describe('task planner', () => {
  ...
  const tsConfigBaseJson = JSON.stringify({
    compilerOptions: {
      paths: {
        '@khulnasoft/parent': ['libs/parent/src/index.ts'],
        '@khulnasoft/child': ['libs/child/src/index.ts'],
        '@khulnasoft/grandchild': ['libs/grandchild/src/index.ts'],
      },
    },
  });
    ...
    await withEnvironmentVariables({ TESTENV: 'env123' }, async () => {
      ...
      builder.addNode({
        ...
        data: {
          ...
          targets: {
            build: {
              executor: 'khulnasoft:run-commands',
              inputs: [
                'default',
                '^default',
      ...
      const taskGraph = createTaskGraph(projectGraph, {}, ['parent'], ['build'], undefined, {}, false);

      let khulnasoftJson = {} as any;

      const ref = transferProjectGraph(transformProjectGraphForRust(projectGraph));
      const planner = new HashPlanner(khulnasoftJson as any, ref);
  ...
  it('should plan the task where the project has dependencies', async () => {
    ...
    let taskGraph = createTaskGraph(projectGraph, { build: ['^build'] }, ['parent'], ['build'], undefined, {});
    let khulnasoftJson = {} as any;
    const planner = new HashPlanner(
      khulnasoftJson as any,
  ...
  it('should make a plan with multiple filesets of a project', async () => {
    ...
    let taskGraph = createTaskGraph(projectGraph, {}, ['parent'], ['build', 'test'], undefined, {});
    let khulnasoftJson = {
      namedInputs: {
        prod: ['!{projectRoot}/**/*.spec.ts'],
      },
  ...
  it('should be able to handle multiple filesets per project', async () => {
    await withEnvironmentVariables({ MY_TEST_HASH_ENV: 'MY_TEST_HASH_ENV_VALUE' }, async () => {
      let projectFileMap = {
        parent: [
          { file: 'libs/parent/filea.ts', hash: 'a.hash' },
          { file: 'libs/parent/filea.spec.ts', hash: 'a.spec.hash' },
      ...
      builder.addNode({
        ...
        data: {
          root: 'libs/child',
          namedInputs: {
            prod: ['!{projectRoot}/**/*.spec.ts', '{workspaceRoot}/global2', { env: 'MY_TEST_HASH_ENV' }],
          },
          targets: {
            test: {
              inputs: ['default'],
      ...
      builder.addStaticDependency('parent', 'child', 'libs/parent/filea.ts');
      let projectGraph = builder.getUpdatedProjectGraph();
      let taskGraph = createTaskGraph(projectGraph, { build: ['^build'] }, ['parent'], ['test'], undefined, {});
      let khulnasoftJson = {
        namedInputs: {
  ...
  it('should hash executors', async () => {
    ...
    builder.addNode({
      ...
      data: {
        ...
        targets: {
          lint: {
            inputs: ['default'],
            executor: '@khulnasoft/eslint:lint',
          },
        },
      },
    });
    builder.addExternalNode({
      type: 'npm',
      name: 'npm:@khulnasoft/eslint',
      data: {
        packageName: '@khulnasoft/eslint',
        hash: 'hash1',
        version: '1.0.0',
      },
    });
    builder.addExternalNode({
      type: 'npm',
      name: 'npm:@khulnasoft/devkit',
      data: {
        packageName: '@khulnasoft/devkit',
        hash: 'hash2',
        version: '1.0.0',
      },
    });
    builder.addDependency('npm:@khulnasoft/eslint', 'npm:@khulnasoft/devkit', DependencyType.static);
    let projectGraph = builder.getUpdatedProjectGraph();
    let taskGraph = createTaskGraph(projectGraph, { build: ['^build'] }, ['proj'], ['lint'], undefined, {});
    let khulnasoftJson = {
      namedInputs: {
        default: ['{projectRoot}/**/*', '{workspaceRoot}/global1'],
        prod: ['!{projectRoot}/**/*.spec.ts'],
  ...
  it('should build plans where the project graph has circular dependencies', async () => {
    ...
    builder.addNode({
      ...
      data: {
        root: 'libs/parent',
        targets: { build: { executor: 'khulnasoft:run-commands' } },
      },
    });
    builder.addNode({
      name: 'child',
  ...
  it('should build plans where a project specifies no external dependencies', async () => {
    ...
    let projectGraph = builder.getUpdatedProjectGraph();
    let taskGraph = createTaskGraph(projectGraph, {}, ['proj'], ['build'], undefined, {});
    let khulnasoftJson = {} as any;
    const planner = new HashPlanner(khulnasoftJson, transferProjectGraph(transformProjectGraphForRust(projectGraph)));
    const taskIds = Object.keys(taskGraph.tasks);
  ...
  it('should include npm projects', async () => {
    ...
    let projectGraph = builder.getUpdatedProjectGraph();
    let taskGraph = createTaskGraph(projectGraph, { build: ['^build'] }, ['app'], ['build'], undefined, {});
    let khulnasoftJson = {} as any;
    const transformed = transferProjectGraph(transformProjectGraphForRust(projectGraph));
    const planner = new HashPlanner(khulnasoftJson as any, transformed);
  ...
  describe('dependentTasksOutputFiles', () => {
    it('should depend on dependent tasks output files', async () => {
      ...
      const transformed = transferProjectGraph(transformProjectGraphForRust(projectGraph));
      const planner = new HashPlanner(khulnasoftJson, transformed);
