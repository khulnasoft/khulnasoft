...
import { KhulnasoftJsonConfiguration } from '../../config/khulnasoft-json';
...
describe('Workspace Context', () => {
  ...
  it('should gather workspace file information', async () => {
    ...
    const khulnasoftJson: KhulnasoftJsonConfiguration = {};
    await fs.createFiles({
      './khulnasoft.json': JSON.stringify(khulnasoftJson),
    ...
    const context = new WorkspaceContext(fs.tempDir, cacheDirectoryForWorkspace(fs.tempDir));
  ...
  describe('hashing', () => {
    ...
    beforeEach(async () => {
      ...
      context = new WorkspaceContext(fs.tempDir, cacheDirectoryForWorkspace(fs.tempDir));
    });

    it('should hash consistently when nothing changes', () => {
      let hash = context.hashFilesMatchingGlob(['**/*.txt']);
      for (let i = 0; i < 100; i++) {
        const newContext = new WorkspaceContext(fs.tempDir, cacheDirectoryForWorkspace(fs.tempDir));
