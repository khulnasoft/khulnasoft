...
use crate::native::types::{Input, KhulnasoftJson};
use crate::native::{
...
pub struct HashPlanner {
    khulnasoft_json: KhulnasoftJson,
...
impl HashPlanner {
    #[napi(constructor)]
    pub fn new(khulnasoft_json: KhulnasoftJson, project_graph: External<ProjectGraph>) -> Self {
        enable_logger();
        Self {
            khulnasoft_json,
            project_graph,
    ...
    pub fn get_plans_internal(
        ...
        task_ids
            ...
            .map(|id| {
                ...
                let inputs = get_inputs(task, &self.project_graph, &self.khulnasoft_json)?;
                ...
                let mut inputs: Vec<HashInstruction> = target
                    ...
                    .into_iter()
                    .chain(vec![
                        HashInstruction::Environment("KHULNASOFT_CLOUD_ENCRYPTION_KEY".into()),
                        HashInstruction::WorkspaceFileSet(vec![
                            "{workspaceRoot}/khulnasoft.json".to_string(),
                            "{workspaceRoot}/.gitignore".to_string(),
                            "{workspaceRoot}/.khulnasoftignore".to_string(),
                        ]),
                    ])
                    .chain(self_inputs)
                    .collect();
    ...
    fn target_input<'a>(
        ...
        // we can only vouch for @khulnasoft packages's executor dependencies
        // if it's "run commands" or third-party we skip traversing since we have no info what this command depends on
        if target
            .executor
            .as_ref()
            .is_some_and(|e| e.starts_with("@nrwl/") || e.starts_with("@khulnasoft/"))
        {
    ...
    fn gather_project_inputs(
        ...
        for project in project_inputs {
            ...
            for project in projects {
                let named_inputs =
                    get_named_inputs(&self.khulnasoft_json, &self.project_graph.nodes[project]);
