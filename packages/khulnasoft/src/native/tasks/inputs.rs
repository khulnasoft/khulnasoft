...
pub(super) fn get_inputs<'a>(
    ...
    khulnasoft_json: &'a KhulnasoftJson,
    ...
    let named_inputs = get_named_inputs(khulnasoft_json, project_node);
...
pub(super) fn get_inputs_for_dependency<'a>(
    ...
    let inputs = get_named_inputs(khulnasoft_json, project);
...
If "{file_set}" is a named input, make sure it is defined in khulnasoft.json.
"#,
            file_set = s,
...
pub(super) fn get_named_inputs<'a>(
    ...
    let iterable_structs = [&khulnasoft_json.named_inputs, &project.named_inputs];
