...
export function getNativeFileCacheLocation() {
  if (process.env.KHULNASOFT_NATIVE_FILE_CACHE_DIRECTORY) {
    return process.env.KHULNASOFT_NATIVE_FILE_CACHE_DIRECTORY;
  } else {
    ...
    return join(tmpdir(), `khulnasoft-native-file-cache-${hash.digest('hex').substring(0, 7)}`);
  }
}
