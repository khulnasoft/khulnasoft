...
fn process_file(
    ...
    let mut lines_with_khulnasoft_ignore_comments: HashSet<usize> = HashSet::new();
    let leading_comments = leading_comments.borrow();
    for (_, comments) in leading_comments.iter() {
        for comment in comments {
            let comment_text = comment.text.trim();

            if comment_text.contains("khulnasoft-ignore-next-line") {
                let line_where_comment_ends = cm
                    .lookup_line(comment.span.hi)
                    .expect("Comments end on a line");

                lines_with_khulnasoft_ignore_comments.insert(line_where_comment_ends);
            }
        }
    }
    ...
    let code_is_not_ignored = |(specifier, pos): (String, BytePos)| {
        ...
        if line_with_code > 0
            && lines_with_khulnasoft_ignore_comments.contains(&(line_with_code - 1))
        {
            None
        } else {
            Some(specifier)
...
mod find_imports {
    ...
    use crate::native::walker::khulnasoft_walker;
...
/* prettier-ignore */ /* khulnasoft-ignore-next-line */
import 'a4'; import 'a5';

/* khulnasoft-ignore-next-line */ /* prettier-ignore */
import 'a4'; import 'a5';
...
<template>
  ...
  <WelcomeItem>
    ...
    fn should_find_imports_in_tsx_files() {
        ...
        temp_dir
            ...
            .write_str(
                ...
                const KhulnasoftProjectGraphViz = dynamic(
    ...
    #[test]
    fn should_ignore_lines_with_khulnasoft_ignore() {
        let temp_dir = TempDir::new().unwrap();
        temp_dir
            .child("test.ts")
            .write_str(
...
import(myTag`react@${version}`);
    ...
    #[test]
    #[ignore]
    fn should_find_imports_properly_for_all_files_in_khulnasoft_repo() {
        ...
        let files = khulnasoft_walker(root.clone(), true)
            .filter(|file| glob.is_match(&file.full_path))
            .map(|file| file.full_path)
            .collect::<Vec<_>>();

        let results: HashMap<_, _> =
            find_imports(HashMap::from([(String::from("khulnasoft"), files.clone())]))
                .unwrap()
                .into_iter()
        ...
        for dep in deps {
            ...
            if line_with_dep > 0 {
                if let Some(line_before_dep) = cm.get_line(line_with_dep - 1) {
                    ...
                    if trimmed_line == "// khulnasoft-ignore-next-line"
                        || trimmed_line == "/* khulnasoft-ignore-next-line */"
                    {
                        continue;
                    }
                }
