...
import { ListNotificationsResponse, Notification, KhulnasoftError, isSameFilter, NotificationFilter } from '@khulnasoft/js';
...
export type UseNotificationsProps = {
  ...
  onError?: (error: KhulnasoftError) => void;
};
...
export const useNotifications = (props?: UseNotificationsProps) => {
  ...
  const { notifications, on } = useKhulnasoft();
  const [data, setData] = useState<Array<Notification>>();
  const [error, setError] = useState<KhulnasoftError>();
