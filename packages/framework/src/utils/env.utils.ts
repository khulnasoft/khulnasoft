...
export const getBridgeUrl = async (): Promise<string> => {
   ...
   */
  if (process.env.KHULNASOFT_BRIDGE_ORIGIN) {
    return `${process.env.KHULNASOFT_BRIDGE_ORIGIN}/api/khulnasoft`;
  }

  // Vercel preview deployments
  if (process.env.NEXT_PUBLIC_VERCEL_ENV === 'preview' && process.env.NEXT_PUBLIC_VERCEL_URL) {
    return `https://${process.env.NEXT_PUBLIC_VERCEL_URL}/api/khulnasoft`;
  }

  // Local environments
  try {
    if (process.env.NODE_ENV === 'development') {
      const response = await fetch('http://localhost:2022/.well-known/khulnasoft');
