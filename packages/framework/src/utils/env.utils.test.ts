...
describe('env.utils', () => {
  ...
  describe('getBridgeUrl', () => {
    beforeEach(() => {
      delete process.env.KHULNASOFT_BRIDGE_ORIGIN;
    ...
    it('should return KHULNASOFT_BRIDGE_ORIGIN if defined', async () => {
      process.env.KHULNASOFT_BRIDGE_ORIGIN = 'http://example.com';
      const url = await getBridgeUrl();
      expect(url).toBe('http://example.com/api/khulnasoft');
    });
    ...
    it('should return NEXT_PUBLIC_VERCEL_URL if NEXT_PUBLIC_VERCEL_ENV is preview', async () => {
      ...
      const url = await getBridgeUrl();
      expect(url).toBe('https://vercel.example.com/api/khulnasoft');
    });
    ...
    it('should return local bridge URL in development environment', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        json: vi.fn().mockResolvedValue({
          ...
          route: '/api/khulnasoft',
        }),
      });
      global.fetch = mockFetch;
      const url = await getBridgeUrl();
      expect(url).toBe('http://localhost:2022/api/khulnasoft');
    });
