...
import { EmailProviderIdEnum, IEmailOptions } from '@khulnasoft/shared';
...
export class SendgridEmailProvider extends BaseProvider implements IEmailProvider {
  ...
  private createMailData(options: IEmailOptions) {
    ...
    const mailData: Partial<MailDataRequired> = {
      ...
      customArgs: {
        id: options.id,
        khulnasoftTransactionId: options.notificationDetails?.transactionId,
        khulnasoftMessageId: options.id,
        khulnasoftWorkflowIdentifier: options.notificationDetails?.workflowIdentifier,
        khulnasoftSubscriberId: options.notificationDetails?.subscriberId,
        ...options.customData,
      },
      attachments,
      personalizations: [
