...
export const configName = 'khulnasoft.toml'

function getConfigHeader(config: KHULNASOFTConfig) {
  return `# This is a config for KhulnaSoft sandbox template.
...
export type KHULNASOFTConfig = yup.InferType<typeof configSchema>
...
export async function loadConfig(configPath: string) {
  ...
  return (await configSchema.validate(migratedConfig)) as KHULNASOFTConfig
}

export async function saveConfig(
  configPath: string,
  config: KHULNASOFTConfig,
  overwrite?: boolean,
) {
  try {
    if (!overwrite) {
    ...
    throw new Error(
      `KhulnaSoft sandbox template config ${asFormattedSandboxTemplate(
        {
