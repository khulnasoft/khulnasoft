...
async function getTemplateBuildLogs({
  ...
  handleKHULNASOFTRequestError(res.error, 'Error getting template build status')
  return res.data as khulnasoft.paths['/templates/{templateID}/builds/{buildID}/status']['get']['responses']['200']['content']['application/json']
}

async function requestTemplateBuild(
  args?: khulnasoft.paths['/templates']['post']['requestBody']['content']['application/json']
) {
...
async function requestTemplateRebuild(
  templateID: string,
  args?: khulnasoft.paths['/templates/{templateID}']['post']['requestBody']['content']['application/json']
) {
...
async function triggerTemplateBuild(templateID: string, buildID: string) {
  ...
  handleKHULNASOFTRequestError(res.error, 'Error triggering template build')
  return res.data
}
...
export const buildCommand = new commander.Command('build')
  ...
  .option(
    '-d, --dockerfile <file>',
    `specify path to Dockerfile. By default KhulnaSoft tries to find ${asLocal(
      defaultDockerfileName
      ...
      try {
        ...
        try {
          child_process.execSync(
            `echo "${accessToken}" | docker login docker.${connectionConfig.domain} -u _khulnasoft_access_token --password-stdin`,
            {
              stdio: 'inherit',
              cwd: root,
            }
          ...
          console.error(
            'Docker login failed. Please try to log in with `khulnasoft auth login` and try again.'
          )
          process.exit(1)
        }
        process.stdout.write('\n')
        ...
        const cmd = `docker build . -f ${dockerfileRelativePath} --pull --platform linux/amd64 -t docker.${connectionConfig.domain
          }/khulnasoft/custom-envs/${templateID}:${template.buildID} ${Object.entries(
            dockerBuildArgs
          )
        ...
        try {
          child_process.execSync(
            `docker push docker.${connectionConfig.domain}/khulnasoft/custom-envs/${templateID}:${template.buildID}`,
            {
              stdio: 'inherit',
              cwd: root,
            }
  ...
  do {
    ...
    switch (template.status) {
      case 'building':
        ...
        break
      case 'ready': {
        const pythonExample = asPython(`from khulnasoft import Sandbox, AsyncSandbox
...
sandbox = await AsyncSandbox.create("${aliases?.length ? aliases[0] : template.templateID
        ...
        const typescriptExample = asTypescript(`import { Sandbox } from 'khulnasoft'
      ...
      case 'error':
        ...
        throw new Error(
          `\n❌ Building sandbox template ${asFormattedSandboxTemplate({
            ...
            '(https://khulnasoft.com/docs/getting-help)'
          )} to get help.\n`
        )
    }
...
function getDockerfile(root: string, file?: string) {
  ...
  // Check if default dockerfile khulnasoft.Dockerfile exists
...
async function requestBuildTemplate(
  ...
  Omit<
    khulnasoft.paths['/templates']['post']['responses']['202']['content']['application/json'],
    'logs'
  >
> {
  let res
  ...
  handleKHULNASOFTRequestError(res.error, 'Error requesting template build')
  return res.data
}
