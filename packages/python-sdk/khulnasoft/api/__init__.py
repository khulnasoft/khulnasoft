...
from khulnasoft.api.client.client import AuthenticatedClient
from khulnasoft.connection_config import ConnectionConfig
from khulnasoft.api.metadata import default_headers
from khulnasoft.exceptions import (
    AuthenticationException,
    SandboxException,
    RateLimitException,
)
from khulnasoft.api.client.types import Response
...
class ApiClient(AuthenticatedClient):
    """
    The client for interacting with the KHULNASOFT API.
    """
        ...
        **kwargs,
        ...
        if require_api_key:
            if config.api_key is None:
                raise AuthenticationException(
                    "API key is required, please visit the Team tab at https://khulnasoft.com/dashboard to get your API key. "
                    "You can either set the environment variable `KHULNASOFT_API_KEY` "
                    'or you can pass it directly to the sandbox like Sandbox(api_key="khulnasoft_...")',
                )
        ...
        if require_access_token:
            if config.access_token is None:
                raise AuthenticationException(
                    "Access token is required, please visit the Personal tab at https://khulnasoft.com/dashboard to get your access token. "
                    "You can set the environment variable `KHULNASOFT_ACCESS_TOKEN` or pass the `access_token` in options.",
                )
