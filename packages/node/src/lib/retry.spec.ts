...
import { defaultRetryCondition, Khulnasoft } from '../index';
...
describe('Khulnasoft Node.js package - Retries and idempotency-key', () => {
  ...
  const khulnasoft = new Khulnasoft('fake-key', {
  ...
  it('should retry trigger and generate idempotency-key only once for request', async () => {
    ...
    const result = await khulnasoft.trigger('fake-workflow', {
      to: { subscriberId: '123' },
      payload: {},
    });
  ...
  it('should retry getting topics list', async () => {
    ...
    const result = await khulnasoft.topics.list({});
  ...
  it('should fail after reaching max retries', async () => {
    ...
    await expect(khulnasoft.topics.list({})).rejects.toMatchObject({
      response: { status: 500 },
    });
  });
  ...
  it('should retry on various errors until it reach successful response', async () => {
    ...
    const khulnasoftClient = new Khulnasoft('fake-key', {
      backendUrl: BACKEND_URL,
      retryConfig: {
        initialDelay: 0,
        waitMin: 0.2,
    ...
    const result = await khulnasoftClient.topics.list({});
    expect(result.status).toEqual(200);
  });
