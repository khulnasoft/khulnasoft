...
describe('Khulnasoft Node.js package - Messages class', () => {
  ...
  test('should delete message by messageId', async () => {
    ...
    const result = await khulnasoft.messages.deleteById(messageId);
  ...
  test('should list messages with filter', async () => {
    ...
    const result = await khulnasoft.messages.list({
      page: 1,
      limit: 5,
