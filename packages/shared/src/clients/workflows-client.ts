...
import { createKhulnasoftBaseClient, HttpError, KhulnasoftRestResult } from './khulnasoft-base-client';

export const createWorkflowClient = (baseUrl: string, headers: HeadersInit = {}) => {
  const baseClient = createKhulnasoftBaseClient(baseUrl, headers);

  const createWorkflow = async (
    createWorkflowDto: CreateWorkflowDto
  ): Promise<KhulnasoftRestResult<WorkflowResponseDto, HttpError>> => {
  ...
  const getWorkflow = async (workflowId: string): Promise<KhulnasoftRestResult<WorkflowResponseDto, HttpError>> => {
  ...
  const getWorkflowStepData = async (
    workflowId: string,
    stepId: string
  ): Promise<KhulnasoftRestResult<StepResponseDto, HttpError>> => {
  ...
  const deleteWorkflow = async (workflowId: string): Promise<KhulnasoftRestResult<void, HttpError>> => {
    return await baseClient.safeDelete(`/v2/workflows/${workflowId}`);
  };

  const searchWorkflows = async (
    queryParams: GetListQueryParams
  ): Promise<KhulnasoftRestResult<ListWorkflowResponse, HttpError>> => {
   ...
   * @returns {Promise<KhulnasoftRestResult<ListWorkflowResponse, HttpError>>} - A promise that resolves to the result of the workflow retrieval.
   */
  const searchWorkflowsV1 = async (queryParams?: string): Promise<KhulnasoftRestResult<WorkflowResponseDto[], HttpError>> => {
    const query = new URLSearchParams();
  ...
  ): Promise<KhulnasoftRestResult<GeneratePreviewResponseDto, HttpError>> => {
  ...
  const getWorkflowTestData = async (
    workflowId: string
  ): Promise<KhulnasoftRestResult<WorkflowTestDataResponseDto, HttpError>> => {
