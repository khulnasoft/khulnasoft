  ...
  const renderEpic = (epic: Epic, depth = 0) => (
    <div key={epic.iid} className={`mb-4 ${depth > 0 ? 'ml-6' : ''}`}>
      <div className="mb-2 flex items-center">
        <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => toggleEpicExpansion(epic.iid)}>
      ...
      {expandedEpics.includes(epic.iid) && (
        <div className="ml-6">
          ...
          {epic.childEpics?.length > 0 && (
            <div className="mt-2">
              <h4 className="mb-1 mt-4 font-semibold">Child Epics:</h4>
      ...
      <CardContent>
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span>
              {formatDateForUI(epicData.createdAt)} - {formatDateForUI(epicData.closedAt)}
            </span>
          </div>
          <div className="w-1/3">
        ...
        <Tabs defaultValue="summary">
          ...
          <TabsContent value="summary">
            <Card>
              ...
              <CardContent>
                <h3 className="mb-2 mt-4 font-semibold">Summary:</h3>
                <Markdown contents={epicData.summary?.summary || ''} />
                <h3 className="mb-2 mt-4 font-semibold">Key Points:</h3>
                <Markdown contents={epicData.summary?.keyPoints || ''} />
                <h3 className="mb-2 mt-4 font-semibold">Discussion Summary:</h3>
                <Markdown contents={epicData.summary?.discussionSummary || ''} />
                <h3 className="mb-2 mt-4 font-semibold">Closed Issues:</h3>
                <Markdown contents={epicData.summary?.closedIssues || ''} />
                <h3 className="mb-2 mt-4 font-semibold">Pending Issues:</h3>
                <Markdown contents={epicData.summary?.pendingIssues || ''} />
                <h3 className="mb-2 mt-4 font-semibold">Stats:</h3>
              ...
              <CardContent>
                {epicData.releaseNotes?.map(({ milestone: release, summary }) => (
                  <div key={release} className="mb-4">
                    <h3 className="mb-2 font-semibold">Release {release}</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {['CLOSED', 'OPEN'].map((status) => (
                    <div key={status} className="rounded border p-4">
                      ...
                      <ul className="list-disc pl-5">
                        {epicData.issues
                          .filter((issue) => issue.state === status)
                          .map((issue) => (
                            <li key={issue.id}>
                              {issue.title} (Release {issue.milestone?.title || 'Not Assigned'})
                            </li>
                          ))}
                      </ul>
                    </div>
          ...
          <TabsContent value="structure">
            <Card>
              ...
              <CardContent>{renderEpic(epicData)}</CardContent>
            </Card>
          </TabsContent>
        </Tabs>
      </CardContent>
