...
export default function BugReportTab({ data }: { data: BugReport }) {
  return (
    <div className="bug-report-tab">
      <h2 className="mb-4 text-2xl font-bold">Bug Report Insights</h2>

      {/* Common Bug Types */}
      <section className="mb-8">
        <h3 className="mb-2 text-xl font-semibold">Common Bug Types</h3>
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {data.common_bug_types.map((bugType, index) => (
              <tr key={index}>
                <td className="border border-gray-300 px-4 py-2">{bugType.type}</td>
                <td className="border border-gray-300 px-4 py-2">{bugType.count}</td>
                <td className="border border-gray-300 px-4 py-2">{bugType.percentage}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Frequency Over Time */}
      <section className="mb-8">
        <h3 className="mb-2 text-xl font-semibold">Frequency Over Time</h3>
        <table className="w-full border-collapse border border-gray-300">
          <thead className="bg-gray-100">
            <tr>
          ...
          <tbody>
            {data.frequency_over_time.map((freq, index) => (
              <tr key={index}>
                <td className="border border-gray-300 px-4 py-2">{freq.period}</td>
                <td className="border border-gray-300 px-4 py-2">{freq.bug_count}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <section className="mb-8">
        <h3 className="mb-2 text-xl font-semibold">Average Resolution Time</h3>
        <p className="text-gray-700">{data.average_resolution_time} days</p>
      </section>

      {/* Potential Root Causes */}
      <section className="mb-8">
        <h3 className="mb-2 text-xl font-semibold">Potential Root Causes</h3>
