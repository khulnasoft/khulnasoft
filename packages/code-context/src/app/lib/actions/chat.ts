...
export async function* sendChatMessage(
  ...
  trackAction(session?.user?.name, 'chat_message').catch((e) => console.error('Could not track action:', e));
    ...
    messages = [
      {
        ...
        ----------
        Open Actions:
        ${mrDetails.discussionsAnalysis.actions
          .map(
            (action) => `
