...
// --------------------------------------------------
export async function fetchHighLevelInsights(
  blob: InsightsBlob
): Promise<Pick<BlobAnalysis, 'explanation' | 'code_flow'>> {
  ...
  const prompt = `
Analyze the following file data from a GitLab repository. Focus on two areas only:
...
// 2. Functions & Classes
// --------------------------------------------------
export async function fetchFunctionsAndClasses(
  blob: InsightsBlob
): Promise<Pick<BlobAnalysis, 'functions' | 'classes'>> {
  const session = await getServerSession(authOptions);
  if (!session) {
    throw new Error('No session found. Please log in.');
  }
  ...
  const prompt = `
Analyze the following file data from a GitLab repository. Focus on:
...
// --------------------------------------------------
export async function fetchOtherInsights(
  blob: InsightsBlob
): Promise<Pick<BlobAnalysis, 'dependencies' | 'security' | 'performance_improvements' | 'data_dictionary'>> {
  const session = await getServerSession(authOptions);
  if (!session) {
    throw new Error('No session found. Please log in.');
  }
  ...
  const prompt = `
Analyze the following file data from a GitLab repository. Focus on these four areas:
1) Dependencies
2) Security
3) Performance
4) Data Dictionary
