...
export async function fetchBlob(url: string): Promise<InsightsBlob | null> {
  ...
  trackRun(user?.name, user?.email, url, 'blob').catch((e) => console.error('Could not track run:', e));
  ...
  const response = await fetch(
    `${GITLAB_BASE_URL}/projects/${encodeURIComponent(project)}/repository/files/${encodeURIComponent(path)}/raw?ref=${ref}`,
    {
      headers,
    }
  );

  const rawGitLabBlob = await response.text();

  const blob: InsightsBlob = {
    contents: rawGitLabBlob,
    project: project || '',
    path: path || '',
    ref: ref || '',
  };
  ...
  const [
    { explanation, code_flow },
    { functions, classes },
    { dependencies, security, performance_improvements, data_dictionary },
  ] = await Promise.all([fetchHighLevelInsights(blob), fetchFunctionsAndClasses(blob), fetchOtherInsights(blob)]);
...
function parseBlobURL(url: string): ParsedURL {
  try {
    ...
    if (refTypeIndex === -1) {
      throw new Error('Invalid GitLab URL format: cannot find blob or tree');
    }
