...
const EXPLANATION_MODEL = 'claude-3-7-sonnet-latest';
...
export async function fetchTopicExplanation(
  ...
  trackRun(user?.name, user?.email, level, 'learn').catch((e) => console.error('Could not track run:', e));
  ...
  const prompt = `
  You are an expert software developer who has read and understood the GitLab user handbook and best practices.
  ...
  const response = await callAnthropic(prompt, EXPLANATION_MODEL, EXPLANATION_MAX_OUTPUT_TOKENS);
