...
export async function generateMRAnalysis(mrDetails: MergeRequest, reviewType: ReviewType): Promise<Analysis | null> {
  ...
  try {
    ...
    const architecturalComponentsMatch = response.match(
      /<architecturalComponents>([\s\S]*?)<\/architecturalComponents>/
    );
    ...
    const analysis: Analysis = {
      ...
      suggestedQuestions: suggestedQuestionsMatch
        ? suggestedQuestionsMatch[1].trim()
        : 'No suggested questions available',
      architecturalComponents: architecturalComponentsMatch
        ? architecturalComponentsMatch[1].trim()
        : 'No architectural components found',
      testingDocumentation: testingDocumentationMatch
        ? testingDocumentationMatch[1].trim()
