...
const ISSUE_ANALYSIS_MODEL = 'claude-3-7-sonnet-latest';
const ISSUE_ANALYSIS_MAX_TOKENS = 8192;
const DISCUSSIONS_ANALYSIS_MODEL = 'claude-3-7-sonnet-latest';
...
export async function getIssueUnderstanding(issue: Issue): Promise<Understanding> {
  ...
  const prompt = `
    ...
    <unfamiliarTerms>Are there any unfamiliar terms or keywords in the issue? What do they mean according to GitLab’s documentation or handbook?</unfamiliarTerms>
...
export async function getIssueSummaries(issues: Issue[], gitlabAuthHeaders: Record<string, string>): Promise<Issue[]> {
  const issueSummaries = await Promise.all(
    issues.map(async (issue) => {
      // Fetch the discusssions for the issue
      const discussionResponse = await fetch(
        `https://gitlab.com/api/v4/projects/${issue.project_id}/issues/${issue.iid}/notes`,
        {
      ...
      const prompt = `Summarize the following GitLab issue:\n\n
        Title: ${issue.title}
        Description: ${issue.description}
...
export async function getMRSummaries(
  mrs: MergeRequest[],
  gitlabAuthHeaders: Record<string, string>
): Promise<MergeRequest[]> {
  const issueSummaries = await Promise.all(
    mrs.map(async (mr) => {
      // Fetch the discusssions for the issue
      const discussionResponse = await fetch(
        `https://gitlab.com/api/v4/projects/${mr.project_id}/merge_requests/${mr.iid}/notes`,
        {
          headers: gitlabAuthHeaders,
        }
      );
      ...
      const diffResponse = await fetch(
        `https://gitlab.com/api/v4/projects/${mr.project_id}/merge_requests/${mr.iid}/diffs`,
        {
          headers: gitlabAuthHeaders,
        }
      );
      ...
      const prompt = `Summarize the following GitLab Merge Request:\n\n
        Title: ${mr.title}
