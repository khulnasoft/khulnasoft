  ...
  data: {
    group: {
      workItem: {
        ...
        widgets: Array<GQWorkItemWidgetHierarchy | GQWorkItemWidgetNotes | object>;
      };
    };
  };
...
// Type Guards
function isGQWorkItemWidgetNotes(widget: any): widget is GQWorkItemWidgetNotes {
  return widget && 'discussions' in widget;
}

function isGQWorkItemWidgetHierarchy(widget: any): widget is GQWorkItemWidgetHierarchy {
  return widget && ('children' in widget || 'parent' in widget);
}

function isGQWorkItemWidgetMilestone(widget: any): widget is GQWorkItemWidgetMilestone {
...
function mapDiscussionNode(node: GQDiscussionNode): Discussion {
  return node.notes.edges.length > 0
    ? {
        ...
        id: parseInt(node.notes.edges[0].node.id.replace('gid://gitlab/Note/', '')),
...
function extractIDFromGID(string: string): string | undefined {
  const regex = /gid:\/\/gitlab\/Project\/(\d+)/;
