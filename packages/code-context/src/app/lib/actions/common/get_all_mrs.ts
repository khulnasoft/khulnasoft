  ...
  try {
    ...
    while (true) {
      const response = await fetch(
        `https://gitlab.com/api/v4/merge_requests?scope=all&author_id=${user.id}&created_after=${dateRange?.from.toISOString()}&created_before=${dateRange?.to.toISOString()}&state=merged&per_page=${perPage}&page=${page}`,
        {
          headers,
        }
      );
    ...
    const publicMergeRequests = allMergeRequests.filter((mr) => publicProjectIds.includes(mr.project_id));

    // Optionally fetch MR diff stats
    if (fetchDiffs) {
      const diffPromises = publicMergeRequests.map(async (mr) => {
        const diffResponse = await fetch(
          `https://gitlab.com/api/v4/projects/${mr.project_id}/merge_requests/${mr.iid}/changes`,
          {
            headers,
          }
        );
        ...
        if (!diffResponse.ok) {
          console.warn(`Failed to fetch diff stats for MR ${mr.iid}: ${diffResponse.statusText}`);
