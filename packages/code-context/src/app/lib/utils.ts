...
/**
 * Parses the GitLab issue URL to extract project ID and issue IID.
 * Example URL: "https://gitlab.com/gitlab-org/gitlab/-/issues/502414"
 */
export function parseGitlabIssueUrl(url: string) {
  const urlPattern = /https:\/\/gitlab\.com\/(.+?)\/-\/issues\/(\d+)/;
  const match = url.match(urlPattern);

  if (!match) {
    throw new Error('Invalid GitLab issue URL format');
  }
...
export function parseGitlabEpicUrl(url: string) {
  const urlPattern = /https:\/\/gitlab\.com\/groups\/(.+?)\/-\/epics\/(\d+)/;
  const match = url.match(urlPattern);

  if (!match) {
    throw new Error('Invalid GitLab epic URL format');
  }
