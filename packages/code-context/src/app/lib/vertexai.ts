...
export async function getTextEmbeddings(texts: string | string[], options: EmbeddingOptions = {}): Promise<number[]> {
  const {
    project = PROJECT_ID,
    location = LOCATION,
    model = MODEL_ID,
  ...
  const parameters: Value =
    dimensionality > 0 ? { structValue: { fields: { outputDimensionality: { numberValue: dimensionality } } } } : {};
