import KhulnaSoftProvider from "next-auth/providers/khulnasoft";
...
export const authOptions: NextAuthOptions = {
  providers: [
    KhulnaSoftProvider({
      clientId: process.env.KHULNASOFT_CLIENT_ID || '',
      clientSecret: process.env.KHULNASOFT_CLIENT_SECRET || '',
  ...
  callbacks: {
    async signIn({ user, account }: { user: User; account?: Account | null }) {
      if (user.email && user.email.endsWith("@khulnasoft.com")) {
        trackLogin(user)
        ...
        if (account?.access_token) {
          ...
          // Check if user is a member of khulnasoft-org
