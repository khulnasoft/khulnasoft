...
import { useKhulnasoftClient } from '@khulnasoft/auth-helpers-react'
...
const questions = [
  'How do I get started with Khulnasoft?',
  'How do I run Khulnasoft locally?',
...
const DocsSearch = () => {
  ...
  const khulnasoftClient = useKhulnasoftClient()
    ...
    switch (pageType) {
      ...
      case PageType.Reference:
        if (site === 'docs') {
          ...
          window.open(`https://khulnasoft.com/docs${link}`, '_blank')
          setIsOpen(false)
        }
        break
      case PageType.Integration:
        if (site === 'website') {
          ...
          window.open(`https://khulnasoft.com${link}`, '_blank')
          setIsOpen(false)
        }
        break
  ...
  const handleSearch = useCallback(
    async (query: string) => {
      ...
      khulnasoftClient.rpc('docs_search_fts', { query: query.trim() }).then(({ data, error }) => {
        sourcesLoaded += 1
      ...
      fetch(`${process.env.NEXT_PUBLIC_KHULNASOFT_URL}${FUNCTIONS_URL}search-embeddings`, {
        method: 'POST',
