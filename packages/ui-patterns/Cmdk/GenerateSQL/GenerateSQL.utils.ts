...
export function getEdgeFunctionUrl() {
  const khulnasoftUrl = process.env.NEXT_PUBLIC_KHULNASOFT_URL?.replace(/\/$/, '')

  if (!khulnasoftUrl) return undefined

  // https://github.com/khulnasoft/khulnasoft-js/blob/10d3423506cbd56345f7f6ab2ec2093c8db629d4/src/KhulnasoftClient.ts#L96
  const isPlatform = khulnasoftUrl.match(/(khulnasoft\.co)|(khulnasoft\.in)/)

  if (isPlatform) {
    const [schemeAndProjectId, domain, tld] = khulnasoftUrl.split('.')
    return `${schemeAndProjectId}.functions.${domain}.${tld}`
  } else {
    return `${khulnasoftUrl}/functions/v1`
  }
}
