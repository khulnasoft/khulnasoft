  ...
  const submit = useCallback(
    async (query: string) => {
      ...
      const eventSource = new SSE(`${BASE_PATH}/api/ai/docs`, {
        headers: {
          apikey: process.env.NEXT_PUBLIC_KHULNASOFT_ANON_KEY ?? '',
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_KHULNASOFT_ANON_KEY}`,
    ...
    <div onClick={(e) => e.stopPropagation()}>
                        ...
                        {message.status === MessageStatus.Pending ? (
                          ...
                          <ReactMarkdown
                            ...
                            transformLinkUri={(href) => {
                              const khulnasoftUrl = new URL('https://khulnasoft.com')
                              const linkUrl = new URL(href, 'https://khulnasoft.com')

                              if (linkUrl.origin === khulnasoftUrl.origin) {
      ...
      <div className="absolute bottom-0 w-full bg-background py-3">
        ...
        <Input
          ...
          autoFocus
          placeholder={
            isLoading || isResponding ? 'Waiting on an answer...' : 'Ask Khulnasoft AI a question...'
          }
          value={search}
          actions={
            <>
