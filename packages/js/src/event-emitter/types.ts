...
type KhulnasoftPendingEvent<A, D = undefined> = {
  args: A;
  data?: D;
};
type KhulnasoftResolvedEvent<A, D> = KhulnasoftPendingEvent<A, D> & {
  error?: unknown;
};
...
type EventObject<K extends string, ARGS, DATA, EVENT_STATUS = EventStatus<K>> = EVENT_STATUS extends 'pending'
  ? KhulnasoftPendingEvent<ARGS, DATA>
  : KhulnasoftResolvedEvent<ARGS, DATA>;
...
/**
 * Events that are emitted by Khulnasoft Event Emitter.
 *
