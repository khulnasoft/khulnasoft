...
import { useKhulnasoftEvent } from '../helpers/useKhulnasoftEvent';
import { useWebSocketEvent } from '../helpers/useWebSocketEvent';
import { useInboxContext } from './InboxContext';
import { useKhulnasoft } from './KhulnasoftContext';
...
export const CountProvider = (props: ParentProps) => {
  ...
  const updateTabCounts = async () => {
    ...
    const { data } = await khulnasoft.notifications.count({ filters });
    if (!data) {
      return;
    }
  ...
  const updateNewNotificationCountsOrCache = (notification: Notification, tags: string[]) => {
    const notificationsCache = khulnasoft.notifications.cache;
