...
const PRODUCTION_SOCKET_URL = 'https://ws.khulnasoft.co';
...
export class Socket extends BaseModule {
  ...
  async #handleConnectSocket(): Result<void> {
    try {
      ...
      return { error: new KhulnasoftError('Failed to initialize the socket', error) };
    }
  }
  ...
  async #handleDisconnectSocket(): Result<void> {
    try {
      ...
      return { error: new KhulnasoftError('Failed to disconnect from the socket', error) };
    }
  }
