...
- name: src-warning-receiver
  ...
  - send_resolved: true
    ...
    title_link: https://khulnasoft.com/docs/admin/observability/alerts#{{ .CommonLabels.service_name
    ...
    - type: button
      text: Next steps
      url: https://khulnasoft.com/docs/admin/observability/alerts#{{ .CommonLabels.service_name
        }}-{{ .CommonLabels.name | reReplaceAll "_" "-" }}
    - type: button
      text: Dashboard
      url: https://khulnasoft.com/-/debug/grafana/d/{{ .CommonLabels.service_name
...
- name: src-critical-receiver
  ...
  - send_resolved: true
    ...
    html: |-
      ...
      For next steps, please refer to our documentation: https://khulnasoft.com/docs/admin/observability/alerts#{{ .CommonLabels.service_name }}-{{ .CommonLabels.name | reReplaceAll "_" "-" }}
      For more details, please refer to the service dashboard: https://khulnasoft.com/-/debug/grafana/d/{{ .CommonLabels.service_name }}/{{ .CommonLabels.service_name }}?viewPanel={{ .CommonLabels.grafana_panel_id }}{{ $start := (index .Alerts 0).StartsAt.Unix }}{{ $end := (index .Alerts 0).EndsAt.Unix }}{{ if gt $end 0 }}&from={{ $start }}000&end={{ $end }}000{{ else }}&time={{ $start }}000&time.window=3600000{{ end }}{{ else }}{{ .CommonLabels.level | title }} alert '{{ .CommonLabels.name }}' for service '{{ .CommonLabels.service_name }}' has resolved.{{ end }}</body>
    text: |-
...
- name: src-warning-on-distribution
  ...
  - send_resolved: true
    ...
    source: https://khulnasoft.com/-/debug/grafana/d/{{ .CommonLabels.service_name
      }}/{{ .CommonLabels.service_name }}?viewPanel={{ .CommonLabels.grafana_panel_id
    ...
    details:
      Next steps: https://khulnasoft.com/docs/admin/observability/alerts#{{ .CommonLabels.service_name
        }}-{{ .CommonLabels.name | reReplaceAll "_" "-" }}
...
`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			problems := []string{}
			receivers, routes := newRoutesAndReceivers(tt.args.newAlerts, "https://khulnasoft.com", func(err error) {
