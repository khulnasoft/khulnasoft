...
def generate_category_from_name(category_fragment, category_array):
    """
    Takes a category ID in splitted form ("." as delimiter) and the array of the categories, and returns the proper category name that Docs expects.
    """
...
<img src="https://khulnasoft.com/img/{filename}" width="150"/>
"""
...
def build_readme_from_integration(integration, mode=''):
    ...
    if mode == 'collector':
        ...
        try:
            ...
            docs_rel_path = generate_category_from_name(
...
"""
    ...
    # EXPORTERS
        ...
        try:
            ...
            docs_rel_path = generate_category_from_name(integration['meta']['categories'][0].split("."), categories)
...
sidebar_label: "{sidebar_label}"
docs_status: "Published"
docs_rel_path: "Exporting Metrics"
...
sidebar_label: "{sidebar_label}"
docs_status: "Published"
docs_rel_path: "{docs_rel_path.replace("notifications", "Alerts & Notifications/Notifications")}"
...
sidebar_label: "{sidebar_label}"
docs_status: "Published"
docs_rel_path: "{docs_rel_path.replace("logs", "Logs")}"
...
sidebar_label: "{sidebar_label}"
docs_status: "Published"
docs_rel_path: "{docs_rel_path.replace("authentication", "Khulnasoft Cloud/Authentication & Authorization/Cloud Authentication & Authorization Integrations")}"
...
"""
    ...
    if "community" in integration['meta'].keys():
        ...
        community = "<img src=\"https://img.shields.io/badge/maintained%20by-Khulnasoft-%2300ab44\" />"

    return meta_yaml, sidebar_label, docs_rel_path, md, community
...
def build_path(meta_yaml_link):
    ...
    """
    return meta_yaml_link.replace("https://github.com/khulnasoft/", "") \
        .split("/", 1)[1] \
        .replace("edit/master/", "") \
...
def write_to_file(path, md, meta_yaml, sidebar_label, community, mode='default'):
    ...
    if mode == 'default':
        # Only if the path exists, this caters for running the same script on both the go and khulnasoft repos.
        if Path(path).exists():
...
for integration in integrations:
    ...
    if integration['integration_type'] == "collector":
        ...
        meta_yaml, sidebar_label, docs_rel_path, md, community = build_readme_from_integration(
