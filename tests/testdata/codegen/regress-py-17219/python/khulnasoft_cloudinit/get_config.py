...
class GetConfigResult:
    def __init__(__self__, base64_encode=None, boundary=None, gzip=None, id=None, parts=None, rendered=None):
        ...
        khulnasoft.set(__self__, "rendered", rendered)
...
def get_config(base64_encode: Optional[bool] = None,
               ...
               opts: Optional[khulnasoft.InvokeOptions] = None) -> AwaitableGetConfigResult:
    """
    ...
    __ret__ = khulnasoft.runtime.invoke('cloudinit:index/getConfig:getConfig', __args__, opts=opts, typ=GetConfigResult).value

    return AwaitableGetConfigResult(
        base64_encode=khulnasoft.get(__ret__, 'base64_encode'),
        boundary=khulnasoft.get(__ret__, 'boundary'),
        gzip=khulnasoft.get(__ret__, 'gzip'),
        id=khulnasoft.get(__ret__, 'id'),
        parts=khulnasoft.get(__ret__, 'parts'),
        rendered=khulnasoft.get(__ret__, 'rendered'))
def get_config_output(base64_encode: Optional[khulnasoft.Input[Optional[bool]]] = None,
                      boundary: Optional[khulnasoft.Input[Optional[str]]] = None,
                      gzip: Optional[khulnasoft.Input[Optional[bool]]] = None,
                      parts: Optional[khulnasoft.Input[Optional[Sequence[Union['GetConfigPartArgs', 'GetConfigPartArgsDict']]]]] = None,
                      opts: Optional[khulnasoft.InvokeOptions] = None) -> khulnasoft.Output[GetConfigResult]:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    ...
    __args__['parts'] = parts
    opts = khulnasoft.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = khulnasoft.runtime.invoke_output('cloudinit:index/getConfig:getConfig', __args__, opts=opts, typ=GetConfigResult)
    return __ret__.apply(lambda __response__: GetConfigResult(
        base64_encode=khulnasoft.get(__response__, 'base64_encode'),
        boundary=khulnasoft.get(__response__, 'boundary'),
        gzip=khulnasoft.get(__response__, 'gzip'),
        id=khulnasoft.get(__response__, 'id'),
        parts=khulnasoft.get(__response__, 'parts'),
        rendered=khulnasoft.get(__response__, 'rendered')))
