...
secret_value = khulnasoft.Output.secret("hello")
plain_value = khulnasoft.Output.unsecret(secret_value)
current_stack = khulnasoft.get_stack()
current_project = khulnasoft.get_project()
working_directory = os.getcwd()
file_mime_type = mimetypes.guess_type("./base64.txt")[0]
# using the filebase64 function
first = aws.s3.BucketObject("first",
    bucket=bucket.id,
    source=khulnasoft.StringAsset((lambda path: base64.b64encode(open(path).read().encode()).decode())("./base64.txt")),
...
second = aws.s3.BucketObject("second",
    bucket=bucket.id,
    source=khulnasoft.StringAsset(computeFilebase64sha256("./base64.txt")))
# using the sha1 function
third = aws.s3.BucketObject("third",
    bucket=bucket.id,
    source=khulnasoft.StringAsset(hashlib.sha1("content".encode()).hexdigest()))
