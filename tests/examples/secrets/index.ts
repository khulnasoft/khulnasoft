...
// A plaintext message.  We could turn this into a secret after the fact by passing it to `khulnasoft.secret` if we wished.
const message = c.require("message");

// Secrets are viral. When you combine secrets with `khulnasoft.all`, if any of the input values are secret, the entire
...
const combined = khulnasoft.all([apiKey, message]).apply(([s, p]) => {
    return p;
})
...
export const richStructure = new ReflectResource("rValue", {
    plain: khulnasoft.output("plaintext"),
    secret: khulnasoft.secret("secret value"),
}).value;
