...
const d = new Resource("dependent-3", { state: 99, noReplace: khulnasoft.all([a.state, b.state]).apply(([sa, sb]) => sa + sb) });

// Dependent-4 depends on Dependent and Dependent-2 via a replace property.
const e = new Resource("dependent-4", { state: khulnasoft.all([b.state, c.state]).apply(([sb, sc]) => sb + sc) });
