...
// directory using version 0.4.1-dev of the khulnasoft/policy sdk.
//
//nolint:paralleltest // mutates environment variables
func TestPolicyWithConfig(t *testing.T) {
	t.Skip("Skip test that is causing unrelated tests to fail - khulnasoft/khulnasoft#4149")
	...
	name, _ := e.RunCommand("khulnasoft", "whoami")
	...
	packsOutput, _ := e.RunCommand("khulnasoft", "policy", "ls", "--json")
	var packs []policyPacksJSON
	assertJSON(e, packsOutput, &packs)

	groupsOutput, _ := e.RunCommand("khulnasoft", "policy", "group", "ls", "--json")
	var groups []policyGroupsJSON
	assertJSON(e, groupsOutput, &groups)

	// Enable, Disable and then Delete the Policy Pack.
	e.RunCommand("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName), "0.0.1")

	// Validate Policy Pack Configuration.
	e.RunCommand("khulnasoft", "policy", "validate-config", fmt.Sprintf("%s/%s", orgName, policyPackName),
		"--config=configs/valid-config.json", "0.0.1")
	// Valid config, but no version specified.
	e.RunCommandExpectError("khulnasoft", "policy", "validate-config", fmt.Sprintf("%s/%s", orgName, policyPackName),
	...
	e.RunCommand("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName),
		"--config=configs/valid-config.json", "0.0.1")
	e.RunCommandExpectError("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName),
		"--config=configs/invalid-config.json", "0.0.1")

	// Disable Policy Pack specifying version.
	e.RunCommand("khulnasoft", "policy", "disable", fmt.Sprintf("%s/%s", orgName, policyPackName), "--version=0.0.1")

	// Enable and Disable without specifying the version number.
	e.RunCommand("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName), "latest")
	e.RunCommand("khulnasoft", "policy", "disable", fmt.Sprintf("%s/%s", orgName, policyPackName))

	e.RunCommand("khulnasoft", "policy", "rm", fmt.Sprintf("%s/%s", orgName, policyPackName), "0.0.1")
	e.RunCommand("khulnasoft", "policy", "rm", fmt.Sprintf("%s/%s", orgName, policyPackName), "all")
}

// TestPolicyWithoutConfig runs integration tests against the policy pack in the policy_pack_w_config
// directory. This tests against version 0.4.0 of the khulnasoft/policy sdk, prior to policy config being supported.
//
//nolint:paralleltest // mutates environment variables
func TestPolicyWithoutConfig(t *testing.T) {
	...
	// Publish the Policy Pack twice.
	e.RunCommand("khulnasoft", "policy", "publish", orgName)
	e.RunCommand("khulnasoft", "policy", "publish", orgName)
	...
	// Enable, Disable and then Delete the Policy Pack.
	e.RunCommand("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName), "1")
	e.RunCommand("khulnasoft", "policy", "disable", fmt.Sprintf("%s/%s", orgName, policyPackName), "--version=1")

	// Enable and Disable without specifying the version number.
	e.RunCommand("khulnasoft", "policy", "enable", fmt.Sprintf("%s/%s", orgName, policyPackName), "latest")
	e.RunCommand("khulnasoft", "policy", "disable", fmt.Sprintf("%s/%s", orgName, policyPackName))

	e.RunCommand("khulnasoft", "policy", "rm", fmt.Sprintf("%s/%s", orgName, policyPackName), "1")
	e.RunCommand("khulnasoft", "policy", "rm", fmt.Sprintf("%s/%s", orgName, policyPackName), "all")
}
