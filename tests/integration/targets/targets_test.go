...
func TestUntargetedCreateDuringTargetedUpdate(t *testing.T) {
	t.Skip() // TODO[khulnasoft/khulnasoft#4149]
	t.Parallel()
	...
	e.RunCommand("khulnasoft", "up", "--non-interactive", "--skip-preview", "--yes")
	urn, _ := e.RunCommand("khulnasoft", "stack", "output", "urn")
	...
	e.RunCommand("khulnasoft", "up", "--target", strings.TrimSpace(urn), "--non-interactive", "--skip-preview", "--yes")
	e.RunCommand("khulnasoft", "refresh", "--non-interactive", "--yes")
...
func TestDeleteManyTargets(t *testing.T) {
	...
	resourceType := tokens.Type("khulnasoft-nodejs:dynamic:Resource")
	...
	stdout, _ := e.RunCommandExpectError("khulnasoft", "destroy", "--skip-preview", "--yes", "--non-interactive",
		"--target", mkURNStr("a", ""))
	...
	e.RunCommand("khulnasoft", "destroy", "--skip-preview", "--yes", "--non-interactive",
