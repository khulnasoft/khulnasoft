...
import (
	...
	"cloud.google.com/go/kms/apiv1/kmspb" //nolint:staticcheck // See https://github.com/khulnasoft/khulnasoft/issues/45843
	...
	"github.com/khulnasoft/khulnasoft/internal/encryption/envelope"
	"github.com/khulnasoft/khulnasoft/internal/encryption/wrapper"
...
func (k *Key) Version(ctx context.Context) (encryption.KeyVersion, error) {
	key, err := k.client.GetCryptoKey(ctx, &kmspb.GetCryptoKeyRequest{ //nolint:staticcheck // See https://github.com/khulnasoft/khulnasoft/issues/45843
		Name: k.name,
	})
	if err != nil {
...
func (k *Key) Decrypt(ctx context.Context, cipherText []byte) (_ *encryption.Secret, err error) {
	...
	case mechanismEncrypt:
		res, err := k.client.Decrypt(ctx, &kmspb.DecryptRequest{ //nolint:staticcheck // See https://github.com/khulnasoft/khulnasoft/issues/45843
			Name:       k.name,
...
func (k *Key) Encrypt(ctx context.Context, plaintext []byte) (_ []byte, err error) {
	...
	res, err := k.client.Encrypt(ctx, &kmspb.EncryptRequest{ //nolint:staticcheck // See https://github.com/khulnasoft/khulnasoft/issues/45843
		Name:            k.name,
		Plaintext:       ev.Key,
