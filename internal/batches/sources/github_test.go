...
func TestGithubSource_CreateChangeset_CreationLimit(t *testing.T) {
	...
	assert.Equal(
		t,
		"reached GitHub's internal creation limit: see https://khulnasoft.com/docs/admin/config/batch_changes#avoiding-hitting-rate-limits: error in GraphQL response: error in GraphQL response: was submitted too quickly",
		err.Error(),
	)
}
