...
import (
	...
	githubapps_auth "github.com/khulnasoft/khulnasoft/internal/github_apps/auth"
	...
	if err != nil {
		if err != github.ErrPullRequestAlreadyExists {
			...
			if strings.Contains(err.Error(), "was submitted too quickly") {
				return exists, errors.Wrapf(err, "reached GitHub's internal creation limit: see https://khulnasoft.com/docs/admin/config/batch_changes#avoiding-hitting-rate-limits")
			}
			return exists, err
		}
