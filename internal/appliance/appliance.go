...
import (
	...
	"github.com/khulnasoft/khulnasoft/internal/appliance/config"
	pb "github.com/khulnasoft/khulnasoft/internal/appliance/v1"
	"github.com/khulnasoft/khulnasoft/internal/releaseregistry"
...
func (a *Appliance) ensureBackingSecretKeysExist(ctx context.Context, secret *corev1.Secret) error {
	...
	if _, ok := secret.Data[dataSecretEncryptedPasswordKey]; !ok {
		...
		if err := a.client.Get(ctx, initialPasswordSecretNsName, &initialPasswordSecret); err != nil {
			a.logger.Info("no initial password secret exists. Please refer to https://github.com/khulnasoft/khulnasoft/blob/main/cmd/appliance/README.md#development")
		...
		if adminPassword, ok := initialPasswordSecret.Data[initialPasswordSecretPasswordKey]; ok {
			...
			a.logger.Info("The k8s secret appliance-password exists, but it does not contain a key password. Please refer to https://github.com/khulnasoft/khulnasoft/blob/main/cmd/appliance/README.md#development")
			// We don't return an error here because we don't want to crash on
			// startup. We want to show the user an error message guiding them
			// to configure a password.
		}
