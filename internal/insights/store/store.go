...
import (
	...
	"github.com/khulnasoft/khulnasoft/internal/database/batch"
...
func (s *Store) SeriesPoints(ctx context.Context, opts SeriesPointsOpts) ([]SeriesPoint, error) {
	...
	// set of repos installed on Khulnasoft. To make this faster, we query Postgres for a list of repos the current user cannot see, and then exclude those from the
	// time series results. 🚨
	...
	// 2. We have quite a bit of customer feedback that suggests they don't even use repo permissions - they just don't install their private repos onto that Khulnasoft instance.
...
func FilterSeriesPoints(denyList []api.RepoID, points []SeriesPointForExport) []SeriesPointForExport {
	...
	for _, x := range points {
		...
		// See https://github.com/khulnasoft/khulnasoft/pull/61580#discussion_r1552271816 for more details.
