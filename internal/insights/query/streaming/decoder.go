...
import (
	...
	"github.com/khulnasoft/khulnasoft/internal/compute/client"

	"github.com/khulnasoft/khulnasoft/internal/api"
	streamapi "github.com/khulnasoft/khulnasoft/internal/search/streaming/api"
			...
			for _, result := range results {
				...
				for _, match := range result.Matches {
					for _, data := range match.Environment {
						value := data.Value
						if value == "" {
							continue // a bug in upstream compute processing means we need to check for empty replacements (https://github.com/khulnasoft/khulnasoft/issues/37972)
						}
