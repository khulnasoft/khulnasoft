...
func (s *Service) Search(ctx context.Context, args search.SymbolsParameters) (_ result.Symbols, err error) {
	...
	if args.Timeout > 0 {
		...
		defer func() {
			...
			err = errors.Newf("Processing symbols is taking a while, try again later ([more details](https://khulnasoft.com/docs/code_navigation/explanations/rockskip)).")
			for _, status := range s.status.threadIdToThreadStatus {
				if strings.HasPrefix(status.Name, fmt.Sprintf("indexing %s", args.Repo)) {
					err = errors.Newf("Still processing symbols ([more details](https://khulnasoft.com/docs/code_navigation/explanations/rockskip)). Estimated completion: %s.", status.Remaining())
				}
			}
		}()
	}
