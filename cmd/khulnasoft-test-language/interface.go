...
import (
	...
	backendDisplay "github.com/khulnasoft/khulnasoft/pkg/v3/backend/display"
	...
	b64secrets "github.com/khulnasoft/khulnasoft/pkg/v3/secrets/b64"
...
func (eng *languageTestServer) PrepareLanguageTests(
	...
	languageClient := plugin.NewLanguageRuntimeClient(
		pctx, req.LanguagePluginName, khulnasoftrpc.NewLanguageRuntimeClient(conn))
...
// TODO(https://github.com/khulnasoft/khulnasoft/issues/13944): We need a RunLanguageTest(t *testing.T) function that
...
func (eng *languageTestServer) RunLanguageTest(
	...
	languageClient := plugin.NewLanguageRuntimeClient(
		pctx, token.LanguagePluginName, khulnasoftrpc.NewLanguageRuntimeClient(conn))
	...
	for i, run := range test.runs {
		...
		for _, pkg := range pkgs {
			if pkg.Name() == "khulnasoft" {
				// No need to write the khulnasoft package, it's builtin to core SDKs
				continue
			}
	...
	// We always override the core "khulnasoft" package to point to the local core SDK we built as part of test
	// setup.
	localDependencies := map[string]string{}
	if token.CoreArtifact != "" {
		localDependencies["khulnasoft"] = token.CoreArtifact
	}
	for _, pkg := range packages {
		...
		//
		// TODO[khulnasoft/issues/16079]: This could probably be a per-sdk lock to be more fine grained and allow more
		// parallelism.
	...
	for i, run := range test.runs {
		...
		// TODO(https://github.com/khulnasoft/khulnasoft/issues/13940): We don't report back warning diagnostics here
		...
		// TODO(https://github.com/khulnasoft/khulnasoft/issues/13941): We don't capture stdout/stderr from the language
		...
		// TODO(https://github.com/khulnasoft/khulnasoft/issues/13942): This should only add new things, don't modify

		// Query the language plugin for what it thinks the project dependencies are, we expect to see khulnasoft and the SDKs.
		...
		if token.CoreVersion != "" {
			expectedDependencies = append(expectedDependencies, plugin.DependencyInfo{
				Name: "khulnasoft", Version: token.CoreVersion,
			})
		}
		for _, provider := range test.providers {
		...
		for _, expectedDependency := range expectedDependencies {
			...
			// library, which is generally _not_ just the plugin name. e.g. "@khulnasoft/aws" for the nodejs aws library.
