...
func newProviderServer(provider plugin.Provider) khulnasoftrpc.ResourceProviderServer {
...
func wrapProviderWithGrpc(provider plugin.Provider) (plugin.Provider, io.Closer, error) {
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			khulnasoftrpc.RegisterResourceProviderServer(srv, newProviderServer(provider))
			return nil
		},
	...
	wrapped := plugin.NewProviderWithClient(
		nil, provider.Pkg(), khulnasoftrpc.NewResourceProviderClient(conn), false)
