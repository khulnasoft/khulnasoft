...
import (
	...
	"github.com/khulnasoft/khulnasoft/cmd/frontend/internal/app/errorutil"
	"github.com/khulnasoft/khulnasoft/cmd/frontend/internal/app/router"
	"github.com/khulnasoft/khulnasoft/cmd/frontend/internal/app/ui"
	"github.com/khulnasoft/khulnasoft/cmd/frontend/internal/auth/accessrequest"
...
func NewHandler(db database.DB, logger log.Logger) http.Handler {
	...
	r.Get(router.GopherconLiveBlog).Handler(trace.Route(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, "https://khulnasoft.com/go", http.StatusFound)
	})))
