...
import (
	...
	"github.com/khulnasoft/khulnasoft/cmd/frontend/internal/batches/resolvers/apitest"
	...
	"github.com/khulnasoft/khulnasoft/internal/batches/state"
...
func TestChangesetCountsOverTimeIntegration(t *testing.T) {
	...
	githubExtSvc := &types.ExternalService{
		...
		Config: extsvc.NewUnencryptedConfig(bt.MarshalJSON(t, &schema.GitHubConnection{
			...
			Token: "abc",
			Repos: []string{"khulnasoft/khulnasoft"},
		})),
	}
	...
	githubRepo, err := githubSrc.GetRepo(ctx, "khulnasoft/khulnasoft")
	if err != nil {
		t.Fatal(err)
	}
