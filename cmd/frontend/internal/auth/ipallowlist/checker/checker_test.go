...
func Test_isAuthorized(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			trustedClientIps, trustedClientRanges, err := parseIPs(tc.trustedClientIps)
			require.NoError(t, err)
...
func Test_getConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: true,
			environ: map[string]string{
				AUTH_ALLOWED_IP_ADDRESS_USER_IP_REQUEST_HEADERS_OVERRIDE: "CF-Connecting-IP",
			},
		},
	}
	for _, tc := range tests {
...
func Test_getEnvCommaSeparated(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name string
		s    string
		...
		{
			name: "ip and cidr",
			s:    "1.1.1.1,1.2.3.4/32",
			want: autogold.Expect([]string{"1.1.1.1", "1.2.3.4/32"}),
		},
		{
			name: "ip and cidr with spaces",
			s:    " 1.1.1.1  ,    1.2.3.4/32    ",
			want: autogold.Expect([]string{"1.1.1.1", "1.2.3.4/32"}),
		},
		{
			name: "only one",
			s:    "somestr",
			want: autogold.Expect([]string{"somestr"}),
		},
	}
	for _, tc := range tests {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			os.Setenv(t.Name(), tc.s)
			t.Cleanup(func() {
				os.Unsetenv(t.Name())
			})
			tc.want.Equal(t, getEnvCommaSeparated(t.Name()))
		})
	}
}
