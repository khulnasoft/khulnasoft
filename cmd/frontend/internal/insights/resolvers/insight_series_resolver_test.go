...
func TestInsightStatusResolver_IncompleteDatapoints(t *testing.T) {
	...
	t.Run("without aggregating for timeouts should yield repositories", func(t *testing.T) {
		...
		repoStore.GetFunc.SetDefaultReturn(&internalTypes.Repo{ID: api.RepoID(repo), Name: "github.com/khulnasoft/khulnasoft"}, nil)
		...
		for _, alert := range got {
			...
			if isTimeout {
				...
				for _, repositoryResolver := range *resolvers {
					require.Equal(t, "github.com/khulnasoft/khulnasoft", repositoryResolver.Name())
				}
			}
		}
	})
