...
func WriteSubscriptionErrorResponseForFeature(w http.ResponseWriter, featureNameHumanReadable string) {
	WriteSubscriptionErrorResponse(
		...
		fmt.Sprintf("To use the %s feature, a site admin must upgrade the Khulnasoft license in the Khulnasoft [**site configuration**](/site-admin/configuration). (The site admin may also remove the site configuration that enables this feature to dismiss this message.)", featureNameHumanReadable),
	)
}
...
func WriteSubscriptionErrorResponse(w http.ResponseWriter, statusCode int, title, message string) {
	...
	fmt.Fprintln(w, `
<title>`+html.EscapeString(title)+` - Khulnasoft</title>
<style>
.bg {
	position: absolute;
	user-select: none;
...
<body>
<div class=bg></div>
<div class=msg><h1>`+html.EscapeString(title)+`</h1><p>`+html.EscapeString(message)+`</p><p>See <a href="https://khulnasoft.com/pricing">khulnasoft.com</a> for more information.</p></div>`)
}
