...
func TestExcludeRepoFromExternalServices_NoExistingExcludedRepos_NewExcludedRepoAdded(t *testing.T) {
	...
	repos.GetFunc.SetDefaultHook(func(_ context.Context, id api.RepoID) (*types.Repo, error) {
		...
		metadata := &github.Repository{NameWithOwner: "khulnasoft/khulnasoft"}
		return &types.Repo{ID: api.RepoID(1), Name: "github.com/khulnasoft/khulnasoft", ExternalRepo: spec, Metadata: metadata}, nil
	})
	...
	expectedConfig := `{"exclude":[{"name":"khulnasoft/khulnasoft"}],"repositoryQuery":["none"],"token":"abc","url":"https://github.com"}`
...
func TestExcludeRepoFromExternalServices_ExcludedRepoExists_AnotherExcludedRepoAdded(t *testing.T) {
	...
	externalServices.ListFunc.SetDefaultHook(func(_ context.Context, options database.ExternalServicesListOptions) ([]*types.ExternalService, error) {
		return []*types.ExternalService{{
			ID:     options.IDs[0],
			Kind:   extsvc.KindGitHub,
			Config: extsvc.NewUnencryptedConfig(`{"exclude":[{"name":"khulnasoft/khulnasoft"}],"repositoryQuery":["none"],"token":"abc","url":"https://github.com"}`),
		}}, nil
	})
	...
	expectedConfig := `{"exclude":[{"name":"khulnasoft/khulnasoft"},{"name":"sourcegraph/horsegraph"}],"repositoryQuery":["none"],"token":"abc","url":"https://github.com"}`
	assert.Equal(t, expectedConfig, *externalServices.UpdateFunc.History()[0].Arg3.Config)
}
...
func TestExcludeRepoFromExternalServices_ExcludedRepoExists_SameRepoIsNotExcludedAgain(t *testing.T) {
	...
	externalServices.ListFunc.SetDefaultHook(func(_ context.Context, options database.ExternalServicesListOptions) ([]*types.ExternalService, error) {
		return []*types.ExternalService{{
			ID:     options.IDs[0],
			Kind:   extsvc.KindGitHub,
			Config: extsvc.NewUnencryptedConfig(`{"exclude":[{"name":"khulnasoft/khulnasoft"},{"name":"sourcegraph/horsegraph"}],"repositoryQuery":["none"],"token":"abc","url":"https://github.com"}`),
		}}, nil
	})
...
func TestExcludeRepoFromExternalServices_ExcludedFromTwoExternalServices(t *testing.T) {
	...
	expectedConfig2 := `{"exclude":[{"name":"khulnasoft/khulnasoft"},{"name":"sourcegraph/horsegraph"}],"repositoryQuery":["none"],"token":"abc","url":"https://github.com"}`
