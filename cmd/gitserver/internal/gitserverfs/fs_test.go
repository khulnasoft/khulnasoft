...
func TestGitserverFS_RepoDir(t *testing.T) {
	...
	tts := []struct {
	...
	}{
		{
			...
			want:     "/data/repos/github.com/khulnasoft/khulnasoft/.git",
		},
		{
		...
		{
			repoName: "DELETED-123123.123123-github.com/khulnasoft/khulnasoft",
			want:     "/data/repos/github.com/khulnasoft/khulnasoft/.git",
		},
		{
			// This is invalid, but as a protection make sure that we still don't
			// allow a path outside of /data/repos.
			repoName: "github.com/khulnasoft/khulnasoft/../../../../../src-cli",
...
func TestIgnorePath(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{path: filepath.Join(reposDir, p4HomeName+"   "), shouldIgnore: true},
		{path: filepath.Join(reposDir, "khulnasoft/khulnasoft"), shouldIgnore: false},
	} {
