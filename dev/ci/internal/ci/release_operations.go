...
import (
	...
	"github.com/khulnasoft/khulnasoft/dev/ci/runtype"
...
func releasePromoteImages(c Config) operations.Operation {
	additionalProdRegistries := strings.Join([]string{images.KhulnasoftArtifactRegistryPublicRegistry, images.CloudEphemeralRegistry}, " ")
	image_args := strings.Join(images.KhulnasoftDockerImages, " ")
	return func(pipeline *bk.Pipeline) {
		pipeline.AddStep("Promote release to public",
			bk.Agent("queue", AspectWorkflows.QueueDefault),
			bk.Env("VERSION", c.Version),
			bk.Env("INTERNAL_REGISTRY", images.KhulnasoftInternalReleaseRegistry),
			bk.Env("PUBLIC_REGISTRY", images.KhulnasoftDockerPublishRegistry),
...
func releaseTestOperation(c Config) operations.Operation {
	devRegistry := images.KhulnasoftDockerDevRegistry
	prodRegistry := images.KhulnasoftDockerPublishRegistry

	if c.RunType.Is(runtype.InternalRelease) {
		prodRegistry = images.KhulnasoftInternalReleaseRegistry
	}
