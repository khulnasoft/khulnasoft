...
EOF
    ...
    if [ ${#modified_packages[@]} -gt 0 ]; then
      cat <<-EOF >>"${REPO_DIR}/annotations/${file}"
NOTE: Any modified package will <strong>not</strong> be present in the image once merged - <a href="https://docs-legacy.khulnasoft.com/dev/how-to/wolfi/add_update_packages#update-an-existing-packaged-dependency">check the docs</a> for more details.
EOF
    fi
  fi
fi
