...
func clearPreciseIndexesOnce(_ context.Context, client *gqltestutil.Client) (requery bool, _ error) {
	...
	if err := client.GraphQL(internal.KhulnasoftAccessToken, precisesIndexesQuery, nil, &payload); err != nil {
		return false, err
	}
	...
	for _, preciseIndex := range payload.Data.PreciseIndexes.Nodes {
		...
		if preciseIndex.State == "DELETING" {
			...
			if err := client.GraphQL(internal.KhulnasoftAccessToken, deletePreciseIndexQuery, map[string]any{"id": preciseIndex.ID}, nil); err != nil {
				return false, err
			}
		}
