...
import (
	...
	"github.com/khulnasoft/khulnasoft/dev/codeintel-qa/internal"
	"github.com/khulnasoft/khulnasoft/lib/errors"
)

// monitor periodically polls Khulnasoft via the GraphQL API for the status of each
		...
		for repoName, data := range state {
			...
			for _, uploadState := range data.uploadStates {
				...
				if uploadState.state == "COMPLETED" {
					...
					if err := internal.GraphQLClient().GraphQL(internal.KhulnasoftAccessToken, preciseIndexesQueryFragment, nil, &payload); err != nil {
...
func queryRepoState(_ context.Context, repoNames []string, uploads []uploadMeta) (map[string]repoState, error) {
	...
	if err := internal.GraphQLClient().GraphQL(internal.KhulnasoftAccessToken, makeRepoStateQuery(repoNames, uploadIDs), nil, &payload); err != nil {
		return nil, err
	}
