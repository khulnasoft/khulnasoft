...
func upload(ctx context.Context, repoName, commit, file, root string) (string, error) {
	...
	cmd := exec.CommandContext(ctx, srcPath, append([]string{"code-intel", "upload", "-json"}, args...)...)
	cmd.Dir = tempDir
	cmd.Env = os.Environ()
	cmd.Env = append(cmd.Env, fmt.Sprintf("SRC_ENDPOINT=%s", internal.KhulnasoftEndpoint))
	cmd.Env = append(cmd.Env, fmt.Sprintf("SRC_ACCESS_TOKEN=%s", internal.KhulnasoftAccessToken))
