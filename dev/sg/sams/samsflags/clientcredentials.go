...
func ClientCredentials() []cli.Flag {
	return []cli.Flag{
		...
		&cli.StringFlag{
			...
			Usage: fmt.Sprintf("Client ID of the Khulnasoft Accounts Management System (SAMS) client - defaults to shared local dev credentials for %q",
				SAMSDevURL),
		},
		&cli.StringFlag{
			Name:    "client-secret",
			EnvVars: []string{"SG_SAMS_CLIENT_SECRET"},
			Usage: fmt.Sprintf("Client Secret of the Khulnasoft Accounts Management System (SAMS) client - defaults to shared local dev credentials for %q",
				SAMSDevURL),
		},
	}
}
