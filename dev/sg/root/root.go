...
var ErrNotInsideKhulnasoft = errors.New("not running inside khulnasoft/khulnasoft")
...
func RepositoryRoot() (string, error) {
	once.Do(func() {
		...
		// cases where we really do not need to be khulnasoft/khulnasoft repo ie. generate help docs.
...
// findRootFromCwd finds root path of the khulnasoft/khulnasoft repository from
...
// findRoot finds the root path of khulnasoft/khulnasoft from wd
func findRoot(wd string) (string, error) {
	for {
		contents, err := os.ReadFile(filepath.Join(wd, "go.mod"))
		if err == nil {
			for _, line := range strings.Split(string(contents), "\n") {
				if line == "module github.com/khulnasoft/khulnasoft" {
					return wd, nil
				}
			}
