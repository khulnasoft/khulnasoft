...
var srcInstanceCommand = &cli.Command{
	...
	Usage:     "Interact with Khulnasoft instances that 'sg src' will use",
	Category:  category.Dev,
	Subcommands: []*cli.Command{
		{
			Name:      "register",
			Usage:     "Register (or edit an existing) Khulnasoft instance to target with src-cli",
			...
			Action: func(cmd *cli.Context) error {
				...
				accessToken, err := std.Out.PromptPasswordf(
					os.Stdin,
					`Please enter the access token for Khulnasoft instance named %s (%s):`,
					name,
					endpoint,
				)
				if err != nil {
