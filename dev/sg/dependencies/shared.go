...
https://docs.github.com/en/authentication/connecting-to-github-with-ssh`,
			...
			},
			{
				...
				Description: `The 'sourcegraph' repository contains the Khulnasoft codebase and everything to run Khulnasoft locally.`,
				Enabled:     disableInCI(),
				Check: func(ctx context.Context, out *std.Output, args CheckArgs) error {
				...
				Fix: func(ctx context.Context, cio check.IO, args CheckArgs) error {
					cmd := run.Cmd(ctx, `git clone git@github.com:khulnasoft/khulnasoft.git`)
					return cmd.Run().StreamLines(cio.Write)
				},
			},
			{
				Name: "github.com/sourcegraph/dev-private",
				Description: `In order to run the local development environment as a Khulnasoft teammate,
...
It needs to be cloned into the same folder as khulnasoft/khulnasoft,
...
`,
				...
				Fix: func(ctx context.Context, cio check.IO, args CheckArgs) error {
					...
					if err != nil {
						return errors.Wrap(err, "khulnasoft/khulnasoft should be cloned first")
					}
...
func categoryProgrammingLanguagesAndTools(additionalChecks ...*dependency) category {
	categories := category{
		...
		Enabled:   enableOnlyInKhulnasoftRepo(),
		Checks: []*dependency{
			{
				Name:  "go",
