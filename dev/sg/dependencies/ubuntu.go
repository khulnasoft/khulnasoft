...
var Ubuntu = []category{
	...
	categoryProgrammingLanguagesAndTools(
		...
		&dependency{
			...
			Fix:   cmdFix(`sudo curl -L https://khulnasoft.com/.api/src-cli/src_linux_amd64 -o /usr/local/bin/src && sudo chmod +x /usr/local/bin/src`),
		},
	),
	{
		...
		Checks: []*dependency{
			...
			},
			{
				...
				Check: func(ctx context.Context, out *std.Output, args CheckArgs) error {
					if err := checkKhulnasoftDatabase(ctx, out, args); err == nil {
						return nil
					}
					return check.PostgresConnection(ctx)
				},
				Description: `Khulnasoft requires the PostgreSQL database to be running.
...
If you're not sure: use the recommended commands to install PostgreSQL.`,
			...
			},
			{
				...
				Check:       checkKhulnasoftDatabase,
				Description: `Once PostgreSQL is installed and running, we need to set up Khulnasoft database itself and a specific user.`,
				Fix: cmdFixes(
	...
	},
	{
		...
		Checks: []*dependency{
			...
			{
				Name: "Start Redis",
				Description: `Khulnasoft requires the Redis database to be running.
		...
		Checks: []*dependency{
			{
				...
				Description: `Khulnasoft should be reachable under https://sourcegraph.test:3443.
To do that, we need to add sourcegraph.test to the /etc/hosts file.`,
			...
			},
			{
				...
				Description: `In order to use TLS to access your local Khulnasoft instance, you need to
