...
import (
	...
	"github.com/khulnasoft/khulnasoft/dev/sg/internal/rfc"
...
var rfcCommand = &cli.Command{
	Name:  "rfc",
	Usage: `List, search, and open Khulnasoft RFCs`,
	Description: fmt.Sprintf("Khulnasoft RFCs live in the following drives - see flags to configure which drive to query:\n\n%s", func() (out string) {
...
`,
	...
	Subcommands: []*cli.Command{
		...
		},
		{
			Name:      "open",
			ArgsUsage: "[number]",
			Usage:     "Open a Khulnasoft RFC - find and list RFC numbers with 'sg rfc list' or 'sg rfc search'",
			Action: func(c *cli.Context) error {
				driveSpec := rfc.PrivateDrive
				if c.Bool("public") {
					driveSpec = rfc.PublicDrive
