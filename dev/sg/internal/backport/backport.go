...
import (
	...
	"github.com/khulnasoft/khulnasoft/internal/execute"
...
func Run(cmd *cli.Context, prNumber int64, version string) error {
	...
	_, err = execute.GH(cmd.Context, "api", fmt.Sprintf("/repos/khulnasoft/khulnasoft/branches/%s", version))
	if err != nil {
		p.Destroy()
		return errors.Wrapf(err, "%q does not exist in khulnasoft/khulnasoft", version)
	}
