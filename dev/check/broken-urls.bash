...
URL_MATCHES=$(git grep -h -e https://khulnasoft.com --and --not -e '^\s*//' --and --not -e 'CI\:URL_OK' -- '*.go' '*.js' '*.jsx' '*.ts' '*.tsx' '*.json' ':(exclude)vendor' ':(exclude)*testdata*' | grep -Eo 'https://khulnasoft.com[^'"'"'`)>" ]+' | sed 's/\.$//' | sort -u)
...
if [ -n "$BAD_URLS" ]; then
    echo
    echo "Error: Found broken khulnasoft.com URLs:"
    echo "$BAD_URLS" | sed 's/ /\n/g' | sed 's/^/  /'
