...
configuration as an Other External Service in Khulnasoft:

  {
    ...
    // "http://host.docker.internal:%s" works from Khulnasoft when using Docker for Desktop.
...
func main() {
	...
	var (
		...
		globalReposDir = globalFlags.String("repos-dir", "", "src-expose's git directories. src-expose creates a git repo per directory synced. The git repo is then served to Khulnasoft. The repositories are stored and served relative to this directory. Default: ~/.sourcegraph/src-expose-repos")
	...
	serve := &ffcli.Command{
		...
		ShortHelp: "Serve git repos for Khulnasoft to list and clone.",
		LongHelp: `src-expose serve will serve the git repositories over HTTP. These can be git
cloned, and they can be discovered by Khulnasoft.
...
See https://github.com/khulnasoft/khulnasoft/tree/main/dev/src-expose/examples`,
