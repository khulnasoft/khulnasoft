...
// parseKhulnasoftCommitFromDeploymentManifestsDiff parses the diff output, returning
// the new and old commits that were used to build this specific image.
func parseKhulnasoftCommitFromDeploymentManifestsDiff(output []byte) *ServiceVersionDiff {
...
func diffDeploymentManifest(path string) (*ServiceVersionDiff, error) {
	...
	imageDiff := parseKhulnasoftCommitFromDeploymentManifestsDiff(output)
