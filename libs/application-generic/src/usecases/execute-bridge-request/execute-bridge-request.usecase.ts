...
export class ExecuteBridgeRequest {
  ...
  private async buildRequestHeaders(command: ExecuteBridgeRequestCommand) {
    const khulnasoftSignatureHeader = await this.buildRequestSignature(command);

    return {
      [HttpRequestHeaderKeysEnum.BYPASS_TUNNEL_REMINDER]: 'true',
      [HttpRequestHeaderKeysEnum.CONTENT_TYPE]: 'application/json',
      [HttpHeaderKeysEnum.KHULNASOFT_SIGNATURE]: khulnasoftSignatureHeader,
    };
  }
  ...
  private async buildRequestSignature(command: ExecuteBridgeRequestCommand) {
    ...
    const khulnasoftSignatureHeader = `t=${timestamp},v1=${this.createHmacBySecretKey(
      secretKey,
      timestamp,
      command.event || {}
    )}`;

    return khulnasoftSignatureHeader;
  }
   ...
   *
   * - Khulnasoft Cloud workflows go to the Khulnasoft API Bridge
  ...
  private getBridgeUrl(
    ...
    switch (workflowOrigin) {
      case WorkflowOriginEnum.KHULNASOFT_CLOUD: {
