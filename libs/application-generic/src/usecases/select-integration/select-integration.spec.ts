...
const khulnasoftIntegration: IntegrationEntity = {
  _environmentId: 'env-test-123',
  _id: 'integration-test-khulnasoft-123',
  _organizationId: 'org-test-123',
  active: true,
  channel: ChannelTypeEnum.EMAIL,
  credentials: {},
  ...
  deleted: false,
  identifier: 'test-khulnasoft-integration-identifier',
  name: 'test-khulnasoft-integration-name',
  primary: true,
  priority: 1,
  deletedAt: null,
  deletedBy: null,
...
jest.mock('@khulnasoft/dal', () => ({
  ...jest.requireActual('@khulnasoft/dal'),
...
jest.mock('../get-decrypted-integrations', () => ({
  ...
  GetDecryptedIntegrations: jest.fn(() => ({
    execute: jest.fn(() => khulnasoftIntegration),
  })),
}));
...
describe('select integration', function () {
  ...
  it('should return the khulnasoft integration', async function () {
    ...
    expect(integration).not.toBeNull();
    expect(integration?.providerId).toEqual(EmailProviderIdEnum.Khulnasoft);
  });
