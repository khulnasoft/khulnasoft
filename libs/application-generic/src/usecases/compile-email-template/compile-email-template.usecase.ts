...
import { IEmailBlock, CommunityOrganizationRepository } from '@khulnasoft/dal';
...
import { GetKhulnasoftLayout } from '../get-khulnasoft-layout';
...
export class CompileEmailTemplate extends CompileTemplateBase {
  constructor(
    ...
    private getKhulnasoftLayoutUsecase: GetKhulnasoftLayout,
  ...
  public async execute(
    ...
    if (command.layoutId) {
      ...
      layoutContent = await this.getKhulnasoftLayoutUsecase.execute({});
    }
    ...
    try {
      ...
      throw new ApiException(e?.message || `Email subject message content could not be generated`);
    }
