...
import { NotificationGroupEntity, NotificationGroupRepository, NotificationTemplateRepository } from '@khulnasoft/dal';
import {
...
export class CreateWorkflow {
  ...
  async execute(usecaseCommand: CreateWorkflowCommand): Promise<WorkflowInternalResponseDto> {
    ...
    try {
      if (
        ...
        storedWorkflow.origin === WorkflowOriginEnum.KHULNASOFT_CLOUD_V1
      ) {
  ...
  private async processBlueprint(command: CreateWorkflowCommand) {
    ...
    return CreateWorkflowCommand.create({
      ...
      origin: command.origin ?? WorkflowOriginEnum.KHULNASOFT_CLOUD,
    });
  }
