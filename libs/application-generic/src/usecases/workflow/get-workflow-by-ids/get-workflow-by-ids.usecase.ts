...
import { NotificationTemplateEntity, NotificationTemplateRepository } from '@khulnasoft/dal';
import { buildWorkflowPreferencesFromPreferenceChannels, DEFAULT_WORKFLOW_PREFERENCES } from '@khulnasoft/shared';
...
export class GetWorkflowByIdsUseCase {
  ...
  @InstrumentUsecase()
  async execute(command: GetWorkflowByIdsCommand): Promise<WorkflowInternalResponseDto> {
  ...
  @Instrument()
  private async getWorkflowPreferences(command: GetWorkflowByIdsCommand, workflowEntity: NotificationTemplateEntity) {
