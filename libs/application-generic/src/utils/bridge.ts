import { DigestTypeEnum } from '@khulnasoft/shared';
import { DigestOutput, DigestRegularOutput, DigestTimedOutput } from '@khulnasoft/framework/internal';
...
export const BRIDGE_EXECUTION_ERROR = {
  ...
  TUNNEL_NOT_FOUND: {
    ...
    message: (url: string) =>
      ...
      `Unable to establish tunnel connection to \`${url}\`. Run npx khulnasoft@latest dev in Local mode, or ensure your Tunnel app deployment is available.`,
  },
  ...
  BRIDGE_ENDPOINT_UNAVAILABLE: {
    ...
    message: (url: string) =>
      // eslint-disable-next-line max-len
      `Unable to reach Bridge Endpoint at \`${url}\`. Run npx khulnasoft@latest dev in Local mode, or ensure your Bridge app deployment is available.`,
  },
  BRIDGE_METHOD_NOT_CONFIGURED: {
    code: 'BridgeMethodNotConfigured',
    message: (url: string) =>
      // eslint-disable-next-line max-len
      `Bridge Endpoint at \`${url}\` is not correctly configured. Ensure your \`@khulnasoft/framework\` integration exposes the \`POST\`, \`GET\`, and \`OPTIONS\` methods.`,
  },
