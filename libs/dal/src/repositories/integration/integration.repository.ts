...
import { ApiServiceLevelEnum, ChannelTypeEnum, KHULNASOFT_PROVIDERS } from '@khulnasoft/shared';
...
export class IntegrationRepository extends BaseRepository<IntegrationDBModel, IntegrationEntity, EnforceEnvOrOrgIds> {
  ...
  async setRemoveKhulnasoftBranding(organizationId: string, value: boolean) {
    return await this.update(
      {
        _organizationId: organizationId,
        channel: ChannelTypeEnum.IN_APP,
      },
      { removeKhulnasoftBranding: value }
    );
  }
  ...
  async countActiveExcludingKhulnasoft({
    _organizationId,
    _environmentId,
    channel,
  }: Pick<IntegrationEntity, '_environmentId' | '_organizationId' | 'channel'>) {
