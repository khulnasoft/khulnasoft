...
const KHULNASOFT_ICON_REGEX = /^Icon(?!Button)[A-Z0-9]{1}[a-zA-Z0-9]+$/;
...
module.exports = {
   ...
   * Currently includes: khulnasoft, antd, mantine, and local imports
   *
   * To see only local imports, replace with: /(\.(\.){0,}\/.*)/gim
   */
  importedFrom:
    /(@khulnasoft\/(khulnasofti|design-system|shared-web|notification-center)|@mantine\/core|@ant-design)(\/[a-z0-9\-)]+){0,}|(\.(\.){0,}\/.*)/gim,
...
function getComponentName({ imported, local, moduleName, importType }) {
  ...
  // get the module namespace / org (AKA khulnasoft or mantine), but remove @
  const moduleOrg = moduleName.split('/').join('_').replace('@', '');

  // group Icons if from Khulnasoft Design System or AntD
  const name =
    (moduleName === '@khulnasoft/design-system' && KHULNASOFT_ICON_REGEX.test(importedName)) || moduleName === ANTD_ICON_MODULE_NAME
      ? 'Icon'
      : importedName;
 ...
 * Processor for grouping by namespace (i.e. Khulnasoft, Mantine, etc)
 */
