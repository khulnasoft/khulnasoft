...
# PNPM is the package manager used in Khulnasoft's monorepo
install_pnpm () {
    PNPM_VERSION="8.9.0"
...
start_database() {
  ...
  if [ $already_installed -ne 1 ]; then
      ...
      echo "To manually start the containerized databases by going to /docker in the khulnasoft project"
  fi
}
...
clone_monorepo () {
    ...
    if [[ -z "$SKIP" ]]; then
        echo ""
        echo "❓ Do you want to clone Khulnasoft's monorepo? ($POSITIVE_RESPONSE / $NEGATIVE_RESPONSE)"
        read -p " > " RESPONSE
	echo ""

    	if [[ "$RESPONSE" == "$POSITIVE_RESPONSE" ]]; then
            REPOSITORY="git@github.com:khulnasoft/khulnasoft.git"
            DESTINATION_FOLDER="$HOME/Dev"
            KHULNASOFT_FOLDER="$DESTINATION_FOLDER/khulnasoft"

            [[ ! -d "$DESTINATION_FOLDER" ]] && mkdir "$DESTINATION_FOLDER"
            if [[ ! -d "$KHULNASOFT_FOLDER" ]]; then
                git clone "$REPOSITORY $KHULNASOFT_FOLDER"
	        success_message "Khulnasoft monorepo"
            else
                already_installed_message "Khulnasoft monorepo"
            fi
        fi
    else
        skip_message "Khulnasoft monorepo"
        echo "$SKIP"
    fi
}

# Khulnasoft set of tools chosen
install_khulnasoft_tools () {
    check_git
    make_zsh_default_shell
    install_ohmyzsh
