...
async function setupRunner() {
  ...
  inquirer.prompt(questions).then(async (answers) => {
    if (answers.action === DEV_ENVIRONMENT_SETUP) {
      ...
      shell.exec('nx run-many --target=build --projects=@khulnasoft/api-service,@khulnasoft/worker');
      ...
      try {
        shell.exec('nx run-many --target=build --projects=@khulnasoft/api-service,@khulnasoft/worker,@khulnasoft/ws');
...
function showWelcomeScreen() {
  ...
  console.log(chalk.bold(`                        Hi, I'm Jarvis by KHULNASOFT!`));
