...
export interface PreviewContent {
    ...
    lineNumber?: number
    // Note: Although Khulnasoft's API returns results as Unicode code point offsets, JetBrains
    ...
    // Khulnasoft API data accordingly.
...
async function createPreviewContentForSymbolMatch(
    ...
    return {
        ...
        lineNumber: getLineFromKhulnasoftUrl(symbolMatch.url),
        absoluteOffsetAndLengths: getAbsoluteOffsetAndLengthsFromKhulnasoftUrl(content, symbolMatch.url),
        relativeUrl: '',
    }
}
...
function getLineFromKhulnasoftUrl(url: string): number {
    const offsets = extractStartAndEndOffsetsFromKhulnasoftUrl(url)
    if (offsets === null) {
        return -1
    }
...
function getAbsoluteOffsetAndLengthsFromKhulnasoftUrl(content: string | null, url: string): number[][] {
    const offsets = extractStartAndEndOffsetsFromKhulnasoftUrl(url)
    if (offsets === null) {
        return []
    }
...
// Parses a Khulnasoft URL and extracts the offsets from it. E.g.:
//
...
//       }
//    }
function extractStartAndEndOffsetsFromKhulnasoftUrl(
    url: string
