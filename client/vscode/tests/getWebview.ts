...
export async function getVSCodeWebviewFrames(page: puppeteer.Page): Promise<VSCodeWebviewFrames> {
    ...
    // Find the Khulnasoft icon in the activity bar
    // to open the Khulnasoft sidebar and active the extension,
    // which should open a search panel in the process.
    await page.waitForSelector('[aria-label="Khulnasoft"]')
    await page.click('[aria-label="Khulnasoft"]')
