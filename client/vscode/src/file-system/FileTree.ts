...
export class FileTree {
    constructor(public readonly uri: KhulnasoftUri, public readonly files: string[]) {
        files.sort()
    }
        ...
        while (index < this.files.length) {
            ...
            if (file.startsWith(directory)) {
                ...
                while (allowRecursion && nestedChildren.length === 1) {
                    const child = KhulnasoftUri.parse(nestedChildren[0])
                ...
                const uri = KhulnasoftUri.fromParts(this.uri.host, this.uri.repositoryName, {
