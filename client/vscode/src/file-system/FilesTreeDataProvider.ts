...
import type { KhulnasoftFileSystemProvider } from './KhulnasoftFileSystemProvider'
import { KhulnasoftUri } from './KhulnasoftUri'

export class FilesTreeDataProvider implements vscode.TreeDataProvider<string> {
    constructor(public readonly fs: KhulnasoftFileSystemProvider) {
    ...
    public activeTextDocument(): KhulnasoftUri | undefined {
    ...
    public async getParent(uriString?: string): Promise<string | undefined> {
        ...
        try {
            ...
            // `KhulnasoftUri.parse(uri).parentUri()` because that would return
            ...
            const uri = KhulnasoftUri.parse(uriString)
            if (!uri.path) {
                return undefined
            }
    ...
    public async getChildren(uriString?: string): Promise<string[] | undefined> {
        try {
            ...
            for (const child of directChildren) {
                this.treeItemCache.set(child, this.newTreeItem(KhulnasoftUri.parse(child), uri, directChildren.length))
            }
    ...
    public async getTreeItem(uriString: string): Promise<vscode.TreeItem> {
        try {
            ...
            return this.newTreeItem(uri, parentUri ? KhulnasoftUri.parse(parentUri) : undefined, 0)
        } catch (error) {
    ...
    private async didFocusString(
        ...
        try {
            if (this.treeView) {
                ...
                if (parent) {
                    await this.didFocusString(KhulnasoftUri.parse(parent), false, token)
                } else {
    ...
    private newTreeItem(
        uri: KhulnasoftUri,
        parent: KhulnasoftUri | undefined,
        ...
        if (
            ...
            uri.path === KhulnasoftUri.parse(vscode.window.activeTextEditor?.document.uri.toString()).path
        ) {
            selectedFile = true
        }
        return {
