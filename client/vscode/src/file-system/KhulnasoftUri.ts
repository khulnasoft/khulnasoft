...
/**
 * KhulnasoftUri encodes a URI like `sourcegraph://HOST/REPOSITORY@REVISION/-/blob/PATH?L1337`.
 *
 * This class is used in both webviews and extensions, so try to avoid state management in this class or module.
 */
export class KhulnasoftUri {
    private constructor(
    ...
    public withRevision(newRevision: string | undefined): KhulnasoftUri {
        const newRevisionPath = newRevision ? `@${newRevision}` : ''
        return KhulnasoftUri.parse(
    ...
    public with(optionals: KhulnasoftUriOptionals): KhulnasoftUri {
        return KhulnasoftUri.fromParts(this.host, this.repositoryName, {
            path: this.path,
    ...
    public withPath(newPath: string): KhulnasoftUri {
        return KhulnasoftUri.parse(`${this.repositoryUri()}/-/blob/${newPath}${this.positionSuffix()}`)
    }
    ...
    public withIsDirectory(isDirectory: boolean): KhulnasoftUri {
        return KhulnasoftUri.fromParts(this.host, this.repositoryName, {
            isDirectory,
            path: this.path,
            revision: this.revision,
    ...
    public static fromParts(host: string, repositoryName: string, optional?: KhulnasoftUriOptionals): KhulnasoftUri {
    ...
    public treeItemLabel(parent?: KhulnasoftUri): string {
        if (this.path) {
            if (parent?.path) {
    ...
    public static parse(uri: string, URLModule = URL): KhulnasoftUri {
        ...
        const lineRange = KhulnasoftURL.from(url.toString()).lineRange
        if (lineRange.line) {
            position = {
                line: lineRange.line,
        ...
        return KhulnasoftUri.fromParts(url.host, repoName, {
            revision,
            path,
            position,
            isDirectory,
