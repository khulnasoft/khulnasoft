...
export class KhulnasoftFileSystemProvider implements vscode.FileSystemProvider {
    ...
    // We don't implement this because Khulnasoft files are read-only.
    ...
    public async readDirectory(vscodeUri: vscode.Uri): Promise<[string, vscode.FileType][]> {
        ...
        return children.map(childUri => {
            const child = KhulnasoftUri.parse(childUri)
    ...
    public async allFilesFromOpenRepositories(folder?: KhulnasoftUri): Promise<RepositoryFileNames[]> {
        ...
        for (const [repositoryUri, downloadingFileNames] of this.fileNamesByRepository.entries()) {
            ...
            try {
                ...
                const uri = KhulnasoftUri.parse(repositoryUri)
                promises.push({
    ...
    public toVscodeLocation(node: LocationNode): vscode.Location {
        ...
        return new vscode.Location(
            vscode.Uri.parse(
                KhulnasoftUri.fromParts(endpointHostnameSetting(), node.resource.repositoryName, {
                    revision,
     ...
     */
    public async defaultFileUri(repositoryName: string): Promise<KhulnasoftUri> {
        ...
        const uri = KhulnasoftUri.fromParts(endpointHostnameSetting(), repositoryName, { revision: defaultBranch })
        const files = await this.downloadFiles(uri)
        ...
        return KhulnasoftUri.fromParts(endpointHostnameSetting(), repositoryName, {
            revision: defaultBranch,
            path: defaultFile,
        })
    }

    public async fetchBlob(uri: KhulnasoftUri): Promise<Blob> {
    ...
    public downloadFiles(uri: KhulnasoftUri): Promise<string[]> {
    ...
    public sourcegraphUri(uri: vscode.Uri): KhulnasoftUri {
        const sourcegraphUri = KhulnasoftUri.parse(uri.toString(true))
        if (sourcegraphUri.host !== new URL(this.instanceURL).host) {
            const message = 'Khulnasoft instance URL has changed. Close files opened through the previous instance.'
    ...
    public async getFileTree(uri: KhulnasoftUri): Promise<FileTree> {
        const files = await this.downloadFiles(uri)
