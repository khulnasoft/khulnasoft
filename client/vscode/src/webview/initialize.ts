...
interface KhulnasoftWebviewConfig {
...
export async function initializeSearchPanelWebview({
    ...
    const panel = vscode.window.createWebviewPanel('sourcegraphSearch', 'Khulnasoft', vscode.ViewColumn.One, {
        enableScripts: true,
...
// TODO expand CSP for Khulnasoft extension loading
export function initializeSearchSidebarWebview({
    ...
    // Get a proxy for the Khulnasoft Webview API to communicate with the Webview.
    const searchSidebarAPI = Comlink.wrap<SearchSidebarAPI>(proxy)

    // Expose the Khulnasoft VS Code Extension API to the Webview.
    Comlink.expose(extensionCoreAPI, expose)
