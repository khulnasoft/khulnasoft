...
import type { initializeKhulnasoftSettings } from '../backend/sourcegraphSettings'
...
export function registerWebviews({
    ...
    sourcegraphSettings: ReturnType<typeof initializeKhulnasoftSettings>
    ...
    // Open Khulnasoft search tab on `sourcegraph.search` command.
    context.subscriptions.push(
...
export async function openKhulnasoftLinks(uri: string): Promise<void> {
    await vscode.env.openExternal(vscode.Uri.parse(uri))
    return
}

export async function copyKhulnasoftLinks(uri: string): Promise<void> {
    try {
