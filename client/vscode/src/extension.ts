...
import { initializeKhulnasoftSettings } from './backend/sourcegraphSettings'
...
import { openKhulnasoftUriCommand } from './file-system/commands'
import { initializeKhulnasoftFileSystem } from './file-system/initialize'
import { KhulnasoftUri } from './file-system/KhulnasoftUri'
...
import { copyKhulnasoftLinks, focusSearchPanel, openKhulnasoftLinks, registerWebviews } from './webview/commands'
import { KhulnasoftAuthActions } from './webview/platform/AuthProvider'
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    const sourcegraphSettings = initializeKhulnasoftSettings({ context })
    ...
    const { fs } = initializeKhulnasoftFileSystem({ context, initialInstanceURL })
    ...
    const authActions = new KhulnasoftAuthActions(secretStorage)
    const extensionCoreAPI: ExtensionCoreAPI = {
        ...
        observeKhulnasoftSettings: () => proxySubscribable(sourcegraphSettings.settings),
        ...
        openKhulnasoftFile: (uri: string) => openKhulnasoftUriCommand(fs, KhulnasoftUri.parse(uri)),
        openLink: uri => openKhulnasoftLinks(uri),
        copyLink: uri => copyKhulnasoftLinks(uri),
