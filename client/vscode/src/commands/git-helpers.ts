...
export async function repoInfo(filePath: string): Promise<RepositoryInfo | undefined> {
    try {
        ...
        // as \\ does not work in Khulnasoftl links
        ...
        branch = (await isOnKhulnasoft(remoteURL, getDefaultBranch() || branch)) ? getDefaultBranch() || branch : ''
 ...
 */
export function getKhulnasoftFileUrl(
    ...
    const uri = new URL('/-/editor', KhulnasoftUrl)
...
/**
 * Check if branch exists on Khulnasoft instance
 * Return 'HEAD' if it does not exists remotely
 */
export async function isOnKhulnasoft(remoteURL: string, currentBranch: string): Promise<boolean> {
    ...
    const isOnKhulnasoft = await requestGraphQLFromVSCode<CheckBranchResult>(checkBranchQuery, {
        repoName,
    ...
    console.log(isOnKhulnasoft)
    return isOnKhulnasoft || false
}
