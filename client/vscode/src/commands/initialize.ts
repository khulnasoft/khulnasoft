...
/**
 * Generates a link to a blob on a Khulnasoft instance.
 *
 ...
 */
export function generateKhulnasoftBlobLink(
    uri: vscode.Uri,
    startLine: number,
    startChar: number,
    endLine: number,
    ...
    // Using KhulnasoftUri.parse to properly decode repo revision
    const decodedUri = KhulnasoftUri.parse(uri.toString())
    const finalUri = new URL(decodedUri.uri)
    // Khulnasoft expects 1-based line and character values
