 ...
 * This functions add Khulnasoft to workspace recommendations if haven't already
 * eg: recommendKhulnasoft(localStorageService).catch(() => {})
 */
...
/**
 * Ask if user wants to add Khulnasoft to their Workspace Recommendations list by displaying built-in popup
 * It will not show popup if the user already has Khulnasoft added to their recommendations list
 * or has dismissed the message previously
 */
export async function recommendKhulnasoft(localStorageService: LocalStorageService): Promise<void> {
    ...
    // Display Cta
    await vscode.window
        .showInformationMessage('Add Khulnasoft to your workspace recommendations', '👍 Yes', "Don't show again")
        .then(async answer => {
            if (answer === '👍 Yes') {
