...
const ShareOptionComponent: React.FunctionComponent<
    React.PropsWithChildren<Omit<ShareOption, 'namespaceId'> & { isKhulnasoftDotCom: boolean }>
> = ({ isKhulnasoftDotCom, namespaceType, namespaceName, isPublic }) => {
    if (namespaceType === 'User') {
        if (isPublic) {
            const publicText = isKhulnasoftDotCom ? 'Public' : 'Instance'
            return (
                <>
                    <Icon
                        className="mr-2"
                        svgPath={isKhulnasoftDotCom ? mdiWeb : mdiDomain}
                        inline={false}
                        height="1.15rem"
                        width="1.15rem"
                        aria-hidden={true}
    ...
    return (
        <Menu>
                ...
                {isOpen => (
                    <>
                        <span className="d-flex align-items-center">
                            <ShareOptionComponent {...selectedShareOption} isKhulnasoftDotCom={isKhulnasoftDotCom} />
                        </span>
                        <span className="ml-5">
                            {isOpen ? (
            ...
            <MenuList
                ...
                // see https://github.com/khulnasoft/khulnasoft/pull/41654#issuecomment-1246672813
            ...
            >
                {shareOptions.map(option => (
                    ...
                    >
                        <ShareOptionComponent {...option} isKhulnasoftDotCom={isKhulnasoftDotCom} />
                    </MenuItem>
                ))}
            </MenuList>
        </Menu>
