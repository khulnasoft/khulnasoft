...
describe('SurveyToast', () => {
        ...
        describe('on day 3', () => {
            ...
            it('correctly proceed to use case form', () => {
                const recommendRadioGroup = renderResult.getByLabelText(
                    'How likely is it that you would recommend Khulnasoft to a friend?'
                )
                ...
                expect(renderResult.getByLabelText('What do you use Khulnasoft for?')).toBeVisible()
            })
        })
    ...
    describe('user has submitted rating score', () => {
        ...
        it('Should render use case form correctly', () => {
            expect(renderResult.getByLabelText('What do you use Khulnasoft for?')).toBeVisible()
            expect(renderResult.getByLabelText('How can we make Khulnasoft better?')).toBeVisible()
        })

        it('Should show some gratitude after usecase submission', async () => {
            const reasonInput = renderResult.getByLabelText('How can we make Khulnasoft better?')
            expect(reasonInput).toBeVisible()
            fireEvent.change(reasonInput, { target: { value: mockVariables.better } })

            const otherUseCaseInput = renderResult.getByLabelText('What do you use Khulnasoft for?')
