...
export const UserAreaGQLFragment = gql`
    fragment UserAreaUserFields on User {
        ...
        builtinAuth
        createdAt
        emails @skip(if: $isKhulnasoftDotCom) {
            email
            isPrimary
        }
        roles @skip(if: $isKhulnasoftDotCom) {
            nodes {
                name
                system
            }
...
export const USER_AREA_USER_PROFILE = gql`
    query UserAreaUserProfile($username: String!, $isKhulnasoftDotCom: Boolean!) {
 ...
 */
export const UserArea: FC<UserAreaProps> = ({ useBreadcrumb, userAreaRoutes, isKhulnasoftDotCom, ...props }) => {
    ...
    const { data, error, loading, previousData } = useQuery<UserAreaUserProfileResult, UserAreaUserProfileVariables>(
        ...
        {
            variables: { username: username!, isKhulnasoftDotCom },
        }
    )
