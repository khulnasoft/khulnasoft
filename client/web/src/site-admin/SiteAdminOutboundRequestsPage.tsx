    ...
    return (
        <div className="site-admin-outbound-requests-page">
            ...
            <PageHeader
                ...
                description={
                    <>
                        This is the log of recent external requests sent by the Khulnasoft instance. Handy for seeing
                        what's happening between Khulnasoft and other services.{' '}
...
function formatStackFrameLine(line: string): React.ReactNode {
    ...
    return (
        <li key={`${fileName}:${lineIndex}`}>
            <Code>
                <Link to={buildKhulnasoftUrl(fileName, parseInt(lineIndex, 10))} target="_blank" rel="noopener">
                    {fileName}:{lineIndex}
                </Link>{' '}
