...
describe('SignInPage', () => {
    const authProviders: KhulnasoftContext['authProviders'] = [
        {
    ...
    const render = (
        route: string,
        props: {
            authProviders?: KhulnasoftContext['authProviders']
            authenticatedUser?: AuthenticatedUser
            sourcegraphDotComMode?: KhulnasoftContext['sourcegraphDotComMode']
            allowSignup?: KhulnasoftContext['allowSignup']
            primaryLoginProvidersCount?: KhulnasoftContext['primaryLoginProvidersCount']
            authAccessRequest?: KhulnasoftContext['authAccessRequest']
        }
    ) =>
    ...
    describe('with Khulnasoft operator auth provider', () => {
        const withKhulnasoftOperator: KhulnasoftContext['authProviders'] = [
            ...authProviders,
            {
        ...
        it('renders page with 2 providers', () => {
            const rendered = render('/sign-in', { authProviders: withKhulnasoftOperator })
            expect(
                within(rendered.baseElement).queryByText(txt => txt.includes('Khulnasoft Operators'))
            ).not.toBeInTheDocument()
            expect(rendered.asFragment()).toMatchSnapshot()
        })

        it('renders page with 3 providers (url-param present)', () => {
            const rendered = render('/sign-in?sourcegraph-operator', { authProviders: withKhulnasoftOperator })
            expect(
                within(rendered.baseElement).queryByText(txt => txt.includes('Khulnasoft Operators'))
            ).toBeInTheDocument()
            expect(rendered.asFragment()).toMatchSnapshot()
    ...
    describe('with Gerrit auth provider', () => {
        const withGerritProvider: KhulnasoftContext['authProviders'] = [
            ...authProviders,
            {
                displayName: 'Gerrit',
                isBuiltin: false,
    ...
    it('does not render redirect when there is only 1 auth provider with request access enabled', () => {
        const withGitHubProvider: KhulnasoftContext['authProviders'] = [
            {
                serviceType: 'github',
                displayName: 'GitHub',
                isBuiltin: false,
