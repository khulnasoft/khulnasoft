...
interface Props extends TelemetryProps, TelemetryV2Props {
    ...
    context: Pick<KhulnasoftContext, 'externalURL' | 'authPasswordPolicy' | 'authMinPasswordLength'>
...
/**
 * Sign up page specifically for Khulnasoft.com
 */
export const CloudSignUpPage: React.FunctionComponent<React.PropsWithChildren<Props>> = ({
    ...
    const { data } = useQuery<UserAreaUserProfileResult, UserAreaUserProfileVariables>(USER_AREA_USER_PROFILE, {
        variables: { username: invitedBy || '', isKhulnasoftDotCom },
        skip: !invitedBy,
    })
                        ...
                        {invitedByUser ? (
                            <>
                                <UserAvatar
                                    ...
                                    user={invitedByUser}
                                />
                                <strong className="mr-1">{invitedBy}</strong> has invited you to join Khulnasoft
                            </>
                        ) : (
                            title
                        )}
