...
import type { AuthProvider, KhulnasoftContext } from '../jscontext'
...
export const SignInPage: React.FunctionComponent<React.PropsWithChildren<SignInPageProps>> = props => {
    ...
    const shouldShowProvider = function (provider: AuthProvider): boolean {
        ...
        // Hide the Khulnasoft Operator authentication provider by default because it is
...
const SignUpNotice: React.FunctionComponent<SignUpNoticeProps> = ({
    ...
    const dotcomCTAs = (
        <>
            ...
            >
                consider Khulnasoft Enterprise
            </Link>
            .
        </>
    )
    ...
    if (allowSignup) {
        return (
            <Text className="mt-3 text-center">
                New to Khulnasoft? <Link to="/sign-up">Sign up</Link>{' '}
                {sourcegraphDotComMode && <>To use Khulnasoft on private repositories, {dotcomCTAs}</>}
            </Text>
        )
    }
