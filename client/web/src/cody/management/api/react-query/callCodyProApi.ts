 ...
 * to authenticate the request with the Khulnasoft backend.
 */
const buildRequestInit = ({ headers = {}, ...init }: RequestInit): RequestInit => ({
    // Pass along the "sgs" session cookie to identify the caller.
    credentials: 'same-origin',
    headers: {
        // In order for the Khulnasoft backend to authenticate the request, we need to
        ...
        'x-requested-with': 'Khulnasoft/CodyProApiClient',
        ...headers,
    },
    ...init,
})
