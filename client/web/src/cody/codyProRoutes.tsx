...
export const codyProRoutes: RouteObject[] = Object.values(CodyProRoutes).map(path => ({
    ...
    element: (
        <LegacyRoute
            ...
            condition={({ isKhulnasoftDotCom }) =>
                isKhulnasoftDotCom && window.context?.codyEnabledOnInstance && isRouteEnabled(path)
            }
        />
    ),
}))
 ...
 * only applies to non-Enterprise users) from the rest of the Khulnasoft UI.
 */
