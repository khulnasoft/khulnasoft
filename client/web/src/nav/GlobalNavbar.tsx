...
export const GlobalNavbar: React.FunctionComponent<React.PropsWithChildren<GlobalNavbarProps>> = ({
    ...
    const showSearchContext = searchContextsEnabled && !isKhulnasoftDotCom && !disableCodeSearchFeatures
    const showCodeMonitoring = codeMonitoringEnabled && !isKhulnasoftDotCom && !disableCodeSearchFeatures
    const showSearchNotebook = notebooksEnabled && !isKhulnasoftDotCom && !disableCodeSearchFeatures
    const showSearchJobs = searchJobsEnabled && !disableCodeSearchFeatures
    const showBatchChanges =
        props.batchChangesEnabled && isLicensed && !isKhulnasoftDotCom && !disableCodeSearchFeatures
    ...
    const codeInsights = (codeInsightsEnabled && !isKhulnasoftDotCom && !disableCodeSearchFeatures) ?? false
                        ...
                        <>
                            <NavAction>
                                <div>
                                    ...
                                    {!isKhulnasoftDotCom && window.context?.allowSignup && (
...
export const InlineNavigationPanel: FC<InlineNavigationPanelProps> = props => {
    ...
    const codyItem = window.context?.codyEnabledOnInstance ? (
        <NavItem icon={() => <CodyLogoWrapper />} key="cody">
            <NavLink variant={navLinkVariant} to={linkForCodyNavItem(isKhulnasoftDotCom)}>
                Cody
            </NavLink>
        </NavItem>
    ) : null
...
export function linkForCodyNavItem(isKhulnasoftDotCom: boolean): string {
