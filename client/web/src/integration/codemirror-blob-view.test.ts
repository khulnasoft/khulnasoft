...
function createBlobPageData<T extends BlobInfo>({
    ...
    const repositoryKhulnasoftUrl = `/${repoName}`
    ...
    return {
        ...
        graphqlResults: {
            ResolveRepoRev: () => createResolveRepoRevisionResult(repositoryKhulnasoftUrl),
            FileExternalLinks: ({ filePath }) =>
                createFileExternalLinksResult(`https://${repoName}/blob/master/${filePath}`),
            TreeEntries: () => createTreeEntriesResult(repositoryKhulnasoftUrl, fileNames),
            FileTreeEntries: () => createFileTreeEntriesResult(repositoryKhulnasoftUrl, fileNames),
