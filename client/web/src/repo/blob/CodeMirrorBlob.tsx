...
const staticExtensions: Extension = [
    ...
    EditorView.contentAttributes.of({
        ...
        // See https://github.com/khulnasoft/khulnasoft/issues/43375
        role: 'generic',
    }),
...
export const CodeMirrorBlob: React.FunctionComponent<BlobProps> = props => {
    ...
    const position = useMemo(
        ...
        () => KhulnasoftURL.from(props.activeURL || location).lineRange,
    ...
    const onSelection = useCallback(
        (range: SelectedLineRange) => {
            const url = KhulnasoftURL.from(locationRef.current)
    ...
    useEffect(() => {
        ...
        if (editor) {
            ...
            if (navigateToLineOnAnyClick) {
                 ...
                 * We don't need code intel and keyboard navigation in the references panel blob: https://github.com/khulnasoft/khulnasoft/pull/41615.
                 */
                return
            }
                      ...
                      pin: {
                          onPin(position) {
                              updateBrowserHistoryIfChanged(
                                  navigate,
                                  locationRef.current,
                                  KhulnasoftURL.from(locationRef.current)
                          ...
                          onUnpin() {
                              updateBrowserHistoryIfChanged(
                                  navigate,
                                  locationRef.current,
                                  KhulnasoftURL.from(locationRef.current).deleteSearchParameter('popover')
                              )
                          },
                      },
                      navigate,
