...
<HeroPage title="Repository not found" icon={ILucideBookX}>
    {#if viewerCanAdminister}
        <p>
            As a site admin, you can add <code>{repoName}</code> to Khulnasoft to allow users to search and view it by
            <a href="/site-admin/external-services">connecting an external service</a> referencing it.
        </p>
    {:else}
        <p>To access this repository, contact the Khulnasoft admin.</p>
    {/if}
</HeroPage>
