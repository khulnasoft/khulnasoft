...
<script lang="ts">
    ...
    import { isErrorLike, pluralize, KhulnasoftURL, type LineOrPositionOrRange } from '$lib/common'
    ...
    function viewModeURL(viewMode: CodeViewMode) {
        switch (viewMode) {
            case CodeViewMode.Code: {
                const url = KhulnasoftURL.from($page.url)
                if (isRichFile) {
            ...
            case CodeViewMode.Default:
                return KhulnasoftURL.from($page.url).deleteSearchParameter('view').toString()
        }
    }
...
{#if revisionOverride}
    <div class="revision-info">
        ...
        <a href={KhulnasoftURL.from($page.url).deleteSearchParameter('rev').toString()}>
        ...
        {#key blob.canonicalURL}
            <CodeMirrorBlob
                ...
                on:selectline={({ detail: range }) => {
                    goto(
                        KhulnasoftURL.from(embedded ? `${repoURL}/-/blob/${filePath}` : $page.url.searchParams)
