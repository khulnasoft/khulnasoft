...
import { KEY, type KhulnasoftContext } from '$lib/stores'
...
type KhulnasoftContextKey = keyof KhulnasoftContext
type MockedKhulnasoftContextValue<T> = T extends Readable<infer U> ? Writable<U> : T

// Sets up stubs for mocking the Khulnasoft context. The sourcegraph context makes
...
const mockedSourcgraphContext: {
    [key in KhulnasoftContextKey]: MockedKhulnasoftContextValue<KhulnasoftContext[key]> | typeof unmocked
} = {
    user: writable(null),
    settings: writable({}),
...
// Creates a proxy object for the mocked Khulnasoft context object.
    ...
    Object.defineProperties(
        ...
        Object.fromEntries(
            Object.keys(mockedSourcgraphContext).map(key => [
                ...
                {
                    get: () => {
                        if (mockedSourcgraphContext[key as KhulnasoftContextKey] === unmocked) {
                            signale.warn(`Khulnasoft context ${key} is unmocked`)
                        }
                        return mockedSourcgraphContext[key as KhulnasoftContextKey]
                    },
                },
            ])
        )
