...
/**
 * This plugin proxies certain requests to a real Khulnasoft instance. These include
 ...
 *
 * It does this by first fetching the sign-in page from the real Khulnasoft instance
 ...
 * Those that are not will be proxied to the real Khulnasoft instance.
 *
...
export function sgProxy(options: Options): Plugin {
    ...
    // Additional endpoints that should be proxied to the real Khulnasoft instance.
    const additionalEndpoints = [
        ...
        // proxy them to the real Khulnasoft instance, for consistency with the production setup.
        '/-/raw/',
        '/-/batch-changes/?',
        '/-/settings/?',
        '/-/code-graph/?',
    ...
    // Routes known by the server that need to (potentially) be proxied to the real Khulnasoft instance.
    ...
    return {
        ...
        async config() {
            ...
            // At startup we fetch the sign-in page from the real Khulnasoft instance to extract the `knownRoutes` array
            // from the JS context object. This is used to determine which requests should be proxied to the real Khulnasoft
            // instance.
            // We keep trying to connect to the origin server in case it is not yet available (e.g. when just starting up a
            // local Khulnasoft instance).
            let backoff = 1
            while (true) {
                try {
            ...
            const proxyConfig: Record<string, ProxyOptions> = {
                // Proxy requests to specific endpoints to a real Khulnasoft instance.
