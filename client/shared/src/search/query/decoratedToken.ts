...
/**
 * A token that is labeled and interpreted as a field, like "repo:" in the Khulnasoft language syntax.
 */
...
export type MetaRevisionKind = MetaGitRevision | MetaKhulnasoftRevision

/**
 * A custom revision syntax that is only valid in Khulnasoft search queries.
 */
export enum MetaKhulnasoftRevision {
...
/**
 * A token that denotes a revision separator in the Khulnasoft query language. I.e., the '@' in
        ...
        switch (current) {
            case '*': {
                ...
                if (atRevision(start - 1) && token.value[start] !== '!') {
                    ...
                    appendDecoratedToken(start, MetaKhulnasoftRevision.IncludeGlobMarker)
                    ...
                    accumulator.push('*!')
                    appendDecoratedToken(start, MetaKhulnasoftRevision.ExcludeGlobMarker)
                } else {
                    accumulator.push(current)
            ...
            case ':': {
                ...
                accumulator.push(current)
                appendDecoratedToken(start, MetaKhulnasoftRevision.Separator)
                break
            }
            default: {
                accumulator.push(current)
