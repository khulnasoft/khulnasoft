    ...
    error: (type, eventSource, observer) =>
        fromEvent(eventSource, type).subscribe(event => {
            ...
            if (isErrorLike(error)) {
                ...
                observer.error(
                    new Error(
                        'The connection was closed before your search was completed. This may be due to a problem with a firewall, VPN or proxy, or a failure with the Khulnasoft server.'
                    )
                )
            }
            eventSource.close()
 ...
 * This is a type safe wrapper around Khulnasoft's streaming search API (using Server Sent Events). The observable will emit each event returned from the backend.
