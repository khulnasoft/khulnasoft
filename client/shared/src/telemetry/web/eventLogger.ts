...
/**
 * Indicates if the webapp ever receives a message from the user's Khulnasoft browser extension,
    ...
    fromEvent<CustomEvent<{ platform?: string; version?: string }>>(
        ...
        map(({ detail }) => {
            try {
                ...
                // Temporary to fix issues on Firefox (https://github.com/khulnasoft/khulnasoft/issues/25998)
                if (
                    isFirefox() &&
                    isErrorLike(error) &&
