...
/**
 * Registers the builtin client commands that are required for Khulnasoft extensions. See
...
export function registerBuiltinClientCommands(
    ...
    /**
     * Sends a GraphQL request to the Khulnasoft GraphQL API and returns the result. The request is performed
    ...
    subscription.add(
        registerCommand({
            ...
            run: (query: string, variables: { [name: string]: any }): Promise<any> =>
                ...
                // from being sent to Khulnasoft.com.
                lastValueFrom(
    ...
    /**
     * Sends a telemetry event to the Khulnasoft instance with the correct anonymous user id.
     */
    subscription.add(
        registerCommand({
...
export function urlForOpenPanel(viewID: string, urlHash: string): string {
    return KhulnasoftURL.from({ hash: urlHash }).setViewState(viewID).toString()
}
