...
import { encodeURIPathComponent, escapeRevspecForURL, KhulnasoftURL } from '@sourcegraph/common'
...
export interface RepoSpec {
    /**
     * The name of this repository on a Khulnasoft instance,
 ...
 * phased out in favor of URLs to resources in the Khulnasoft raw API, which do not require out-of-band
 * information to fetch the contents of.
 * @deprecated Migrate to using URLs to the Khulnasoft raw API (or other concrete URLs) instead.
 */
...
/**
 * Returns an absolute URL to the blob (file) on the Khulnasoft instance.
 */
...
export function parseBrowserRepoURL(href: string): ParsedRepoURI & Pick<ParsedRepoRevision, 'rawRevision'> {
    const url = KhulnasoftURL.from(href)
