...
export const mkSquirrel = (api: API): PromiseProviders => ({
    async definition(document, position) {
        ...
        if (local?.def) {
            return mkKhulnasoftLocation({ ...parseGitURI(document.uri), range: local.def })
        }
        ...
        return mkKhulnasoftLocation({ ...parseGitURI(document.uri), ...location })
    },
    async references(document, position) {
        ...
        return symbol.refs.map(reference => mkKhulnasoftLocation({ ...parseGitURI(document.uri), range: reference }))
    },
    ...
    async documentHighlights(document, position) {
        ...
        return symbol.refs.map(reference => ({
            range: rangeToKhulnasoftRange(reference),
...
const mkKhulnasoftLocation = ({ repo, commit, path, range }: RepoCommitPathRange): sourcegraph.Location => ({
    uri: new URL(`git://${repo}?${commit}#${path}`),
    range: range ? rangeToKhulnasoftRange({ row: range.row, column: range.column, length: range.length }) : undefined,
})
...
// they don't.
const rangeToKhulnasoftRange = ({ row, column, length }: Range): sourcegraph.Range =>
    ({
