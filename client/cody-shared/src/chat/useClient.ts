...
export const useClient = ({
    ...
    const { graphqlClient, chatClient, intentDetector } = useMemo(() => {
        const completionsClient = new KhulnasoftBrowserCompletionsClient(config)
        const chatClient = new ChatClient(completionsClient)
        const graphqlClient = new KhulnasoftGraphQLAPIClient(config)
    ...
    const codebaseIds: Promise<string[]> = useMemo(async () => {
        ...
        if (isError(results)) {
            // eslint-disable-next-line no-console
            console.error(
                `Cody could not access the repositories on your Khulnasoft instance. Details: ${results.message}`
            )
            return []
        }
        ...
        async (count: number): Promise<string[]> =>
            graphqlClient
                ...
                .catch(error => {
                    // eslint-disable-next-line no-console
                    console.error(
                        `Cody could not fetch the list of repositories on your Khulnasoft instance. Details: ${error}`
                    )
        ...
        async (
            ...
            if (activeEditor?.repoName && !repoNames.includes(activeEditor.repoName)) {
                ...
                if (isError(additionalRepoId)) {
                    // eslint-disable-next-line no-console
                    console.error(
                        `Cody could not access the ${activeEditor.repoName} repository on your Khulnasoft instance. Details: ${additionalRepoId.message}`
                    )
                } else {
