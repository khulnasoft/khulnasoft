...
import { KhulnasoftEmbeddingsSearchClient } from '../embeddings/client'
import { KhulnasoftIntentDetectorClient } from '../intent-detector/client'
import { KhulnasoftBrowserCompletionsClient } from '../sourcegraph-api/completions/browserClient'
import type { CompletionsClientConfig, KhulnasoftCompletionsClient } from '../sourcegraph-api/completions/client'
import { KhulnasoftGraphQLAPIClient } from '../sourcegraph-api/graphql'
...
export interface ClientInit {
    ...
    createCompletionsClient?: (config: CompletionsClientConfig) => KhulnasoftCompletionsClient
}
...
export interface Client {
    ...
    graphqlClient: KhulnasoftGraphQLAPIClient
}
...
export async function createClient({
    ...
    editor,
    initialTranscript,
    createCompletionsClient = config => new KhulnasoftBrowserCompletionsClient(config),
}: ClientInit): Promise<Client | null> {
    const fullConfig = { debugEnable: false, ...config }

    const graphqlClient = new KhulnasoftGraphQLAPIClient(fullConfig)
    ...
    if (sourcegraphStatus.authenticated && codyStatus.enabled) {
        ...
        if (isError(repoId)) {
            throw new Error(
                `Cody could not access the '${config.codebase}' repository on your Khulnasoft instance. Details: ${repoId.message}`
            )
        }

        const embeddingsSearch = repoId ? new KhulnasoftEmbeddingsSearchClient(graphqlClient, repoId, true) : null
        const codebaseContext = new CodebaseContext(config, config.codebase, embeddingsSearch, null, null, null)

        const intentDetector = new KhulnasoftIntentDetectorClient(graphqlClient, completionsClient)
