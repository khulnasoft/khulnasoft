...
import { KhulnasoftCompletionsClient } from './client'
import { parseEvents } from './parse'
import type { CompletionCallbacks, CompletionParameters } from './types'

export class KhulnasoftNodeCompletionsClient extends KhulnasoftCompletionsClient {
    public stream(params: CompletionParameters, cb: CompletionCallbacks): () => void {
        ...
        const request = requestFn(
            this.completionsEndpoint,
            {
                ...
                // So we can send requests to the Khulnasoft local development instance, which has an incompatible cert.
        ...
        request.on('error', e => {
            ...
            if (message.includes('ECONNREFUSED')) {
                message =
                    'Could not connect to Cody. Please ensure that Cody app is running or that you are connected to the Khulnasoft server.'
            }
            log?.onError(message)
            cb.onError(message)
        })
