...
export class KhulnasoftGraphQLAPIClient {
    ...
    public async getSiteVersion(): Promise<string | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<SiteVersionResponse>>(CURRENT_SITE_VERSION_QUERY, {}).then(
            response =>
                extractDataOrError(
                    response,
                    data =>
    ...
    public async getSiteIdentification(): Promise<{ siteid: string; hashedLicenseKey: string } | Error> {
        const response = await this.fetchKhulnasoftAPI<APIResponse<SiteIdentificationResponse>>(
            CURRENT_SITE_IDENTIFICATION,
            {}
        )
    ...
    public async getSiteHasIsCodyEnabledField(): Promise<boolean | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<SiteGraphqlFieldsResponse>>(
            CURRENT_SITE_GRAPHQL_FIELDS_QUERY,
            {}
        ).then(response =>
    ...
    public async getSiteHasCodyEnabled(): Promise<boolean | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<SiteHasCodyEnabledResponse>>(
            CURRENT_SITE_HAS_CODY_ENABLED_QUERY,
            {}
    ...
    public async getCurrentUserId(): Promise<string | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<CurrentUserIdResponse>>(CURRENT_USER_ID_QUERY, {}).then(response =>
    ...
    public async getCurrentUserIdAndVerifiedEmail(): Promise<{ id: string; hasVerifiedEmail: boolean } | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<CurrentUserIdHasVerifiedEmailResponse>>(
            CURRENT_USER_ID_AND_VERIFIED_EMAIL_QUERY,
            {}
        ).then(response =>
            extractDataOrError(response, data =>
    ...
    public async getCodyLLMConfiguration(): Promise<undefined | CodyLLMSiteConfiguration | Error> {
        ...
        const [configResponse, providerResponse] = await Promise.all([
            this.fetchKhulnasoftAPI<APIResponse<CodyLLMSiteConfigurationResponse>>(CURRENT_SITE_CODY_LLM_CONFIGURATION),
            this.fetchKhulnasoftAPI<APIResponse<CodyLLMSiteConfigurationProviderResponse>>(
                CURRENT_SITE_CODY_LLM_PROVIDER
            ),
        ])
    ...
    public async getRepoIds(names: string[]): Promise<{ id: string; name: string }[] | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<RepositoryIdsResponse>>(REPOSITORY_IDS_QUERY, {
            names,
            first: names.length,
    ...
    public async getRepoId(repoName: string): Promise<string | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<RepositoryIdResponse>>(REPOSITORY_ID_QUERY, {
            name: repoName,
        }).then(response =>
            extractDataOrError(response, data =>
    ...
    public async getRepoNames(first: number): Promise<string[] | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<RepositoryNamesResponse>>(REPOSITORY_NAMES_QUERY, { first }).then(
            response =>
                extractDataOrError(
                    response,
    ...
    public async getRepoIdIfEmbeddingExists(repoName: string): Promise<string | null | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<RepositoryEmbeddingExistsResponse>>(
            REPOSITORY_EMBEDDING_EXISTS_QUERY,
            {
                name: repoName,
            }
    ...
    /**
     * Checks if Cody is enabled on the current Khulnasoft instance.
     * @returns
     * enabled: Whether Cody is enabled.
     * version: The Khulnasoft version.
     *
     * This method first checks the Khulnasoft version using `getSiteVersion()`.
    ...
    private async sendEventLogRequestToDotComAPI(event: event): Promise<LogEventResponse | Error> {
        const response = await this.fetchKhulnasoftDotcomAPI<APIResponse<LogEventResponse>>(LOG_EVENT_MUTATION, event)
    ...
    private async sendEventLogRequestToAPI(event: event): Promise<LogEventResponse | Error> {
        const initialResponse = await this.fetchKhulnasoftAPI<APIResponse<LogEventResponse>>(LOG_EVENT_MUTATION, event)
        const initialDataOrError = extractDataOrError(initialResponse, data => data)

        if (isError(initialDataOrError)) {
            const secondResponse = await this.fetchKhulnasoftAPI<APIResponse<LogEventResponse>>(
                LOG_EVENT_MUTATION_DEPRECATED,
                event
            )
    ...
    private async sendEventLogRequestToTestingAPI(event: event): Promise<LogEventResponse | Error> {
        const initialResponse = await this.fetchKhulnasoftTestingAPI<APIResponse<LogEventResponse>>(event)
        const initialDataOrError = extractDataOrError(initialResponse, data => data)

        if (isError(initialDataOrError)) {
            const secondResponse = await this.fetchKhulnasoftTestingAPI<APIResponse<LogEventResponse>>(event)
            return extractDataOrError(secondResponse, data => data)
        }
    ...
    public async getCodyContext(
        ...
        return this.fetchKhulnasoftAPI<APIResponse<GetCodyContextResponse>>(GET_CODY_CONTEXT_QUERY, {
            repos,
            query,
            codeResultsCount,
            textResultsCount,
    ...
    public async searchEmbeddings(
        ...
        return this.fetchKhulnasoftAPI<APIResponse<EmbeddingsMultiSearchResponse>>(SEARCH_EMBEDDINGS_QUERY, {
            repos,
            query,
            codeResultsCount,
            textResultsCount,
    ...
    public async legacySearchEmbeddings(
        ...
        return this.fetchKhulnasoftAPI<APIResponse<EmbeddingsSearchResponse>>(LEGACY_SEARCH_EMBEDDINGS_QUERY, {
            repo,
            query,
            codeResultsCount,
            textResultsCount,
    ...
    public async searchAttribution(snippet: string): Promise<SearchAttributionResults | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<SearchAttributionResponse>>(SEARCH_ATTRIBUTION_QUERY, {
            snippet,
    ...
    public async isContextRequiredForQuery(query: string): Promise<boolean | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<IsContextRequiredForChatQueryResponse>>(IS_CONTEXT_REQUIRED_QUERY, {
            query,
    ...
    public async getEvaluatedFeatureFlags(): Promise<Record<string, boolean> | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<EvaluatedFeatureFlagsResponse>>(GET_FEATURE_FLAGS_QUERY, {}).then(
            response =>
                extractDataOrError(response, data =>
    ...
    public async evaluateFeatureFlag(flagName: string): Promise<boolean | null | Error> {
        return this.fetchKhulnasoftAPI<APIResponse<EvaluateFeatureFlagResponse>>(EVALUATE_FEATURE_FLAG_QUERY, {
            flagName,
        }).then(response => extractDataOrError(response, data => data.evaluateFeatureFlag))
    }

    private fetchKhulnasoftAPI<T>(query: string, variables: Record<string, any> = {}): Promise<T | Error> {
        ...
        return fetch(url, {
            ...
            .catch(error => new Error(`accessing Khulnasoft GraphQL API: ${error} (${url})`))
    }

    // make an anonymous request to the dotcom API
    private fetchKhulnasoftDotcomAPI<T>(query: string, variables: Record<string, any>): Promise<T | Error> {
        ...
        return fetch(url, {
            ...
            .then(verifyResponseCode)
            .then(response => response.json() as T)
            .catch(error => new Error(`error fetching Khulnasoft GraphQL API: ${error} (${url})`))
    }

    // make an anonymous request to the Testing API
    private fetchKhulnasoftTestingAPI<T>(body: Record<string, any>): Promise<T | Error> {
        ...
        return fetch(url, {
            ...
            .then(verifyResponseCode)
            .then(response => response.json() as T)
            .catch(error => new Error(`error fetching Testing Khulnasoft API: ${error} (${url})`))
    }
}
