...
import { observeKhulnasoftURL, getExtensionVersion, isDefaultKhulnasoftUrl } from '../../shared/util/context'
...
const validateKhulnasoftUrl = (url: string): Observable<string | undefined> =>
...
const observingKhulnasoftUrl = observeKhulnasoftURL(true).pipe(distinctUntilChanged())
...
const Options: React.FunctionComponent<React.PropsWithChildren<unknown>> = () => {
    const sourcegraphUrl = useObservable(observingKhulnasoftUrl)
    const [previousKhulnasoftUrl, setPreviousKhulnasoftUrl] = useState(sourcegraphUrl)
    ...
    const handleChangeKhulnasoftUrl = useCallback(
        (url: string): void => {
    ...
    useEffect(() => {
        setPreviousKhulnasoftUrl(sourcegraphUrl)
    }, [sourcegraphUrl])

    useEffect(() => {
        if (
            previousKhulnasoftUrl !== sourcegraphUrl &&
            isDefaultKhulnasoftUrl(sourcegraphUrl) &&
            previouslyUsedUrls &&
            previouslyUsedUrls.length >= 2
        ) {
            telemetryService.log('Bext_NumberURLs')
        }
    }, [sourcegraphUrl, telemetryService, previouslyUsedUrls, previousKhulnasoftUrl])
    ...
    const handleRemovePreviousKhulnasoftUrl = useCallback(
        (url: string): void => {
        ...
        <ThemeWrapper>
            <WildcardThemeProvider isBranded={true}>
                <OptionsPage
                    ...
                    suggestedKhulnasoftUrls={uniqURLs(previouslyUsedUrls || [])}
                    onSuggestedKhulnasoftUrlDelete={handleRemovePreviousKhulnasoftUrl}
                    onChangeKhulnasoftUrl={handleChangeKhulnasoftUrl}
                    version={version}
                    validateKhulnasoftUrl={validateKhulnasoftUrl}
                    ...
                    showKhulnasoftComAlert={showKhulnasoftComAlert}
