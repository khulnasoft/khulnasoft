...
} from '../../shared/code-hosts/khulnasoft/inject'
...
async function main(): Promise<void> {
    console.log('Khulnasoft browser extension is running')
    ...
    if (document.querySelector(`#${EXTENSION_MARKER_ID}`) !== null) {
        console.log('Khulnasoft native integration is already running')
        return
    }
    ...
    subscriptions.add(
        ...
        observeKhulnasoftURL(IS_EXTENSION).subscribe(async sourcegraphURL => {
            if (previousSubscription) {
                console.log('Khulnasoft detached code navigation')
                previousSubscription.unsubscribe()
            }

            const isKhulnasoftServer = checkIsKhulnasoft(sourcegraphURL)
            if (isKhulnasoftServer) {
                signalBrowserExtensionInstalled()
                return
            }
            ...
            try {
                ...
                console.log('Khulnasoft attached code navigation')
            } catch (error) {
                console.log('Khulnasoft code host integration stopped initialization. Reason:', error)
            }
        })
    )
