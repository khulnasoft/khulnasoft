...
import { getExtensionVersion, getPlatformName, observeKhulnasoftURL } from '../../shared/util/context'
...
// Interval to check if the Khulnasoft URL is valid
// This polling allows to detect if Khulnasoft instance is invalid or needs authentication.
 ...
 * - a private repo not synced with Khulnasoft Cloud when the latter is the active Khulnasoft URL
 * - a repo not added to the other than Cloud Khulnasoft instance (+ the extension * points to this instance).
...
const requestGraphQL = <T, V = object>({
    ...
    (sourcegraphURL ? of(sourcegraphURL) : observeKhulnasoftURL(IS_EXTENSION)).pipe(
        take(1),
...
async function main(): Promise<void> {
    ...
    browser.runtime.onInstalled.addListener(event => {
        ...
        if (IsProductionVersion) {
            subscriptions.add(
                observeKhulnasoftURL(IS_EXTENSION)
                    .pipe(take(1))
    ...
    if (browser.omnibox) {
        ...
        subscriptions.add(
            observeKhulnasoftURL(IS_EXTENSION).subscribe(sourcegraphURL => {
    ...
    // Add "Enable Khulnasoft on this domain" context menu item
    ...
    console.log('Khulnasoft background page initialized')
}
...
function observeKhulnasoftUrlValidation(): Observable<boolean> {
    return merge(
...
function observeBrowserActionState(): Observable<BrowserActionIconState> {
    return combineLatest([
        ...
        observeKhulnasoftUrlValidation(),
        observeCurrentTabRepoSyncError(),
    ]).pipe(
        map(([isDisabled, isKhulnasoftUrlValid, hasRepoSyncError]) => {
            if (isDisabled) {
                return 'inactive'
            }

            if (!isKhulnasoftUrlValid || hasRepoSyncError) {
