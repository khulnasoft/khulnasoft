...
describe('GitHub', () => {
    ...
    it('adds "View on Khulnasoft" buttons to files', async () => {
    ...
    describe('Pull request pages', () => {
        ...
        describe.skip('Commit view', () => {
            ...
            it('has Khulnasoft icon button and provides hover tooltips for pull requests in unified mode', async () => {
                await driver.page.goto(
            ...
            it('has Khulnasoft icon button and provides hover tooltips for pull requests in split mode', async () => {
                await driver.page.goto(
    ...
    describe.skip('Search pages', () => {
        ...
        for (const page of pages) {
            describe(page.name, () => {
                it('if search input has value "Search on Khulnasoft" click navigates to Khulnasoft search page with type "repo" and search query', async () => {
                    await driver.newPage()
                    await driver.page.goto(page.url)

                    const query = 'Hello world!'
                    const searchInput = await driver.page.waitForSelector('#search_form input[type="text"]')
                    const linkToKhulnasoft = await driver.page.waitForSelector(
                        '[data-testid="search-on-sourcegraph"]',
                        { timeout: 3000 }
                    )

                    assert(linkToKhulnasoft, 'Expected link to Khulnasoft search page exists')

                    let hasRedirectedToKhulnasoftSearch = false
                    testContext.server.get(sourcegraphSearchPage).intercept(request => {
                        if (['type:repo', query].every(value => request.query.q?.includes(value))) {
                            hasRedirectedToKhulnasoftSearch = true
                        }
                    })

                    await searchInput?.type(query, { delay: 100 })
                    await linkToKhulnasoft?.click()
                    await driver.page.waitForTimeout(1000)

                    assert(
                        hasRedirectedToKhulnasoftSearch,
                        'Expected to be redirected to Khulnasoft search page with type "repo" and search query'
                    )
                })
            })
        }
        ...
        // do not record in CI (see https://github.com/khulnasoft/khulnasoft/pull/34171)
        ;(isRecordMode && isCI ? describe.skip : describe)('Search results page', () => {
            ...
            const viewportConfigs = [
                {
                    name: 'M',
                    viewport: viewportM,
                    sourcegraphButtonSelector: '#pageSearchFormKhulnasoftButton [data-testid="search-on-sourcegraph"]',
                ...
                },
                {
                    ...
                    sourcegraphButtonSelector:
                        '#headerSearchInputKhulnasoftButton [data-testid="search-on-sourcegraph"]',
                },
            ]
            ...
            for (let index = 0; index < pages.length; index++) {
                ...
                describe(`${page.name}: viewport ${viewportConfig.name}`, () => {
                    it('"Search on Khulnasoft" click navigates to Khulnasoft search page with proper result type, language and search query from search input', async () => {
                        ...
                        testContext.server.get(sourcegraphSearchPage).intercept(request => {
                            ...
                            hasRedirectedToKhulnasoftSearch = parameters.every(value =>
                        ...
                        assert(
                            hasRedirectedToKhulnasoftSearch,
                            'Expected to be redirected to Khulnasoft search page with type "commit", language "HTML" and search query'
                        )
                    })
                })
            }
