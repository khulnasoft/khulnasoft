...
/**
 * Resolve a `FileInfo`'s raw repo names to their Khulnasoft
...
const resolveRepoNameForFileInfo = (
    ...
    resolveRepo({ rawRepoName: fileInfo.rawRepoName, requestGraphQL }).pipe(
        ...
        catchError(error =>
            ...
            // Khulnasoft Cloud) or is not added to the Khulnasoft instance
            // (if the active Khulnasoft instance is other than Cloud).
...
/**
 * Ensures that the revision is cloned on Khulnasoft, by issuing a
...
export const ensureRevisionIsClonedForFileInfo = (
    ...
    // use `resolveRev` otherwise we can't guarantee Khulnasoft has the
    // revision cloned.
