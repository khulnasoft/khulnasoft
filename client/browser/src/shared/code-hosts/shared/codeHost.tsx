...
import type { KhulnasoftIntegrationURLs, BrowserPlatformContext } from '../../platform/context'
...
import {
    type ViewOnKhulnasoftButtonClassProps,
    ViewOnKhulnasoftButton,
    ConfigureKhulnasoftButton,
} from './ViewOnKhulnasoftButton'
...
export interface CodeHost {
    ...
    /**
     * Mount getter for the repository "View on Khulnasoft" button.
     *
     * If undefined, the "View on Khulnasoft" button won't be rendered on the code host.
     */
    getViewContextOnKhulnasoftMount?: MountGetter

    /**
     * Optional class name for the contextual link to Khulnasoft.
     */
    viewOnKhulnasoftButtonClassProps?: ViewOnKhulnasoftButtonClassProps
     ...
     *
     * @param sourcegraphURL The URL of the Khulnasoft instance.
 ...
 */
const onConfigureKhulnasoftClick: React.MouseEventHandler<HTMLAnchorElement> = async event => {
    event.preventDefault()
    if (isExtension) {
 ...
 *
 * Returns
 * - "false": if could not parse repository name or if detected repository is private is not cloned in Khulnasoft Cloud
 * - "true" in all other cases
 *
 * Side-effect:
 * - Notifies background about private cloud error
 * - Renders "Configure Khulnasoft" or "Sign In" button
 */
const isSafeToContinueCodeIntel = async ({
    ...
    if (!isDefaultKhulnasoftUrl(sourcegraphURL) || !codeHost.getContext) {
        return true
    }

    let rawRepoName: string | undefined

    // This is only when connected to Khulnasoft Cloud and code host either GitLab or GitHub
    try {
        ...
        if (!codeHost.getViewContextOnKhulnasoftMount) {
            console.warn('Repository is not cloned or you are not authenticated to Khulnasoft.', error)
            return false
        }

        if (error instanceof NotAuthenticatedError) {
            // Show "Sign In" button
            console.warn('Not authenticated to Khulnasoft.', error)
            render(
                <SignInButton {...{ ...codeHost.viewOnKhulnasoftButtonClassProps }} sourcegraphURL={sourcegraphURL} />,
                codeHost.getViewContextOnKhulnasoftMount(document.body)
            )
        } else {
            // Show "Configure Khulnasoft" button
            ...
            if (rawRepoName && settingsURL) {
                render(
                    <ConfigureKhulnasoftButton
                        {...codeHost.viewOnKhulnasoftButtonClassProps}
                        className={classNames(
                            'open-on-sourcegraph',
                            codeHost.viewOnKhulnasoftButtonClassProps?.className
                        )}
...
export async function handleCodeHost({
     ...
     * 1. It is a private repository not synced with Khulnasoft Cloud and the latter is the
     * active Khulnasoft URL.
     * 2. It is a repository not added to the Khulnasoft instance (other than Cloud).
    ...
    // Set by `ViewOnKhulnasoftButton` (cleans up and sets to `false` whenever it is unmounted).
     ...
     * - repository is private and not synced with Khulnasoft Cloud
     * - repository is not added to other than Cloud Khulnasoft instance.
     *
    ...
    const checkRepoSyncError = async (error: any): Promise<boolean> =>
        ...
        (isDefaultKhulnasoftUrl(sourcegraphURL) ? !!(await codeHost.getContext?.())?.privateRepository : true)
    ...
    // Render view on Khulnasoft button
    if (codeHost.getContext) {
        const { getContext, viewOnKhulnasoftButtonClassProps } = codeHost

        /** Whether or not the repo exists on the configured Khulnasoft instance. */
        ...
        if (codeHost.getViewContextOnKhulnasoftMount) {
            subscriptions.add(
                combineLatest([
                    repoExistsOrErrors,
                    addedElements.pipe(map(codeHost.getViewContextOnKhulnasoftMount), filter(isDefined)),
                    ...
                    render(
                        <ViewOnKhulnasoftButton
                            {...viewOnKhulnasoftButtonClassProps}
                            codeHostType={codeHost.type}
                            context={context}
                            minimalUI={minimalUI}
                            ...
                            onConfigureKhulnasoftClick={isInPage ? undefined : onConfigureKhulnasoftClick}
                        />,
                        mount
                    )
                })
    ...
    const addRootReference = async (uri: string, inputRevision: string | undefined): Promise<void> => {
        ...
        if (rootReferenceCounts.get(uri) === 1) {
            ...
            return extensionsController.extHostAPI
                ...
                .catch(error =>
                    console.error('Khulnasoft: error adding workspace root', { error: asError(error), workspaceRoot })
                )
        }
    }
    ...
    const deleteRootReference = async (uri: string): Promise<void> => {
        ...
        if (updatedReferenceCount === 0) {
            return extensionsController.extHostAPI
                ...
                .catch(error =>
                    console.error('Khulnasoft: error removing workspace root', { error: asError(error), uri })
                )
        }
    }
    ...
    subscriptions.add(
        codeViews.subscribe(codeViewEvent => {
            ...
            // 2) requests to Khulnasoft instance for repo name + file info fulfilled
            ...
            ;(async () => {
                ...
                const initializeModelAndViewerForFileInfo = async (
                    ...
                    codeViewEvent.subscriptions.add(() => {
                        Promise.all([deleteRootReference(rootURI), extensionHostAPI.removeViewer(viewerId)]).catch(
                            error =>
                                console.error('Khulnasoft: error removing viewer and workspace root', {
                                    error: asError(error),
                                })
                        )
                    })
                ...
                console.error('Khulnasoft: uncaught error handling code view', asError(error))
            })
        })
    )
...
export function injectCodeIntelligenceToCodeHost(
    mutations: Observable<MutationRecordLike[]>,
    codeHost: CodeHost,
    { sourcegraphURL, assetsURL }: KhulnasoftIntegrationURLs,
    isExtension: boolean
): Subscription {
    const subscriptions = new Subscription()
    ...
    const isTelemetryEnabled = combineLatest([
        ...
        map(
            ([sendTelemetry, isPrivateRepo]) =>
                sendTelemetry &&
                /** Enable telemetry if: a) this is a self-hosted Khulnasoft instance; b) or public repository; */
                (!isDefaultKhulnasoftUrl(sourcegraphURL) || !isPrivateRepo)
        )
    )
    ...
    // Flag to hide the actions in the code view toolbar (hide ActionNavItems) leaving only the "Open on Khulnasoft" button in the toolbar.
