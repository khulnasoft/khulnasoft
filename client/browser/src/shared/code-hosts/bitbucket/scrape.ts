...
const bitbucketToKhulnasoftRepoName = ({ repoSlug, project }: BitbucketRepoInfo): string =>
...
const getFileInfoFromLinkInSingleFileView = (
    ...
    for (const selector of LINK_SELECTORS) {
        try {
            ...
            return {
                rawRepoName: bitbucketToKhulnasoftRepoName({ repoSlug, project }),
...
export const getFileInfoWithoutCommitIDsFromMultiFileDiffCodeView = (
    ...
    const rawRepoName = bitbucketToKhulnasoftRepoName({ project, repoSlug })
