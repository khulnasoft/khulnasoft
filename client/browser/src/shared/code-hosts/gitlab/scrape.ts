...
export const getGitlabRepoURL = (): string => {
    ...
    return projectLink.href // e.g. 'https://gitlab.com/KhulnasoftCody/jsonrpc2'
}
...
export function getPageInfo(): GitLabInfo {
    ...
    /**
     * Get the repository name that we can use to interact with the Khulnasoft API.
     * It is possible that this differs from `projectNameOnGitLab` if the Khulnasoft instance
     * has `nameTransformations` or `repositoryPathPattern` set.
     */
    const sourcegraphCompatibleProjectName = repoNameOnKhulnasoft.value
