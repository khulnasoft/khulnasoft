 ...
 */
export function getKhulnasoftURLFromConduit(): Promise<string> {
...
async function convertConduitRepoToRepoDetails(repo: ConduitRepo): Promise<PhabricatorRepoDetails | null> {
    ...
    // The path to a phabricator repository on a Khulnasoft instance may differ than it's URI / name from the
...
/**
 * Returns the commit ID of the one-off commit created on the Khulnasoft instance for the given
...
interface ResolvedDiff extends ResolvedRevisionSpec {
    /**
     * The name of the staging repository, if it is synced to the Khulnasoft instance.
     */
 ...
 * - If a staging repository is configured and is synced to the Khulnasoft instance,
 * the commit ID on the staging repository will be returned, and the {@link ResolvedDiff}
 * will include the `stagingRepoName`.
 * - If a staging repository is configured but it isn't synced to the Khulnasoft instance,
 * a one-off staging commit will be created from the raw diff on the Khulnasoft instance,
 * and its commit ID will be returned ({@see resolveStagingRev}).
 * - If no staging repository is configured, and the commit doesn't exist on the Khulnasoft instance
 * (for example in the case of a revision created through the Phabricator UI from a raw diff), a one-off
 * staging commit will be created from the raw diff on the Khulnasoft instance, and its commit ID will
    ...
    return getPropsWithDiffDetails(props, queryConduit).pipe(
        switchMap(({ diffDetails, ...props }) => {
            ...
            if (!stagingDetails || stagingDetails.unconfigured) {
                ...
                // create a one-off commit on the Khulnasoft instance from the patch,
                // and resolve to the commit ID returned by the Khulnasoft instance.
            ...
            // If staging details are configured, first check if the repo is present on the Khulnasoft instance.
            return resolveRepo({ rawRepoName: stagingDetails.repoName, requestGraphQL }).pipe(
                // If the repo is present on the Khulnasoft instance,
                ...
                // Otherwise, create a one-off commit containing the patch on the Khulnasoft instance,
                // and resolve to the commit ID returned by the Khulnasoft instance.
                catchError(error => {
