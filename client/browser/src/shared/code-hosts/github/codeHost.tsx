...
import { KhulnasoftIconButton } from '../../components/KhulnasoftIconButton'
...
const genericCodeViewResolver: ViewResolver<CodeView> = {
    selector: target => {
        ...
        // https://github.com/khulnasoft/khulnasoft/issues/18337
...
type KhulnasoftResultType = 'repo' | 'commit'

const getKhulnasoftResultType = (): KhulnasoftResultType | '' => {
...
const getKhulnasoftResultLanguage = (): string | null =>
    new URLSearchParams((windowLocation__testingOnly.value ?? window.location).search).get('l')

const buildKhulnasoftQuery = (searchTerms: string[]): string => {
    const queryParameters = searchTerms.filter(Boolean).map(parameter => parameter.trim())
    const sourcegraphResultType = getKhulnasoftResultType()
    const resultsLanguage = getKhulnasoftResultLanguage()
...
/**
 * Adds "Search on Khulnasoft buttons" to GitHub search pages
 */
function enhanceSearchPage(sourcegraphURL: string): void {
    ...
    const renderKhulnasoftButton = ({
        ...
        root.render(
            <KhulnasoftIconButton
                label="Search on Khulnasoft"
                title="Search on Khulnasoft to get hover tooltips, go to definition and more"
                ariaLabel="Search on Khulnasoft to get hover tooltips, go to definition and more"
                ...
                onClick={event => {
                    const searchQuery = buildKhulnasoftQuery(getSearchQuery())
    ...
    if (isSearchResultsPage()) {
        ...
        if (
            ...
            /*
                Separate search form is visible for screen sizes xs-md, so we add a Khulnasoft button
                next to form submit button and track search query changes from the corresponding form input.
                On screen sizes smaller than md Github *Submit* and *Search on Khulnasoft* buttons are hidden while search input remains visible.
            */
            ...
            if (pageSearchInput && pageSearchFormSubmitButton) {
                const buttonContainer = queryByIdOrCreate('pageSearchFormKhulnasoftButton', 'ml-2 d-none d-md-block')
                pageSearchFormSubmitButton.after(buttonContainer)

                renderKhulnasoftButton({
            ...
            /*
                On screen sizes lg and larger separate search form is hidden, so we add a Khulnasoft button
            ...
            */
            ...
            if (headerSearchInput && (emptyResultsContainer || searchResultsContainerHeading)) {
                const buttonContainer: HTMLElement = queryByIdOrCreate(
                    'headerSearchInputKhulnasoftButton',
    ...
    if (searchInputContainer && inputElement) {
        const buttonContainer = queryByIdOrCreate('searchInputKhulnasoftButton', 'ml-0 ml-md-2 mt-2 mt-md-0')
...
export const githubCodeHost: GithubCodeHost = {
    ...
    getViewContextOnKhulnasoftMount: createOpenOnKhulnasoftIfNotExists,
