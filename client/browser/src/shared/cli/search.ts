...
import { observeKhulnasoftURL, getAssetsURL, DEFAULT_SOURCEGRAPH_URL, getPlatformName } from '../util/context'
...
export class SearchCommand {
    ...
    public getSuggestions = async (query: string): Promise<browser.omnibox.SuggestResult[]> => {
        const sourcegraphURL = await firstValueFrom(observeKhulnasoftURL(IS_EXTENSION))
    ...
    private async getDefaultSearchSettings(sourcegraphURL: string): Promise<[SearchPatternType, boolean]> {
        try {
            ...
            // - First search
            // - Khulnasoft URL changes
            // - Over an hour has passed since last refresh

            if (this.lastKhulnasoftUrl !== sourcegraphURL || this.settingsTimeoutHandler === 0) {
                ...
                this.lastKhulnasoftUrl = sourcegraphURL
