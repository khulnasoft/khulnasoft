...
import { getExtensionVersion, isDefaultKhulnasoftUrl, observeKhulnasoftURL } from '../util/context'
...
export function initSentry(script: 'content' | 'options' | 'background', codeHost?: string): void {
    ...
    observeKhulnasoftURL(IS_EXTENSION).subscribe(url => {
        Sentry.configureScope(scope => {
            scope.setTag('using_dot_com', isDefaultKhulnasoftUrl(url) ? 'true' : 'false')
        })
    })
}
