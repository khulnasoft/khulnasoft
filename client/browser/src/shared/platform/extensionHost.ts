...
import type { KhulnasoftIntegrationURLs } from './context'
import { browserPortToMessagePort } from './ports'

function createInPageExtensionHost({
    assetsURL,
}: Pick<KhulnasoftIntegrationURLs, 'assetsURL'>): Promise<ClosableEndpointPair> {
 ...
 */
export function createExtensionHost(urls: Pick<KhulnasoftIntegrationURLs, 'assetsURL'>): Promise<ClosableEndpointPair> {
    if (isInPage) {
