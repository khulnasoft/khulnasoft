...
export interface KhulnasoftIntegrationURLs {
    /**
     * The URL of the configured Khulnasoft instance. Used for extensions, find-references, ...
     */
...
export interface BrowserPlatformContext extends PlatformContext {
    /**
     * Re-fetches the settings cascade from the Khulnasoft instance.
     */
    ...
    const context: BrowserPlatformContext = {
        ...
        refreshSettings: async () => {
            try {
                ...
                if (isHTTPAuthError(error)) {
                    ...
                    console.warn(
                        `Could not fetch Khulnasoft settings from ${sourcegraphURL} because user is not signed into Khulnasoft`
                    )
        ...
        updateSettings: async (subject, edit) => {
            if (subject === 'Client') {
                ...
                // Khulnasoft viewers can update settings.
        ...
        urlToFile: ({ rawRepoName, ...target }, context) => {
            ...
            // Otherwise fall back to linking to Khulnasoft (with an absolute URL).
