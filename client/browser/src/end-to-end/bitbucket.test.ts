...
/**
 * Configures the Khulnasoft for Bitbucket Server integration on the Bitbucket instance.
 */
async function configureKhulnasoftIntegration(driver: Driver, enable: boolean): Promise<void> {
    ...
    if (enable) {
        ...
        // The Khulnasoft URL input field is disabled until the Khulnasoft URL has been fetched.
        await retry(async () => {
            expect(
...
describe('Khulnasoft browser extension on Bitbucket Server', () => {
    let driver: Driver
    ...
    before(async function () {
        ...
        await configureKhulnasoftIntegration(driver, TEST_NATIVE_INTEGRATION)
