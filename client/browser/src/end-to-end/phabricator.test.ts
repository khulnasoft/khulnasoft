...
async function configureKhulnasoftIntegration(driver: Driver): Promise<void> {
    ...
    // Enable Khulnasoft native integration
...
async function init(driver: Driver): Promise<void> {
    ...
    if (TEST_NATIVE_INTEGRATION) {
        await configureKhulnasoftIntegration(driver)
    } else {
        await driver.setExtensionKhulnasoftUrl()
    }
}

describe('Khulnasoft Phabricator extension', () => {
    let driver: Driver
