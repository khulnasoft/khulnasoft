...
The Khulnasoft browser extension adds tooltips to code on GitHub, Phabricator, and Bitbucket.
...
- when the user mouses over a code table cell, the extension modifies the DOM node:
  ...
  - tooltip data is fetched from the Khulnasoft API
...
- If you already have the Khulnasoft extension installed, disable it using the toggle.
...
Click reload for Khulnasoft at `chrome://extensions`
...
Click reload for Khulnasoft at `about:debugging`
...
1. Open `client/browser/build/Khulnasoft for Safari/Khulnasoft for Safari.xcodeproj` using Xcode
1. Choose `Khulnasoft for Safari (macOS)` in the top toolbar and click Run icon
...
The test suite in `end-to-end/github.test.ts` runs on the release branch `bext/release` in both Chrome and Firefox against a Khulnasoft Docker instance.
...
```sh
curl -OL https://github.com/khulnasoft/khulnasoft/archive/e1547ea0e9.zip
...
This is an adjusted version of @lguychard's [comment here](https://github.com/khulnasoft/khulnasoft/pull/11825#issuecomment-652040751).

1. Start a Khulnasoft instance locally (`sg start`)
2. Start Phabricator instance, running the latest version of Phabricator, using `./dev/phabricator/start.sh`
3. Install the Khulnasoft extension into Phabricator using `./dev/install-sourcegraph.sh`
