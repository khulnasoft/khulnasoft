    ...
    if (options.disableFilterCompletion !== true) {
        sources.push(
            ...
            createFilterSource((_context, _tokens, token, resolvedFilter) => {
                ...
                return {
                    ...
                    options: resolvedFilter.definition
                        .discreteValues(value, options.isKhulnasoftDotCom)
                        .map(({ label, insertText, asSnippet }, index) => {
                            const apply = (insertText || label) + ' '
                            return {
                                label,
                                // See issue https://github.com/khulnasoft/khulnasoft/issues/38254
            ...
            createFilterSource(async (context, tokens, token, resolvedFilter) => {
                // On Khulnasoft.com, prompt only static suggestions (above) if there is no value to use for generating dynamic suggestions yet.
                if (
                    options.isKhulnasoftDotCom &&
