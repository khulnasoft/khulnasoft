 ...
 *
 * NOTE: Prefer to use {@link KhulnasoftURL} instead of this function.
 *
...
export class KhulnasoftURL {
    ...
    /**
     * Creates a new KhulnasoftURL instance from a string, URL, URLSearchParams, or location object.
     *
    ...
    public static from(
        ...
        if (typeof url === 'string' || url instanceof URL) {
            return new KhulnasoftURL(url)
        }
        if (url instanceof URLSearchParams) {
            return new KhulnasoftURL(`?${formatSearchParameters(url.toString())}`)
        }
        return KhulnasoftURL.fromLocation(url)
    }
    ...
    private static fromLocation(location: {
        ...
        return new KhulnasoftURL(`${pathname}${search}${hash}`)
    }
     ...
     * Example:
     * ```
     * const url = new KhulnasoftURL('/foo?bar')
