...
import { encodeURIPathComponent, LineOrPositionOrRange, KhulnasoftURL } from './url'

describe('KhulnasoftURL', () => {
    describe('from', () => {
        ...
        it('accepts URLSearchParams', () => {
            expect(KhulnasoftURL.from(new URLSearchParams('some=param')).search).toBe('?some=param')
        })
        ...
        describe('location object', () => {
            ...
            `(
                ...
                ({ pathname, search, hash, expected }) => {
                    expect(KhulnasoftURL.from({ pathname, search, hash }).toString()).toBe(expected)
                }
            )
        })
    })
    ...
    describe('get lineRange', () => {
        ...
        `('$input => $expected', ({ input, expected }) => {
            ...
            expect(KhulnasoftURL.from(`/some/path?${input}`).lineRange).toEqual(expected)
            // Hash position
            expect(KhulnasoftURL.from(`/some/path#${input}`).lineRange).toEqual(expected)
        })
        ...
        `('$input ($message) => {}', ({ input }) => {
            // Search parameter position
            expect(KhulnasoftURL.from(`/some/path?${input}`).lineRange).toEqual({})
            // Hash position
            expect(KhulnasoftURL.from(`/some/path#${input}`).lineRange).toEqual({})
        })
    })
    ...
    describe('setLineRange', () => {
        ...
        `('$input => $expected', ({ input, lpr, expected }) => {
            expect(KhulnasoftURL.from(input).setLineRange(lpr).toString()).toBe(expected)
        })
    })
    ...
    describe('get viewState', () => {
        ...
        `('$input => $expected', ({ input, expected }) => {
            expect(KhulnasoftURL.from(input).viewState).toBe(expected)
        })
    })
    ...
    describe('setViewState', () => {
        ...
        `('$input => $expected', ({ input, viewState, expected }) => {
            expect(KhulnasoftURL.from(input).setViewState(viewState).toString()).toBe(expected)
        })
    })
})
...
describe('parse legacy hash', () => {
    function parseHash(hash: string): LineOrPositionOrRange & { viewState?: string } {
        const url = KhulnasoftURL.from({ pathname: '', hash })
