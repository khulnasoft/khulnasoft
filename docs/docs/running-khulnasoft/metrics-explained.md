...
permalink: /running-khulnasoft/metrics-explained/
...
KhulnaSoft Fair Web Analytics does not rely on an IP database for deriving geo location, but instead asks the browser for the selected timezone and tries to map this to a country.
...
[campaigns sources]: /running-khulnasoft/campaigns-sources/
