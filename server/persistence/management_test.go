...
func TestPersistenceLayer_ShareAccount(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"invitee@khulnasoft.com",
			"develop@khulnasoft.com",
			"develop",
			"",
			ShareAccountResult{},
			true,
		},
		{
			...
			&mockShareAccountDatabase{
				findAcccountUsersResult: []AccountUser{
					(func() AccountUser {
						a, _ := newAccountUser("admin@khulnasoft.com", "develop", AccountUserAdminLevelSuperAdmin)
						return *a
					})(),
				},
			},
		...
		},
		{
			...
			&mockShareAccountDatabase{
				findAcccountUsersResult: []AccountUser{
					(func() AccountUser {
						a, _ := newAccountUser("develop@khulnasoft.com", "d3v3lop", AccountUserAdminLevelSuperAdmin)
						return *a
					})(),
				},
			},
		...
		},
		{
			...
			&mockShareAccountDatabase{
				findAcccountUsersResult: []AccountUser{
					(func() AccountUser {
						a, _ := newAccountUser("develop@khulnasoft.com", "develop", AccountUserAdminLevelSuperAdmin)
						return *a
					})(),
				},
			...
			&mockShareAccountDatabase{
				findAcccountUsersResult: []AccountUser{
					(func() AccountUser {
						...
						emailDerivedKey, _ := keys.DeriveKey("develop@khulnasoft.com", a.Salt)
					...
					(func() AccountUser {
						a, _ := newAccountUser("invitee@khulnasoft.com", "develop", AccountUserAdminLevelSuperAdmin)
						return *a
					})(),
				},
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := p.ShareAccount(test.invitee, test.email, test.password, test.accountID, true)
