...
import (
	...
	"github.com/khulnasoft/khulnasoft/server/keys"
...
var demoUsage = `
"demo" starts a one-off KhulnaSoft instance that you can use for testing the software
on your local machine. You can use the username "demo@khulnasoft.com" with password
...
func cmdDemo(subcommand string, flags []string) {
	...
	a := newApp(false, true, "")
	{
		...
		cfg.Database.ConnectionString = config.EnvString(fmt.Sprintf("/tmp/khulnasoft-demo-%s.db", dbID.String()))
		if runtime.GOOS == "windows" {
			cfg.Database.ConnectionString = config.EnvString(fmt.Sprintf("%%Temp%%\\khulnasoft-%s.db", dbID.String()))
		}
	...
	a.logger.Info("KhulnaSoft is generating some random usage data for your demo, this might take a little while.")
	...
	a.logger.Infof("You can now start your KhulnaSoft demo by visiting")
	a.logger.Infof("")
