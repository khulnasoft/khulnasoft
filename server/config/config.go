...
import (
	...
	"github.com/khulnasoft/khulnasoft/server/mailer"
	"github.com/khulnasoft/khulnasoft/server/mailer/localmailer"
	"github.com/khulnasoft/khulnasoft/server/mailer/sendmailmailer"
	"github.com/khulnasoft/khulnasoft/server/mailer/smtpmailer"
)

const envFileName = "khulnasoft.env"

var (
	// EventRetention defines the duration for which events are expected to
	//  be kept before expired. This value can be overridden by setting KHULNASOFT_APP_RETENTION_DAYS
...
func walkConfigurationCascade() (string, error) {
	...
	// in case there is an khulnasoft.env file next to the binary, it will be loaded
	...
	case "darwin", "linux":
		cascade = []string{
			...
			path.Join("/etc/khulnasoft", envFileName),
		}
	}
...
func New(populateMissing bool, override string) (*Config, error) {
	...
	err := envconfig.Process("khulnasoft", &c)
	if err != nil {
	...
	if populateMissing {
		...
		for _, val := range []autopopulatedValue{
			{"KHULNASOFT_SECRET", c.Secret.IsZero},
		} {
			if !val.isEmpty() {
