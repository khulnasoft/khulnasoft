...
func TestRouter_postLogin(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			strings.NewReader(`{"username":"mail@khulnasoft.com","password":"secret!"}`),
...
func TestRouter_postResetPassword(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"bad token",
			strings.NewReader(`{"emailAddress":"admin@khulnasoft.com","password":"new","token":"made up token"}`),
			mockPostResetPasswordDatabase{},
			http.StatusBadRequest,
		},
		{
			...
			func() io.Reader {
				s, _ := signer.Encode("credentials", &forgotPasswordCredentials{
					EmailAddress: "mail@khulnasoft.com",
				})
				return strings.NewReader(
					fmt.Sprintf(
						`{"emailAddress":"admin@khulnasoft.com","password":"new","token":"%s"}`, s,
					),
				)
			}(),
			mockPostResetPasswordDatabase{},
		...
		},
		{
			...
			func() io.Reader {
				s, _ := signer.Encode("credentials", &forgotPasswordCredentials{
					EmailAddress: "admin@khulnasoft.com",
				})
				return strings.NewReader(
					fmt.Sprintf(
						`{"emailAddress":"admin@khulnasoft.com","password":"new","token":"%s"}`, s,
...
func TestRouter_postForgotPassword(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mockMailer{},
			strings.NewReader(`{"emailAddress":"mail@khulnasoft.com","urlTemplate":"/{token}/"}`),
			http.StatusNoContent,
		},
		{
			"error sending email",
			...
			strings.NewReader(`{"emailAddress":"mail@khulnasoft.com","urlTemplate":"/reset/{token}/"}`),
			http.StatusInternalServerError,
		},
		{
			"ok",
