...
func TestRouter_postShareAccount(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			"oingo-boingo",
			strings.NewReader(`{"invitee":"mail@khulnasoft.com","emailAddress":"admin@khulnasoft.com","password":"ok","urlTemplate":"/join/{token}","grantAdminPrivileges":false}`),
			mockMailer{},
			http.StatusBadRequest,
		},
		{
...
func TestRouter_postJoin(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			"bad token",
			mockPostJoinDatabase{},
			strings.NewReader(`{"emailAddress":"admin@khulnasoft.com","password":"pass","token":"something something"}`),
			http.StatusBadRequest,
		},
		{
			"email mismatch",
			mockPostJoinDatabase{},
			func() io.Reader {
				token, _ := signer.Encode("credentials", "mail@khulnasoft.com")
				return strings.NewReader(
					fmt.Sprintf(
						`{"emailAddress":"admin@khulnasoft.com","password":"pass","token":"%s"}`,
						token,
					),
				)
			}(),
		...
		},
		{
			...
			func() io.Reader {
				token, _ := signer.Encode("credentials", "admin@khulnasoft.com")
				return strings.NewReader(
					fmt.Sprintf(
						`{"emailAddress":"admin@khulnasoft.com","password":"pass","token":"%s"}`,
						token,
